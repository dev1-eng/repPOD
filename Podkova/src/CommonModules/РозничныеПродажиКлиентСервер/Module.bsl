	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		Для Каждого СтрокаРаспределения Из РаспределениеПоСтроке Цикл
			...
			Иначе
				...
				СтрокаТоварыРазобранные.КоличествоЕдиниц   = СтрокаРаспределения.Количество;
			...
			Если СтрокаТовары.КоличествоУпаковок = СтрокаРаспределения.КоличествоУпаковок Тогда
			...
			Иначе
				СуммаСкидки = Окр(СтрокаТовары.СуммаСкидки / СтрокаТовары.КоличествоЕдиниц * Коэффициент, 2);
				СуммаНДС    = Окр(СтрокаТовары.СуммаНДС    / СтрокаТовары.КоличествоЕдиниц * Коэффициент, 2);
				СуммаСНДС   = Окр(СтрокаТовары.СуммаСНДС   / СтрокаТовары.КоличествоЕдиниц * Коэффициент, 2);
				Цена        = СуммаСНДС - СуммаСкидки;
			КонецЕсли;
			...
			СтрокаТовары.КоличествоУпаковок = СтрокаТовары.КоличествоУпаковок - СтрокаРаспределения.КоличествоУпаковок;
...
//1softpro pok +++ Закрытие и открытие кассовой смены в регистре сведений ШМ_Смены при автоматическом закрытии и открытии кассовой смены ККТ из-за ошибки 24 часа 
&НаСервере
Процедура ЗакрытьСменуВРегистре() Экспорт          
	
	Смены = РегистрыСведений.ШМ_Смены.СоздатьНаборЗаписей(); 
	Смены.Отбор.СменаЗакрыта.Установить(Ложь);    
	Смены.Отбор.ИмяКомпьютера.Установить(МенеджерОборудованияВызовСервера.ПолучитьРабочееМестоКлиента().Наименование);
	
	Для Каждого стр из Смены Цикл   
		стр.СменаЗакрыта = Истина;
	КонецЦикла;                   
	...
	Запрос.Текст = "ВЫБРАТЬ Первые 1
	               |	ШМ_СменыСрезПоследних.НомерСмены КАК НомерСмены
	               |ИЗ
	...
	Смены = РегистрыСведений.ШМ_Смены.СоздатьНаборЗаписей();
	Смена = Смены.Добавить();   
	
	Смена.НомерСмены = Выборка[0].НомерСмены;
	Смена.Период = ТекущаяДата();
	Смена.ИмяКомпьютера = МенеджерОборудованияВызовСервера.ПолучитьРабочееМестоКлиента().Наименование;
