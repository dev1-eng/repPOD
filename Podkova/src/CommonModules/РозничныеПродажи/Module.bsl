...
Процедура ТаблицаТоваровСРаспределениемКодовМаркировкиИСМП(Товары, ДанныеДляИСМП) Экспорт
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Для каждого СтрокаДляДобавленияВТовары из МассивСтрокДляДобавленияВТовары Цикл
			...
			Если КоэффициентПересчетаУпаковки > 1
				И ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "Упаковка")
				И ЗначениеЗаполнено(СтрокаТЧ.Упаковка) Тогда                
				
				РеквизитыУпаковки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаТЧ.Упаковка, "Числитель, Знаменатель");
			...
			Если ЭтоМаркируемаяПродукция И ЗначениеЗаполнено(СтрокаДляДобавленияВТовары.Упаковка) Тогда
				СтрокаТоваровСРаспределением.УпаковкаНаименование = СтрокаДляДобавленияВТовары.Упаковка;
			КонецЕсли;
...
Процедура ПодписатьГорячиеКлавишиНаКнопках(Форма) Экспорт
	...
	Для каждого Команда Из Форма.Команды Цикл
		...
		Если СтрНайти(Команда.Заголовок, ПредставлениеСочетанияКлавиш) = 0 Тогда
			...
			МассивСтрок.Добавить(Команда.Заголовок);
			МассивСтрок.Добавить(ПредставлениеСочетанияКлавиш);
			Команда.Заголовок = СтрСоединить(МассивСтрок, " ");
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьДанныеПродажЗаПериод(ПараметрыРасчета) Экспорт
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютеРегл КАК СуммыДокументовВВалютеРегл
...
Функция ПолучитьДанныеИсправленийПродажЗаПериод(ПараметрыРасчета) Экспорт
	...
	|		ЧекККМТовары.Номенклатура.ТипНоменклатуры,
	|		ЧекККМТовары.СтавкаНДС,
	|		НАЧАЛОПЕРИОДА(ЧекККМТовары.Ссылка.ДатаИсходнойПродажи, МЕСЯЦ)) КАК РозничныеПродажи
	|
	|СГРУППИРОВАТЬ ПО
	|	РозничныеПродажи.Организация,
	|	РозничныеПродажи.Период,
