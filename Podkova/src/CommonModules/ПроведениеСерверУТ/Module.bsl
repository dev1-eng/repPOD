...
Процедура СформироватьЗаписиРегистровЗаданий(Объект) Экспорт
	...
	ВзаиморасчетыСервер.ОтразитьЗаданияКРаспределениюРасчетовСКлиентами(Объект.Ссылка, Объект.ДополнительныеСвойства);
	ВзаиморасчетыСервер.ОтразитьЗаданияКРаспределениюРасчетовСПоставщиками(Объект.Ссылка, Объект.ДополнительныеСвойства);
	...
	ПартионныйУчетСервер.ОтразитьЗаданияКРасчетуСебестоимости(Объект.Ссылка, Объект.ДополнительныеСвойства);
	ЗакрытиеМесяцаСервер.ОтразитьЗаданияКЗакрытиюМесяца(Объект.Ссылка, Объект.ДополнительныеСвойства);
	УчетНДСУП.ОтразитьЗаписиРегистраЗаданий(Объект.Ссылка, Объект.ДополнительныеСвойства);
...
Процедура СообщитьОбОшибкахПроведенияПриВозвратеТоваровОтХранителей(Объект, Отказ, РезультатЗапроса)
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструПереданнаяВозвратнаяТара(Объект, Отказ, РезультатЗапроса)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.НомерГТД) Тогда
			...
			АналитикаПередачиСтрока = СтрЗаменить(АналитикаПередачиСтрока, "%НомерГТД%",  Строка(Выборка.НомерГТД));
			АналитикаПередачи.Добавить(АналитикаПередачиСтрока);
		КонецЕсли;
