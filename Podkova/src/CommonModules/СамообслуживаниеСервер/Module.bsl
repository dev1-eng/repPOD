...
Процедура ОформитьЗаказ(СтруктураЗаказа, Проводить = Истина) Экспорт
	...
	Для Каждого СтрокаТовара Из ДокументЗаказ.Товары Цикл
	...
	Если ДокументЗаказ.НеОтгружатьЧастями Тогда
		МаксимальнаяДата = ОбеспечениеСервер.МаксимальноеЗначениеВКоллекции(ДокументЗаказ.Товары, "ДатаОтгрузки", '00010101');
		ДокументЗаказ.ДатаОтгрузки = ?(ЗначениеЗаполнено(МаксимальнаяДата), МаксимальнаяДата, ДокументЗаказ.ДатаОтгрузки);
		ОбеспечениеСервер.ЗаполнитьРеквизитВКоллекции(ДокументЗаказ.Товары, "ДатаОтгрузки", ДокументЗаказ.ДатаОтгрузки);
	КонецЕсли;
...
Процедура УправлениеЭлементомФормыСклад(Объект, Данные, Элемент) Экспорт
	...
	НеПолучатьГруппыСкладов = НЕ ПолучитьФункциональнуюОпцию("ИспользоватьСкладыВТабличнойЧастиДокументовПродажи")
	                          ИЛИ ((ТипЗнч(Объект) = Тип("ФормаКлиентскогоПриложения") И Объект.ИмяФормы = "ПланОбмена.ОбменССайтом.Форма.ФормаУзла"))
