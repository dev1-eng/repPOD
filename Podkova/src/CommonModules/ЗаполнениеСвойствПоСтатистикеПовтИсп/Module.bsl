...
#Область ЗаказНаВнутреннееПотребление
	Если ОбъектМетаданных = Метаданные.Документы.ЗаказНаВнутреннееПотребление Тогда
		
		СтруктураСинонимов.Вставить("Автор", "Ответственный");
		
		СтруктураНезаполняемыхРеквизитов.Вставить("ПотреблениеДляДеятельности");
		
	КонецЕсли;
#КонецОбласти
#Область ЗаказНаПеремещение
	Если ОбъектМетаданных = Метаданные.Документы.ЗаказНаПеремещение Тогда
		...
		ЗаполнениеСвойствПоСтатистикеСервер.ДобавитьЗаполняемыеРеквизиты(ДополнительныеРеквизиты,
			"СкладПолучатель, ОрганизацияПолучатель");
		
	КонецЕсли;
#КонецОбласти
#Область ЗаказНаСборку
	Если ОбъектМетаданных = Метаданные.Документы.ЗаказНаСборку Тогда
		
		СтруктураНезаполняемыхРеквизитов.Вставить("СборкаПодДеятельность");
...
#Область ОтчетКомиссионера
	Если ОбъектМетаданных = Метаданные.Документы.ОтчетКомиссионера Тогда
		...
		СтруктураСинонимов.Вставить("БанковскийСчетОрганизации", "БанковскийСчет");
			...
			ЗаполнениеСвойствПоСтатистикеСервер.ДобавитьЗаполняемыеРеквизиты(ДополнительныеРеквизиты,
			"Услуга, СпособРасчетаВознаграждения, ПроцентВознаграждения");
		
		ЗаполнениеСвойствПоСтатистикеСервер.ДобавитьЗаполняемыеРеквизиты(ДополнительныеРеквизиты,
			"СтавкаНДСВознаграждения", "","","Организация");
...
Функция ЗначениеРеквизитаПоСтатистике(
	...
	Запрос.Текст = СтрШаблон(Запрос.Текст, Формат(РазмерВыборки, "ЧГ=0"), ИмяРеквизита, ИмяОбъектаМетаданных,
		?(ЗначениеЗаполнено(СсылкаНаОбъект), "И НЕ ТаблицаОбъекта.Ссылка = &Ссылка", ""),
