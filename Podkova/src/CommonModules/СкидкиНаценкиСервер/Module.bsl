...
// Параметры:
//  Объект - ДанныеФормыСтруктура - Объект.
//  ДополнительныеПараметры - Структура - возможность определить имена реквизитов.
//
// Возвращаемое значение:
...
Функция РассчитатьПоРеализацииТоваровУслугПоЗаказу(Объект) Экспорт
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		ТекСуммаАвтоматическойСкидки = Окр(ВыборкаТовары.ЗаказСуммаАвтоматическойСкидки * (ВыборкаТовары.РеализацияКоличество / ВыборкаТовары.ЗаказКоличество) * ВыборкаТовары.ВалютаКоэффициентПересчета, 2);
		ТекСуммаРучнойСкидки 		 = Окр(ВыборкаТовары.ЗаказСуммаРучнойСкидки * (ВыборкаТовары.РеализацияКоличество / ВыборкаТовары.ЗаказКоличество) * ВыборкаТовары.ВалютаКоэффициентПересчета, 2);
	...
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		...
		Если (НЕ Объект.ИспользоватьОплатуБонуснымиБаллами) ИЛИ ИспользуетсяОдновременноеСписаниеИНачисление Тогда
			...
			Для Каждого СтрокаНачислениеБаллов Из НайденныеСтроки Цикл
				...
				НоваяСтрока.СуммаБонусныхБаллов = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(НоваяСтрока.СуммаБонусныхБаллов,
...
//
// Параметры:
//  Форма  - ФормаКлиентскогоПриложения - форма, для которой заполняются строковые переменные, содержащие информацию о количестве
//                              скидок (наценок)
...
//
// Параметры:
//  Источник  - СправочникСсылка.Склады, СправочникСсылка.ВидыКартЛояльности, СправочникСсылка.СоглашенияСКлиентом -
...
//	ТаблицаЗначений - Расшифровка скидок.
//
Функция СкидкиНаценкиДляОптовойТорговли(ТекущаяДата, Склад, Соглашение, КартаЛояльности, Параметры = Неопределено)
	...
	Если ТипЗнч(Параметры) = Тип("Структура") Тогда
		Если Параметры.Свойство("НеИспользоватьСоглашенияСКлиентами") Тогда
...
//
Процедура ЗаполнитьКлючиСвязиВТабличнойЧастиТовары(Объект, ИмяТЧ, РеализацияСверхЗаказа = Ложь)
...
Функция КонвертироватьСуммуВБонусы(СтрокаДерева, Сумма, ПараметрыРасчета) Экспорт
	...
	Если СтрокаДерева.СпособПримененияСкидки = Перечисления.СпособыПримененияСкидокНаценок.НачислитьБонусныеБаллы Тогда
		...
		Значение = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Сумма * СтрокаДерева.КурсКонвертацииБонусовВВалюту,
...
// Параметры:
//  Объект - ДокументОбъект, ДанныеФормыСтруктура - Объект, в котором требуется рассчитать скидки (наценки).
//  ИмяТЧ  - имя табличной части для расчета скидок
//  ПримененныеСкидки - Структура со свойствами:
//   Структура - Структура со свойствами:
//    * ДеревоСкидок - ДеревоЗначений - Дерево скидок (наценок).
//    * ТаблицаСкидкиНаценки - ТаблицаЗначений - Таблица с рассчитанными скидками.
...
//  РассчитыватьСуммуВзаиморасчетов
...
Процедура ПередЗаписью(ТекущийОбъект, Отказ) Экспорт
	...
	Если ТипЗнч(НастройкиКомпоновкиДанных) = Тип("Структура") Тогда
		...
		Если НастройкиКомпоновкиДанных.Свойство("Запрос", ЗапросНастройкиКомпоновкиДанных) Тогда
				...
				Если ЗапросНастройкиКомпоновкиДанных.Свойство("ТекстЗапроса", ТекстЗапросаНастройкиКомпоновкиДанных) И 
					 ...
					 Для каждого ПараметрЗапроса из ЗапросДляПроверкиПараметров.Параметры Цикл
						 Если Не ПараметрыЗапросаНастройкиКомпоновкиДанных.Свойство(ПараметрЗапроса.Ключ) Тогда
							 Отказ = Истина;
							 ТекстИсключения = НСтр("ru = 'В реквизите ХранилищеНастроекКомпоновкиДанных коллекция параметров не соответствует параметрам текста запроса'");
							 Прервать;
						 КонецЕсли;	 
					 КонецЦикла;	 
			...
			Иначе	
				Отказ = Истина;	
				ТекстИсключения = НСтр("ru = 'В реквизите ХранилищеНастроекКомпоновкиДанных нет данных о запросе'");
			КонецЕсли;
		КонецЕсли;
...
Функция РассчитатьСуммуНДС(Сумма, СтавкаНДС, Объект)
	
	Если ТипЗнч(Объект) = Тип("ДанныеФормыСтруктура")
		И Объект.Свойство("Ссылка")
		И ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.КоммерческоеПредложениеКлиенту")
