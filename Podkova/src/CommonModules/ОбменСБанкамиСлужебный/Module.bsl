...
Процедура ПроверитьВозможностьПередачиДанныхПлатежногоПоручения(
	...
	Если ЗначениеЗаполнено(КодВидаДохода) Тогда
		Если (ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
			...
			ШаблонОшибки = НСтр("ru = 'В соответствии с указаним Банка России № 5286-У в платежах физическим лицам необходимо указывать Код вида дохода.
									...
									|Ваш банк либо использует устарешую версию формата, либо нужно получить настройки обмена из банка.
		...
		ИначеЕсли ПрограммаБанка = Перечисления.ПрограммыБанка.АльфаБанкОнлайн Тогда
			ТекстОшибки = НСтр("ru = 'В соответствии с указаним Банка России № 5286-У в платежах физическим лицам необходимо указывать Код вида дохода.
		...
		Если ЗначениеЗаполнено(ДатаСписано) И ЗначениеЗаполнено(ИдПлатежа) Тогда
			...
			Если ЗначениеЗаполнено(ПлатежныйДокумент) Тогда
				...
				Если ЗначениеЗаполнено(ШтампБанкаСтатус) Тогда
					...
					ШтампБанкаГород = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаТЧ, "ШтампБанкаГород");
					...
					ДанныеШтампа.Вставить("ГородБанка", ШтампБанкаГород);
