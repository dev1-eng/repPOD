...
// Значение Истина будет установлена в том случае, если существует номенклатурная позиция, в которой
...
Процедура УстановитьФОИспользуетсяУчетВЕдиницеИзмеренияТНВЭД() Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	МАКСИМУМ(Номенклатура.ЕдиницаИзмеренияТНВЭД <> Номенклатура.ЕдиницаИзмерения
	|			И Номенклатура.ЕдиницаИзмеренияТНВЭД <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)) КАК ЕстьРазницаЕдиниц
	|ПОМЕСТИТЬ ВтТовары
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВтТовары.ЕстьРазницаЕдиниц КАК ЕстьРазницаЕдиниц,
	|	ВтТовары.ЕстьРазницаЕдиниц <> ИспользуетсяУчетВЕдиницеИзмеренияТНВЭД.Значение КАК ТребуетсяПерезаписьКонстанты
	|ИЗ
	|	ВтТовары КАК ВтТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Константа.ИспользуетсяУчетВЕдиницеИзмеренияТНВЭД КАК ИспользуетсяУчетВЕдиницеИзмеренияТНВЭД
	|		ПО (ИСТИНА)
	|ГДЕ
	|	НЕ ВтТовары.ЕстьРазницаЕдиниц ЕСТЬ NULL";
	
	УстановитьПривилегированныйРежим(Истина);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий()
		И Выборка.ТребуетсяПерезаписьКонстанты Тогда
		Константы.ИспользуетсяУчетВЕдиницеИзмеренияТНВЭД.Установить(Выборка.ЕстьРазницаЕдиниц);
	КонецЕсли;
...
// 		ПутьКПолюОтбора - Строка - Полный путь к реквизиту "ВедетсяУчетПоРНПТ",
//										если он отличается от "Объект.Товары.ВедетсяУчетПоРНПТ".
// 		ПутьКПолюДатаДокумента - Строка - Полный путь к реквизиту "Дата",
//										если он отличается от "Объект.Дата".
//
...
Процедура УстановитьВидимостьКоличестваРНПТ(Форма,
	...
	Форма.Элементы[ЭлементГруппаРНТП].Видимость = ИспользоватьУчетПрослеживаемыхИмпортныхТоваров
													И ИспользуетсяУчетВЕдиницеИзмеренияТНВЭД;
	//-- Локализация
