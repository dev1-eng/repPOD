...
Функция ТекстЗапросаПоЗаказам()
	...
	|			И МАКСИМУМ(ЗаказыПоставщикам.ПорядокРасчетов) = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
...
// ИмяДокумента - Строка - имя документа в метаданных, для которго осуществляется проверка
//
// Возвращаемое значение:
// Булево
// Истина, если в переданных заказах отличаются реквизиты шапки и нельзя проводить документ с данными заказами.
//
Функция ПроверитьСовместимостьЗаказовВОдномДокументе(Знач МассивСсылок, ИмяДокумента) Экспорт
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияГФУ Тогда
