...
//  * Наличие              - Число  - наличие товара на складах (0 - по запросу, 1 - мало, 2 - достаточно, 3 - много).
...
Процедура ИнициализацияЗапросаПубликуемыхТоваров(Запрос) Экспорт
	...
	|			{(&Период)},
	|			НЕ Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|				И ИСТИНА В
	|					(ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|							ПрайсЛисты КАК ПрайсЛисты
	|						ГДЕ
	|							ПрайсЛисты.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))) {(Номенклатура).* КАК Номенклатура, (Характеристика).* КАК Характеристика}) КАК СвободныеОстатки
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Склад
	|;
	...
	|			ГДЕ
	|				ПозицииТорговыхПредложенийСОстатками.Номенклатура = ЦеныНоменклатуры.Номенклатура
	...
	|	ПозицииТорговыхПредложенийСОстатками.ВидЦены КАК ВидЦены,
	...
	|	КОНЕЦ КАК ЕдиницаИзмерения,
	|	ПозицииТорговыхПредложенийСУточнениями.Номенклатура.Наименование КАК НаименованиеНоменклатуры,
	...
	|	ВЫРАЗИТЬ(ПозицииТорговыхПредложенийСУточнениями.Номенклатура.Описание КАК СТРОКА(250)) КАК ОписаниеНоменклатуры,
	|	ПозицииТорговыхПредложенийСУточнениями.Номенклатура.СтавкаНДС КАК СтавкаНДС,
...
Процедура ОпределитьВладельцевНезаполненныхРеквизитов(ПроверяемыйРеквизит, СтрокаТорговогоПредложения, ДополнительныйРеквизит, СсылкаНаОбъект, ПутьКДанным, ТекстСообщения) Экспорт 
	...
	ИначеЕсли ПроверяемыйРеквизит = "Контакты.ЭлектроннаяПочта" Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(СтрокаТорговогоПредложения.ПрайсЛист.Менеджер) Тогда
			СсылкаНаОбъект  = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТорговогоПредложения.ПрайсЛист, "Менеджер");
			ПутьКДанным     = СтрШаблон("КонтактнаяИнформацияПоле%1%2", 
				СтрЗаменить(Справочники.ВидыКонтактнойИнформации.EmailПользователя.УникальныйИдентификатор(), "-", "x"), 1);
		КонецЕсли;		
		ТекстСообщения = НСтр("ru = 'Не заполнен адрес электронной почты менеджера соглашения'");
