...
//
Процедура СохранитьПользовательскиеИзмененияВДокументахИсточниках(ТаблицаСИзменениями, Отказ) Экспорт
	
	ТаблицаИсточники = ТаблицаСИзменениями.Скопировать(,"Источник");
	ТаблицаИсточники.Свернуть("Источник");
	
	Для Каждого СтрокаТЧ Из ТаблицаИсточники Цикл
		
		Если Метаданные.Документы.Содержит(СтрокаТЧ.Источник.Метаданные()) Тогда
			
			Отбор = Новый Структура("Источник", СтрокаТЧ.Источник);
			НайденныеСтроки = ТаблицаСИзменениями.НайтиСтроки(Отбор);
			
			НачатьТранзакцию();
			
			Попытка
				Блокировка = Новый БлокировкаДанных();
				ЭлементБлокировки = Блокировка.Добавить(СтрокаТЧ.Источник.Метаданные().ПолноеИмя());
				ЭлементБлокировки.УстановитьЗначение("Ссылка", СтрокаТЧ.Источник);
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
				Блокировка.Заблокировать();
				
				ДокументОбъект = СтрокаТЧ.Источник.ПолучитьОбъект();
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Коммерческие предложения.Сохранения настроек'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,
					, СтрокаТЧ.Источник, НСтр("ru = 'Сохранение настроек запроса коммерческих предложений'"));
...
//      для печатных форм которого определяются получатели,
...
Процедура ПослеДобавленияСтрокИзЗапросаВКоммерческомПредложениеКлиенту(Форма, МассивИдентификаторовДобавленныхСтрок) Экспорт
	...
	ПараметрыЗаполненияСтавкиНДС.Вставить("НалогообложениеНДС", 
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт 
	...
	ИначеЕсли Форма.ИмяФормы = "Документ.КоммерческоеПредложениеКлиенту.Форма.ФормаСписка" Тогда
		...
		КоммерческоеПредложениеКлиентуФормаСпискаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка);
...
Процедура ПриОпределенииНастроекУчета(Настройки) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоВалют") Тогда
		...
		Если Выборка.Количество() = 1 Тогда 
			...
			Настройки.ЕдинственнаяВалюта = Выборка.Ссылка;
	...
	Настройки.СтатусыКоммерческихПредложенийКлиентамДействует.Добавить(Перечисления.СтатусыКоммерческихПредложенийКлиентам.Действует);
	Настройки.СтатусыКоммерческихПредложенийКлиентамЧерновик.Добавить(Перечисления.СтатусыКоммерческихПредложенийКлиентам.Черновик);
...
Процедура ПриФормированииЗаголовкаПечатнойФормы(Макет, ОбластьМакета, ДанныеОбъекта) Экспорт
	...
	ФормированиеПечатныхФорм.ВывестиЛоготипВТабличныйДокумент(Макет, ОбластьМакета, "Заголовок", ДанныеОбъекта.Организация);
...
Процедура ПриКопированииСтрокВБуферОбмена(Форма, ТабличнаяЧасть, ВыделенныеСтроки, ИмяТабличнойЧасти, СтандартнаяОбработка) Экспорт
	...
	ИменаКолонок.Вставить("ЕдиницаИзмерения", "Упаковка");
	ИменаКолонок.Вставить("Количество",       "КоличествоУпаковок");
	
	КопированиеСтрокСервер.ПоместитьВыделенныеСтрокиВБуферОбмена(ВыделенныеСтроки, ТабличнаяЧасть, ИменаКолонок);
...
Процедура КоммерческоеПредложениеКлиентуПриВыполненииПереопределяемойКоманды(Форма, ИмяКоманды, РезультатОбработки)
	...
	Если ИмяКоманды = "ПоказатьСообщения" Тогда
		...
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ИмяРеквизитаПартнер", "Клиент");
		РезультатОбработки = СкидкиНаценкиСервер.СтруктураСообщений(Форма.Объект, ДополнительныеПараметры);
...
Процедура КоммерческоеПредложениеКлиентуРассчитатьСкидки(Форма, РезультатОбработки)
	...
	ПараметрыРасчета.Вставить("ПрименятьКОбъекту",                Истина);
	...
	ПараметрыРасчета.Вставить("УправляемыеСкидки",                Форма.УправляемыеСкидки);
	ПараметрыРасчета.Вставить("ИмяКолонкиУпаковка",               "ЕдиницаИзмерения");
	ПараметрыРасчета.Вставить("ИмяКолонкиКоличествоУпаковок",     "Количество");
...
Процедура КоммерческоеПредложениеКлиентуПриНазначенииРучнойСкидкиНаСервере(Форма, РезультатОбработки)
	...
	РезультатОбработки = СкидкиНаценкиСервер.АдресДанныхДляРасчетаРучныхСкидокВоВременномХранилище(Форма, "Товары", Форма.УникальныйИдентификатор, Истина);
...
Процедура КоммерческоеПредложениеКлиентуПослеНазначенияРучнойСкидкиНаСервере(Форма, ВыделенныеСтроки, РезультатОбработки, ДополнительныеПараметры)
	...
	ПараметрыСкидки.Вставить("АдресВоВременномХранилище",        ДополнительныеПараметры.АдресВоВременномХранилище);
	ПараметрыСкидки.Вставить("ИмяКолонкиКоличество",             "Количество");
	
	СкидкиНаценкиСервер.НазначитьРучнуюСкидку(Объект, "Товары", РезультатОбработки, ПараметрыСкидки);
...
Процедура КоммерческоеПредложениеКлиентуПриНазначенииАвтоматическихСкидокНаСервере(Форма, Результат)
	...
	ПараметрыРасчета.Вставить("ТолькоПредварительныйРасчет",      Истина);
	ПараметрыРасчета.Вставить("ВосстанавливатьУправляемыеСкидки", Истина);
	ПараметрыРасчета.Вставить("УправляемыеСкидки",                Форма.УправляемыеСкидки);
	ПараметрыРасчета.Вставить("ИмяКолонкиУпаковка",               "ЕдиницаИзмерения");
	ПараметрыРасчета.Вставить("ИмяКолонкиКоличествоУпаковок",     "Количество");
...
Функция РезультатРасчетаСкидок(Форма, ПараметрыРасчета, ВзятьИзВременногоХранилища = Ложь)
	...
	Иначе
		ПримененныеСкидки = СкидкиНаценкиСервер.Рассчитать(Объект, ПараметрыРасчета);
	...
	РезультатОбработки = СкидкиНаценкиСервер.СтруктураСообщений(Объект, ДополнительныеПараметры);
	СкидкиНаценкиСервер.НастроитьКомандуПоказатьСообщения(Объект, Элементы.ПоказатьСообщения,,ДополнительныеПараметры);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "КоммерческиеПредложенияДокументыУТ.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.4.9.25";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("4f9c4794-0067-4d38-9db0-f2f90911f6a4");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "КоммерческиеПредложенияДокументыУТ.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЗапускатьТолькоВГлавномУзле = Истина;
	Обработчик.Комментарий = НСтр("ru = 'Обновление документа ""Коммерческое предложение клиенту""'");
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(Метаданные.БизнесПроцессы.СогласованиеПродажи.ПолноеИмя());
	Читаемые.Добавить(Метаданные.Документы.ЗаказКлиента.ПолноеИмя());
	Читаемые.Добавить(Метаданные.Документы.КоммерческоеПредложениеКлиенту.ПолноеИмя());
	...
	Изменяемые.Добавить(Метаданные.БизнесПроцессы.СогласованиеПродажи.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.Документы.КоммерческоеПредложениеКлиенту.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.Задачи.ЗадачаИсполнителя.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.РегистрыСведений.СостоянияКоммерческихПредложенийКлиентам.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.КоммерческоеПредложениеКлиенту.ПолноеИмя());
...
Процедура КоммерческиеПредложенияКлиентуЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	
	ПараметрыВыборки = Параметры.ПараметрыВыборки;
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.КоммерческоеПредложениеКлиенту";
	...
	|	Документ.КоммерческоеПредложениеКлиенту.Товары КАК КоммерческоеПредложениеКлиентуТовары
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(КоммерческоеПредложениеКлиентуТовары.СтавкаНДС) = ТИПзначения(&ПустаяСтавкаНДС)";
...
Процедура КоммерческиеПредложенияКлиентуОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ИспользоватьУчетНДС И ДокументОбъект <> Неопределено Тогда
...
Процедура КоммерческиеПредложенияКлиентуЗарегистрироватьДокументыДляПереводаНаВерсиюБЭД(Параметры) Экспорт
...
Процедура КоммерческиеПредложенияКлиентуОбработатьДокументыДляПереводаНаВерсиюБЭД(Параметры) Экспорт
	
	ИнициализироватьПараметрыОбработкиДляПереходаНаНовуюВерсию(Параметры);
	
	МетаданныеОбъекта = Метаданные.Документы.КоммерческоеПредложениеКлиенту;
	...
	КоммерческиеПредложенияСсылки = Новый Массив;
	
	Пока Выборка.Следующий() Цикл
		КоммерческиеПредложенияСсылки.Добавить(Выборка.Ссылка);
	КонецЦикла;
	
	Контрагенты = КонтрагентыПоДокументам(КоммерческиеПредложенияСсылки);
	
	Для Каждого ДокументСсылка Из КоммерческиеПредложенияСсылки Цикл
		...
		Попытка
			...
			Если ДокументОбъект = Неопределено Тогда
				ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(ДокументСсылка);
				ЗафиксироватьТранзакцию();
				Продолжить;
			КонецЕсли;
			...
			ЗаполнитьДокументКоммерческоеПредложениеКлиенту(ДокументОбъект, Контрагенты);
			
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ДокументОбъект);
			
			// Завершение бизнес процессов
			
			ЗавершитьСогласованиеПродажПоДокументу(ДокументОбъект.Ссылка);
			...
			Если ЗначениеЗаполнено(Состояние) Тогда
				...
				НаборЗаписей.Отбор.КоммерческоеПредложениеКлиенту.Установить(ДокументОбъект.Ссылка);
				
				Запись = НаборЗаписей.Добавить();
				Запись.КоммерческоеПредложениеКлиенту = ДокументОбъект.Ссылка;
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru = 'Не удалось записать документ %1 по причине:'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ДокументСсылка) + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				МетаданныеОбъекта, ДокументСсылка, ТекстСообщения);
	...
	Если ОбработанныхОбъектов = 0 И ПроблемныхОбъектов <> 0 Тогда
		ШаблонСообщения = НСтр("ru = 'Не удалось обработать некоторые коммерческие предложения контрагенту (пропущены): %1'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ШаблонСообщения = НСтр("ru = 'Обработана очередная порция коммерческих предложений клиенту: %1'");
...
Процедура ЗавершитьСогласованиеПродажПоДокументу(ДокументСсылка)
	...
	"ВЫБРАТЬ
	|	СогласованиеПродажи.Ссылка КАК БизнесПроцессСсылка,
	|	ЗадачаИсполнителя.Ссылка КАК ЗадачаСсылка
	|ИЗ
	|	БизнесПроцесс.СогласованиеПродажи КАК СогласованиеПродажи
	|		ЛЕВОЕ СОЕДИНЕНИЕ Задача.ЗадачаИсполнителя КАК ЗадачаИсполнителя
	|		ПО СогласованиеПродажи.Ссылка = ЗадачаИсполнителя.БизнесПроцесс
	|			И (НЕ ЗадачаИсполнителя.Выполнена)
	|ГДЕ
	|	НЕ СогласованиеПродажи.Завершен
	|	И СогласованиеПродажи.Предмет = &ДокументСсылка
	|ИТОГИ ПО
	|	БизнесПроцессСсылка";
	...
	Пока Выборка.Следующий() Цикл
		...
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("БизнесПроцесс.СогласованиеПродажи");
		ЭлементБлокировки.УстановитьЗначение("Ссылка", Выборка.БизнесПроцессСсылка);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
				
		БизнесПроцессОбъект = Выборка.БизнесПроцессСсылка.ПолучитьОбъект();
		
		БизнесПроцессОбъект.Завершен = Истина;
		
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(БизнесПроцессОбъект);
		
		ДетальныеЗаписи = Выборка.Выбрать();
		
		Пока ДетальныеЗаписи.Следующий() Цикл
			
			Если Не ЗначениеЗаполнено(ДетальныеЗаписи.ЗадачаСсылка) Тогда
				Продолжить;
			КонецЕсли;
		
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("Задача.ЗадачаИсполнителя");
			ЭлементБлокировки.УстановитьЗначение("Ссылка", ДетальныеЗаписи.ЗадачаСсылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			Блокировка.Заблокировать();
			
			ЗадачаОбъект = ДетальныеЗаписи.ЗадачаСсылка.ПолучитьОбъект();
...
Процедура ЗаполнитьДокументКоммерческоеПредложениеКлиенту(ДокументОбъект, Контрагенты)
	
	ДокументОбъект.Клиент                  = ДокументОбъект.УдалитьПартнер;
	ДокументОбъект.МожетВыкупатьсяЧастично = Истина;
	
	Если Не ЗначениеЗаполнено(ДокументОбъект.СрокДействия) Тогда
		ДокументОбъект.СрокДействия = КонецГода(ДобавитьМесяц(ДокументОбъект.Дата, 12));
	КонецЕсли;
	
	// Условие доставки
	
	УсловияДоставкиТекст = "";
	
	Если ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.Самовывоз Тогда
		УсловияДоставкиТекст = НСтр("ru = 'Самовывоз (с нашего склада)'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.ДоКлиента Тогда
		УсловияДоставкиТекст = НСтр("ru = 'Наша транспортная служба до клиента'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчикаПоАдресу Тогда
		УсловияДоставкиТекст = НСтр("ru = 'Наша транспортная служба до перевозчика'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчика Тогда
		УсловияДоставкиТекст = НСтр("ru = 'Перевозчик (с нашего склада)'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.КПолучателюОпределяетСлужбаДоставки Тогда
		УсловияДоставкиТекст = НСтр("ru = 'На усмотрение транспортной службы'");
	Иначе
		УсловияДоставкиТекст = ДокументОбъект.УдалитьСпособДоставки;	
	КонецЕсли;
	
	ДокументОбъект.УсловияДоставкиТекст = УсловияДоставкиТекст;
	
	// Хозяйственная операция
	
	Если ДокументОбъект.УдалитьХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию Тогда
	...
	СтрокаСКонтрагентом = Контрагенты.Найти(ДокументОбъект.Ссылка, "КоммерческоеПредложение");
	
	Если СтрокаСКонтрагентом <> Неопределено Тогда
		ДокументОбъект.Контрагент = СтрокаСКонтрагентом.Контрагент;
	КонецЕсли;
	
	// Источник условий оплаты
	
	ИсточникУсловийОплаты = Неопределено;
	
	Если ЗначениеЗаполнено(ДокументОбъект.УдалитьСоглашение) Тогда
		ИсточникУсловийОплаты = ДокументОбъект.УдалитьСоглашение;
	ИначеЕсли ЗначениеЗаполнено(ДокументОбъект.УдалитьГрафикОплаты) Тогда	
		ИсточникУсловийОплаты = ДокументОбъект.УдалитьГрафикОплаты;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ИсточникУсловийОплаты) Тогда
		ДокументОбъект.УсловияОплатыТекст = 
			КоммерческиеПредложенияДокументыУТВызовСервера.УсловияОплатыТекстом(ИсточникУсловийОплаты);
	КонецЕсли;
	
	// Дополнительная информация
	
	Если ЗначениеЗаполнено(ДокументОбъект.УдалитьДополнительнаяИнформация) Тогда
		ШаблонHTML = НСтр("ru = '<HTML>%1</HTML>'");
		ДокументОбъект.ДополнительнаяИнформацияКлиентуHTML = СтрШаблон(ШаблонHTML, ДокументОбъект.УдалитьДополнительнаяИнформация);
	КонецЕсли;
	
	// Вариант указания срока поставки
	
	ДокументОбъект.ВариантУказанияСрокаПоставки = ?(ЗначениеЗаполнено(ДокументОбъект.УдалитьСрокПоставки),
		Перечисления.ВариантыСроковПоставкиКоммерческихПредложений.УказываетсяВДняхСМоментаЗаказа,
		Перечисления.ВариантыСроковПоставкиКоммерческихПредложений.НеУказывается);
		
	// Статус	
		
	ЭтоЧерновик = (Не ДокументОбъект.Проведен 
		И ДокументОбъект.УдалитьСтатус = Перечисления.СтатусыКоммерческихПредложенийКлиентам.НеСогласовано)
			ИЛИ НЕ ЗначениеЗаполнено(ДокументОбъект.УдалитьСтатус);
		
	ДокументОбъект.Статус = ?(ЭтоЧерновик, Перечисления.СтатусыКоммерческихПредложенийКлиентам.Черновик, ДокументОбъект.УдалитьСтатус);
	
	ИспользоватьУчетНДС = ПолучитьФункциональнуюОпцию("ИспользоватьУчетНДС");

	// Налогообложение
	Если ИспользоватьУчетНДС Тогда
		ДокументОбъект.Налогообложение = Не (ДокументОбъект.УдалитьНалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС
			Или ДокументОбъект.УдалитьНалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД);
	КонецЕсли;

	// Товары	
	Для Каждого ТекущийТовар Из ДокументОбъект.Товары Цикл
		
		ТекущийТовар.Количество       = ТекущийТовар.УдалитьКоличествоУпаковок;
		ТекущийТовар.ЕдиницаИзмерения = ТекущийТовар.УдалитьУпаковка;
		
		Если ЗначениеЗаполнено(ДокументОбъект.УдалитьСрокПоставки) Тогда
			ТекущийТовар.СрокПоставки = ДокументОбъект.УдалитьСрокПоставки;
		КонецЕсли;
		
		Если Не ДокументОбъект.Проведен И Не ЗначениеЗаполнено(ТекущийТовар.Номенклатура) Тогда
			ТекущийТовар.КомментарийДляПокупателя = ТекущийТовар.УдалитьТекстовоеОписание;
		КонецЕсли;
...
Функция КонтрагентыПоДокументам(СсылкиНаДокументы)
	...
	|	МАКСИМУМ(ЕСТЬNULL(ЗаказКлиента.Контрагент, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка))) КАК Контрагент
	|ПОМЕСТИТЬ КонтрагентыПоЗаказам
	|ИЗ
	|	Документ.КоммерческоеПредложениеКлиенту КАК КоммерческоеПредложениеКлиенту
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента КАК ЗаказКлиента
	|		ПО КоммерческоеПредложениеКлиенту.Ссылка = ЗаказКлиента.ДокументОснование
	|ГДЕ
	|	КоммерческоеПредложениеКлиенту.Ссылка В(&Ссылки)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КонтрагентыПоЗаказам.КоммерческоеПредложение КАК КоммерческоеПредложение,
	|	КонтрагентыПоЗаказам.Контрагент КАК Контрагент
	|ИЗ
	|	КонтрагентыПоЗаказам КАК КонтрагентыПоЗаказам
	|ГДЕ
	|	КонтрагентыПоЗаказам.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	КонтрагентыПоЗаказам.КоммерческоеПредложение,
	|	МАКСИМУМ(Контрагенты.Ссылка)
	|ИЗ
	|	КонтрагентыПоЗаказам КАК КонтрагентыПоЗаказам
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
	|		ПО КонтрагентыПоЗаказам.КоммерческоеПредложение.УдалитьПартнер = Контрагенты.Партнер
	|ГДЕ
	|	КонтрагентыПоЗаказам.Контрагент = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("Ссылки", СсылкиНаДокументы);
...
Процедура ИнициализироватьПараметрыОбработкиДляПереходаНаНовуюВерсию(Параметры)
	...
	Если Параметры.ПрогрессВыполнения.ВсегоОбъектов = 0 Тогда
		...
		Выборка = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, "Документ.КоммерческоеПредложениеКлиенту", ПараметрыВыборки);
