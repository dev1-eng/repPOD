...
Процедура ЗаполнитьДанныеПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, Форма = Неопределено) Экспорт
	...
	Если Не ЭтоПроверкаКодовМаркировкиИСМП Тогда
	...
	Если ЭтоПроверкаКодовМаркировкиИСМП
		...
		Справочники.ШтрихкодыУпаковокТоваров.ЗаполнитьШтрихкодыУпаковокПоДеревуМаркируемойПродукции(
...
Процедура ДополнитьТаблицуДанныеКодовМаркировкиПоСтрокамДереваУпаковок(ДанныеКодовМаркировки, ДеревоУпаковок, ДобавленныеСтроки = Неопределено, ПараметрыСканирования = Неопределено, ИменаКолонокДляКопированияСвойств = Неопределено, ЭтоЗагрузкаИзТСД = Ложь) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Иначе
			...
			Входящий = (СтрокаДерева.Родитель = Неопределено Или СтрокаДерева.Входящий);
...
Функция ДанныеПоШтрихкодуУжеПолучены(ДанныеШтрихкода)
	...
	Возврат (ДанныеШтрихкода.Свойство("Номенклатура") И ЗначениеЗаполнено(ДанныеШтрихкода.Номенклатура))
		Или (ДанныеШтрихкода.Свойство("МаркируемаяПродукция") И ДанныеШтрихкода.МаркируемаяПродукция);
...
Функция ОбработатьДанныеПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки, ФормаУникальныйИдентификатор, РезультатыПроверок) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		ИначеЕсли ДанныеПоШтрихкодам.УточнитьУПользователя.Найти(ДанныеШтрихкода.Штрихкод) <> Неопределено
			...
			И СтрокаТаблицы.СтрокаДерева.Строки.Количество()>0 Тогда
...
Процедура ЗаполнитьПоДокументуИОснованию(ДанныеПоШтрихкодам, ПараметрыСканирования, Форма)
	...
	МодульСпецификиИСМП = МодульСпецификиИСМП();
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ИнтеграцияИСКлиентСервер.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции) Тогда
