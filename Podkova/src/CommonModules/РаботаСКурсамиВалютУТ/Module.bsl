...
Функция ПересчитатьСуммуДокументаВВалюту(СуммаДокумента, ТекущаяВалюта, НоваяВалюта, Дата) Экспорт
	...
	НоваяСуммаДокумента = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(СуммаДокумента, СтруктураКурсовТекущейВалюты, СтруктураКурсовНовойВалюты);
...
Функция ПолучитьКроссКурсВалют(Валюта1, Валюта2) Экспорт
	...
	Если Валюта2.Курс = 0 Тогда
		КурсПриведенный = 0;
	Иначе
		КурсПриведенный = Валюта1.Курс / Валюта2.Курс;
	КонецЕсли;
	Если Валюта2.Кратность = 0 Тогда
		КратностьПриведенная = 0;
	Иначе
