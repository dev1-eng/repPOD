	...
	Если НеобходимРасчет Тогда
		...
		Если ЗначениеЗаполнено(Организация) Тогда
			...
			Комментарий = СтрЗаменить(Комментарий,
				"%Организация%", 
				РасчетСебестоимостиПрикладныеАлгоритмы.ПредставлениеОрганизаций(Организация));
		КонецЕсли;
...
Функция ДанныеДляПартийПрочих(НачалоПериода, ОкончаниеПериода, МассивОрганизаций)
	...
	ИсходныйЗапрос.УстановитьПараметр("ОрганизацииСФИФОСкользящая",
		РегистрыСведений.УчетнаяПолитикаОрганизаций.ОрганизацииСЗаданнымПараметромПолитики(
...
Процедура КорректировкаНачальныхОстатковРегистраСебестоимостьТоваров(НачалоПериода, МассивОрганизаций, ПартионныйУчетВключен)
	...
	ПараметрыРасчета = РасчетСебестоимостиПрикладныеАлгоритмы.ИнициализироватьОсновныеПараметрыРасчета(НачалоПериода, МассивОрганизаций);
...
Процедура ЗавершитьКорректировкуОстатковРегистра(ПараметрыРасчета, ПараметрыКорректировки) Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ОтборПоТипуЗаписиНеКорректировки",
		?(ПараметрыКорректировки.ОписаниеРегистра.ЕстьТипЗаписи, "И НЕ (Т.ТипЗаписи В (&ТипыЗаписейКонвертацииДанных))", ""));
...
Процедура ЗаполнитьСтатьюИАналитикуСписанияНДС(РасчетнаяПартия, Расход, Приход)
	...
	Если ПараметрыУчетнойПолитики.ВариантУчетаНДСПриИзмененииВидаДеятельности = Перечисления.ВариантыУчетаНДСПриИзмененииВидаДеятельностиНаНеоблагаемую.ВключатьВСтоимость Тогда
		Возврат;
	КонецЕсли;
	...
	Если УчетНДСУП.ВозможноСписаниеНДС(РасчетнаяПартия.НалогообложениеПартии, РасчетнаяПартия.НалогообложениеНДС)
	 ИЛИ (РасчетнаяПартия.ВидДвижения = ВидДвиженияНакопления.Приход
	  И УчетНДСУП.ВозможноСписаниеНДС(Приход.НалогообложениеПартии, Приход.НалогообложениеНДС)) Тогда
		...
		Если ПараметрыУчетнойПолитики.ВариантУчетаНДСПриИзмененииВидаДеятельности = Перечисления.ВариантыУчетаНДСПриИзмененииВидаДеятельностиНаНеоблагаемую.ВключатьВСтоимостьИлиРасходыВЗависимостиОтПериода Тогда
		...
		Если УчетНДСУП.ВозможноСписаниеНДС(РасчетнаяПартия.НалогообложениеПартии, РасчетнаяПартия.НалогообложениеНДС) Тогда
...
Функция ЕстьФИФОСкользящая(НачалоПериода, ОкончаниеПериода, МассивОрганизаций)
	...
	РегистрыСведений.УчетнаяПолитикаОрганизаций.СформироватьВТУчетныеПолитикиОрганизаций(Запрос, МассивОрганизаций, НачалоПериода);
	
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДД.МетодОценкиСтоимостиТоваров
	|ИЗ
	|	ВТУчетныеПолитикиОрганизаций КАК ДД
	|ГДЕ
...
Процедура ВременнаяТаблицаДанныхПоОтбору(ПолеОтбора, ТекстСортировка, Таблица, МВТ)
	...
	ПараметрыРасчета.Вставить("ВерсияПлатформыНеНиже8_3_13", РасчетСебестоимостиПрикладныеАлгоритмы.ВерсияПлатформыНеНижеУказанной("8.3.13.0"));
