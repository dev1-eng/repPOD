...
Процедура ЗаполнитьШапкуИсходящейТранспортнойОперации(ДокументОбъект, ДанныеЗаполнения, Запрос)
	...
	Если Не ЭтоПерезаполнение Тогда
		...
		ИначеЕсли ТранспортныеНакладные.КоличествоТранспортныхНакладных = 1 Тогда
			...
			Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТранспортнаяНакладная,"Номер,Дата");
		...
		Если ЗначениеЗаполнено(ДокументОбъект.ГрузоотправительПредприятие) И ДокументОбъект.Маршрут.Количество() = 0 Тогда
			...
			Если ЗначениеЗаполнено(ДанныеПеревозки.АдресПогрузки) Тогда
...
Процедура ЗаполнитьВнутреннееПотреблениеТоваровНаОснованииИнвентаризацииПродукцииВЕТИС (ДокументОбъект, ДанныеЗаполнения) Экспорт
...
Процедура ЗаполнитьСоответствиеШапкиДляИнвентаризацииПродукцииВЕТИС(ДокументОснование, ДокументОбъект)
	...
	РеквизитыВЕТИС = Реквизиты(ДокументОбъект,
		...
		ЗаписьДокументаВЕТИС);
	ТипОснования = ТипЗнч(ДокументОснование);
	Если ТипОснования = Тип("ДокументСсылка.ВнутреннееПотреблениеТоваров")
		Или ТипОснования = Тип("ДокументОбъект.ВнутреннееПотреблениеТоваров")
