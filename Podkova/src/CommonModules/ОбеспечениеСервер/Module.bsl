...
// Определяет даты обеспечения обособленных потребностей, зафиксированных заказом исходя из обеспечения наличием,
// ожидаемыми поступлениями и плановых дат поставок Параметры:
...
//                                                                              для отображения состояния обеспечения
//                                                                              наличием в таблице формы.
...
//   ОформитьПоНакладным       - ТаблицаЗначений - таблица содержащая количество, на которое уже оформлена накладная по
//                                                 строкам документа.
//   ТаблицаНевыделенныеСтроки - ТаблицаЗначений - используется если необходимо предварительно учитывать данные
//                                                 редактируемого документа.
//   Ссылка                    - ДокументСсылка  - используется если необходимо предварительно сторнировать движения
//                                                 редактируемого документа.
// 
// Возвращаемое значение:
...
//   * ДатаОтгрузкиВДокументе - Дата   - дата отгрузки, должна быть указана, если все позиции заказа планируется
...
// Обрабатывает данные обеспечения, полученные механизмами обеспечения, в зависимости от операции их получения и
// параметров документа, который эти данные получает. Параметры:
//   ДанныеЗаполнения - ДанныеФормыКоллекция, ТаблицаЗначений - данные поставляемые механизмом обеспечения (обычно
//       коллекция или таблица с вариантами обеспечения количеством и датой отгрузки).
...
//    * Товары - ТаблицаЗначений - Таблица содержащая товарные позиции и склады, для которых необходимо получить даты
//      поставки. Состав колонок:
//      ** Номенклатура   - СправочникСсылка.Номенклатура               - номенклатура.
//      ** Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - характеристика.
//      ** Склад          - СправочникСсылка.Склады                     - склад.
//    * Работы - ТаблицаЗначений - Таблица содержащая товарные позиции, для которых необходимо получить даты поставки.
//      Состав колонок:
//      ** Номенклатура   - СправочникСсылка.Номенклатура               - номенклатура.
//      ** Характеристика - СправочникСсылка.ХарактеристикиНоменклатуры - характеристика.
//
...
//   * ДатаДоступности - Дата - дата плановой поставки, пустая дата, если схемы обеспечения и/или способы обеспечения не
//                              настроены.
//
...
// Формирует временную таблицу параметров обеспечения с рассчитанными минимальным и максимальным количествами запаса
// для всех методов обеспечения.
//
// Параметры:
//   МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - менеджер содержащий таблицу ВтТовары.
//
// Возвращаемое значение:
//   МенеджерВременныхТаблиц - менеджер содержащий таблицу ВтПараметрыОбеспеченияТоваров.
//
Функция СформироватьВтПараметровОбеспеченияТоваров(МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.УстановитьПараметр("НачалоТекущегоДня", НачалоДня(ТекущаяДатаСеанса()));
	УстановитьПараметрыЗапросаРасчетаСтатистики(Запрос);
	...
	ДобавитьВременнуюТаблицуСтатистикиПотребления(Запрос, "ВтТоварыПоСтатистике");
...
// см. ТекстЗапросаСтатистикиПотребления().
//
// Параметры:
...
// Заполняет вариант обеспечения в таблице значением по умолчанию.
// Используется, например, при копировании заказов, заполнении заказов на основании других документов.
// 
// Параметры:
...
//   СтатусЗаказаКлиента  - ПеречислениеСсылка.СтатусыЗаказовКлиентов - используется для расчета варианта обеспечения
//       при включенной ФО ИспользоватьПострочнуюОтгрузкуВЗаказеКлиента.
// 
// Возвращаемое значение:
...
// Возвращает текст ошибки при создании накладных на основании заказов, если в заказах не указано Отгрузить или
// Отгрузить обособленно.
//
// Возвращаемое значение:
...
//   Статус - ПеречислениеСсылка.СтатусыВнутреннихЗаказов, ПеречислениеСсылка.СтатусыЗаказовНаПеремещение,
...
// 
// Параметры:
//   ИмяТаблицыТоваров - Строка - Имя временной таблицы с товарами для которых необходимо рассчитать доступный остаток.
//                                Временная таблица имеет следующие поля: "Номенклатура", "Характеристика", "Склад" - товар.
//   ИмяТаблицыРезультата - Строка - Имя временной таблицы, для помещения результата расчета. Временная таблица имеет
//                                   следующие поля: "Номенклатура", "Характеристика", "Склад" - товар, "Период",
//                                   "Количество" - количество доступного товара и дата, в которую это количество будет доступно.
...
Функция ДоступныеОстаткиСамообслуживание(ИмяТаблицыТоваров, ИмяТаблицыРезультата, ИмяПараметраНачалоДня = "НачалоТекущегоДня") Экспорт
	...
	// Если для товара нет свободного количества на складе и нет свободного количества в ожидаемых поступлениях, то
	// доступно ноль.
...
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Таблица колонки которой соответствуют ключам структуры. Таблица содержит одну строку, заполненную
//                    по данным структуры.
//
...
//                          Элементы массива - ссылки на документы, которым необходимо установить требуемый статус.
...
Процедура ДобавитьВременнуюТаблицуСезонныеКоэффициенты(Запрос)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРасширенноеОбеспечениеПотребностей")
		...
		Запрос.Текст =
			...
			|	ВЫБОР
			|		КОГДА СУММА(ВЫБОР
			|					КОГДА СезонныеКоэффициенты.НомерПериода <= ДЕНЬГОДА(&ПериодКлассификацииДатаОкончания) ТОГДА
			...
			|					0
			|			КОНЕЦ)
			|	КОНЕЦ * &КоличествоПериодовКлассификации КАК Коэффициент
			...
			|			И ВЫБОР
			|				КОГДА ГОД(&ПериодКлассификацииДатаНачала) <> ГОД(&РасчетныйПериодДатаОкончания)
			|					ТОГДА СезонныеКоэффициенты.НомерПериода >= 1
			|								И СезонныеКоэффициенты.НомерПериода <= ДЕНЬГОДА(&РасчетныйПериодДатаОкончания)
			|							ИЛИ СезонныеКоэффициенты.НомерПериода >= ДЕНЬГОДА(&ПериодКлассификацииДатаНачала)
			|								И СезонныеКоэффициенты.НомерПериода <= 365
			|				ИНАЧЕ СезонныеКоэффициенты.НомерПериода >= ДЕНЬГОДА(&ПериодКлассификацииДатаНачала)
			|						И СезонныеКоэффициенты.НомерПериода <= ДЕНЬГОДА(&РасчетныйПериодДатаОкончания)
			|			КОНЕЦ
			|СГРУППИРОВАТЬ ПО
			|	Группы.СезоннаяГруппа,
			|	Группы.СезоннаяГруппаБизнесРегиона
			|ИНДЕКСИРОВАТЬ ПО
			...
			|	Товары.Номенклатура                       КАК Номенклатура,
			|	Товары.Склад                              КАК Склад,
			|	СезонныеКоэффициентыПоГруппам.Коэффициент КАК Коэффициент
			|ПОМЕСТИТЬ СезонныеКоэффициенты
			|ИЗ
			|	РазличныеТоварыИСклады КАК Товары
			|		
			|		ЛЕВОЕ СОЕДИНЕНИЕ СезонныеКоэффициентыПоГруппам КАК СезонныеКоэффициентыПоГруппам
			|		ПО СезонныеКоэффициентыПоГруппам.СезоннаяГруппа = Товары.Номенклатура.СезоннаяГруппа
			|		 И СезонныеКоэффициентыПоГруппам.СезоннаяГруппаБизнесРегиона
			|			= ВЫБОР КОГДА Товары.Склад.БизнесРегион.СезоннаяГруппа <> ЗНАЧЕНИЕ(Справочник.СезонныеГруппыБизнесРегионов.ПустаяСсылка) ТОГДА
			|					Товары.Склад.БизнесРегион.СезоннаяГруппа
			|				ИНАЧЕ
			|					ЗНАЧЕНИЕ(Справочник.СезонныеГруппыБизнесРегионов.Прочие)
			|			КОНЕЦ
			|ГДЕ
			|	НЕ СезонныеКоэффициентыПоГруппам.СезоннаяГруппа ЕСТЬ NULL
			|ИНДЕКСИРОВАТЬ ПО
			|	Номенклатура, Склад
			|;
			|
...
Функция ДополнительныеСвойстваДляЗаписиНабораОбеспечениеЗаказов(ДополнительныеСвойства) Экспорт
	
	// Инициализация свойств набора записей регистра "Обеспечение заказов", который пишется при записи регистра Движение товаров.
	РассчитыватьИзменения = Неопределено;
	Если Не ДополнительныеСвойства.Свойство("РассчитыватьИзменения", РассчитыватьИзменения) Тогда
		РассчитыватьИзменения = Истина;
	КонецЕсли;
	
	ДляПроведения = Новый Структура("СтруктураВременныеТаблицы", ДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы);
	
	ДопСвойства = Новый Структура();
	ДопСвойства.Вставить("РассчитыватьИзменения", РассчитыватьИзменения);
	ДопСвойства.Вставить("ЭтоНовый", ДополнительныеСвойства.ЭтоНовый);
	ДопСвойства.Вставить("ДляПроведения", ДляПроведения);
