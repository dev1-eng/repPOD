...
Функция ПараметрыОперацииФискализацииЧекаДокументоваПоставкиСВидамиЗапасов(ДокументСсылка, СуммаПредоплатыКорректировка, ВерсияФФД = Неопределено)
	...
	Для Каждого СтрокаТаблицаДокументовПоставки Из ТаблицаДокументовПоставки Цикл
		...
		Если ТипЗнч(ДокументПоставки) = Тип("ДокументСсылка.АктВыполненныхРабот")
			ИЛИ ТипЗнч(ДокументПоставки) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			Запрос = Новый Запрос;
			Запрос.Текст =
			"ВЫБРАТЬ ПЕРВЫЕ 1
		...
		ФискальнаяОперацияПоставкиДанныеЖурнала = МенеджерОборудованияВызовСервера.ДанныеФискальнойОперации(ДокументПоставки);
		
		ПризнакСпособаРасчета = Перечисления.ПризнакиСпособаРасчета.ОплатаКредита;
		
		Если ФискальнаяОперацияПоставкиДанныеЖурнала = Неопределено Тогда
...
Процедура ЗаполнитьПозицииИТаблицуОплатЧека(ПараметрыФискализацииЧека, ДанныеДокумента, СуммаПредоплатыКорректировка, ВерсияФФД)
	...
	|	ДанныеДокумента.Ссылка = &Ссылка
	|	И ДанныеДокумента.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОплатыКлиенту)
	|	И ДанныеДокумента.ОснованиеПлатежа = НЕОПРЕДЕЛЕНО
	|	И ТИПЗНАЧЕНИЯ(ДанныеДокумента.Заказ) В (&ТипыЗаказы)
	|	
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДокумента.Ссылка,
	|	ДанныеДокумента.Заказ,
	...
	|	И (ДанныеДокумента.Ссылка НЕ В (ВЫБРАТЬ Расшифровка.Ссылка ИЗ Расшифровка)
	|		ИЛИ ДанныеДокумента.Заказ = &Ссылка
	|		ИЛИ ТИПЗНАЧЕНИЯ(ДанныеДокумента.Заказ) = ЗНАЧЕНИЕ(Документ.ПоступлениеБезналичныхДенежныхСредств))
	|	
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДокумента.Заказ,
	|	ДанныеДокумента.СтавкаНДС
	...
	|///////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ДанныеДокумента.ОснованиеПлатежа.НалогообложениеНДС,
	...
	ТипыЗаказы = Новый Массив;
	ТипыЗаказы.Добавить(Тип("ДокументСсылка.ЗаказКлиента"));
	Запрос.УстановитьПараметр("ТипыЗаказы", ТипыЗаказы);
	...
	Если ПараметрыФискализацииЧека.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоход
		ИЛИ ПараметрыФискализацииЧека.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.УСНДоходРасход
		ИЛИ ПараметрыФискализацииЧека.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.ЕНВД
		ИЛИ ПараметрыФискализацииЧека.СистемаНалогообложения = Перечисления.ТипыСистемНалогообложенияККТ.Патент Тогда
...
// Процедура заполняет реквизит НалогообложениеНДС.
//
// Параметры:
//	Объект - ДанныеФормыСтруктура - Текущий объект;
//	НалогообложениеНСДПоУмолчанию - ПеречислениеСсылка.ТипыНалогообложенияНДС - Исходное значение налогообложения НДС для документа;
//	ЭлементНалогообложениеНДС - ПолеФормы - элемент формы для выбора налогообложения НДС.
//
Процедура ЗаполнитьНалогообложениеНДС(Объект, НалогообложениеНСДПоУмолчанию, ЭлементНалогообложениеНДС) Экспорт
	
	ТипДокумента = ТипЗнч(Объект.Ссылка);
	
	СписоХозяйственныхОперацийПродаж = ПолучитьХозяйственныеОперацииПродажДляДокумента(ТипДокумента);
	СписокХозяйственныхОперацийЗакупок = ПолучитьХозяйственныеОперацииЗакупкиДляДокумента(ТипДокумента);
	...
	Если НЕ СписоХозяйственныхОперацийПродаж.Найти(Объект.ХозяйственнаяОперация) = Неопределено Тогда
		...
		УчетНДСУП.ЗаполнитьНалогообложениеНДСПродажи(НалогообложениеНСДПоУмолчанию, ПараметрыЗаполнения, Неопределено);
		УчетНДСУП.ЗаполнитьСписокВыбораНалогообложенияНДСПродажи(ЭлементНалогообложениеНДС, НалогообложениеНСДПоУмолчанию, ПараметрыЗаполнения, Неопределено);
	...
	ИначеЕсли НЕ СписокХозяйственныхОперацийЗакупок.Найти(Объект.ХозяйственнаяОперация) = Неопределено Тогда
		...
		УчетНДСУП.ЗаполнитьНалогообложениеНДСЗакупки(НалогообложениеНСДПоУмолчанию, ПараметрыЗаполнения, Неопределено);
		УчетНДСУП.ЗаполнитьСписокВыбораНалогообложенияНДСЗакупки(ЭлементНалогообложениеНДС, НалогообложениеНСДПоУмолчанию, ПараметрыЗаполнения, Неопределено);
...
Процедура ОбновитьСтавкуНДСИПересчитатьСуммуНДС(Объект, ИдСтроки) Экспорт
	...
	СтруктураПараметровДействия.Вставить("НалогообложениеНДС", Объект.НалогообложениеНДС);
	СтруктураПараметровДействия.Вставить("Дата", Объект.Дата);
		
	Если НЕ ИдСтроки = Неопределено Тогда
		СтрокаПлатежа = Объект.РасшифровкаПлатежа.НайтиПоИдентификатору(ИдСтроки);
		
		Если НЕ ЗначениеЗаполнено(СтрокаПлатежа.Заказ) ИЛИ СтрокаПлатежа.Заказ = Объект.Ссылка Тогда
			СтруктураДействий = Новый Структура;
			СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС", СтруктураПараметровДействия);
			СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
			ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаПлатежа, СтруктураДействий, Неопределено);
		Иначе
			ДанныеПоНДС = ДенежныеСредстваСервер.РасшифровкаПлатежаНДС(Объект.Дата, Объект.Валюта, СтрокаПлатежа.Заказ, Ложь);
			Если ДанныеПоНДС.Количество() = 1 Тогда
				СтрокаПлатежа.СтавкаНДС = ДанныеПоНДС[0].СтавкаНДС;
				СтруктураДействий = Новый Структура;
				СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
				ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаПлатежа, СтруктураДействий, Неопределено);
			КонецЕсли;
...
Функция ПолучитьХозяйственныеОперацииПродажДляДокумента(ТипИсходногоДокумента)
	
	МассивХозяйственныхОпераций = Новый Массив();
	
	Если ТипИсходногоДокумента = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
	...
	ИначеЕсли ТипИсходногоДокумента = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
	...
	ИначеЕсли ТипИсходногоДокумента = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств") Тогда
		
		МассивХозяйственныхОпераций.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента);
		МассивХозяйственныхОпераций.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойОрганизации);
		
	ИначеЕсли ТипИсходногоДокумента = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда
		
		МассивХозяйственныхОпераций.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента);
		МассивХозяйственныхОпераций.Добавить(Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту);
		
	ИначеЕсли ТипИсходногоДокумента = Тип("ДокументСсылка.СписаниеБезналичныхДенежныхСредств") Тогда
		
		МассивХозяйственныхОпераций.Добавить(Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту);
		МассивХозяйственныхОпераций.Добавить(Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствВДругуюОрганизацию);
		
	ИначеЕсли ТипИсходногоДокумента = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
...
Функция ПолучитьХозяйственныеОперацииЗакупкиДляДокумента(ТипИсходногоДокумента)
...
Процедура ОчиститьНалогообложениеНДС(Объект)
	...
	Если ЗначениеЗаполнено(Объект.НалогообложениеНДС) Тогда
		...
		СписокДоступныОпераций = ПолучитьХозяйственныеОперацииПродажДляДокумента(ТипДокумента);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СписокДоступныОпераций, ПолучитьХозяйственныеОперацииЗакупкиДляДокумента(ТипДокумента));
		...
		Иначе
			ЕстьОбъектРасчетовТекущийДокумент = Ложь;
			Для каждого СтрокаПлатежа Из Объект.РасшифровкаПлатежа Цикл
				Если НЕ ЗначениеЗаполнено(СтрокаПлатежа.Заказ) ИЛИ СтрокаПлатежа.Заказ = Объект.Ссылка
					ИЛИ (ЗначениеЗаполнено(СтрокаПлатежа.Заказ) И ТипЗнч(СтрокаПлатежа.Заказ) = ТипДокумента) Тогда
					ЕстьОбъектРасчетовТекущийДокумент = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Если НЕ ЕстьОбъектРасчетовТекущийДокумент Тогда
				Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка();
			КонецЕсли;
		КонецЕсли;
