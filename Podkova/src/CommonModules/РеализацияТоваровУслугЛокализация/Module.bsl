...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|						КОНЕЦ
	|				ИНАЧЕ 
	|					ВЫБОР КОГДА ТаблицаТоваров.Количество < ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 0)
	|						ТОГДА ТаблицаТоваров.Количество
...
Функция ПолучитьДанныеДляПечатнойФормыУПД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|
	|	ВЫБОР КОГДА ДанныеДокумента.Грузоотправитель <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) ТОГДА
	...
	|	КОГДА ДанныеДокумента.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) ТОГДА
	...
	ШтрихкодыУпаковок         = МассивРезультатов[8].Выгрузить();
