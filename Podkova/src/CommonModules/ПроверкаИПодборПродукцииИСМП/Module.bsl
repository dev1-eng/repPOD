...
Процедура ВывестиИнформациюОПроблемахУПД(ОшибкиПоУПД, ВидПродукции) Экспорт
	...
	Если ОшибкиПоУПД.Свойство("СтрокиНеМаркируемойПродукции") Тогда
		...
		ВывестиИнформациюОПроблемахУПДПоТовару(ОшибкиПоУПД.СтрокиНеМаркируемойПродукции, ВидПродукцииРодительный, ШаблонСообщения, НаименованиеСправочника);
	...
	Если ОшибкиПоУПД.Свойство("СтрокиСОтсутствующимиВидамиПродукцииВДокументе") Тогда
		...
		Для Каждого СтрокаТоварнойГруппе Из ОшибкиПоУПД.СтрокиСОтсутствующимиВидамиПродукцииВДокументе Цикл
			ВывестиИнформациюОПроблемахУПДПоТовару(СтрокаТоварнойГруппе.Строки, СтрокаТоварнойГруппе.ВидПродукции, ШаблонСообщения, НаименованиеСправочника);
		КонецЦикла;
...
Процедура ВывестиИнформациюОПроблемахУПДПоТовару(СтрокиНоменклатураШтрихкод, ВидПродукции, ШаблонСообщения, НаименованиеСправочника, КоличествоВыводимыхШтрихкодов = 3)
	...
	Для Каждого Строка Из СтрокиНоменклатураШтрихкод Цикл
		ШтрихкодыНоменклатуры = НоменклатураИШтрихкоды[Строка.Номенклатура];
		Если ШтрихкодыНоменклатуры = Неопределено Тогда
			ШтрихкодыНоменклатуры = Новый Массив;
			НоменклатураИШтрихкоды[Строка.Номенклатура] = ШтрихкодыНоменклатуры;
			СписокНоменклатур.Добавить(Строка.Номенклатура);
		КонецЕсли;
		ШтрихкодыНоменклатуры.Добавить(Строка.Штрихкод);
	КонецЦикла;
	...
	Для Каждого ЭлементСписка Из СписокНоменклатур Цикл
		...
		ШтрихкодыНоменклатуры = НоменклатураИШтрихкоды[Номенклатура];
		
		ШтрихкодыСтрокой = ПолучитьПроблемныеШтрихкодыИзУПДСтрокой(ШтрихкодыНоменклатуры, КоличествоВыводимыхШтрихкодов);
...
Функция ПолучитьПроблемныеШтрихкодыИзУПДСтрокой(ШтрихкодыНоменклатуры, КоличествоВыводимыхШтрихкодов)
	
	ШтрихкодыСтрокой = "";
	
	Разделитель = "," + " ";
	Если ШтрихкодыНоменклатуры.Количество() <= КоличествоВыводимыхШтрихкодов Тогда
		
		ШтрихкодыСтрокой = СтрСоединить(ШтрихкодыНоменклатуры, Разделитель);
		
	Иначе
		
		Штрихкоды = Новый Массив;
		Для Сч = 0 По КоличествоВыводимыхШтрихкодов - 1 Цикл
			Штрихкоды.Добавить(ШтрихкодыНоменклатуры[Сч]);
		КонецЦикла;
		
		ШтрихкодыСтрокой = СтрШаблон(
			НСтр("ru = '%1 (+ еще %2)'"),
			СтрСоединить(Штрихкоды, Разделитель),
			ШтрихкодыНоменклатуры.Количество() - КоличествоВыводимыхШтрихкодов);
