...
Процедура ПодготовитьКПередачеСообщениеАбстрактнаяАгрегацияИзменениеРасчетСтатуса(СообщенияJSON, ОбщийЭлементОчереди, 
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииВводВОборотИмпортСФТС(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Иначе
		...
		Если СообщенияJSONТрансформация.Количество() Тогда
			ПодготовитьКПередачеСообщениеАбстрактнаяАгрегацияИзменениеРасчетСтатуса(СообщенияJSON, ЭлементОчереди, СообщенияJSONТрансформация);
		КонецЕсли;
...
Процедура ПодготовитьСообщенияПоАбстрактнойАгрегации(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Иначе
		...
		Для Каждого СтрокаДанныхОтчета Из Штрихкоды Цикл
			...
			Если ДанныеРазбора = Неопределено Тогда
				ВызватьИсключение СтрШаблон(
					НСтр("ru = 'Код маркировки %1 не соответствует виду продукции: %2'"),
					СтрокаДанныхОтчета.Штрихкод,
					ДанныеШапки.ВидПродукции);
			КонецЕсли;
	...
	Иначе
		...
		ПодготовитьКПередачеСообщениеАбстрактнаяАгрегацияИзменениеРасчетСтатуса(СообщенияJSON, ЭлементОчереди, СообщенияJSONОперацияУдаление, СообщенияJSONОперацияТрансформация);
