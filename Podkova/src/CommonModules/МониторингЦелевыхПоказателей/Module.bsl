...
Процедура ЗаполнитьСтруктуруЦелейИВариантыАнализа() Экспорт
	...
	КаталогВременныхФайлов = ОбщегоНазначения.СоздатьВременныйКаталог("kpitmp");
...
// Возвращаемое значение:
//
//   Структура     - Значение      - Число,  экстраполированное значение для смещения по оси Х, или неопределено, при
//                   ошибке в расчете.
...
Процедура УстановитьПользовательскиеНастройкиСерийДиаграммы(СерииДиаграммы, ИсточникДанных)
	...
	ИначеЕсли ТипАнализа = ТипыАнализа.ПокомпонентноеСравнение 
		...
		Если ТолькоЦветОсновнойСерии Тогда
			...
			ПолучитьКрасныйЗеленыйСинийВебЦвета(ЦветОсновнойСерии, ИсходныйКрасный, ИсходныйЗеленый, ИсходныйСиний);
			ПолучитьТонНасыщенностьЯркостьПоКрасныйЗеленыйСиний(ИсходныйКрасный, ИсходныйЗеленый, ИсходныйСиний, ИсходныйТон, ИсходнаяНасыщенность, ИсходнаяСветлота);
				...
				Если ЕстьГрадиент Тогда
					Если ПоВесуСерии Тогда
						...
						Если ВыделятьМаксимальноеЗначение И ВесСерии = ВесМаксимальнойСерии Тогда
							...
							ПолучитьКрасныйЗеленыйСинийПоТонуНасыщенностиЯркости(ИсходныйТон, 100, ИсходнаяСветлота + (100 - ИсходнаяСветлота) * (1 - КоэффициентВесаСерии), СерияКрасный, СерияЗеленый, СерияСиний);
							ПолучитьДополнительныйЦветПоКрасныйЗеленыйСиний(СерияКрасный, СерияЗеленый, СерияСиний, СерияКрасный, СерияЗеленый, СерияСиний);
						Иначе
							ПолучитьКрасныйЗеленыйСинийПоТонуНасыщенностиЯркости(ИсходныйТон, ИсходнаяНасыщенность * КоэффициентВесаСерии, ИсходнаяСветлота + (100 - ИсходнаяСветлота) * (1 - КоэффициентВесаСерии), СерияКрасный, СерияЗеленый, СерияСиний);
						КонецЕсли;
					...
					Иначе
						...
						Если МаксимумГрадаций > 1 Тогда
							...
							ПолучитьКрасныйЗеленыйСинийПоТонуНасыщенностиЯркости(ИсходныйТон, НовыйКоэффициентНасыщенности, НовыйКоэффициентСветлоты, 
								СерияКрасный, СерияЗеленый, СерияСиний);
						Иначе
							ПолучитьКрасныйЗеленыйСинийПоТонуНасыщенностиЯркости(ИсходныйТон, ИсходнаяНасыщенность, ИсходнаяСветлота, 
								СерияКрасный, СерияЗеленый, СерияСиний);
						КонецЕсли;
					...
					Если ЕстьГрадиент И ВыделятьМаксимальноеЗначение И СерияДиаграммы.Значение = ОбъектАнализаМаксимум Тогда
						Если СерииДиаграммы.Количество() > 1 Тогда
							...
							ИначеЕсли ЦветОсновнойСерии.Вид = ВидЦвета.WebЦвет Тогда
								ПолучитьКрасныйЗеленыйСинийВебЦвета(ЦветОсновнойСерии, ИсходныйКрасныйМаксСерии, ИсходныйЗеленыйМаксСерии, ИсходныйСинийМаксСерии);
							КонецЕсли;
							
							ПолучитьДополнительныйЦветПоКрасныйЗеленыйСиний(ИсходныйКрасныйМаксСерии, ИсходныйЗеленыйМаксСерии, ИсходныйСинийМаксСерии, 
...
Функция ИнициализироватьИсточникДанных(ВариантАнализа) Экспорт
	...
	ИсточникДанных.Вставить("ДинамическиеСвойстваВариантаАнализа", Неопределено);
	...
	ИсточникДанных.Вставить("СвойстваДанныхПоПериодам",          Неопределено);
	ИсточникДанных.Вставить("СвойстваДанныхПоПериодамСравнения", Неопределено);
	ИсточникДанных.Вставить("СвойстваДанныхПоОбъектамАнализа",   Неопределено);
	ИсточникДанных.Вставить("СвойстваПрогнозаДанныхПоПериодам",  Неопределено);
	ИсточникДанных.Вставить("СвойстваЗоныАнализаИТренда",        Неопределено);
	ИсточникДанных.Вставить("СвойстваОбщейСтатистики",           Неопределено);
	...
	СвойстваОбщейСтатистики.Вставить("ПоОбъектамАнализа", ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(СтруктураСтатистики));
