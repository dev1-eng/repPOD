...
Процедура СинхронизироватьСБанком(
	...
	РеквизитыНастройкиОбмена = Новый Структура("ПрограммаБанка, АутентификацияПоСертификату, ИспользуетсяКриптография,
		...
		|ДополнительнаяОбработка, ПравоВыполненияОбмена, ПодписываетсяЗапросВыписки");
	...
	Если РеквизитыНастройкиОбмена.ПрограммаБанка = СбербанкОнлайн Тогда
		Если РеквизитыНастройкиОбмена.ИспользуетсяКриптография Тогда
			...
			УстановитьСоединениеИАутентифицироватьсяНаСервереЧерезТокенСбербанк(Оповещение, НастройкаОбмена);
		Иначе
			РезультатОтправки = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияСинхронизацииСбербанк(НастройкаОбмена);
			...
			ВыполнитьОбработкуОповещения(ОповещениеЗапускаДлительнойОперации, РезультатОтправки.ИдентификаторЗадания);
		КонецЕсли;
	ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен") Тогда
		Если РеквизитыНастройкиОбмена.ПодписываетсяЗапросВыписки Тогда
			Оповещение = Новый ОписаниеОповещения(
		...
		ИначеЕсли РеквизитыНастройкиОбмена.АутентификацияПоСертификату Тогда
			...
			ПолучитьОтпечаткиСертификатов(Оповещение);
		ИначеЕсли ПарольКСертификатуПолучен(НастройкаОбмена, ДанныеАвторизации) Тогда
...
Процедура АутентификацияНаСервереПослеПолученияДанныхАутентификацииСбербанк(
	...
	Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияАутентификацииПоЛогинуСбербанк(
		ДополнительныеПараметры.НастройкаОбмена, Результат.Логин, Результат.Пароль, ДополнительныеПараметры.ФродПараметры);
...
Процедура ТихийСтартПослеПодключенияВКСбербанка(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ДополнительныеПараметры.ЭтоТест И НЕ ДополнительныеПараметры.ПринудительнаяАутентификация
		И ЗначениеИзКэшаСбербанк("АутентификацияНаТокенеВыполнена") = Истина
		И ЗначениеЗаполнено(ДополнительныеПараметры.НастройкаОбмена)
		И ЗначениеИзКэшаСбербанк("ТекущаяНастройкаОбмена") = ДополнительныеПараметры.НастройкаОбмена Тогда
		Оповещение = Новый ОписаниеОповещения(
...
Процедура ПослеАутентификацииПоЛогинуСбербанк(РезультатЗадания, ДополнительныеПараметры) Экспорт
	...
	Иначе // выполнено
		...
		ИначеЕсли ПараметрыВозврата.ТребуетсяСМСАутентификация Тогда
			ОбменСБанкамиСлужебныйВызовСервера.СохранитьСессиюНаСервереСбербанк(ДополнительныеПараметры.КлючСессии,
				ПараметрыВозврата.ИдентификаторСессии, ДополнительныеПараметры.ФродПараметры, ДополнительныеПараметры.Логин);
...
Процедура ОтправитьСообщениеВБанкСАутентификациейЛогинПароль(ПараметрыАутентификации, Параметры) Экспорт
	...
	Иначе
		...
		СформироватьИОтправитьПакетыВБанк(
			ОписаниеОповещения, МассивСообщенийОбменаКОтправке, СоотвНастроекОбменаИСтруктурСертификатов);
	КонецЕсли;
...
Процедура ОтправкаСообщенийОбмена(Параметры)
	...
	Если НЕ Параметры.Свойство("ПодтверждениеПлатежейВБанке") Тогда
		Параметры.Вставить("ПодтверждениеПлатежейВБанке", ОбменСБанкамиКлиентСервер.СтруктураПодтвержденияПлатежа());
	КонецЕсли;
	...
	ИначеЕсли СтруктураКОтправке.ЧерезДополнительнуюОбработку.Количество() Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ЗавершитьОтправкуСообщенийОбменаЧерезОбработку", ЭтотОбъект, Параметры);
	...
	ИначеЕсли СтруктураКОтправке.САутентификациейПоСертификату.Количество() Тогда
		Для Каждого ТекЭл Из СтруктураКОтправке.САутентификациейПоСертификату Цикл
			...
			Для Каждого ОтправляемоеСообщениеОбмена Из ТекЭл.Значение Цикл
				...
				Если ДокументПодписанПолностью Тогда
					Параметры.МассивКОтправке.Добавить(ОтправляемоеСообщениеОбмена);
					ЕстьСообщениеОбменаКОтправке = Истина;
				КонецЕсли;
			КонецЦикла;
			Прервать;
		КонецЦикла;
		СтруктураКОтправке.САутентификациейПоСертификату.Удалить(НастройкаОбмена);
		Если ЕстьСообщениеОбменаКОтправке Тогда
			...
			ДопПараметры.Вставить("СоотвСертификатовИИхСтруктур", Параметры.СоотвСертификатовИИхСтруктур);
...
Процедура ВыполнитьДействияПослеОтправки(Параметры)
	...
	ВывестиИнформациюОбОбработанныхЭД(ТекстЗаголовка, Параметры.КоличествоНовыхЭД, Параметры.КоличествоПодписанных,
...
Процедура ПродолжитьОтправкуСообщенийОбменаПослеРасшифровкиМаркера(Результат, Параметры) Экспорт
	...
	СформироватьИОтправитьПакетыВБанк(ОписаниеОповещения, Параметры.МассивКОтправке, СоотвНастроекОбменаИСтруктурСертификатов);
...
Процедура СформироватьИОтправитьПакетыВБанк(Оповещение, МассивСообщенийОбмена, ИдентификаторыСессий = Неопределено)
	
	РезультатОтправки = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияПоОтправкеДокументовВБанк(
		МассивСообщенийОбмена, ИдентификаторыСессий);
...
Процедура ПродолжитьОбработкуДокументовПослеПереопределения(МассивСсылокНаОбъект, ДополнительныеПараметры) Экспорт
	...
	ОбработатьЭлектронныеДокументы(МассивСсылокНаОбъект, ДополнительныеПараметры.Действия);
...
Процедура ВывестиИнформациюОбОбработанныхЭД(
	...
	Если КоличествоОтправленных > 0 И ПодтверждениеПлатежейВБанке.БанкиТребующиеПодтверждениеПлатежаВЛК <> Неопределено
		И ПодтверждениеПлатежейВБанке.БанкиТребующиеПодтверждениеПлатежаВЛК.Количество() Тогда
