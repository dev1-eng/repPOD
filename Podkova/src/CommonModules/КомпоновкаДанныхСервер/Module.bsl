...
//
// Параметры:
//		ПараметрыОтчета - Массив - представления неиспользуемых параметров отчета.
//
...
Процедура НастроитьДинамическийПериод(СхемаКомпоновкиДанных, КомпоновщикНастроек, ДополнятьПериод = Ложь, ДополнениеПути = "") Экспорт 
	...
	Если ПараметрПериодичность <> Неопределено 
		...
		Если ИскомоеПоле <> Неопределено Тогда
			...
			ИскомоеПоле.Выражение = ДополнениеПути + "Период" + СтрокаДлительностьПериода;
...
// Вызывается из отчета АнализСебестоимостиТоваровРасширенный.
//
// Параметры
...
Функция МодульЧисла(Число) Экспорт
	...
	Если Число <> Неопределено И Число <> NULL Тогда
...
//
// Параметры:
//  СхемаНабор   - СхемаКомпоновкиДанных, НаборыДанныхСхемыКомпоновкиДанных, НаборДанныхОбъединениеСхемыКомпоновкиДанных
//                 - схема или набор компоновки данных, в которой необходимо создать новый набор
...
//  * НастройкиКомпоновки - Строка - адрес во временном хранилище настроек компоновки
//							         или имя варианта настроек компоновки
...
//
// Параметры:
//  Параметры	 - структура - см. КомпоновкаДанныхСервер.ПараметрыВыгрузитьРезультатСКД
...
Функция ВыгрузитьРезультатСКД(Параметры, АдресВременногоХранилища = "") Экспорт
	...
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки   = КомпоновщикМакета.Выполнить(СхемаКомпоновки, НастройкиКомпоновкиПоПараметру(СхемаКомпоновки, Параметры.НастройкиКомпоновки), , ,
	...
	Иначе
		ПроцессорКомпоновки.Инициализировать(МакетКомпоновки, Параметры.ВнешниеНаборыДанных,,Параметры.ВозможностьИспользованияВнешнихФункций);
	КонецЕсли;
...
Функция СхемаКомпоновкиПоПараметру(Знач СхемаКомпоновки) Экспорт
	...
	ИначеЕсли ТипЗнч(СхемаКомпоновки) = Тип("СхемаКомпоновкиДанных") Тогда
		Возврат СхемаКомпоновки;
	Иначе
		ТекстИсключения = НСтр("ru = 'Неподдерживаемый тип параметра ""СхемаКомпоновки""'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
