...
Процедура ИнтернетПоддержкаИСервисы_ВключитьРаботуСНовостямиПриИзменении(Форма, Элемент) Экспорт
	...
	Если НЕ Форма.БИПВключитьРаботуСНовостями Тогда
		...
		МассивСтрок.Добавить(СтроковыеФункцииКлиентСервер.ФорматированнаяСтрока(НСтр("ru='Настоятельно <b>не рекомендуется</b> отключать общую опцию получения новостей.'")));
...
Процедура ПанельКонтекстныхНовостей_ЭлементПанелиНовостейОбработкаНавигационнойСсылки(
	...
	Если СтандартнаяОбработкаПодсистемой <> Ложь Тогда
		...
		Если Найти(ВРег(НавигационнаяСсылкаЭлемента), ВРег("news1C:Open?")) >= 1 Тогда // Ссылка на новость
			Описатель = "news1C:Open?";
			ГдеОписатель = Найти(ВРег(НавигационнаяСсылкаЭлемента), ВРег(Описатель));
...
#Область АнализГиперссылки
	...
	СтруктураГиперссылки = Новый Структура;
		СтруктураГиперссылки.Вставить("ЭтоВебКлиент"             , ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент());
	...
	Если (ДанныеСобытия.Anchor <> Неопределено) Тогда
		...
		Попытка
			BaseURI = ДанныеСобытия.Anchor.baseURI;
		Исключение
			BaseURI = "";
		...
		Если (BaseURI = Неопределено) ИЛИ ПустаяСтрока(BaseURI) Тогда
			BaseURI = "";
			Если СтруктураГиперссылки.ЭтоВебКлиент = Истина Тогда
				...
				Если ПараметрыСтрокиСоединения.Свойство("ws") Тогда
					BaseURI = ПараметрыСтрокиСоединения.ws;
		...
		Href = ДанныеСобытия.Anchor.Href;
		СтрокаДляПоиска = "about:blank";
		Если СтрНачинаетсяС(НРег(Href), СтрокаДляПоиска) Тогда
			Href = Прав(Href, СтрДлина(Href) - СтрДлина(СтрокаДляПоиска));
