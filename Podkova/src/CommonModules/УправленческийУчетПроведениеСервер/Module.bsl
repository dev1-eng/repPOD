			...
			Для Каждого Организация Из Документ.Строки Цикл
				...
				Иначе
					Для Каждого ОбъектРасчетов Из Организация.Строки Цикл
						Отбор.Вставить("ВладелецОбъектаРасчета", ОбъектРасчетов.ВладелецОбъектаРасчета);
						СтрокиКредита = ДокументыКредита.НайтиСтроки(Отбор);
						Если ЗначениеЗаполнено(СтрокиКредита) Тогда
							ВзаиморасчетыСервер.ПровестиВзаимозачет(Организация.Строки, СтрокиКредита, ТаблицаДвижений);
						КонецЕсли;
...
//
// Параметры:
//  СсылкаНаДокумент - ДокументСсылка - документ для которого необходимо сформировать и записать движения по управленческому балансу.
...
Функция ДанныеДляФормированияДвиженийДенежныеСредстваКонтрагент(Документ, Период, ТаблицыДляДвижений)
	...
	|				  (КурсВалютыВзаиморасчетов.Кратность * КурсВалютыПлатежа.Курс) КАК ЧИСЛО(31,2))
...
Процедура ПолучитьРасчетыСКлиентами(МенеджерВременныхТаблиц, СписокДокументов)
	...
	#Область ТекстЗапросаПоРасчетамСКлиентамиПоСрокам
		...
		|	ВЫБОР КОГДА РасчетыСКлиентами.Долг < 0
		|		ТОГДА -РасчетыСКлиентами.Долг
		|		ИНАЧЕ РасчетыСКлиентами.Долг
		|	КОНЕЦ КАК Долг,
		|	ВЫБОР КОГДА РасчетыСКлиентами.ДолгУпр < 0
		|		ТОГДА -РасчетыСКлиентами.ДолгУпр
		|		ИНАЧЕ РасчетыСКлиентами.ДолгУпр
		|	КОНЕЦ КАК ДолгУпр,
		|	ВЫБОР КОГДА РасчетыСКлиентами.ДолгРегл < 0
		|		ТОГДА -РасчетыСКлиентами.ДолгРегл
		|		ИНАЧЕ РасчетыСКлиентами.ДолгРегл
		|	КОНЕЦ КАК ДолгРегл,
		|	ВЫБОР КОГДА РасчетыСКлиентами.Предоплата < 0
		|		ТОГДА -РасчетыСКлиентами.Предоплата
		|		ИНАЧЕ РасчетыСКлиентами.Предоплата
		|	КОНЕЦ КАК Предоплата,
		|	ВЫБОР КОГДА РасчетыСКлиентами.ПредоплатаУпр < 0
		|		ТОГДА -РасчетыСКлиентами.ПредоплатаУпр
		|		ИНАЧЕ РасчетыСКлиентами.ПредоплатаУпр
		|	КОНЕЦ КАК ПредоплатаУпр,
		|	ВЫБОР КОГДА РасчетыСКлиентами.ПредоплатаРегл < 0
		...
		|		РегистрСведений.АналитикаУчетаПоПартнерам КАК Аналитика
		|	ПО
		|		РасчетыСПартнерами.АналитикаУчетаПоПартнерам = Аналитика.КлючАналитики
		|ГДЕ
		|	РасчетыСПартнерами.ДокументРегистратор В (&СписокДокументов)
...
Функция ТекстЗапросаРасчетыСПартнерами()
	...
	#Область ТекстЗапросаПоРасчетамСПартнерамиПоСрокам
		...
		|	ВЫБОР
		|		КОГДА РасчетыСПоставщиками.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И РасчетыСПоставщиками.ДолгРегл <> 0
		|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|		КОГДА РасчетыСПоставщиками.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) И РасчетыСПоставщиками.ДолгРегл <> 0
		...
		|
		|	РасчетыСПоставщиками.ХозяйственнаяОперация КАК ХозОперацияРегистра,
		...
		|			ПО РасчетыСПоставщиками.ДокументРегистратор = Т.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК КлючиАналитики
		...
		|		ИЛИ РасчетыСПоставщиками.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) 
		...
		|	ВЫБОР
		|		КОГДА РасчетыСКлиентами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И РасчетыСКлиентами.ПредоплатаРегл <> 0
		|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|		КОГДА РасчетыСКлиентами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) И РасчетыСКлиентами.ПредоплатаРегл <> 0
		|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|		ИНАЧЕ
		...
		|	КлючиАналитики.Партнер = ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие) КАК НашеПредприятие,
		|	ТИПЗНАЧЕНИЯ(КлючиАналитики.Контрагент) = ТИП(Справочник.Организации) КАК КонтрагентОрганизация,
		|	РасчетыСКлиентами.ХозяйственнаяОперация КАК ХозОперацияРегистра,
		|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВзаимозачетЗадолженности) КАК ХозяйственнаяОперация,
		|	РасчетыСКлиентами.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам
		|	
		|ИЗ
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ  втРегистраторы КАК Т
		|			ПО РасчетыСКлиентами.ДокументРегистратор = Т.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК КлючиАналитики
		...
		|			И РасчетыСКлиентами.ДолгРегл <> 0 
		|		ИЛИ РасчетыСКлиентами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) 
	...
	#Область ТекстЗапросаПоРасчетамСПартнерамиПоДокументам
		ТекстЗапросаРасчетыСПартнерами = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Движения.Регистратор КАК Ссылка
		|ПОМЕСТИТЬ втРегистраторы
...
//Платежные документы: ПоступлениеБДС, ПКО
		#Область РасходЗадолженностиФилиала
		// Поступление денег в головную организацию закрывает задолженость в филиале
		РасходЗадолженностиФилиала =  
		...
		|	РасчетыСПартнерами.Организация.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
		|	ЕСТЬNULL(ВозвратОплаты.Организация, РасчетыСПартнерами.Организация) КАК ВладелецОбъектаРасчета,
		|	РасчетыСПартнерами.НаправлениеДеятельности КАК НаправлениеДеятельности,
		...
		|	""РасходЗадолженностиФилиала"" КАК ИмяТаблицы
		|ИЗ
		|	РасчетыСПартнерами КАК РасчетыСПартнерами
		|	ЛЕВОЕ СОЕДИНЕНИЕ втВозвратОплаты КАК ВозвратОплаты
		...
		|	И РасчетыСПартнерами.Организация <> РасчетыСПартнерами.ОбъектРасчетов.Организация";
		#КонецОбласти

//Платежные документы: СписаниеБДС, РКО
		#Область РасходВГоловнойОрганизации
		РасходВГоловнойОрганизации = //в филиале это приход от головной организации
		...
		|	РасчетыСПартнерами.Сумма КАК КорСуммаВВалютеВзаиморасчетов,
		|	РасчетыСПартнерами.ОбъектРасчетов КАК КорИсточникГФУРасчетов,
		|	""РасходВГоловнойОрганизации"" КАК ИмяТаблицы
		|ИЗ
		|	РасчетыСПартнерами КАК РасчетыСПартнерами
		|ГДЕ
		...
		|	И РасчетыСПартнерами.Организация <> РасчетыСПартнерами.Организация.ГоловнаяОрганизация
		|	И РасчетыСПартнерами.Организация.ГоловнаяОрганизация <> РасчетыСПартнерами.ОбъектРасчетов.Организация";
		#КонецОбласти
...
//Возврат оплаты по курсу аванса: СписаниеБДС, РКО
		...
		|ГДЕ
		|	РасчетыСПартнерами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	И НЕ РасчетыСПартнерами.ХозОперацияРегистра В (
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика),
		|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОплатыКлиенту))
		|	И РасчетыСПартнерами.Организация = РасчетыСПартнерами.Организация.ГоловнаяОрганизация";
		#КонецОбласти
		...
		РасходЗадолженности.Добавить(РасходЗадолженностиФилиала);
		РасходЗадолженности.Добавить(РасходВГоловнойОрганизации);
...
//Платежные документы: СписаниеБДС, РКО
		#Область ПриходЗадолженностиВФилиале
		ПриходЗадолженностиВФилиале = 
		...
		|	РасчетыСПартнерами.Сумма КАК СуммаВВалютеВзаиморасчетов,
		|	РасчетыСПартнерами.ОбъектРасчетов КАК ИсточникГФУРасчетов,
		|	""ПриходЗадолженностиВФилиале"" КАК ИмяТаблицы
		|ИЗ
		|	РасчетыСПартнерами КАК РасчетыСПартнерами
		|ГДЕ
		|	РасчетыСПартнерами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
...
//Платежные документы: ПоступлениеБДС, ПКО
		#Область ПриходВГоловнойОрганизации
		ПриходВГоловнойОрганизации = //в филиале это расход в головную организацию
		...
		|	РасчетыСПартнерами.Сумма КАК СуммаВВалютеВзаиморасчетов,
		|	РасчетыСПартнерами.ОбъектРасчетов КАК ИсточникГФУРасчетов,
		|	""ПриходВГоловнойОрганизации"" КАК ИмяТаблицы
		|ИЗ
		|	РасчетыСПартнерами КАК РасчетыСПартнерами
		|ГДЕ
		|	РасчетыСПартнерами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
...
//Возврат по курсу аванса: ПоступлениеБДС, ПКО
		...
		ПриходЗадолженности.Добавить(ПриходЗадолженностиВФилиале);
		ПриходЗадолженности.Добавить(ПриходВГоловнойОрганизации);
...
Функция ТекстЗапросаДвиженияКонтрагентДоходыРасходы()
	...
	#Область ТекстЗапросаОбновить
		...
		|	КОНЕЦ КАК Сумма,
		|	ВЫБОР КОГДА ВложенныйЗапрос.СуммаУпр > 0
		|		ТОГДА ВложенныйЗапрос.СуммаУпр
		|		ИНАЧЕ -ВложенныйЗапрос.СуммаУпр
		|	КОНЕЦ КАК СуммаУпр,
		|	ВЫБОР КОГДА ВложенныйЗапрос.СуммаРегл > 0
		...
		|	ГДЕ
		|		ТИПЗНАЧЕНИЯ(Расчеты.ДокументРегистратор) = ТИП(Документ.СписаниеЗадолженности)
		|		И Расчеты.ДокументРегистратор В(&МассивДокументов)
		|	
		|	СГРУППИРОВАТЬ ПО
		...
		|		АналитикаПартнеров.НаправлениеДеятельности,
		|		Расчеты.ОбъектРасчетов,
		|		Расчеты.ХозяйственнаяОперация) КАК ВложенныйЗапрос";
		#КонецОбласти
	...
	#Область ТекстЗапросаКонтрагентДоходыРасходы
	
	ТекстЗапросаКонтрагентДоходыРасходы = 
	"ВЫБРАТЬ
	...
	|	
	|	ЕСТЬNULL(Расчеты.СуммаУпр - ЕСТЬNULL(РезервыПоДолгам.СуммаУпр, 0), КонтрагентДоходыРасходы.Сумма) КАК Сумма,
	|	КонтрагентДоходыРасходы.СуммаБезНДС КАК СуммаБезНДС,
	|	ЕСТЬNULL(Расчеты.СуммаРегл - ЕСТЬNULL(РезервыПоДолгам.СуммаРегл, 0), КонтрагентДоходыРасходы.СуммаРегл) КАК СуммаРегл,
	|	КонтрагентДоходыРасходы.СуммаРеглБезНДС КАК СуммаРеглБезНДС,
	|	
	|	КонтрагентДоходыРасходы.Валюта КАК Валюта,
	|	ЕСТЬNULL(Расчеты.Сумма - ЕСТЬNULL(РезервыПоДолгам.Сумма, 0), КонтрагентДоходыРасходы.СуммаВВалюте) КАК СуммаВВалюте,
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ втРезервыПоДолгам КАК РезервыПоДолгам
	|	ПО КонтрагентДоходыРасходы.Регистратор = РезервыПоДолгам.Регистратор
	|		И КонтрагентДоходыРасходы.Организация = РезервыПоДолгам.Организация
	|		И КонтрагентДоходыРасходы.Партнер = РезервыПоДолгам.Партнер
	|		И КонтрагентДоходыРасходы.Контрагент = РезервыПоДолгам.Контрагент
	|		И КонтрагентДоходыРасходы.Договор = РезервыПоДолгам.Договор
	|		И КонтрагентДоходыРасходы.НаправлениеДеятельностиКонтрагента = РезервыПоДолгам.НаправлениеДеятельности
	|		И КонтрагентДоходыРасходы.ОбъектРасчетов = РезервыПоДолгам.ОбъектРасчетов
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(КонтрагентДоходыРасходы.Регистратор) = ТИП(Документ.СписаниеЗадолженности)
	|	И КонтрагентДоходыРасходы.Регистратор В(&МассивДокументов)
	|	
	|УПОРЯДОЧИТЬ ПО
	|	Регистратор,
	|	НомерСтроки
	...
	Тексты = Новый Массив;
	Тексты.Добавить(ТекстЗапросаРасчеты);
	Тексты.Добавить(ТекстЗапросаКонтрагентДоходыРасходы);
