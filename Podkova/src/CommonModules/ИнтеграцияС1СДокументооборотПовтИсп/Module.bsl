...
Функция ШаблоныПроцессовОбъекта(ТипОбъекта) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	НастройкиЗаполнения.ЗначениеРеквизитаДО КАК Имя,
		|	НастройкиЗаполнения.ЗначениеРеквизитаДОID КАК ID,
		|	НастройкиЗаполнения.ЗначениеРеквизитаДОТип КАК Тип
		|ИЗ
		|	Справочник.ПравилаИнтеграцииС1СДокументооборотом.ПравилаЗаполненияРеквизитовДО КАК НастройкиЗаполнения
		|ГДЕ
		|	НастройкиЗаполнения.ИмяРеквизитаОбъектаДО = ""documentType""
		|	И НастройкиЗаполнения.ЗначениеРеквизитаДОТип ПОДОБНО ""%DocumentType""
		|	И НастройкиЗаполнения.Ссылка.ТипОбъектаИС = &ТипОбъекта";
	...
	Пока Выборка.Следующий() Цикл
		...
		ВидДокументаXDTO = ИнтеграцияС1СДокументооборот.СоздатьObjectID(Прокси, Выборка.ID, Выборка.Тип);
