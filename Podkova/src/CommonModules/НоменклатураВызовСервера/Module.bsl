...
Процедура СерииНоменклатурыОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ВидНоменклатуры") Тогда
		...
		|	СерииНоменклатуры.ПроизводительВЕТИС КАК ПроизводительВЕТИС,
		|	СерииНоменклатуры.ЗаписьСкладскогоЖурналаВЕТИС КАК ЗаписьСкладскогоЖурналаВЕТИС,
		|	СерииНоменклатуры.ИдентификаторПартииВЕТИС КАК ИдентификаторПартииВЕТИС,
		|	СерииНоменклатуры.МаксимальнаяРозничнаяЦенаМОТП КАК МаксимальнаяРозничнаяЦенаМОТП
		|ИЗ
		...
		|				ТОГДА &ПроизводительВЕТИСНеУказан
		|						ИЛИ СерииНоменклатуры.ПроизводительВЕТИС = &ПроизводительВЕТИС
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ВЫБОР
		|			КОГДА СерииНоменклатуры.ВидНоменклатуры.ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии
		|				ТОГДА &ЗаписьСкладскогоЖурналаВЕТИСНеУказан
		|						ИЛИ СерииНоменклатуры.ЗаписьСкладскогоЖурналаВЕТИС = &ЗаписьСкладскогоЖурналаВЕТИС
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ВЫБОР
		|			КОГДА СерииНоменклатуры.ВидНоменклатуры.ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии
		|				ТОГДА &ИдентификаторПартииВЕТИСНеУказан
		|						ИЛИ СерииНоменклатуры.ИдентификаторПартииВЕТИС = &ИдентификаторПартииВЕТИС
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ВЫБОР
		|			КОГДА СерииНоменклатуры.ВидНоменклатуры.ИспользоватьМРЦМОТПСерии
		|				ТОГДА &МаксимальнаяРозничнаяЦенаМОТПНеУказан
		|						ИЛИ СерииНоменклатуры.МаксимальнаяРозничнаяЦенаМОТП = &МаксимальнаяРозничнаяЦенаМОТП
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|
		|УПОРЯДОЧИТЬ ПО
		...
		Запрос.УстановитьПараметр("ПроизводительВЕТИС",         Параметры.ПроизводительВЕТИС);
		Запрос.УстановитьПараметр("ПроизводительВЕТИСНеУказан", Не ЗначениеЗаполнено(Параметры.ПроизводительВЕТИС));
		Запрос.УстановитьПараметр("ЗаписьСкладскогоЖурналаВЕТИС",          Параметры.ЗаписьСкладскогоЖурналаВЕТИС);
		Запрос.УстановитьПараметр("ЗаписьСкладскогоЖурналаВЕТИСНеУказан",  Не ЗначениеЗаполнено(Параметры.ЗаписьСкладскогоЖурналаВЕТИС));
		Запрос.УстановитьПараметр("ИдентификаторПартииВЕТИС",              Параметры.ИдентификаторПартииВЕТИС);
		Запрос.УстановитьПараметр("ИдентификаторПартииВЕТИСНеУказан",      Не ЗначениеЗаполнено(Параметры.ИдентификаторПартииВЕТИС));
		Запрос.УстановитьПараметр("МаксимальнаяРозничнаяЦенаМОТП",         Параметры.МаксимальнаяРозничнаяЦенаМОТП);
		Запрос.УстановитьПараметр("МаксимальнаяРозничнаяЦенаМОТПНеУказан", Не ЗначениеЗаполнено(Параметры.МаксимальнаяРозничнаяЦенаМОТП));
		...
		Пока Выборка.Следующий() Цикл
			...
			Значение = Новый Структура("Серия,Номер,ГоденДо,НомерКиЗГИСМ,RFIDTID,RFIDUser,RFIDEPC,EPCGTIN,ДатаПроизводства,
				|ПроизводительЕГАИС,Справка2ЕГАИС,ПроизводительВЕТИС,ЗаписьСкладскогоЖурналаВЕТИС,ИдентификаторПартииВЕТИС,
	...
	ИначеЕсли Параметры.СтрокаПоиска <> Неопределено Тогда
		...
		|ГДЕ
		|	СерииНоменклатуры.Наименование %ВидСравнения% &СтрокаПоиска
		|
		|УПОРЯДОЧИТЬ ПО
		|	СерияПредставление,
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ВидСравнения%", "=");
		...
		Если Результат.Пустой() Тогда
			...
			Запрос = Новый Запрос;
			Запрос.Текст = ТекстЗапроса;
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ВидСравнения%", "ПОДОБНО");
			Запрос.УстановитьПараметр("СтрокаПоиска", "%" + СокрЛП(Параметры.СтрокаПоиска) + "%");
...
Процедура УпаковкиЕдиницыИзмеренияОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ВыборРодителя") Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
					"&УсловиеПоСтрокеПоиска",
					?(Параметры.СтрокаПоиска = Неопределено, "ИСТИНА", " УпаковкиНоменклатуры.Наименование ПОДОБНО &СтрокаПоиска"));
...
//
// Параметры:
//  ДанныеГТД  - Структура - Структура содержит ссылку на номер гтд и страну происхождения.
//
