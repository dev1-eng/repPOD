...
Функция СтатьяАналитикаРасходовПоУмолчанию(ХозяйственнаяОперация, Организация, Подразделение, Номенклатура) Экспорт
	...
	|	ВЫБРАТЬ
	|		Операция.Дата							КАК Период,
	|		ДанныеДокумента.СтатьяРасходовДоходов	КАК СтатьяРасходов,
	|		ДанныеДокумента.АналитикаРасходов		КАК АналитикаРасходов
	|	ИЗ
...
//
// Параметры:
// 	 Форма - ФормаКлиентскогоПриложения - Форма, в которой для которой необходимо условное оформление формы
...
//
Процедура УстановитьУсловноеОформлениеАналитикПриСовместномИспользованииСтатей(Форма, 
																			   ИмяТЧ, 
																			   ТипыСтатей, 
	...
	Если ТипыСтатей.Найти(1) <> Неопределено Тогда
		...
		УстановитьУсловноеОформлениеАналитикиПоТипамСтатей(Форма, ИмяТЧ, "АналитикаРасходов", ДанныеТипов);
	...
	Если ТипыСтатей.Найти(2) <> Неопределено Тогда
		...
		УстановитьУсловноеОформлениеАналитикиПоТипамСтатей(Форма, ИмяТЧ, "АналитикаДоходов", ДанныеТипов);
	...
	Если ТипыСтатей.Найти(3) <> Неопределено Тогда
		...
		УстановитьУсловноеОформлениеАналитикиПоТипамСтатей(Форма, ИмяТЧ, "АналитикаАктивовПассивов", ДанныеТипов);
...
// Процедура дополняет условное оформление элементами, обеспечивающими совместное отображение в табличной части
// аналитики статей расходов.
//
// Параметры:
// 	 Форма        - ФормаКлиентскогоПриложения      - Форма, в которой для которой необходимо условное оформление формы
// 	 ИмяТЧ        - Строка                - имя табличной части
//   ИмяРеквизита - Строка                - имя реквизита аналитики
//   ТипыСтатей   - Число, СписокЗначений - типы статей (1 - Статьи расходов, 2 - Статьи доходов, 3 - СтатьиАктивовПассивов)
//									        для которых устанавливаться оформление аналитики.
//											В случаи списка значений можно указать 0 - используется для указания статьи по умолчанию
//
Процедура УстановитьУсловноеОформлениеАналитикиПоТипамСтатей(Форма, ИмяТЧ, ИмяРеквизита, ТипыСтатей) Экспорт
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы[ИмяТЧ + ИмяРеквизита].Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект." + ИмяТЧ + ".ТипСтатьи");
	ОтборЭлемента.ВидСравнения   = ?(ТипЗнч(ТипыСтатей) = Тип("СписокЗначений"), 
									 ВидСравненияКомпоновкиДанных.НеВСписке, ВидСравненияКомпоновкиДанных.НеРавно);
	ОтборЭлемента.ПравоеЗначение = ТипыСтатей;
