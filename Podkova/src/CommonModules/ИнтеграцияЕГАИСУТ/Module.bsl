...
Процедура СформироватьДанныеДокументаОснования(ДокументОснование, ДокументЕГАИС, ТаблицаДанных) Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ВнутреннееПотреблениеТоваров") Тогда
		...
		СформироватьДанныеОснованияВнутреннееПотреблениеТоваров(ДокументОснование, ДокументЕГАИС, ТаблицаДанных);
...
Процедура СформироватьДанныеОснованияВнутреннееПотреблениеТоваров(ДокументОснование, ДокументЕГАИС, ТаблицаДанных)
...
Функция ШаблонКурсыВалют()
	...
	|				ИЛИ Валюта = &%1) КАК КурсыВалютСрезПоследних
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Валюта
	|;
...
Процедура ПроверитьЗаполнениеЧека(Объект, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("Товары",        Объект.Товары.Выгрузить(, "НоменклатураЕГАИС, Номенклатура, Характеристика, Серия, Количество, НомерСтроки, Помещение"));
	Запрос.УстановитьПараметр("Серии",         Объект.Серии.Выгрузить(, "Номенклатура, Характеристика, Серия, Количество, Помещение"));
	Запрос.УстановитьПараметр("АкцизныеМарки", Объект.АкцизныеМарки.Выгрузить(, "АкцизнаяМарка"));
...
Процедура ЗаполнитьАктСписанияЕГАИСНаОснованииВнутреннегоПотребленияТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаЗаполнения(ВозможныеДокументыЕГАИС, "ВнутреннееПотреблениеТоваров");
...
Процедура ЗаполнитьЧекЕГАИСНаОснованииВнутреннегоПотребленияТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаЗаполнения(ВозможныеДокументыЕГАИС, "ВнутреннееПотреблениеТоваров",Настройки);
	//Доработаем запрос шапки
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
	"	Таблица.Ответственный   КАК Ответственный",
	"	Таблица.Ответственный КАК Ответственный,
...
Процедура ЗаполнитьТТНИсходящуюЕГАИСНаОснованииВозвратаТоваровПоставщику(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если Не ЭтоПерезаполнение Тогда
		...
		ТранспортныеНакладные = Документы.ТранспортнаяНакладная.ПолучитьИнформациюТранспортныхНакладных(РеквизитыДокумента.ДокументОснование);
		ТранспортнаяНакладная = Неопределено;
		
		Если ТранспортныеНакладные.КоличествоТранспортныхНакладных = 0 Тогда 
			...
			КонецЕсли;
		ИначеЕсли ТранспортныеНакладные.КоличествоТранспортныхНакладных = 1 Тогда
		    Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТранспортныеНакладные.ТранспортнаяНакладная, "Дата,Номер");
			ДокументОбъект.НомерТТН = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Реквизиты.Номер);
			ДокументОбъект.ДатаТТН  = Реквизиты.Дата;
			ТранспортнаяНакладная   = ТранспортныеНакладные.ТранспортнаяНакладная;
		Иначе
