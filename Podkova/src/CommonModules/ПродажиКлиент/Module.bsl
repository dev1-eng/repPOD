...
#Область ПроцедурыИФункцииРаботыСТранспортнымиНакладными

// Общая процедура обработки навигационной ссылки текста ТТН в документах продажи.
// В зависимости от выбранной ссылки вызывает создание/открытие ТТН документа.
//
// Параметры:
...
// Параметры:
//  СтандартнаяОбработка  - Булево - признак стандартной обработки события.
//  Форма                 - ФормаКлиентскогоПриложения - Форма, в которой было выполнено нажатие.
//
...
Процедура ПроверитьВозможностьСозданияТранспортнойНакладной(Форма)
	...
	ОснованиеПоЗаказам = Ложь;
	
	Если Форма.Объект.Свойство("РеализацияПоЗаказам") Тогда
		ОснованиеПоЗаказам = Форма.Объект.РеализацияПоЗаказам;
	ИначеЕсли Форма.Объект.Свойство("ПередачаПоЗаказам") Тогда
		ОснованиеПоЗаказам = Форма.Объект.ПередачаПоЗаказам;
	КонецЕсли;
	
	Если ОснованиеПоЗаказам Тогда
