...
Процедура ОтправитьЗапросКоммерческихПредложений(Знач ПараметрыОтправки, Знач АдресРезультата) Экспорт
	...
	Если Отказ Тогда
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Отправка Запроса коммерческих предложений поставщикам'"),
			ПредставлениеОшибки);
	...
	Если ПерваяОтправка Тогда
		...
		ИспользоватьОбменЧерезЭДО =
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЗначениеФункциональнойОпции("ИспользоватьОбменЭД");
			
		АдресатыЭДО = Новый Соответствие;
		Если ИспользоватьОбменЧерезЭДО Тогда
			
			ПараметрыПолученияНастроек = Новый Структура;
			ПараметрыПолученияНастроек.Вставить("НаправлениеЭД", Перечисления.НаправленияЭД.Исходящий);
			ПараметрыПолученияНастроек.Вставить("ВидЭД"        , Перечисления.ВидыЭД.ЗапросКоммерческихПредложений);
			ПараметрыПолученияНастроек.Вставить("Организация"  , Организация);
			
			Для Каждого Получатель Из Адресаты Цикл
				
				ПараметрыПолученияНастроек.Вставить("Контрагент", Получатель);
				
				НастройкиОбмена = ОбменСКонтрагентамиСлужебный.ОпределитьНастройкиОбменаЭД(
					ПараметрыПолученияНастроек, ПараметрыОтправки.ОтпечаткиСертификатов);
		...
		Исключение
			...
			ПредставлениеОшибки = НСтр("ru = 'Ошибка подготовки данных для первой отправки.'");
	...
	Если ПерваяОтправка Тогда
		...
		Если ЗначениеЗаполнено(ДанныеЗапроса.ДанныеДокумента) Тогда
			ИдентификаторСервиса = РазместитьЗапросКоммерческихПредложенийВСервисе(ДанныеЗапроса.ДанныеДокумента,
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ИдентификаторСервиса) Тогда
			...
			Если ЗначениеЗаполнено(ТекущаяВерсия) И ЗначениеЗаполнено(ДанныеЗапроса.ДанныеДокумента) Тогда
				НоваяВерсия = ОбновитьЗапросКоммерческихПредложенийВСервисе(ИдентификаторСервиса, ТекущаяВерсия,
					ДанныеЗапроса.ДанныеДокумента, КлючИдемпотентности);
			КонецЕсли;
	...
	Если ПубликацияВСервисеВыполнена Тогда
		...
		ЭлектронноеВзаимодействиеСлужебный.ДобавитьЗначениеВДерево(ДанныеЗапроса.ДанныеДокумента,
	...
	Попытка
		...
		Если ОснованияЭДО.Количество() Тогда
			...
			ДополнительныеПараметры.Вставить("ПараметрыОбмена", ПараметрыОбменаЭДО);
			ДополнительныеПараметры.Вставить("ДанныеДокумента", ДанныеЗапроса.ДанныеДокумента);
			
			РезультатДействия = ОбменСКонтрагентамиСлужебный.ВыполнитьДействияПоЭД(ОснованияЭДО, Новый Массив,
				"Сформировать", ДополнительныеПараметры, Неопределено, Новый Соответствие);
			
			Если РезультатДействия.Свойство("МассивНовыхЭД") Тогда
				НовыеЭлектронныеДокументы =
					ОбменСКонтрагентамиСлужебный.ФайлыЭлектронныхДокументовПоВладельцам(РезультатДействия.МассивНовыхЭД);
			Иначе
				НовыеЭлектронныеДокументы = Новый Массив;
			КонецЕсли;
			
			СостоянияОтправки = ОбменСКонтрагентамиСлужебный.ПолучитьДанныеЭДПоВладельцам(ОснованияЭДО);
			...
			Для Каждого СостояниеВерсии Из СостоянияОтправки Цикл
				...
				СостояниеСинхронизацииПоСостояниюВерсииЭлектронногоДокумента(СостояниеВерсии.СостояниеВерсииЭД, ТекущееСостояние,
					ПредставлениеОшибки);
				
				ОснованиеОбъект = СостояниеВерсии.ВладелецЭД.ПолучитьОбъект(); // СправочникОбъект.ОснованияЭлектронныхДокументовПоЗапросамКоммерческихПредложений
				ОснованиеОбъект.СостояниеСинхронизации = ТекущееСостояние;
				ОснованиеОбъект.ПредставлениеОшибки = ПредставлениеОшибки;
				ОснованиеОбъект.Записать();
				
				Получатель = ОснованияИПолучатели[СостояниеВерсии.ВладелецЭД];
			...
			РезультатОперации.Вставить("НовыеЭлектронныеДокументы", НовыеЭлектронныеДокументы);
			
		КонецЕсли;
		
	Исключение
		
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Отправка Запроса коммерческих предложений поставщика'"),
	...
	Если Источник.ПометкаУдаления Тогда
		...
		Если Не РазрешеноУдалениеЗапросаКоммерческихПредложенийПоставщиков(Источник.Ссылка, ДанныеДокумента) Тогда
			...
			ЭлектронноеВзаимодействие.ОбработатьОшибку(
				...
				НСтр("ru = 'Документ не может быть помечен на удаление, так как опубликован в сервисе.'"), ТекстСообщения,
				"КоммерческиеПредложения", Источник.Ссылка);
...
Процедура ОбработатьИзменениеВложенияЗапросаКоммерческихПредложенийПоставщиков(Источник, Отказ) Экспорт
	...
	Если ЗапросКоммерческихПредложенийПоставщиковЗанят(Документ) Тогда
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(
			...
			ТекстСообщения, "КоммерческиеПредложения", Документ);
...
Процедура ОбновитьСостояниеЗапросаКоммерческихПредложенийПоставщиков(Источник, Отказ) Экспорт
	...
	Если Источник.ОбменДанными.Загрузка
		Или Отказ
		Или Источник.ВидЭД <> Перечисления.ВидыЭД.ЗапросКоммерческихПредложений Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ОснованияДокумента Из Источник.ДокументыОснования Цикл
		Если ТипЗнч(ОснованияДокумента.ДокументОснование) = ТипОснованияЗапроса Тогда
			Основания.Добавить(ОснованияДокумента.ДокументОснование);
		КонецЕсли;
	КонецЦикла;
	...
	ФактическиеСостояния = ОбменСКонтрагентамиСлужебный.ПолучитьДанныеЭДПоВладельцам(
	...
	Для Каждого СостояниеЭлектронногоДокумента Из ФактическиеСостояния Цикл
		...
		Если Не ЗначениеЗаполнено(СостояниеЭлектронногоДокумента.СостояниеВерсииЭД) Тогда
			Продолжить;
		КонецЕсли;
		...
		СостояниеСинхронизацииПоСостояниюВерсииЭлектронногоДокумента(СостояниеЭлектронногоДокумента.СостояниеВерсииЭД,
			ТекущееСостояние, ПредставлениеОшибки);
		
		ДанныеОснования = СостоянияОснований.Найти(СостояниеЭлектронногоДокумента.ВладелецЭД, "Основание");
		
		Если ДанныеОснования.СостояниеСинхронизации <> ТекущееСостояние Тогда
			
			ОснованиеОбъект = СостояниеЭлектронногоДокумента.ВладелецЭД.ПолучитьОбъект(); // СправочникОбъект.ОснованияЭлектронныхДокументовПоЗапросамКоммерческихПредложений
			ОснованиеОбъект.СостояниеСинхронизации = ТекущееСостояние;
			ОснованиеОбъект.ПредставлениеОшибки = ПредставлениеОшибки;
			ОснованиеОбъект.Записать();
...
Функция ДанныеЗапросаКоммерческихПредложенийПоставщиков(Знач Документ, ПроверятьЗаполнение, Отказ) Экспорт
	...
	// Заполнение дерева
	ДанныеДокумента = ЭлектронноеВзаимодействие.ДеревоЭлектронногоДокумента(
	...
	Если Не Отказ
		И ПроверятьЗаполнение Тогда
		ЭлектронноеВзаимодействиеСлужебный.ПроверитьЗаполнениеРеквизитовДереваДанныхРекурсивно(Результат.ДанныеДокумента,
			Результат.Ошибки);
		ОбменСКонтрагентамиВнутренний.ОбработатьОшибкиФормированияЭД(Результат.Ошибки,
			Перечисления.ВидыЭД.ЗапросКоммерческихПредложений, Документ);
	КонецЕсли;
...
//  Параметры - Структура - См. ОбменСКонтрагентамиСлужебный.ЗаполнитьПараметрыЭДПоИсточнику.
//
Процедура ЗаполнитьПараметрыЭлектронногоДокументаПоОснованию(Основание, Параметры) Экспорт
	...
	Параметры.Вставить("ВидЭД",                   Перечисления.ВидыЭД.ЗапросКоммерческихПредложений);
...
Процедура ДополнитьДанныеДокументаПоОснованиюЗапросаКоммерческихПредложений(Знач Основание, ДанныеДокумента) Экспорт
	...
	ЭлектронноеВзаимодействиеСлужебный.ДобавитьЗначениеВДерево(ДанныеДокумента,
...
// Принудительное закрытие неактуального электронного документа по входящему Запросу коммерческих предложений
// при получении новой версии документа.
// Выполняется при записи входящего электронного документа. См. подписку на событие
// ЗакрытьНеактуальныйЭлектронныйДокументПоЗапросуКоммерческихПредложений.
//
// Параметры:
//  Источник - ДокументОбъект.ЭлектронныйДокументВходящий - входящий электронный документ.
//  Отказ	 - Булево - признак отказа от записи документа.
//
Процедура ЗакрытьНеактуальныйЭлектронныйДокументПоЗапросуКоммерческихПредложений(Источник, Отказ) Экспорт
	
	Если Источник.ОбменДанными.Загрузка
		Или Отказ
		Или Источник.ВидЭД <> Перечисления.ВидыЭД.ЗапросКоммерческихПредложений
		Или Не Источник.ДокументыОснования.Количество() Тогда
		Возврат;
	КонецЕсли;
	
	ТекущиеВерсии = ОбменСКонтрагентамиСлужебный.ВладельцыИЭлектронныеДокументы(
		Источник.ДокументыОснования.ВыгрузитьКолонку("ДокументОснование"));
	
	ТребуетсяЗакрыть = Новый Массив;
	Для Каждого ВерсияДокумента Из ТекущиеВерсии Цикл
		Если ВерсияДокумента.Значение <> Источник.Ссылка Тогда
			ТребуетсяЗакрыть.Добавить(ВерсияДокумента.Значение);
		КонецЕсли;
	КонецЦикла;
	
	Если ТребуетсяЗакрыть.Количество() Тогда
		ПричинаЗакрытия = НСтр("ru='Закрыт автоматически при получении новой версии.'");
		КоличествоЗакрытых = 0;
		ОбменСКонтрагентамиСлужебныйВызовСервера.ЗакрытьДокументыПринудительно(ТребуетсяЗакрыть, ПричинаЗакрытия,
			КоличествоЗакрытых);
	КонецЕсли;
...
Процедура ПерезаполнитьЗапросКоммерческихПредложений(УчетныйДокумент, ЭлектронныйДокумент, СпособОбработки) Экспорт
	...
	Если Перезаполнить Тогда
		...
		ПараметрыЗаполнения.Вставить("ЭД",              ЭлектронныйДокумент);
		...
		ОбменСКонтрагентамиСлужебныйВызовСервера.ПерезаполнитьДокумент(ПараметрыЗаполнения, Неопределено);
...
Процедура ИспользуемыеВидыЭлектронныхДокументов(ВидыЭлектронныхДокументов) Экспорт
	...
	Если ИспользоватьЗапросКоммерческихПредложений = Истина Тогда
		ОбменСКонтрагентами.ДобавитьАктуальныйВидЭД(ВидыЭлектронныхДокументов, Перечисления.ВидыЭД.ЗапросКоммерческихПредложений);
	КонецЕсли;
	
	Если ИспользоватьКоммерческоеПредложение = Истина Тогда
		ОбменСКонтрагентами.ДобавитьАктуальныйВидЭД(ВидыЭлектронныхДокументов, Перечисления.ВидыЭД.КоммерческоеПредложение);
	КонецЕсли;
...
Процедура ОтключитьКонтрольОтраженияВУчетеДокументовПодсистемы(ВидыЭлектронныхДокументов) Экспорт
	
	ВидыЭлектронныхДокументов.Удалить(Перечисления.ВидыЭД.ЗапросКоммерческихПредложений);
	ВидыЭлектронныхДокументов.Удалить(Перечисления.ВидыЭД.КоммерческоеПредложение);
	
КонецПроцедуры

Процедура ОпределитьСписокОперацийВходящихЭлектронныхДокументов(Знач ВидЭлектронногоДокумента, СписокОпераций) Экспорт
	
	Если ВидЭлектронногоДокумента = Перечисления.ВидыЭД.ЗапросКоммерческихПредложений
	...
	ИначеЕсли ВидЭлектронногоДокумента = Перечисления.ВидыЭД.КоммерческоеПредложение
		И ОбщегоНазначения.ПодсистемаСуществует(
...
//
// Параметры:
//  ВидЭлектронногоДокумента - см. ОбменСКонтрагентамиПереопределяемый.СписокТиповДокументовПоВидуЭД.ВидЭД
//  СписокТипов              - см. ОбменСКонтрагентамиПереопределяемый.СписокТиповДокументовПоВидуЭД.СписокВозврата
//
Процедура ДополнитьСписокТиповДокументовПоВидуЭлектронногоДокумента(Знач ВидЭлектронногоДокумента, СписокТипов) Экспорт
	
	Если ВидЭлектронногоДокумента = Перечисления.ВидыЭД.ЗапросКоммерческихПредложений Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует(ЗапросКоммерческогоПредложенияОтКлиента) Тогда
			...
			СписокТипов.Добавить(МенеджерДокумента.ПустаяСсылка(),
				Метаданные.Документы["ЗапросКоммерческогоПредложенияОтКлиента"].Представление());
		КонецЕсли;
		
	ИначеЕсли ВидЭлектронногоДокумента = Перечисления.ВидыЭД.КоммерческоеПредложение Тогда
...
//
// Параметры:
//  СостояниеВерсии - ПеречислениеСсылка.СостоянияВерсийЭД - Состояние версии электронного документа.
//  СостояниеСинхронизации - ПеречислениеСсылка.СостоянияСинхронизацииЗапросовКоммерческихПредложений -
...
Процедура СостояниеСинхронизацииПоСостояниюВерсииЭлектронногоДокумента(Знач СостояниеВерсии, СостояниеСинхронизации,
	...
	Если СостояниеВерсии = Перечисления.СостоянияВерсийЭД.НаПодписи Тогда
		СостояниеСинхронизации = СостояниеВыполняется;
	ИначеЕсли СостояниеВерсии = Перечисления.СостоянияВерсийЭД.ОжидаетсяПодтверждение
		Или СостояниеВерсии = Перечисления.СостоянияВерсийЭД.ОжидаетсяОтправка
		Или СостояниеВерсии = Перечисления.СостоянияВерсийЭД.ОжидаетсяОтправкаПолучателю
		Или СостояниеВерсии = Перечисления.СостоянияВерсийЭД.ОжидаетсяПередачаОператору
		Или СостояниеВерсии = Перечисления.СостоянияВерсийЭД.ОжидаетсяПодтверждениеОператора
		Или СостояниеВерсии = Перечисления.СостоянияВерсийЭД.ОжидаетсяИзвещениеОПолучении
		Или СостояниеВерсии = Перечисления.СостоянияВерсийЭД.ОжидаетсяСогласиеКонтрагента
		Или СостояниеВерсии = Перечисления.СостоянияВерсийЭД.ОбменЗавершен Тогда
...
Процедура ЗакрытьЭлектронныйДокументПоЗапросуКоммерческихПредложенийИсходящему(СостояниеДокумента,
	...
	ТребуетсяЗакрыть = Новый Массив;
	ТребуетсяЗакрыть.Добавить(Основание);
...
Функция НастройкиОбменаЭДОПоЗапросуКоммерческихПредложенийИсходящему(Знач Организация, Знач Адресат)
	...
	ПараметрыЭДО = ОбменСКонтрагентамиСлужебный.СтруктураПараметровЭД();
	ПараметрыЭДО.Вставить("ВидЭД",         Перечисления.ВидыЭД.ЗапросКоммерческихПредложений);
	ПараметрыЭДО.Вставить("НаправлениеЭД", Перечисления.НаправленияЭД.Исходящий);
	ПараметрыЭДО.Вставить("Организация",   Организация);
	ПараметрыЭДО.Вставить("Контрагент",    Адресат);
	
	Возврат ОбменСКонтрагентамиСлужебный.ОпределитьНастройкиОбменаЭД(ПараметрыЭДО);
...
Процедура АктуализироватьПланСинхронизацииЗапросаКоммерческихПредложенийПоставщиков(Знач Документ, Отказ,
	...
	Если ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЗначениеФункциональнойОпции("ИспользоватьОбменЭД") Тогда
		...
		ПараметрыПолученияНастроек.Вставить("НаправлениеЭД", Перечисления.НаправленияЭД.Исходящий);
		ПараметрыПолученияНастроек.Вставить("ВидЭД"        , Перечисления.ВидыЭД.ЗапросКоммерческихПредложений);
		ПараметрыПолученияНастроек.Вставить("Организация"  , ПараметрыОтправки.Организация);
		
		Для Каждого Получатель Из ПараметрыОтправки.Адресаты Цикл
			
			ПараметрыПолученияНастроек.Вставить("Контрагент", Получатель);
			
			НастройкиОбмена = ОбменСКонтрагентамиСлужебный.ОпределитьНастройкиОбменаЭД(ПараметрыПолученияНастроек);
...
//
Функция РазместитьЗапросКоммерческихПредложенийВСервисе(Знач ДанныеЗапроса, Знач КлючИдемпотентности)
...
Функция ДанныеЗапросаКоммерческихПредложенийДляИзмененийВСервисе(Знач ДеревоДанных)
	...
	Если Не ЗначениеЗаполнено(ИдентификаторАбонента) Тогда
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Получение идентификатора абонента сервиса 1С:Бизнес-сеть'"),
		...
		Для Каждого Позиция Из Товары.Строки Цикл
			...
			Если ЗначениеЗаполнено(СпособОписанияТовара.Значение) Тогда
				...
				ИначеЕсли СпособОписанияТовара.Значение = "Рубрика" Тогда
					...
					СтрокаДерева = Позиция.Строки.Найти("Товары.НомерСтроки.Товар.АртикулПроизводителя", "ПолныйПуть", Истина);
	...
	Если ЗначениеЗаполнено(ПрикрепленныеФайлы.Значение) Тогда
		...
		Для Каждого ПрикрепленныйФайл Из ПрикрепленныеФайлы.Строки Цикл
			...
			СтрокаДерева = ПрикрепленныйФайл.Строки.Найти("ПрисоединенныеФайлы.НомерСтроки.ДвоичныеДанные",
				"ПолныйПуть", Истина);
			RfpAttachmentFile.Content = ЭлектронноеВзаимодействиеСлужебный.Base64СтрокаИзДвоичныхДанных(СтрокаДерева.Значение);
...
Функция ЗапросКоммерческихПредложенийИзМоделиСервиса(Знач МодельСервиса)
	...
	Если ТипЗнч(ИНН) = Тип("Строка") И Не ПустаяСтрока(ИНН) Тогда
		...
		Если СтрДлина(ИНН) = 12 Тогда
			...
			Отправитель = ЭлектронноеВзаимодействие.НайтиСсылку("Контрагенты", "", ИндивидуальныйПредприниматель);
		...
		Иначе
			...
			Отправитель = ЭлектронноеВзаимодействие.НайтиСсылку("Контрагенты", "", ЮридическоеЛицо);
...
Функция ЗаголовокЗапросаКоммерческихПредложенийПоставщиковВБизнесСети(Знач Номер, Знач Дата)
	...
	Возврат СтрШаблон(НСтр("ru = '%1 %2 от %3'"), Строка(Перечисления.ВидыЭД.ЗапросКоммерческихПредложений),
...
Процедура ОтправитьЗапросКоммерческихПредложенийБезЭлектроннойПодписи(Знач Владелец, Знач Организация,
	...
	ПараметрыЗадания.Вставить("МассивСсылокНаОбъект", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Владелец));
	...
	ДанныеДокументов = ПолучитьИзВременногоХранилища(АдресХранилища);
	
	Для Каждого ДанныеДокумента Из ДанныеДокументов Цикл
		
		АдресХранилища = ПоместитьВоВременноеХранилище(ДанныеДокумента.ДвоичныеДанныеПакета);
		АдресХранилищаПредставления = ПоместитьВоВременноеХранилище(ДанныеДокумента.АдресХранилищаПредставления);
		...
		ПараметрыОтправки.Вставить("ВидЭД",                       Перечисления.ВидыЭД.ЗапросКоммерческихПредложений);
		ПараметрыОтправки.Вставить("Сумма",                       СуммаДокумента);
		ПараметрыОтправки.Вставить("АдресХранилища",              АдресХранилища);
		ПараметрыОтправки.Вставить("АдресХранилищаПредставления", АдресХранилищаПредставления);
...
Функция ВыполнитьЗапрос(ЗапросСервиса, Отказ)
	...
	ОтветСервиса = ОбработатьОтветСервиса(Соединение, ЗапросСервиса, Ответ, Данные, Отказ);
...
Функция ОбработатьОшибкуСервиса(Соединение, ЗапросСервиса, КодСостояния, Данные, Отказ)
	...
	Если Не (ЗапросСервиса.Свойство("БлокироватьСообщенияОбОшибках")
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Торговые предложения.'", ОбщегоНазначения.КодОсновногоЯзыка())
