...
Процедура НачатьПечатьТекстовыхДокументов(ОповещениеПриЗавершении, УникальныйИдентификатор, НефискальныеДокументы, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
	...
	// МОЙ КОД +
    // Передаем текст копии Чека ККМ
    Если ТипЗнч(ОповещениеПриЗавершении.ДополнительныеПараметры) = Тип("Структура") Тогда
        Если ОповещениеПриЗавершении.ДополнительныеПараметры.Свойство("МК_ДетализацияЧека") Тогда
            // Передадим его как второй элемент массива ВходныеПараметры
            ВходныеПараметры.Добавить(ОповещениеПриЗавершении.ДополнительныеПараметры.МК_ДетализацияЧека);
        КонецЕсли;
    КонецЕсли; 
    // МОЙ КОД -
