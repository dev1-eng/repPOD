...
//		ПараметрыЗапроса - Структура - см. Тестирование.ПараметрыЗапроса
		...
		Пока Позиция < Граница Цикл
			Если Не Результат[Позиция].Пустой() Тогда
				...
				Если ДополнительныеПараметрыПоРегистру.ОпределятьОбщееРасхождение Тогда
					...
					СтруктураРегистра.Вставить("Документы", Результат[3].Выгрузить().ВыгрузитьКолонку("Регистратор"));
				Иначе
...
Функция СформироватьЗапрос(ПутьКМетаданным,	СохранитьВоВременнуюТаблицу = Ложь, ОтборПоРегистраторам = Ложь, ЭталоннаяТаблица = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		...
		Если ДополнительныеПараметры.ОпределятьОбщееРасхождение И ИсключаемыеПоля.Свойство("Регистратор") Тогда
			ТекстИзмеренияРеквизитыФинал = СтрЗаменить(ТекстИзмеренияРеквизиты, "КАК ДокументРегистратор", "КАК Регистратор");
		Иначе
			ТекстИзмеренияРеквизитыФинал = ТекстИзмеренияРеквизиты;
		КонецЕсли;
		ПолныйТекстЗапроса = СтрЗаменить(ПолныйТекстЗапроса, "&ТекстИзмеренияРеквизитыФинал,", ТекстИзмеренияРеквизитыФинал + ",");
		...
		Если ДополнительныеПараметры.ОпределятьОбщееРасхождение Тогда
			...
			Если ИсключаемыеПоля.Свойство("Регистратор") Тогда
				ЗапросРасхождений = СтрЗаменить(ЗапросРасхождений, "ДД.Регистратор", "ДД.ДокументРегистратор");
			КонецЕсли;
