...
//                                                     ...".
//      * ИмяГруппыКолонокВТабличнойЧасти   - ГруппаФормы - Имя группы колонок заказа клиента и кода строки в ТЧ
...
//          СправочникСсылка.ПричиныОтменыЗаказовПоставщикам - Причина отмены строк заказа.
...
Функция ОтменитьНеотработанныеСтрокиПоОтгрузке(ДокументОбъект, ПараметрыЗаполнения, ПараметрыОтмены) Экспорт
	...
	Для Каждого Строка Из ДокументОбъект[ПараметрыЗаполнения.ИмяТабличнойЧасти] Цикл
		Если ПараметрыОтмены.УдалятьСтроки Или Не Строка.Отменено Тогда
			...
			ИначеЕсли Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.СоСклада
				ИЛИ Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ИзЗаказов Тогда
	...
	ОформленоПоНакладным.Колонки.Количество.Имя = "КоличествоВНакладной";
	...
	ОформленоПоОрдерам.Колонки.Количество.Имя = "КоличествоВОрдере";
	...
	ТаблицаВыбранныхСтрок.Колонки.НазначениеДокумента.Имя = "Назначение";
...
Функция ПеренестиВТаблицуДокументаОтмененныеСтроки(ДокументОбъект, ТаблицаДокумента, ТаблицаОтмененныхСтрок, ПараметрыОтмены, ТипДвиженияЗапасов)
	...
	Если ПараметрыОтмены.СкорректироватьМерныеТовары Тогда
		...
		ДопустимыеОтклонения = Документы[ДокументОбъект.Метаданные().Имя].ДопустимыеОтклоненияМерныхТоваров(
			ДокументОбъект.Ссылка, ТипДвиженияЗапасов);
	...
	Для Каждого КлючЗначение Из СтрокиКОбработке Цикл
		...
		Если Строка.Количество < СтрокаТовары.Количество Тогда
			...
			Если Строка.Количество > 0 И Не ПараметрыОтмены.УдалятьСтроки Тогда
				...
				Если ПараметрыОтмены.ПричинаОтмены <> Неопределено Тогда
					НоваяСтрока.ПричинаОтмены = ПараметрыОтмены.ПричинаОтмены;
				КонецЕсли;
				Если ПараметрыОтмены.КорректироватьСумму Тогда
		...
		Иначе
			...
			Если ПараметрыОтмены.ПричинаОтмены <> Неопределено Тогда
				СтрокаТовары.ПричинаОтмены = ПараметрыОтмены.ПричинаОтмены;
			КонецЕсли;
