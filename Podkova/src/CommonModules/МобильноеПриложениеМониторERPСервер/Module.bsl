...
// Возвращает результат формирования данных обмена.
//
// Параметры:
...
Процедура ЗаписатьКонтактноеЛицоПартнера(Данные, Сообщение)
	...
	МобильноеКонтактноеЛицо = МобильныйОбъектСКонтактнойИнформацией(Данные, ТипОбъекта, ТипОбъектаКИ);
...
Процедура ЗаписатьПартнера(Данные, Сообщение)
	...
	МобильныйПартнер = МобильныйОбъектСКонтактнойИнформацией(Данные, ТипОбъекта, ТипОбъектаКИ);
...
Функция МобильныйОбъектСКонтактнойИнформацией(Данные, ТипОбъекта, ТипОбъектаКИ)
	...
	Для каждого Свойство Из Данные.Свойства() Цикл
		...
		Если Не ЭтоТабличнаяЧасть Тогда
			УстанавливаемоеСвойство = МобильныйОбъект.Свойства().Получить(СокрЛП(Свойство));
			Если НЕ УстанавливаемоеСвойство = Неопределено Тогда
				МобильныйОбъект.Установить(УстанавливаемоеСвойство, Значение);
			КонецЕсли;
		Иначе
