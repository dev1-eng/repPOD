...
Функция ТекстЗапросаНекорректныхОстаткиВзаиморасчетовПоСрокам(ИмяВТ = "") Экспорт
	...
	ТекстЗапроса = "
		...
		|	ИЗ
		|		Документ.ВводОстатков КАК Ввод
		|	ГДЕ
		|		Ввод.Проведен 
		|		И Ввод.ТипОперации В (ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиКлиентов),
		...
		|	ИЛИ РасчетыПоСрокам.ПредоплатаОстаток < 0
		|;
		|УНИЧТОЖИТЬ ПериодВводаОстатков
		|;
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Аналитика.Организация             КАК Организация,
...
// Возвращаемое значение:
// 	Строка - Текст запроса
Функция ТекстЗапросаРазвернутоеСальдо(ИмяВт = "") Экспорт
...
Процедура ИсправитьОстаткиВзаиморасчетов(Период, ЗНАЧ МассивОрганизаций = Неопределено) Экспорт
	...
	Если НекорректныеОстатки.Количество() > 0 Тогда
		...
		Запрос.Текст = "УНИЧТОЖИТЬ ВтАналитика; УНИЧТОЖИТЬ ВтОстатки";
		Запрос.Выполнить();
		...
		Для Каждого Организация Из МассивОрганизацийДляОбработки Цикл
			...
			Для Каждого ТипРасчетов Из ТипыРасчетов Цикл
				...
				Для Каждого ЗаписьНекорректныхОстатков Из ЗаписиНекорректныхОстатков Цикл
					...
					Запрос.УстановитьПараметр("РасчетыПоСрокам",           ДополнительныеСвойства.ДляПроведения[ТипРасчетовСтрокой + "ПоСрокам"]);
					...
					Запрос.Текст = "
						...
						Запрос.Текст = СтрЗаменить(Запрос.Текст, "РасчетыСКлиентами", ТипРасчетовСтрокой);
...
Процедура ИсправитьРазвернутоеСальдо(Период, ЗНАЧ МассивОрганизаций = Неопределено) Экспорт
	...
	Если РазвернутоеСальдо.Количество() > 0 Тогда
		...
		Запрос.Текст = "УНИЧТОЖИТЬ ВтАналитика; УНИЧТОЖИТЬ ВтСальдо; УНИЧТОЖИТЬ АналитикиСуммы";
		Запрос.Выполнить();
	...
	Запрос.Текст = "УНИЧТОЖИТЬ ВтАналитика; УНИЧТОЖИТЬ ВтСальдо";
	Запрос.Выполнить();
...
// * МенеджерВТ - МенеджерВременныхТаблиц - Менеджер с таблицей РасчетыСКлиентамиИзменения или РасчетыСПоставщикамиИзменения.
...
Процедура ВыполнитьОтложенноеРаспределение(Параметры = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	Для Каждого Запись Из ДанныеКОбработке Цикл
		...
		ПараметрыРаспределения.ПорядокФакт = Порядок(Запись.ДатаПересчета,"",,Тип("ДокументСсылка.КорректировкаРегистров"));
		ПараметрыРаспределения.ПорядокПлан = Порядок(Запись.ДатаПересчетаПлан,"",,Тип("ДокументСсылка.КорректировкаРегистров"));
		
		НачатьТранзакцию();
		Попытка
			ОтметитьВыполнениеЗадания(Запись);
			ЗаполнитьОперативныеВзаиморасчеты(ПараметрыРаспределения);
			Если ЗначениеЗаполнено(Запись.ОбъектРасчетов) Тогда
				НовСтр = ТаблицаОбъектовОплаты.Добавить();
				НовСтр.ЭтоРасчетыСКлиентами = Запись.ЭтоРасчетыСКлиентами;
				НовСтр.ОбъектОплаты = Запись.ОбъектРасчетов;
			КонецЕсли;
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
	...
	РегистрыСведений.ГрафикПлатежей.РассчитатьГрафикПлатежейПоРасчетамСКлиентами(
		ТаблицаОбъектовОплаты.Скопировать(Новый Структура("ЭтоРасчетыСКлиентами", Истина)).ВыгрузитьКолонку("ОбъектОплаты"));
	РегистрыСведений.ГрафикПлатежей.РассчитатьГрафикПлатежейПоРасчетамСПоставщиками(
		ТаблицаОбъектовОплаты.Скопировать(Новый Структура("ЭтоРасчетыСКлиентами", Ложь)).ВыгрузитьКолонку("ОбъектОплаты"));
...
Процедура ЗапуститьОтложенноеРаспределениеВзаиморасчетов(РаспределитьФактическиеРасчеты = Ложь) Экспорт
	...
	ИмяМетодаЗадания = "ОперативныеВзаиморасчетыСервер.ВыполнитьФоновоеРаспределениеРасчетов";
	...
	Если (Константы.РаспределятьПлановыеРасчетыФоновымЗаданием.Получить() ИЛИ РаспределитьФактическиеРасчеты)
		...
		И АктивныеЗадания.Количество() = 0 Тогда
		...
		ДлительныеОперации.ВыполнитьВФоне(ИмяМетодаЗадания, ПараметрыЭкспортнойПроцедуры, ПараметрыВыполнения);
	КонецЕсли;
...
Функция ЗаписиДоНачалаРасчета(Запрос, ГлобальныеПеременные, ИмяТаблицы)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицы", "РегистрНакопления." + ИмяТаблицы);
	Если Найти(ИмяТаблицы,"ПоСрокам") <> 0 Тогда
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные)
	...
	|		КОГДА &РасчетыПоЗаказу И НЕ Расчеты.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности ТОГДА &ОбъектРасчетов
	...
	|ГДЕ 
	|	РасчетыОтгрузкиПоставки.КорОбъектРасчетов <> Неопределено
	...
	|		ИЛИ &ВалютаРасчетов =&ВалютаРегл КАК РучнойКурсРегл,
	|	РасчетыОтгрузкиПоставки.ВалютаДокумента = &ВалютаУпр
	|		ИЛИ &ВалютаРасчетов = &ВалютаУпр КАК РучнойКурсУпр,
	|	ВЫБОР
	...
	|		ИЛИ РасчетыОтгрузкиПоставки.КОтгрузкеПоставке < 0
	|		ИЛИ РасчетыОтгрузкиПоставки.КОплате <> 0 И НЕ ТИПЗНАЧЕНИЯ(РасчетыОтгрузкиПоставки.Регистратор) В (&СписокТиповЗаказов))
	|	И РасчетыОтгрузкиПоставки.КорОбъектРасчетов = Неопределено
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ПолучитьКурсыВалют(Запрос, ДатаНачала, ГлобальныеПеременные)
	...
	|		КурсыВалютыРасчетов.Курс / КурсыВалютыРасчетов.Кратность  КАК КурсРегл,
	|		0 КАК КурсУпр
	|	ИЗ
	|		РегистрСведений.КурсыВалют КАК КурсыВалютыРасчетов
	...
	|		0  КАК КурсРегл,
	|		КурсыВалютыУпр.Курс / КурсыВалютыУпр.Кратность КАК КурсУпр
	|	ИЗ
	|		РегистрСведений.КурсыВалют КАК КурсыВалютыУпр
	...
	|		0 КАК КурсУпр
	|	ИЗ
	|		РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНачала) КАК КурсыВалютыРасчетов
	|	ГДЕ КурсыВалютыРасчетов.Валюта = &ВалютаРасчетов
	|	
	|	ОБЪЕДИНИТЬ ВСЕ 
	|	
	...
	|		КурсыВалютыУпр.Курс / КурсыВалютыУпр.Кратность КАК КурсУпр
	|	ИЗ
	|		РегистрСведений.КурсыВалют.СрезПоследних(&ДатаНачала) КАК КурсыВалютыУпр
...
Процедура ЗаполнитьРегистрПереходаНаОнлайн()
	...
	|		ВЫБОР
	|			КОГДА ЕСТЬNULL(Расчеты.КорОбъектРасчетов, Неопределено)
	|					= Неопределено
	|				ТОГДА 1
	|			ИНАЧЕ 2
	|		КОНЕЦ КАК Приоритет
	...
	|					И Расчеты.ЗаказКлиента = РасчетыСПартнерами.ЗаказКлиента
	|		
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		РасчетыСПартнерами.Регистратор КАК Регистратор,
	|		ВЫБОР
	|			КОГДА РасчетыСПартнерами.КорОбъектРасчетов = Неопределено
	|				ТОГДА 1
	|			ИНАЧЕ 2
	|		КОНЕЦ КАК Приоритет
	|	ИЗ 
	...
	|				И РасчетыСПартнерами.ЗаказКлиента = РасчетыПоДокументам.ЗаказКлиента
	...
	|				ПО Расчеты.Регистратор = РасчетыСПартнерами.Регистратор
	|					И Расчеты.АналитикаУчетаПоПартнерам = РасчетыСПартнерами.АналитикаУчетаПоПартнерам
	|					И Расчеты.ЗаказПоставщику = РасчетыСПартнерами.ЗаказПоставщику
	|		
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|			ГДЕ
	|				РасчетыСПартнерами.АналитикаУчетаПоПартнерам = РасчетыПоДокументам.АналитикаУчетаПоПартнерам
	|				И РасчетыСПартнерами.ЗаказПоставщику = РасчетыПоДокументам.ЗаказПоставщику
	|				И РасчетыСПартнерами.Валюта = РасчетыПоДокументам.Валюта
	|		)
	|
	...
	Для Каждого СтрокаДвижений Из ТаблицаДетальныхЗаписей Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаДвижений.ПорядокОперации) Тогда
			Если ПорядокОперации.Получить(СтрокаДвижений.Период) = Неопределено Тогда
				ПорядокОперации.Вставить(СтрокаДвижений.Период, Порядок(СтрокаДвижений.Период, "",, Тип("ДокументСсылка.РасчетКурсовыхРазниц")));
			КонецЕсли;
...
Функция ТекстЗапросаРасчетыСКлиентамиПоДокументам()
	...
	|			И НЕ (РасчетыСКлиентамиПоДокументам.Регистратор ССЫЛКА Документ.ВводОстатков
	|					И РасчетыСКлиентамиПоДокументам.РасчетныйДокумент <> Неопределено)
...
Функция ТекстЗапросаРасчетыСПоставщикамиПоДокументам()
	...
	|			И НЕ (РасчетыСПоставщикамиПоДокументам.Регистратор ССЫЛКА Документ.ВводОстатков
	|					И РасчетыСПоставщикамиПоДокументам.РасчетныйДокумент <> Неопределено)
...
Процедура ВыполнитьФоновоеРаспределениеРасчетов(Параметры = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	Исключение
		...
		Если РаспределитьФактическиеРасчеты Тогда
			Событие = НСтр("ru = 'Фоновое распределение расчетов'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
...
Функция ПолучитьВсеОбъекты(Период = Неопределено) Экспорт
	...
	|	ИСТИНА КАК ЭтоРасчетыСКлиентами,
	|	МАКСИМУМ(ВЫБОР КОГДА РасчетыСКлиентами.КорОбъектРасчетов = Неопределено
	|			ТОГДА 1
	|		ИНАЧЕ 2
	...
	|	ЛОЖЬ КАК ЭтоРасчетыСКлиентами,
	|	МАКСИМУМ(ВЫБОР КОГДА РасчетыСПоставщиками.КорОбъектРасчетов = Неопределено
	|			ТОГДА 1
	|		ИНАЧЕ 2
	|	КОНЕЦ)
	|ИЗ
...
Функция ВидТипДокумента(ТипДокумента, ВидДвижения=Неопределено) Экспорт
	...
	Если Вид = "" Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Оперативные взаиморасчеты с контрагентами'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура УдалитьСистемныеКорректировкиРегистров(ДатаПересчета, Организация = Неопределено, Операция = Неопределено)
...
Функция ТаблицаИзмененийДляПересчета(МенеджерВременныхТаблиц, ЭтоРасчетыСКлиентами, Регистратор) 
	...
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПланОплат КАК Расчеты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РасчетыСКлиентамиИзменения КАК РасчетыИзменения
	...
	|				И Расчеты.ПорядокОперации > РасчетыИзменения.ПорядокОперации
	|				И Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|				И ТИПЗНАЧЕНИЯ(Расчеты.ДокументПлан) В (&ТипыЗаказов)
	|;
	|ВЫБРАТЬ
	...
	|	ВложенныйЗапрос.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|	ВложенныйЗапрос.ОбъектРасчетов            КАК ОбъектРасчетов,
	|	ВложенныйЗапрос.ПустойКорОбъектРасчетов   КАК ПустойКорОбъектРасчетов,
	...
	|	ВЫБРАТЬ
	|		РасчетыСКлиентамиПоСрокам.ПорядокОперации КАК ПорядокФакт,
	|		""30000101""                        КАК ПорядокПлан,
	|		Изменения.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|		Изменения.ОбъектРасчетов            КАК ОбъектРасчетов,
	|		Изменения.КорОбъектРасчетов = Неопределено КАК ПустойКорОбъектРасчетов,
	|		Изменения.ВалютаРасчетов            КАК ВалютаРасчетов
	|	ИЗ
	|		РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК РасчетыСКлиентамиПоСрокам
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РасчетыСКлиентамиИзменения КАК Изменения
	...
	|		РасчетыСКлиентамиПланОплат.ПорядокОперации КАК ПорядокПлан,
	|		Изменения.АналитикаУчетаПоПартнерам        КАК АналитикаУчетаПоПартнерам,
	|		Изменения.ОбъектРасчетов                   КАК ОбъектРасчетов,
	|		Изменения.КорОбъектРасчетов = Неопределено КАК ПустойКорОбъектРасчетов,
	|		Изменения.ВалютаРасчетов                   КАК ВалютаРасчетов 
	...
	|					И РасчетыСКлиентамиПланОплат.Активность
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		""30000101""                                  КАК ПорядокФакт,
	|		РасчетыСКлиентамиПланОтгрузок.ПорядокОперации КАК ПорядокПлан,
	|		Изменения.АналитикаУчетаПоПартнерам           КАК АналитикаУчетаПоПартнерам,
	|		Изменения.ОбъектРасчетов                      КАК ОбъектРасчетов,
	|		Изменения.КорОбъектРасчетов = Неопределено    КАК ПустойКорОбъектРасчетов,
	|		Изменения.ВалютаРасчетов                      КАК ВалютаРасчетов 
	...
	|					И РасчетыСКлиентамиПланОтгрузок.Активность
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|	ВЫБРАТЬ
	|		ВЫБОР КОГДА РасчетыСКлиентамиПоСрокам.ДокументРегистратор ССЫЛКА Документ.ВводОстатков
	|			ТОГДА """"
	|			ИНАЧЕ РасчетыСКлиентамиПоСрокам.ПорядокОперации
	...
	|	ИЗ
	|		РасчетыСКлиентамиИзменения КАК Изменения
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПланОплат КАК ПлановыеОплатыРегистратора
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПланОплат КАК Порядки
	...
	|	ГДЕ
	|		Порядки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|
	|) ВложенныйЗапрос
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.АналитикаУчетаПоПартнерам,
	...
	|УПОРЯДОЧИТЬ ПО
	|	МАКСИМУМ(ВложенныйЗапрос.ПустойКорОбъектРасчетов) УБЫВ
	|;
