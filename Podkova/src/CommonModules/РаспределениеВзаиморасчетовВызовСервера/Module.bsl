...
Процедура РассчитатьРасчетыСКлиентами(НачалоПериода, ОкончаниеПериода, ИсходныеДанные, КоличествоЗаписей = 0)
	...
	Если РегистраторыКОтражению.Количество() > 0 Тогда
		РегистрыСведений.СуммыДокументовВВалютеРегл.РассчитатьПоДаннымВзаиморасчетов(Контекст, РегистраторыКОтражению);
	КонецЕсли;
...
Функция ТекстДвиженияРасчетовСКлиентами()
	Возврат "
		...
		|		ИНАЧЕ 11
		|	КОНЕЦ,
		|	ВЫБОР КОГДА ДД.ЗаказКлиента ССЫЛКА Документ.ПоступлениеБезналичныхДенежныхСредств
		|		ИЛИ ДД.ЗаказКлиента ССЫЛКА Документ.ПриходныйКассовыйОрдер
		|		ИЛИ ДД.ЗаказКлиента ССЫЛКА Документ.ПервичныйДокумент
		|			ТОГДА 1
		|		ИНАЧЕ 2
...
Функция ТекстДвиженияРасчетовСПоставщиками()
	Возврат "
		...
		|		ИНАЧЕ 9
		|	КОНЕЦ КАК Приоритет,
		|	ВЫБОР КОГДА ДД.ЗаказПоставщику ССЫЛКА Документ.СписаниеБезналичныхДенежныхСредств
		|		ИЛИ ДД.ЗаказПоставщику ССЫЛКА Документ.РасходныйКассовыйОрдер
		|		ИЛИ ДД.ЗаказПоставщику ССЫЛКА Документ.ПервичныйДокумент
		|			ТОГДА 1
		|		ИНАЧЕ 2
		|	КОНЕЦ КАК ПриоритетОбъектаРасчетовПлатежки,
		|	ВЫБОР
...
Процедура РассчитатьПартииПоЦепочкамИзВыборки(ОписаниеДвижений, РезультатЗапроса, РасчетныеПартии, ЦепочкиДвижений, Регистраторы,  Тест = Ложь, КоличествоЗаписей = 0)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если РасчетныеПартии.Количество() > 1000
			...
			РегистрыСведений.СуммыДокументовВВалютеРегл.РассчитатьПоДаннымВзаиморасчетов(ОписаниеДвижений.Контекст, РегистраторыКОтражению);
	...
	Если Расход[ОписаниеДвижений.БазисРасхода] <> 0 
		...
		Если ЗначениеЗаполнено(Расход.КорОбъектРасчетов) Тогда
			...
			Если СтрокиДвижений.Количество() > 0 Тогда
				НоваяПартия.ПредоплатаРегл = СтрокиДвижений[0].ПредоплатаРегл * ?(НоваяПартия.Предоплата > 0 И СтрокиДвижений[0].ПредоплатаРегл < 0,(-1),1);
				НоваяПартия.ПредоплатаУпр = СтрокиДвижений[0].ПредоплатаУпр * ?(НоваяПартия.Предоплата > 0 И СтрокиДвижений[0].ПредоплатаРегл < 0,(-1),1);;
			КонецЕсли;
		КонецЕсли;
...
Функция СформироватьЗаданияКРасчетуНалогов(МенеджерРегистра, Расчеты, Регистраторы)
	...
	Если МенеджерРегистра = РегистрыНакопления.РасчетыСПоставщикамиПоДокументам Тогда
		...
		ТекстЗапроса =
		"УНИЧТОЖИТЬ РасчетыСПоставщикамиИзменения
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
