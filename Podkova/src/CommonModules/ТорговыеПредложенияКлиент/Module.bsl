...
Процедура ОткрытьФормуПодсказок(Форма) Экспорт
	...
	ОбработчикЗакрытия = Новый ОписаниеОповещения("СкрытьПодсказкуОбработкаОповещения", ТорговыеПредложенияКлиент, Форма);
	...
	Если Подсказка.ТипРекомендации = "ДляПокупателя" Тогда
		...
		ПараметрыФормы = ОтборАртикуловИШтрихкодов(Подсказка.ДополнительныеПараметры.Номенклатура);
		
		ОткрытьФормуПоискаПоОтборам(ПараметрыФормы, Форма, ОбработчикЗакрытия);
		
	ИначеЕсли Подсказка.ТипРекомендации = "Административный" Тогда	
		
		ПерейтиПоСсылкеРекомендации(Подсказка.Ссылки.АдресСайта);
		
		ОтразитьОткликНаРекомендацию(ОрганизацияИзФормы(Форма), Форма.УникальныйИдентификатор, Подсказка.Ссылки.АдресОтклика);
		...
		Если ЗначениеЗаполнено(Организация)
			И Не БизнесСетьСлужебныйКлиент.ОрганизацияПодключена(Организация) Тогда
			
			БизнесСетьСлужебныйКлиент.ОткрытьФормуПодключенияОрганизации(Организация);
		КонецЕсли;
...
Процедура ОткрытьФормуПоискаПоОтборам(ПараметрыОткрытияФормы = Неопределено, ВладелецФормы = Неопределено, 
	...
	ОткрытьФорму("Обработка.ТорговыеПредложения.Форма.ПоискПоОтборам", ПараметрыОткрытияФормы, ВладелецФормы, КлючУникальности, Окно, ,
		ОписаниеОповещения);
...
//    * Валюта              - Ссылка - валюта торгового предложения.
//    * Номенклатура        - ОпределяемыйТип.НоменклатураБЭД - номенклатура.
//    * Характеристика      - Ссылка - характеристика номенклатуры.
...
//  ОписаниеОповещенияОЗакрытии - ОписаниеОповещения          - оповещение о закрытии формы.
//
Процедура ОткрытьКарточкуПубликуемогоТорговогоПредложения(ПараметрыОткрытияФормы, ФормаВладельца, ОписаниеОповещения = Неопределено) Экспорт 
...
Функция РекомендацииОтключеныДляТекущегоСеанса()
	...
	КэшПодсистемы = КэшПодсистемыТорговыеПредложения();
	
	Результат = КэшПодсистемы.Свойство("РекомендацииОтключены");
...
Процедура ОтключитьРекомендацииДляТекущегоСеанса()
	...
	КэшПодсистемы.Вставить("РекомендацииОтключены", Истина);
...
Функция ОтборАртикуловИШтрихкодов(Номенклатура)
	...
	Если ЗначениеЗаполнено(Артикулы) Тогда
		Результат.Вставить("ОтборАртикулы", Артикулы);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Штрихкоды) Тогда
		Результат.Вставить("ОтборШтрихкоды", Штрихкоды);
	КонецЕсли;
...
Функция КэшПодсистемыТорговыеПредложения()
	
	ИмяПодсистемы = "ЭлектронноеВзаимодействие.ТорговыеПредложения";
...
//   ДополнительныеПараметры - ФормаКлиентскогоПриложения - форма, с размещенными элементами подсказок.
//
Процедура СкрытьПодсказкуОбработкаОповещения(Результат, ДополнительныеПараметры) Экспорт
	
	Если ДополнительныеПараметры = Неопределено ИЛИ ТипЗнч(ДополнительныеПараметры) <> Тип("ФормаКлиентскогоПриложения") Тогда
		Возврат;
	КонецЕсли;
	
	СкрытьПодсказку(ДополнительныеПараметры);
...
Процедура СкрытьПодсказку(Форма)
	...
	БизнесСетьСлужебныйКлиент.ПоказатьОповещениеБизнесСети(НСтр("ru = 'Рекомендация больше показываться не будет'"));
...
Процедура ОбработатьНажатиеПоГиперссылкеРеквизитаКарточкиТорговогоПредложения(ПозицияТорговогоПредложения,
	...
	Перем Действие, ПредупреждениеРедактирования, ИдентификаторНоменклатурыСервиса, ЗначениеСопоставления;
	...
	ИначеЕсли Действие = "ОбработкаЛогикойБиблиотеки" И Расшифровка.ИмяРеквизита = "ПрайсЛистыСлужебный.СпособДоставки" Тогда
	...
	ИначеЕсли Действие = "ОбработкаЛогикойБиблиотеки" И Расшифровка.ИмяРеквизита = "ТоварыСлужебный.КатегорияСервиса" Тогда
		
		Номенклатура = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПозицияТорговогоПредложения.Номенклатура);
		
		ОбщийМодульРаботаСНоменклатуройКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("РаботаСНоменклатуройКлиент");
		ОбщийМодульРаботаСНоменклатуройКлиент.СопоставитьНоменклатуруКатегорииСервиса(Номенклатура, Форма, ОписаниеОповещенияОЗавершении);
				
	ИначеЕсли Действие = "ОбработкаЛогикойБиблиотеки" И Расшифровка.ИмяРеквизита = "ТоварыСлужебный.НоменклатураСервиса" Тогда
		
		ПараметрыФормы = РаботаСНоменклатуройКлиент.ПараметрыФормыСопоставленийНоменклатуры();
		ПараметрыФормы.Номенклатура             = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПозицияТорговогоПредложения.Номенклатура);
		ПараметрыФормы.ОтображатьСопоставленные = Истина;
		
		ОбщийМодульРаботаСНоменклатуройКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("РаботаСНоменклатуройКлиент");
		ОбщийМодульРаботаСНоменклатуройКлиент.ОткрытьФормуСопоставленияНоменклатуры(ПараметрыФормы, Форма, ОписаниеОповещенияОЗавершении);
		
	ИначеЕсли Действие = "ОбработкаЛогикойБиблиотеки" И Расшифровка.ИмяРеквизита = "СвойстваСлужебный.Свойство" Тогда
		
		ОбщийМодульРаботаСНоменклатуройКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("РаботаСНоменклатуройКлиент");
		ОбщийРаботаСНоменклатуройСлужебныйВызовСервера = ОбщегоНазначенияКлиент.ОбщийМодуль("РаботаСНоменклатуройСлужебныйВызовСервера");
		
		СведенияОСопоставленииНоменклатурыБизнесСеть = 
			ОбщийРаботаСНоменклатуройСлужебныйВызовСервера.СведенияОСопоставленииНоменклатурыБизнесСеть(ПозицияТорговогоПредложения.Номенклатура);
