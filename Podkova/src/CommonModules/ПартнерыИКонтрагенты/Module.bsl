...
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма, для которой устанавливаются отборы.
//
...
//  Форма  - ФормаКлиентскогоПриложения - Форма, в которой находится элемент, для которого устанавливается заголовок.
//  ИмяЭлементаФормыПартнер  - Строка - Имя элемента формы, для которого устанавливается заголовок.
//  ХарактерДоговора  - Перечисление.ХарактерыДоговоровФинансовыхИнструментов - характер договора, в зависимости от которого 
//                 устанавливается заголовок.
//
...
Процедура ЗаполнитьДанныеКонтрагентаПартнера(Приемник, Партнер, СИсторическимиДанными = Истина) Экспорт
	...
	|	КонтрагентыИсторияНаименований.НаименованиеПолное КАК НаименованиеПолное
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		...
		Если СИсторическимиДанными Тогда
			...
			Приемник.ИсторияНаименований.Загрузить(РезультатЗапроса[2].Выгрузить());
		КонецЕсли;
...
Процедура ЗаполнитьКонтактнуюИнформациюКонтрагентаПартнера(КонтрагентОбъект, Форма, КонтактнаяИнформацияПартнера, ПереноситьЮридическийАдрес = Истина) Экспорт
	...
	Если ПереноситьЮридическийАдрес
		...
		СтрокаКиКонтрагента.Значение = Форма.ЮридическийАдресКонтрагента;
...
//
// Параметры:
//  Форма   ФормаКлиентскогоПриложения - форма, настраивается доступность элементов.
//
...
Процедура ВключитьХранениеИсторииИзменений(ИмяВидаКонтактнойИнформации) Экспорт
	...
	Если НЕ Справочники.ВидыКонтактнойИнформации[ИмяВидаКонтактнойИнформации].ХранитьИсториюИзменений Тогда
