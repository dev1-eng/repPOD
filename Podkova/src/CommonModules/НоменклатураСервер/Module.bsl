	...
	Для СчетчикОперандов = 0 По СтрДлина(ШаблонНаименования) Цикл
		...
		Для СчетчикРеквизитов = 0 По МассивРеквизитовОперанда.Количество()-1 Цикл
			...
			Иначе
				...
				// Это доп. реквизит
				СвойстваОбъекта = УправлениеСвойствами.СвойстваОбъекта(ЗначениеОперанда);
				...
				Если СвойстваОбъекта.Количество() > 0 Тогда
					...
					|	И ДополнительныеРеквизитыИСведения.Наименование = &ИмяРеквизита";
					
					Запрос.УстановитьПараметр("СвойстваОбъекта", СвойстваОбъекта);
					Запрос.УстановитьПараметр("ИмяРеквизита",    ИмяРеквизита);
					
					ДопРеквизиты = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
					...
					ИначеЕсли ДопРеквизиты.Количество() > 1 Тогда
						...
						|В информационной базе хранится несколько записей дополнительных реквизитов и сведений с наименованием ""%ИмяРеквизита%"", входящих в состав набора дополнительных реквизитов и сведений записываемого элемента справочника ""%ИмяОбъекта%"".'");
...
//			*СуффиксДопРеквизита - Строка - значение по умолчанию "" - если в ТЧ два реквизита "Характеристика", то второй назван с суффиксом. 
...
//																вывести сообщение. Нужно переопределять, когда вместо колонки серии отображается какая-то
//																другая колонка, например, с какими-то строковыми представлениями.
//
...
Процедура ПроверитьЗаполнениеСерий(ДокументОбъект,ПараметрыУказанияСерий,Отказ,МассивНепроверяемыхРеквизитов = Неопределено, ПараметрыПроверки = Неопределено) Экспорт
	...
	Если Не ПараметрыУказанияСерий.ИспользоватьСерииНоменклатуры
		Или (ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументОбъект, "ДополнительныеСвойства")
	...
	Если ВыдаватьСообщения Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.ОшибкаКоличества Тогда
				...
				Иначе
					ТекстСообщения = НСтр("ru = 'Указанное количество товара ""%НоменклатураПредставление%"" с серией ""%Серия%"" равно %КоличествоСерий%. Политика учета серий данного товара предусматривает, что количество по любой серии этого товара всегда будет равно 1.'");
...
#Область ПроцедурыРаботыСНомерамиГТД

// Параметры заполнения номеров ГТД в табличной части объекта.
//
// Возвращаемое значение:
//	Структура - содержит следующие свойства:
//
// ОБЯЗАТЕЛЬНЫЕ:
//	* ПараметрыЗапроса	- Структура	- параметры запроса остатков номеров ГТД.
...
//		** Организация	- СправочникСсылка.Организации			- организация, для которой получаются сведения об остатках 
//																	номеров ГТД (обязательно к заполнению при варианте получения данных 'ПоОстаткамУХранителя').
//		** Партнер		- СправочникСсылка.Партнеры				- партнер, для которого получаются сведения об остатках номеров ГТД
//																	(обязательно к заполнению при варианте получения данных 'ПоОстаткамПереданныхНаКомиссию').
//		** Договор		- СправочникСсылка.ДоговорыКонтрагентов	- договор, по которому получаются сведения об остатках номеров ГТД
//																	(обязательно к заполнению при варианте получения данных 'ПоОстаткамУХранителя').
// НЕОБЯЗАТЕЛЬНЫЕ:
//	* ИмяПоляРаспоряжение	- Строка	- имя поля распоряжения, по которому получаются сведения о номерах ГТД.
//	* СтруктураДействий		- Структура	- действия по обработке строк табличной части.
//
Функция ПараметрыЗаполненияНомеровГТД() Экспорт
	...
	ПараметрыЗаполнения.Вставить("СтруктураДействий");
...
//
// Параметры:
//	ТаблицаТоваров			- ДанныеФормыКоллекция, ТабличнаяЧасть	 - таблица в которой заполняются номера ГТД. Должна содержать
//																		следующие колонки:
...
//	ДополнительныеПараметры	- Структура								 - параметры заполнения номеров ГТД 
//																		(см. НоменклатураСервер.ПараметрыЗаполненияНомеровГТД()).
//
Процедура ЗаполнитьНомераГТД(ТаблицаТоваров, ВариантПолученияДанных, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтрНомерГТДКЗаполнению Из НомераГТДКЗаполнению Цикл
		...
		КоличествоРаспределить = СтрНомерГТДКЗаполнению.Количество;
		...
		Для Каждого СтрТовары Из СтрокиТоваров Цикл
			...
			Иначе
				...
				Если ЗаполнятьКоличествоУпаковок Тогда
					НовоеКоличествоУпаковок = Окр(СтрТовары.КоличествоУпаковок * НовоеКоличество / СтрТовары.Количество,
													3,
													РежимОкругления.Окр15как20);
					
					НоваяСтрТовары.КоличествоУпаковок = НовоеКоличествоУпаковок;
					
					СтрТовары.КоличествоУпаковок = СтрТовары.КоличествоУпаковок - НовоеКоличествоУпаковок;
				КонецЕсли;
				...
				Если СтруктураДействий <> Неопределено Тогда
					...
					ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НоваяСтрТовары, СтруктураДействий, Неопределено);
				КонецЕсли;
			...
			Если ЗаполнятьСтрануПроисхождения Тогда
				СтрТовары.СтранаПроисхождения = СтрНомерГТДКЗаполнению.СтранаПроисхождения;
			КонецЕсли;
...
// Возвращает значение варианта переноса настроек по характеристикам.
//
// Параметры:
//	Приемник - СправочникСсылка.Номенклатура - номенклатура для которой выполняется перенос настроек по характеристикам.
//	Источник - СправочникСсылка.Номенклатура - номенклатура из которой выполняется перенос настроек по характеристикам.
//	СоответствияСкопированныхОбъектов - Структура, Соответствие - коллекция, хранящая информацию о скопированных характеристиках.
//
// Возвращаемое значение:
//	Строка - вариант переноса настроек по упаковкам.
//
...
//
// Параметры:
//  Параметры - Структура - или параметры формы выбора политик учета серий, или параметр обработчика
//                          ПриПолученииДанныхВыбора модуля
//							менеджера политик учета серий.
//
Процедура НастроитьОтборВПараметрахПолитикУчетаСерий(Параметры) Экспорт
	
	// При изменении этой процедуры нужно так же изменять Справочники.ВидыНоменклатуры.ФормаЭлемента.УдалитьНеКорректныеПолитики
	...
	Если Параметры.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияВЕТИС
		Или Параметры.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияПодконтрольнаяВЕТИС Тогда
		Параметры.Отбор.Вставить("УчитыватьОстаткиСерий", Истина);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Склад) Тогда
		Если ТипЗнч(Склад) = Тип("СправочникСсылка.Склады") Тогда
			...
			Если (Параметры.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияВЕТИС
				...
				Параметры.Отбор.Вставить("УчитыватьСебестоимостьПоСериям", Истина);
	...
	Иначе
		Если Параметры.Свойство("ДляСклада") Тогда
			...
			Если (Параметры.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияВЕТИС
					Или Параметры.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияПодконтрольнаяВЕТИС)
				И (СкладыСервер.ЕстьОрдерныйНаПоступлениеСклад()
				Или СкладыСервер.ЕстьОрдерныйНаОтгрузкуСклад()) Тогда
					Параметры.Отбор.Вставить("УчитыватьСебестоимостьПоСериям", Истина);
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВидНоменклатуры)
		...
		// Вообще политику в шапке можно менять, поэтому она не заблокирована БСП, но 
		// если сейчас в шапке указана политика с учетом серий по товарам в пути и неотфактуровке,
		// то менять политику можно только на ту, в которой те же значения флагов.
		
		ИменаРеквизитов = Новый Структура;
		ИменаРеквизитов.Вставить("УчетТоваровВПутиОтПоставщикаПоСериям",        "ПолитикаУчетаСерий.УчетТоваровВПутиОтПоставщикаПоСериям");
		ИменаРеквизитов.Вставить("УчетСерийВНеотфактурованныхПоставкахТоваров", "ПолитикаУчетаСерий.УчетСерийВНеотфактурованныхПоставкахТоваров");
		ИменаРеквизитов.Вставить("УчетСерийВПереданныхНаХранениеТоварах",       "ПолитикаУчетаСерий.УчетСерийВПереданныхНаХранениеТоварах");
		
		РеквизитыТекущейПолитикиУчетаСерий = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидНоменклатуры, ИменаРеквизитов);
		
		Параметры.Отбор.Вставить("УчетТоваровВПутиОтПоставщикаПоСериям",
					РеквизитыТекущейПолитикиУчетаСерий.УчетТоваровВПутиОтПоставщикаПоСериям);
		
		Параметры.Отбор.Вставить("УчетСерийВНеотфактурованныхПоставкахТоваров",
					РеквизитыТекущейПолитикиУчетаСерий.УчетТоваровВПутиОтПоставщикаПоСериям);
...
Функция ДопустимыеТипыПолитикУчетаСерий(Параметры)
	...
	ИначеЕсли Параметры.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар Тогда
		...
		Если Параметры.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара Тогда
			...
			Если Параметры.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПродукцияМаркируемаяДляГИСМ Тогда
...
// ордерная схема Возвращаемое значение:
//  Булево
//
...
Функция ЕстьВидНоменклатурыТребующийОтключенияОрдернойСхемыПриПоступленииИОтгрузки(Склад) Экспорт
	...
	|	НЕ ВидыНоменклатуры.ЭтоГруппа
	|	И ВидыНоменклатуры.ИспользоватьСерии
	|	И (ВидыНоменклатуры.ПодконтрольнаяПродукцияВЕТИС
...
Функция ЗаполнитьСерииПоДаннымТоваровУПартнеров(ТаблицаТовары, ДокументОбъект, ЗаполнятьПоОтрицательнымОстаткам) Экспорт
	...
	|		&СторноРегистраКоличество                 КАК КоличествоПоДаннымПартнера
	|	ИЗ
	...
	Если ЗаполнятьПоОтрицательнымОстаткам Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СторноРегистраКоличество",                  "-СторноРегистра.Количество");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеНаОстаток",                          "ТоварыОрганизацийОстатки.КоличествоОстаток < 0");
	Иначе
