...
// Если перед выводом новости пользователю, разработчику надо "доработать" ее текст - заменить какие-то теги, удалить
// блоки текста и т.п., то это можно сделать в этой процедуре.
...
//   если это новый пользователь, то сбросить важность у новостей, у которых есть категория "Только для новых
//   пользователей = Истина"; если это новый пользователь, то удалить строки с определенной лентой новостей.
//
// Параметры:
//  ИдентификаторМетаданных          - Строка - Идентификатор метаданных, должен совпадать с таким же идентификатором в
//                                              самой новости;
//  ИдентификаторФормы               - Строка - Идентификатор формы, должен совпадать с таким же идентификатором в самой
//                                              новости;
...
//
// Параметры:
//  СписокЛентНовостей - СписокЗначений - Список лент новостей, разрешенных для просмотра (рассчитан по-умолчанию, можно
//                                        изменить).
//
// Возвращаемое значение:
...
// состояние новости. Важное замечание: нельзя вносить изменения в ОбъектНовость, т.к. процедура вызывается из события ПриЗаписи.
...
//                                             тратить время при каждом открытии формы, но позволит реализовать сценарий
//                                             показа новостей при наступлении клиентских событий (ПриИзмененииГалочки,
//                                             ПриУстановкеДатыБольше2014 и т.п.). Можно получать новости в обработчике
//                                             ожидания после открытия формы, также будет задействован клиентский кэш.
//                                             Это сократит время открытия формы, но некоторые сценарии станут
//                                             недоступны. Ключи:
...
//                                             Имеет смысл только если ПолучатьНовостиНаСервере = Истина. По-умолчанию =
//                                             Ложь.
//  ИдентификаторыСобытийПриОткрытии         - Строка, Массив - строка или массив строк идентификаторов события "ПриОткрытии".
//                                             Если передано какое-то значение, то будет подсчитано количество очень
//                                             важных контекстных новостей. И если это количество > 0, то открывающая
//                                             форма инициирует открытие формы просмотра таких новостей;
...
// Например, программно добавить новости, или изменить свойства лент новостей, или изменить расписание регламентного
// задания ВсеОбновленияНовостей.
//
...
//     - 1 - по ленте новостей (без учета корзины), должен быть ключ ЛентаНовостей. Если ключа "ЛентаНовостей" нет, или
//           он пустой, или неправильного типа, то будут отобраны все новости (без учета корзины);
//     - 2 - результат поиска. Также должен быть передан ключ "СписокНайденныхНовостей" типа СписокЗначений;
//     - 3 - контекстные новости (новости по объекту метаданных / форме). Также должны быть переданы структура с ключами
//           "Метаданные" и "Форма". Если значение ключа Форма = Неопределено, то будут получены новости по всем формам
//           для этого объекта метаданных;
...
//
// Параметры:
//  ОченьВажныеНовости - Массив - массив структур, в который будут возвращены новости с важностью "Очень важная", где:
...
//  ВажныеНовости      - Массив - массив структур, в который будут возвращены новости с важностью "Важная", где:
//    * Новость                        - СправочникСсылка.Новости - ссылка на новость;
//    * НовостьУникальныйИдентификатор - УникальныйИдентификатор - УИН от ссылки на новость;
