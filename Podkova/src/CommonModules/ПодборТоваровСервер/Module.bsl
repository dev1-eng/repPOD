...
Функция ВыполнитьПоискНоменклатурыПартнера(Форма) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Если Не Форма.ИспользоватьПолнотекстовыйПоиск Или Форма.НайтиНоменклатуруПартнераПоТочномуСоответствию Тогда
			ВыполнитьНеПолнотекстовыйПоискНоменклатурыПартнера(Форма, РезультатПоиска);
		Иначе
			ВыполнитьПолнотекстовыйПоискНоменклатурыПартнера(Форма, РезультатПоиска);
		КонецЕсли;
		ПоискВыполнен = Истина;
	КонецЕсли;
	
	Форма.ПоискНоменклатурыПартнераНеУдачный = (ПоискВыполнен И ЗначениеЗаполнено(РезультатПоиска.КодОшибки));
	Форма.КодОшибкиПоиска = РезультатПоиска.КодОшибки;
	
	ПодборТоваровКлиентСервер.УстановитьОтборСпискаПоСтрокеПоиска(
		ОбщегоНазначенияУТКлиентСервер.ПолучитьОтборДинамическогоСписка(Форма.СписокНоменклатураПартнера).Элементы,
		РезультатПоиска.НоменклатураПартнеров, ПоискВыполнен);
...
// Выполняет полнотекстовый поиск номенклатуры партнера по строке поиска введенной в форме.
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма списка, форма выбора. форма подбора,
//	РезультатПоиска - Неопределено - переменная, в которую записывается результат поиска.
//
Процедура ВыполнитьПолнотекстовыйПоискНоменклатурыПартнера(Форма, РезультатПоиска)
	...
	СписокПоиска = ПолнотекстовыйПоиск.СоздатьСписок(СтрокаПоиска);
	СписокПоиска.ПолучатьОписание = Ложь;
	СписокПоиска.ОбластьПоиска = ОбластиПоискаНоменклатурыПартнера(Форма);
...
// Выполняет неполнотекстовый поиск номенклатуры партнера по строке поиска.
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма из которой была вызвана функция поиска,
//	РезультатПоиска - Неопределено - переменная, в которую записывается структура, результат поиска.
//
Процедура ВыполнитьНеПолнотекстовыйПоискНоменклатурыПартнера(Форма, РезультатПоиска)
	
	РезультатПоиска = СтруктураРезультатовПоиска();
	СписокПоиска = РезультатПоискаНоменклатурыПартнера(Форма);
	ОбработатьСписокПоиска(СписокПоиска, РезультатПоиска);
	УстановитьКодОшибкиРасширенногоПоиска(СписокПоиска, РезультатПоиска);
...
// Возвращает результат поиска номенклатуры партнера.
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма подбора, форма списка.
//
...
//	Структура - результаты поиска характеристик.
//
Функция РезультатПоискаНоменклатурыПартнера(Форма)
	
	ТочноеСоответствие = Форма.НайтиНоменклатуруПартнераПоТочномуСоответствию;
	
	СтрокаПоиска = ФорматироватьСтрокуПоиска(Форма.СтрокаПоискаНоменклатураПартнера, ТочноеСоответствие);
	ОбластиПоиска = ОбластиПоискаНоменклатурыПартнера(Форма);
	МаксимальноеКоличествоЭлементовПоиска = МаксимальноеКоличествоЭлементовПоиска();
	РезультатПоиска = СтруктураРезультатаПоиска();
...
Функция ОбластиПоискаНоменклатуры(Форма)
	...
	Если ПодборТоваровКлиентСервер.ЭтоФормаПодбораВДокументыЗакупки(Форма) Тогда
		ОбластиПоиска.Добавить(Метаданные.Справочники.НоменклатураКонтрагентов);
	КонецЕсли;
...
// Возвращает массив областей поиска номенклатуры партнера.
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма из которой была вызвана функция поиска.
//
...
//	Массив. Массив метаданных, областей поиска товаров.
//
Функция ОбластиПоискаНоменклатурыПартнера(Форма)
...
Функция СтруктураРезультатовПоиска()
	...
	РезультатПоиска.Вставить("НоменклатураПартнеров", Новый Массив);
...
Процедура ДополнитьРезультатыПоискаНоменклатуры(РезультатПоиска)
	...
	Если РезультатПоиска.НоменклатураПартнеров.Количество() > 0 Тогда
		Запрос.Текст = Запрос.Текст + ?(ЗначениеЗаполнено(Запрос.Текст), " ОБЪЕДИНИТЬ ВСЕ ", "") + 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	НоменклатураКонтрагентов.Ссылка В(&СписокНоменклатурыПартнеров)";
		
		Запрос.УстановитьПараметр("СписокНоменклатурыПартнеров", РезультатПоиска.НоменклатураПартнеров);
	КонецЕсли;
...
// Возвращает форматированную строку поиска - строку с добавлением
...
// Выполняет обработку списка результатов поиска. Вызывается функция определения принадлежности
...
Функция ПараметрыШаблонаТекстаЗапроса()
	...
	ПараметрыШаблона.Добавить("{СправочникНоменклатураПартнеров}");
	
	ПараметрыШаблона.Добавить("{СправочникНоменклатураОбщиеПоля}");
	ПараметрыШаблона.Добавить("{СправочникХарактеристикиОбщиеПоля}");
	ПараметрыШаблона.Добавить("{СправочникНоменклатураПартнеровОбщиеПоля}");
...
Функция ШаблонТекстаЗапросаСпискаНоменклатурыПартнераПоПараметрамСоглашенияСПоставщиком(ВариантРасчетаЦенПоставщика)
	...
	Если ВариантРасчетаЦенПоставщика = "БезЦен" Тогда
		...
		|	{СправочникНоменклатураПартнеровОбщиеПоля}
		...
		|	{СправочникНоменклатураПартнеров}";
	...
	ИначеЕсли ВариантРасчетаЦенПоставщика = "ЦеныПоСоглашению" Тогда
		...
		|			И (ЦеныНоменклатурыПоставщиков.Номенклатура = СправочникНоменклатураПартнеров.Номенклатура)
		|			И (ЦеныНоменклатурыПоставщиков.Характеристика = СправочникНоменклатураПартнеров.Характеристика)
...
Функция ПодстрокаЗаменыПараметраШаблонаТекстаЗапроса(ПодстрокаПоиска, ПараметрыЗамены)
	...
	ИначеЕсли ПодстрокаПоиска = "{СправочникНоменклатураПартнеров}" Тогда
		
		ПодстрокаЗамены = "
		|	Справочник.НоменклатураКонтрагентов КАК СправочникНоменклатураПартнеров
		|";
	...
	ИначеЕсли ПодстрокаПоиска = "{СправочникНоменклатураПартнеровОбщиеПоля}" Тогда
		
		ПодстрокаЗамены = "
		|	СправочникНоменклатураПартнеров.ПометкаУдаления КАК ПометкаУдаления,
		|	СправочникНоменклатураПартнеров.Предопределенный КАК Предопределенный,
		|	СправочникНоменклатураПартнеров.Владелец КАК Владелец,
		|	СправочникНоменклатураПартнеров.Родитель КАК Родитель,
		|	СправочникНоменклатураПартнеров.ЭтоГруппа КАК ЭтоГруппа,
		|	СправочникНоменклатураПартнеров.Наименование КАК Наименование,
		|	СправочникНоменклатураПартнеров.Номенклатура КАК Номенклатура,
		|	СправочникНоменклатураПартнеров.Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
		|	СправочникНоменклатураПартнеров.Номенклатура.ИспользованиеХарактеристик КАК ИспользованиеХарактеристик,
		|	СправочникНоменклатураПартнеров.Номенклатура.НаименованиеПолное КАК НаименованиеПолное,
		|	СправочникНоменклатураПартнеров.Характеристика КАК Характеристика,
		|	СправочникНоменклатураПартнеров.Упаковка КАК Упаковка,
		|	СправочникНоменклатураПартнеров.Артикул КАК Артикул,
		|	СправочникНоменклатураПартнеров.Код КАК Код,
		|	СправочникНоменклатураПартнеров.Номенклатура.СтавкаНДС КАК СтавкаНДС,
		|	СправочникНоменклатураПартнеров.Ссылка КАК Ссылка,
		|	ВЫБОР
		|		КОГДА СправочникНоменклатураПартнеров.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЭтоТовар,
		|	ВЫБОР
		|		КОГДА СправочникНоменклатураПартнеров.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЭтоУслуга,
		|	ВЫБОР
		|		КОГДА СправочникНоменклатураПартнеров.ЭтоГруппа
		|			ТОГДА ВЫБОР
		|					КОГДА СправочникНоменклатураПартнеров.ПометкаУдаления
		|						ТОГДА 13
		|					ИНАЧЕ 12
		|				КОНЕЦ
		|	КОНЕЦ КАК ИндексКартинки,
	...
	ИначеЕсли ПодстрокаПоиска = "{ЦеныНоменклатурыПоставщиковСрезПоследнихНаДату}" Тогда
		...
		ПодстрокаЗамены = ?(ЕстьЦеныНоменклатурыПартнеровВБудущем, "&Дата", "");
	...
	ИначеЕсли ПодстрокаПоиска = "{СвободныеОстатки}" Тогда
		...
		Иначе
			...
			Если ТипСписка = "СписокНоменклатура"
				Или ТипСписка = "СписокНоменклатураПартнера" Тогда
