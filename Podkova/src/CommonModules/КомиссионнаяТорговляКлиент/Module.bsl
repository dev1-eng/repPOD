...
Функция НеобходимоРассчитатьКомиссионноеВознаграждение(ОписаниеОповещения, Форма)
	...
	Если Форма.Объект.Товары.Количество() > 0 Тогда
		...
		ОписаниеОповещенияПродолжение = Новый ОписаниеОповещения("НеобходимоРассчитатьКомиссионноеВознаграждениеВопросЗавершение", КомиссионнаяТорговляКлиент, ДополнительныеПараметры);
		...
		Если ЗначениеЗаполнено(Форма.Объект.СпособРасчетаВознаграждения) Тогда
			...
			ПоказатьВопрос(ОписаниеОповещенияПродолжение,
		...
		Иначе 
			...
			ВыполнитьОбработкуОповещения(ОписаниеОповещенияПродолжение, Истина);   
		КонецЕсли;
...
Процедура НеобходимоРассчитатьКомиссионноеВознаграждениеВопросЗавершение(НеобходимоРассчитать, ДополнительныеПараметры) Экспорт
	...
	КЧ = Новый КвалификаторыЧисла(5, 2, ДопустимыйЗнак.Неотрицательный);
