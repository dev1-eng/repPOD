...
//
Функция НайтиПроверкуПоКлючевымПолям(КонтекстВыполнения, Идентификатор, ВозвращатьТолькоСсылку = Истина) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВЫБОР КОГДА Проверки.КонтекстВыполнения = &КонтекстВыполнения ТОГДА ИСТИНА ИНАЧЕ ЛОЖЬ КОНЕЦ КАК КонтекстЗаполнен,
	|	Проверки.Ссылка,
	|	Проверки.Код,
	...
	|	Проверки.УточнениеКонтекстаВыполнения,
	|	Проверки.ВыполняетсяТолькоВКонтексте,
	|	Проверки.ПредшествующаяПроверка,
	|	Проверки.Обработчик,
	|	Проверки.Комментарий
	|ИЗ
	|	Справочник.ПроверкиСостоянияСистемы КАК Проверки
	|ГДЕ
	|	Проверки.Идентификатор = &Идентификатор
	|	И (Проверки.КонтекстВыполнения = &КонтекстВыполнения
	|			ИЛИ Проверки.КонтекстВыполнения = &ПустойКонтекст
	|				И НЕ Проверки.ПометкаУдаления
	|				И Проверки.Родитель = ЗНАЧЕНИЕ(Справочник.ПроверкиСостоянияСистемы.ПустаяСсылка))
	|
	|УПОРЯДОЧИТЬ ПО
	|	Проверки.ПометкаУдаления,
	|	КонтекстЗаполнен УБЫВ";
	...
	Если Выборка.Следующий() Тогда
		...
		Результат = Новый Структура("Ссылка, Код, Наименование, Важность, Идентификатор, Используется, СпособВыполнения,
			|КонтекстВыполнения, УточнениеКонтекстаВыполнения, ВыполняетсяТолькоВКонтексте, ПредшествующаяПроверка, Обработчик, Комментарий");
...
Функция ЗаполнитьСправочникПроверокСостоянияСистемы(ТипКонтекстаВыполнения = Неопределено, РежимОбновленияПроверок = Ложь) Экспорт
	...
	|	ПроверкиСостоянияСистемы.Родитель КАК Родитель,
	|	ЕСТЬNULL(ПроверкиСостоянияСистемы.Родитель.Идентификатор, """") КАК ИдентификаторРодителя,
...
Функция ЗарегистрироватьГруппуПроверокСостоянияСистемы(Описание, СуществующиеПроверки, РежимОбновленияПроверок = Ложь)
	...
	Попытка
		...
		Если РежимОбновленияПроверок И ЗначениеЗаполнено(ГруппаПроверок) И ПоляГруппыПроверокНеИзменились(ГруппаПроверок, Описание, СуществующиеПроверки) Тогда
	...
	Попытка
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Проверка) Тогда
				...
				ПроверкаОбъект.ПометкаУдаления = Ложь;
...
Функция ПоляГруппыПроверокНеИзменились(Проверка, Описание, СуществующиеПроверки)
	...
	Если ПоляПроверки.Наименование = Описание.Наименование
	 И ПоляПроверки.ИдентификаторРодителя = Описание.ИдентификаторРодителя
	 И НЕ ПоляПроверки.ПометкаУдаления Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ПоляПроверкиНеИзменились(Проверка, Описание, СуществующиеПроверки)
	...
	Если ПоляПроверки.Наименование = Описание.Наименование
	 ...
	 И (ПоляПроверки.Важность = Описание.Важность ИЛИ Описание.ВозможноИзменениеВажности)
	 И НЕ ПоляПроверки.ПометкаУдаления Тогда
		Возврат Истина;
	КонецЕсли;
