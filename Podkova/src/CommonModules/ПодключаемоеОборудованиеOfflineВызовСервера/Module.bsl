...
Функция ПолучитьДанныеДляВесов(Устройство, ТолькоИзмененные = Истина) Экспорт
	...
	ВозвращаемоеЗначение.Вставить("ЧастичнаяВыгрузка", ТолькоИзмененные И Параметры.ВыгружатьИзменения);
	ВозвращаемоеЗначение.Вставить("Параметры", Параметры);
	
	Таблица = ДанныеТоваровДляВыгрузки(Устройство, Параметры, Истина);
...
Процедура ЗаполнитьСтруктуруПрайсЛистаИзДанныхКВыгрузке(СтруктураПрайсЛиста, ТоварыКВыгрузке) Экспорт
	...
	Пока ВыборкаПоНоменклатуре.Следующий() Цикл
		...
		Если ВыборкаПоНоменклатуре.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПродукцияМаркируемаяДляГИСМ Тогда
...
Функция ДанныеТоваровДляВыгрузки(Устройство, Параметры, ОбновитьКодыТоваров = Ложь, Штрихкод = "") Экспорт
	...
	|			//ВесыСПечатьюЭтикеток И ШтрихкодыНоменклатуры.Штрихкод ПОДОБНО &ФорматШтрихкода СПЕЦСИМВОЛ ""\""
...
//
Функция ДанныеТоваровПоПравилуОбмена(ПравилоОбмена, ВидЦены) Экспорт
...
Процедура ЗаполнитьНастройкиККМ(ОфлайнОборудование, НастройкиОборудования) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		//-- Локализация
		Иначе
			НалогообложениеНДСККТ = РозничныеПродажи.СистемаНалогообложенияФискальнойОперации(Выборка.Организация);
		КонецЕсли;
...
Процедура ЗаполнитьПрайсЛистККМ(ОфлайнОборудование, ПрайсЛист, ПолнаяВыгрузка) Экспорт
	...
	ТоварыКВыгрузке = ДанныеТоваровДляВыгрузки(ОфлайнОборудование, Параметры, Истина);
...
Процедура ЗагрузитьОтчетыОПродажахИзККМ(Устройство, ДанныеОПродажах, Отказ, СообщениеОбОшибке) Экспорт
	...
	ТаблицаДатОтчетов = Новый ТаблицаЗначений;
	ТаблицаДатОтчетов.Колонки.Добавить("УникальныйИдентификатор", Новый ОписаниеТипов("Строка"));
	ТаблицаДатОтчетов.Колонки.Добавить("Дата", Новый ОписаниеТипов("Дата",,,,,Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	
	Для Каждого ОтчетОПродажах Из ДанныеОПродажах Цикл
		НоваяСтрока = ТаблицаДатОтчетов.Добавить();
	...
	Пока ВыборкаПоИдентификаторам.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаПоИдентификаторам.УникальныйИдентификатор) Тогда
			СтрокиПоИдентификатору = ТаблицаДатОтчетов.НайтиСтроки(Новый Структура("УникальныйИдентификатор", СокрЛП(ВыборкаПоИдентификаторам.УникальныйИдентификатор)));
			Если СтрокиПоИдентификатору.Количество() > 0 Тогда
				ОтчетОРозничныхПродажахОбъект.Дата       = СтрокиПоИдентификатору[0].Дата;
			Иначе
		...
		Пока ВыборкаПоТоварам.Следующий() Цикл
			...
			Если ВыборкаПоТоварам.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Набор Тогда
				...
				Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура) Тогда
					НоваяСтрока.СтавкаНДС = УчетНДСРФ.СтавкаНДС(
						ОтчетОРозничныхПродажахОбъект.НалогообложениеНДС,
				...
				НоваяСтрока.СуммаНДС            = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(НоваяСтрока.Сумма, НоваяСтрока.СтавкаНДС, Выборка.ЦенаВключаетНДС);
		...
		Если ЗначениеЗаполнено(ВыборкаПоИдентификаторам.УникальныйИдентификатор) Тогда
			СтрокиПоИдентификатору = ТаблицаАкцизныеМарки.НайтиСтроки(Новый Структура("УникальныйИдентификатор", СокрЛП(ВыборкаПоИдентификаторам.УникальныйИдентификатор)));
		Иначе
		...
		Для Каждого ВыборкаПоТоварам Из ТаблицаНаборов Цикл
			...
			ПараметрыЗаполнения.Вставить("Дата", ОтчетОРозничныхПродажахОбъект.Дата);
			ПараметрыЗаполнения.Вставить("Валюта", ОтчетОРозничныхПродажахОбъект.Валюта);
			ПараметрыЗаполнения.Вставить("ВидЦены", ОтчетОРозничныхПродажахОбъект.ВидЦены);
			...
			Для Каждого СтрокаСлужебнойТЧ Из СлужебнаяТЧ Цикл
				...
				СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ОтчетОРозничныхПродажахОбъект);
		...
		ТаблицаОплат.Свернуть("УникальныйИдентификатор, ТипОплаты", "Сумма");
		ОтчетОРозничныхПродажахОбъект.ОплатаПлатежнымиКартами.Очистить();
		
		Если ЗначениеЗаполнено(ВыборкаПоИдентификаторам.УникальныйИдентификатор) Тогда
			ОплатыДокумента = ТаблицаОплат.НайтиСтроки(Новый Структура("УникальныйИдентификатор", СокрЛП(ВыборкаПоИдентификаторам.УникальныйИдентификатор)));
		Иначе
			ОплатыДокумента = ТаблицаОплат;
		КонецЕсли;
		
		Если ПолучитьФункциональнуюОпцию("ИспользоватьОплатуПлатежнымиКартами") Тогда
			Если ЭквайринговыйТерминал = Неопределено Тогда
				ЭквайринговыйТерминал = ПолучитьЭквайринговыйТерминал(РабочееМестоКассира);
			КонецЕсли;
			
			Для каждого Оплата Из ОплатыДокумента Цикл
				Если Оплата.ТипОплаты = Перечисления.ТипыОплатыККТ.Электронно Тогда
					НоваяСтрокаОплаты = ОтчетОРозничныхПродажахОбъект.ОплатаПлатежнымиКартами.Добавить();
...
Функция ПолучитьЭквайринговыйТерминал(РабочееМестоКассира)
