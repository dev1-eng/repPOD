...
Функция ТекстГиперссылкиВДокументеОсновании(Ссылка, ТипНадписи) Экспорт
	...
	Если ТипНадписи = "УведомлениеОбОтгрузке" Тогда
...
Функция ПрочитатьПервичныеДанныеВходящегоСообщенияXML(ТекстСообщенияXML) Экспорт
	...
	ОбъектXTDOДляПолученияВерсии = ИнтеграцияИС.ПроизвольныйОбъектXDTOПоТекстуСообщенияXML(ТекстСообщенияXML);
	...
	Попытка
		...
		Иначе
			...
			Иначе
				...
				ВозвращаемоеЗначение.Результат = ИнтеграцияИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.result.success);
...
Функция ОбработатьОтветНаПередачуДанных(ТекстВходящегоСообщенияSOAP) Экспорт
	...
	Попытка
		...
		Иначе
			...
			ВозвращаемоеЗначение.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийГИСМ.Принято;
			ВозвращаемоеЗначение.Результат = ИнтеграцияИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.success);
	...
	Попытка
		...
		Иначе
			...
			Если ОбъектXDTO.item_body.sender_info <> Неопределено Тогда
				...
				Результат.ДанныеОтправителя = ИнтеграцияИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.item_body.sender_info);
			...
			Для Каждого Свойство Из ОбъектXDTO.item_body.Свойства() Цикл
				...
				Результат.ДанныеДокумента = ИнтеграцияИС.ОбъектXDTOВСтруктуру(ЗначениеСвойства);
...
Функция ОбработатьОтветНаЗапросПолученияДокументов(ТекстВходящегоСообщенияSOAP) Экспорт
	...
	Попытка
		...
		Иначе
			...
			ВозвращаемоеЗначение.Результат = ИнтеграцияИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.item_list);
	...
	Попытка
		...
		Иначе
			...
			Если ОбъектXDTO.success.item_list <> Неопределено Тогда
				ВозвращаемоеЗначение.Результат = ИнтеграцияИС.ОбъектXDTOВСтруктуру(ОбъектXDTO.success.item_list);
			Иначе
...
Функция ИдентификаторОрганизацииГИСМ(Ссылка)
	...
	Если ИнтеграцияИС.ИспользоватьПодразделения() Тогда
		...
		МенеджерОбъектаПодразделение = ИнтеграцияИС.МенеджерОбъектаПоПолномуИмени(ПолноеИмяПодразделение);
...
Функция ПоместитьТекстСообщенияXMLВКонвертSOAP(ТекстСообщенияXML) Экспорт
	...
	ТекстСообщенияXMLБезОтступов = ИнтеграцияИС.ФорматироватьXML(ТекстСообщенияXML, Ложь);
...
Функция ТекстВходящегоСообщенияXML(КонвертSOAP) Экспорт
	...
	ИначеЕсли ТипЗнч(ЭлементDOM) = Тип("ЭлементDOM") Тогда
		...
		ЗаписьXML.УстановитьСтроку(ИнтеграцияИС.ПараметрыФорматированияXML(Ложь));
