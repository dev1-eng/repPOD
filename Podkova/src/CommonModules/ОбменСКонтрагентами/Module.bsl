...
//     * ПрикладнойТипДокумента - ОпределеяемыйТип.ПрикладныеВидыЭлектронныхДокументов - тип прикладного электронного документа.
...
Процедура ОпределитьСвойстваВладельцевЭлектронныхДокументов(СвойстваВладельцевЭД, МассивВладельцевЭД = Неопределено) Экспорт
	...
	|				КОНЕЦ
	|		КОГДА СостоянияЭД.ЭлектронныйДокумент.ВидЭД = ЗНАЧЕНИЕ(Перечисление.ВидыЭД.КорректировочныйСчетФактура)
	|				ИЛИ СостоянияЭД.ЭлектронныйДокумент.ВидЭД = ЗНАЧЕНИЕ(Перечисление.ВидыЭД.УКД)
	|			ТОГДА ВЫБОР
	...
	|				КОНЕЦ
	|		КОГДА СостоянияЭД.ЭлектронныйДокумент.ВидЭД = ЗНАЧЕНИЕ(Перечисление.ВидыЭД.СчетФактура)
	|				ИЛИ СостоянияЭД.ЭлектронныйДокумент.ВидЭД = ЗНАЧЕНИЕ(Перечисление.ВидыЭД.УПД)
	|			ТОГДА ВЫБОР
	|					КОГДА СостоянияЭД.ЭлектронныйДокумент.ТипЭлементаВерсииЭД = ЗНАЧЕНИЕ(Перечисление.ТипыЭлементовВерсииЭД.ЭСФ)
	|						ТОГДА ""СчетФактура""
	|					ИНАЧЕ ""УПД""
...
Процедура УстановитьСоответствиеНоменклатуры(Знач НоменклатураКонтрагента, Знач НоменклатураИБ = Неопределено, 
	...
	Если ОбменСКонтрагентамиСлужебныйПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовСлужебный().УстановитьСоответствиеНоменклатуры(
...
Процедура УдалитьСоответствиеНоменклатуры(Знач НоменклатураКонтрагента) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовСлужебный().УдалитьСоответствиеНоменклатуры(
			НоменклатураКонтрагента);
		Возврат;
	КонецЕсли;
	
	ОбменСКонтрагентамиСлужебный.УдалитьСоответствиеНоменклатуры(НоменклатураКонтрагента);
...
Функция НайтиСоответствиеНоменклатуры(Знач Отбор = Неопределено, Знач ТолькоСопоставленные = Ложь) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		Возврат ОбменСКонтрагентамиСлужебныйПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовСлужебный().НайтиСоответствиеНоменклатуры(
			Отбор, ТолькоСопоставленные);
	КонецЕсли;
	
	Возврат ОбменСКонтрагентамиСлужебный.НайтиСоответствиеНоменклатуры(Отбор, ТолькоСопоставленные);
...
Процедура СоздатьВременнуюТаблицуСоответствияНоменклатуры(Знач ТаблицаОтбора, Знач ИмяВременнойТаблицы, МенеджерВременныхТаблиц) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентов().СоздатьВременнуюТаблицуСоответствияНоменклатуры(
...
Процедура ПриСозданииФормыНоменклатуры(Форма) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентов().ПриСозданииФормыНоменклатуры(Форма);
		Возврат;
	КонецЕсли;

	ОбменСКонтрагентамиСлужебный.ПриСозданииФормыНоменклатуры(Форма);
...
Процедура ПриСозданииФормыХарактеристики(Форма) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
		ОбменСКонтрагентамиСлужебныйПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентов().ПриСозданииФормыХарактеристики(Форма);
		Возврат;
	КонецЕсли;
	
	ОбменСКонтрагентамиСлужебный.ПриСозданииФормыХарактеристики(Форма);
...
// Устанавливает признак функционала внутренних электронных документов.
//
// Параметры:
...
Процедура ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭД") Или Не ПолучитьФункциональнуюОпцию("ИспользоватьВнутренниеДокументыЭДО") Тогда
		Возврат;
	КонецЕсли;
...
#Область РегистрыСведений_СловарьСопоставленияНоменклатурыБЭД_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Справочники.ХарактеристикиНоменклатуры.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "После";	
