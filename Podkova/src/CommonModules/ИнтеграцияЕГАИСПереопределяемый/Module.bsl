...
Процедура ОтменитьПроведениеПоступленияТоваровПриОтказеОтТТН(ТТНВходящаяЕГАИС) Экспорт
	...
	ДокументПоступления = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТТНВходящаяЕГАИС, "ДокументПоступления");
...
Процедура ОпределитьДокументПоступлениеТоваров(ОбъектМетаданных) Экспорт
	...
	//++ НЕ ГОСИС
	ОбъектМетаданных = Метаданные.Документы.ПриобретениеТоваровУслуг;
	//-- НЕ ГОСИС
	Возврат;
...
Процедура ТекстЗапросаВТКоэффициентыПересчетаВЕдиницыЕГАИС(ТекстЗапроса, ИмяТаблицыТовары, ИмяВременнойТаблицы) Экспорт
	...
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА Таблица.АлкогольнаяПродукция.ТипПродукции <> ЗНАЧЕНИЕ(Перечисление.ТипыПродукцииЕГАИС.Неупакованная) ТОГДА 1
	|		КОГДА ЕСТЬNULL(Таблица.Номенклатура.ОбъемДАЛ, 0) = 0 ТОГДА 1
