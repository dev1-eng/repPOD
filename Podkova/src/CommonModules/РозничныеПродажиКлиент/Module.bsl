...
Процедура ЗакрытьКассовуюСмену(ПараметрыКассыККМ, ОписаниеОповещенияЗавершение = Неопределено) Экспорт
	...
	//1softpro +++ закрытие кассовой смены в ргеистре ШМ_Смены при закрытии и открытии кассовой смены при ошибке 24 часа
	РозничныеПродажиКлиентСервер.ЗакрытьСменуВРегистре();
	// ---
...
Процедура ОбработатьСостояниеСмены(Форма, ОписаниеОповещенияЗавершение) Экспорт
	...
	Если Форма.СтруктураСостояниеКассовойСмены.Ошибка24Часа И Форма.ПраваДоступа.ОткрытьСмену И Форма.ПраваДоступа.ЗакрытьСмену Тогда
	...
	ИначеЕсли Не Форма.СтруктураСостояниеКассовойСмены.СменаОткрыта И (Не Форма.ПраваДоступа.ОткрытьСмену ИЛИ Не Форма.ПраваДоступа.ЗакрытьСмену) Тогда
...
Процедура ОтменитьОплатуКартой(РезультатПроведения, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ИдентификаторУстройстваЭТ)
		...
		Если РезультатЭТ Тогда
			...
			Иначе
				ТекущиеДанные.НомерПлатежнойКарты = "1111111111111111";
