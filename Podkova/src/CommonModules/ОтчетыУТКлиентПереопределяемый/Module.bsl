...
Процедура ОбработчикКоманды(ЭтаФорма, Команда, Результат) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтаФорма, "ОтчетСформирован") Тогда
...
Процедура ОткрытьОтчетКарточкаРасчетовСКлиентами(КлючВарианта, Документ) Экспорт
	...
	Если ОтчетыУТВызовСервераПереопределяемый.ЗначениеФункциональнойОпции("НоваяАрхитектураВзаиморасчетов") Тогда
...
//  Расшифровка							 - Строка			 - сериализованная в XML структура, см. Отчет.КонтрольОформленияДокументовТовародвижений.СтруктураОтборовРаспоряженийСтрокаXML.
