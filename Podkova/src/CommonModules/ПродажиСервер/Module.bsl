...
//
Функция ПолучитьХозяйственнуюОперациюРеализацииПоВозврата(Знач ХозяйственнаяОперацияВозврат) Экспорт
...
Функция СкорректироватьСтрокиЗаказа(СтруктураПараметров) Экспорт
	...
	|		,, ЗаказКлиента = &ЗаказКлиента И КодСтроки <> 0) КАК ЗаказыКлиентовОстатки
	|СГРУППИРОВАТЬ ПО
...
Функция ПерезаполнитьНакладнуюПоОрдерам(Объект) Экспорт
	...
	Если ЭтоРеализация Тогда
		...
		СтруктураДействийСИзмененнымиСтроками.Вставить("ПересчитатьСуммуСверхЗаказа",
	...
	Если ЭтоРеализация Тогда
		...
		СтруктураДействийСДобавленнымиСтроками.Вставить("ПересчитатьСуммуСверхЗаказа",
								Новый Структура("РеализацияПоступлениеПоЗаказу, ТребуетсяЗалогЗаТару",
										Объект.РеализацияПоЗаказам, Объект.ТребуетсяЗалогЗаТару));
	КонецЕсли;
...
//
Процедура ПроверитьКорректностьЗаполненияДокументаПродажи(Знач ДокументПродажи, Отказ, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если СогласованиеЧерез1СДокументооборот И
		...
		СогласованВ1СДокументооборот = ИнтеграцияС1СДокументооборотПереопределяемый.
			ПолучитьСостояниеСогласованияВ1СДокументооборот(ДокументПродажи.Ссылка);
	Иначе
...
Процедура ПроверитьКорректностьВозвращаемыхТоваров(Знач ДокументПродажи, ИмяТаблицы, Отказ, ТаблицаПроверяемыеТовары = Неопределено) Экспорт
	...
	Если ТаблицаПроверяемыеТовары.Количество() > 0 Тогда
		...
		Если НЕ ВозвратПоЧеку Тогда
			...
			|	КОНЕЦ			КАК Назначение,
			|	МАКСИМУМ(ВЫБОР
			|			КОГДА ОбщиеОстатки.Количество ЕСТЬ NULL
...
Процедура СформироватьОтчетОтклоненияОтУсловийПродаж(Знач ДокументПродажи, ТаблицаОтчета) Экспорт
	...
	ЗапросПоШапке = Новый Запрос("
		...
		|") + "
		|" + ?(ИмяТаблицы = "Документ.ЗаданиеТорговомуПредставителю","
...
Функция ЗаполнитьЦены(ТабличнаяЧасть, ВыделенныеСтроки = Неопределено, ПараметрыЗаполнения = Неопределено, СтруктураДействий = Неопределено) Экспорт
	...
	Запрос.Текст = Запрос.Текст + ПолучитьТекстЗапросаВременнойТаблицыТоваров("втТаблицаТовары", ИмяКолонкиУпаковка);
...
Функция ПолучитьУсловияПродажПоУмолчанию(Знач Партнер = Неопределено, ПараметрыОтбора = Неопределено) Экспорт
	...
	Если ИспользоватьСоглашенияСКлиентами Тогда
		...
		Запрос = Новый Запрос("
			...
			|			И СоглашениеСКлиентом.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи))
			|		ИЛИ СоглашениеСКлиентом.ХозяйственнаяОперация В(&ХозяйственныеОперации)) И 
...
Функция ПолучитьПоследнююДатуПоГрафику(Знач ДатаДокумента, Знач ГрафикОплаты,  Знач СоглашениеСКлиентом) Экспорт
	...
	Если ЗначениеЗаполнено(ГрафикОплаты) Тогда
		Запрос = Новый Запрос("
			...
			|		ИЛИ ЭтапыГрафикаОплаты.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыКлиентом.КредитСдвиг))
			|
			|СГРУППИРОВАТЬ ПО
...
Процедура ПересчитатьСуммуДокументаВВалюту(Объект, Знач СтараяВалюта, Знач НоваяВалюта, Знач ЕстьСуммаПродажи = Истина, Знач ЕстьСуммаПродажиНДС = Ложь) Экспорт
	...
	НоваяСуммаПродажи = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
		ИтогСуммаПродажи,
		СтруктураКурсовСтаройВалюты,
		СтруктураКурсовНовойВалюты);
...
//	СчетаФактурыВыданныеНаОсновании - Структура, возвращаемая функцией УчетНДСУП.СчетаФактурыВыданныеНаОсновании() 
//	СчетаФактурыПолученныеНаОсновании - Структура, возвращаемая функцией УчетНДСУП.СчетаФактурыПолученныеНаОсновании() 
//
// Возвращаемое значение:
//	Результат - Строка, ФорматированнаяСтрока - Представление в параметре ПараметрыДляФормированияПредставления дополненное форматированным текстом представления фискальной операции в документе.
Функция ДополнитьПредставлениеПредставлениемФискальнойОперацииВДокументеПродажи(Основание, СчетаФактурыВыданныеНаОсновании = Неопределено, СчетаФактурыПолученныеНаОсновании = Неопределено, Контрагент = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") Тогда
		ВозможноПробитиеЧека = Истина;
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВзаимозачетЗадолженности") Тогда
		ВозможноПробитиеЧека = Истина;
	КонецЕсли;
	...
	ВозможноПробитиеЧека = ВозможноПробитиеЧека И ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование") И КонтрагентЯвляетсяФизЛицом;
	
	Если СчетаФактурыВыданныеНаОсновании <> Неопределено И СчетаФактурыВыданныеНаОсновании.СчетаФактуры.Количество() > 0 Тогда
		
		МассивПредставлений.Добавить(СчетаФактурыВыданныеНаОсновании.Представление);
		
	ИначеЕсли СчетаФактурыПолученныеНаОсновании <> Неопределено И СчетаФактурыПолученныеНаОсновании.СчетаФактуры.Количество() > 0 Тогда
	
		МассивПредставлений.Добавить(СчетаФактурыПолученныеНаОсновании.Представление);
		ВозможноПробитиеЧека = Ложь;
		
	ИначеЕсли СчетаФактурыВыданныеНаОсновании <> Неопределено И СчетаФактурыВыданныеНаОсновании.ДокументПоОснованиюТребуется Тогда
		
		МассивПредставлений.Добавить(СчетаФактурыВыданныеНаОсновании.Представление);
		ЕстьПравоИнтерактивногоДобавленияДокументаПоОснованию = СчетаФактурыВыданныеНаОсновании.ЕстьПравоИнтерактивногоДобавленияДокументаПоОснованию;
		
	ИначеЕсли СчетаФактурыПолученныеНаОсновании <> Неопределено И СчетаФактурыПолученныеНаОсновании.ДокументПоОснованиюТребуется Тогда
	
		МассивПредставлений.Добавить(СчетаФактурыПолученныеНаОсновании.Представление);
		ЕстьПравоИнтерактивногоДобавленияДокументаПоОснованию = СчетаФактурыПолученныеНаОсновании.ЕстьПравоИнтерактивногоДобавленияДокументаПоОснованию;
		
	ИначеЕсли СчетаФактурыВыданныеНаОсновании <> Неопределено Тогда
		
		МассивПредставлений.Добавить(СчетаФактурыВыданныеНаОсновании.Представление);
		
	ИначеЕсли СчетаФактурыПолученныеНаОсновании <> Неопределено Тогда
	...
	Если ВозможноПробитиеЧека Тогда
		...
		ФискальнаяОперацияДанныеЖурнала = МенеджерОборудованияВызовСервера.ДанныеФискальнойОперации(Основание);
		Если ФискальнаяОперацияДанныеЖурнала = Неопределено Тогда
			ФискальнаяОперацияДанныеЖурнала = ПодключаемоеОборудованиеУТВызовСервера.ДанныеФискальнойОперацииСУчетомКорректировкиРеализации(Основание);
		КонецЕсли;
		
		Если ФискальнаяОперацияДанныеЖурнала = Неопределено Тогда
		
			ЕстьПравоИнтерактивноПровестиЧек = (ПравоДоступа("ИнтерактивноеПроведение", Основание.Метаданные())
					ИЛИ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание, "Проведен"));
			...
			Если ЕстьПравоИнтерактивноПровестиЧек Тогда
				МассивПредставлений.Добавить(
...
//
// Параметры:
//		ДокументВозврата - ДокументОбъект.ЗаявкаНаВозвратТоваровОтКлиента, ДокументОбъект.ВозвратТоваровОтКлиента - Документ в котором заполняется табличная часть Товары на соновании Чека ККМ
...
Процедура ЗаполнитьВозвращаемыеТоварыПоЧеку(ДокументВозврата, ИмяТаблицы) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДокументВозврата.Валюта <> Выборка.ВалютаРеализации Тогда
			...
			НоваяСтрока.Цена = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
				НоваяСтрока.Цена,
				КурсСтаройВалюты,
				КурсНовойВалюты);
...
// Формирует временную таблицу, содержащую данные документа по поставщику: головную организацию, КПП поставщика.
//
// Параметры:
...
// Переопределенное соответствие (см. ОбеспечениеКлиентСервер.ВариантыОбеспеченияПоУмолчанию) на базе статуса и
...
//    Значение соответствия - ПеречислениеСсылка.ВариантыОбеспечения - Вариант обеспечения, используемый по умолчанию
//    для данного типа номенклатуры.
...
Функция ТекстЗапросаПоФактическойОтгрузке(СтруктураДокументаДляЗаполнения, СтруктураРезультатаЗапроса = Неопределено)
	...
	|		ВЫБОР
	|			КОГДА ЗаказТовары.ВариантОбеспечения В (ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Обособленно), ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
	|				ТОГДА ЗаказыОбороты.ЗаказКлиента.Назначение
	|			КОГДА ЗаказТоварыЗамена.ВариантОбеспечения В (ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Обособленно), ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
	|				ТОГДА ЗаказыОбороты.ЗаказКлиента.Назначение
	|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|		КОНЕЦ КАК Назначение,
	...
	|			И ЗаказыОбороты.ЗаказКлиента = ЗаказТовары.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента.ЗаменяющиеТовары КАК ЗаказТоварыЗамена
	...
	|			И ЗаказыОбороты.ЗаказКлиента = ЗаказТоварыЗамена.Ссылка
	|	ГДЕ
	...
	Если НайденныеСтроки.Количество()>0 Тогда
		...
		Если ЗаказНайден И ПересчитатьСуммы Тогда
			...
			Если ЭтоРеализация
				...
				НоваяСтрока.СуммаАвтоматическойСкидки = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
					НоваяСтрока.СуммаАвтоматическойСкидки,
					КурсВалюты[0],
					СтруктураКурсовНовойВалюты);
				НоваяСтрока.СуммаРучнойСкидки = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
...
Процедура СформироватьЗапросВременнаяТаблицаШапкаДокументаПродажи(ТекстЗапроса,
	...
	Если ПоСсылке Тогда
		...
		Если ИмяТаблицы = "Документ.ЗаказКлиента" Тогда
			...
			ВременныйТекст = "
				...
				|	ДокументПродажи.ГрафикОплаты          КАК ГрафикОплаты,
				|	ДокументПродажи.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
				|	ВЫБОР КОГДА ДокументПродажи.ЖелаемаяДатаОтгрузки = ДАТАВРЕМЯ(1,1,1)
				|			ТОГДА ДокументПродажи.ДатаОтгрузки
				|		ИНАЧЕ ДокументПродажи.ЖелаемаяДатаОтгрузки
	...
	Иначе
		...
		ТекстЗапроса = ТекстЗапроса + "
			...
			|	ДокументПродажи.ХозяйственнаяОперация                         КАК ХозяйственнаяОперация,
			|	ДокументПродажи.СуммаДокумента                                КАК СуммаДокумента,
			|	ВЫБОР КОГДА ДокументПродажи.ЖелаемаяДатаОтгрузки = ДАТАВРЕМЯ(1,1,1) И ДокументПродажи.НеОтгружатьЧастями
			|			ТОГДА ДокументПродажи.ДатаОтгрузки 
			|			ИНАЧЕ ДокументПродажи.ЖелаемаяДатаОтгрузки 
			|	КОНЕЦ                                                         КАК ЖелаемаяДатаОтгрузки,
			|	ДокументПродажи.ПроцентРучнойСкидки                           КАК ПроцентРучнойСкидки,
		...
		// Если ДатаПлатежа не в шапке, а в ТЧ документа
...
Процедура СформироватьЗапросКорректностьЭтаповГрафикаОплаты(ТекстЗапроса, ПараметрыЗапроса)
		...
		ТекстЗапроса = ТекстЗапроса + "
			...
			|				ИЛИ ЭтапыШаблона.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыКлиентом.КредитСдвиг)
			|			ТОГДА ВЫБОР
			...
			|	И НЕ ВременнаяТаблицаДокументПродажи.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоНакладным)
			|	И 0 В (
			|	ВЫБРАТЬ
			...
			|ГДЕ
			|	НЕ ВременнаяТаблицаДокументПродажи.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоНакладным)
			|УПОРЯДОЧИТЬ ПО
			|	НомерСтроки
			|;
			|";
...
Процедура СформироватьЗапросКорректностьДатыПлатежа(ТекстЗапроса, ПараметрыЗапроса, ДокументПродажи, ИмяТаблицы)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	ЭтапыГрафикаОплаты.Ссылка В (
		|		ВЫБРАТЬ
		|			ВременнаяТаблицаДокументПродажи.Соглашение
		|		ИЗ
		|			ВременнаяТаблицаДокументПродажи КАК ВременнаяТаблицаДокументПродажи
		|		ГДЕ
		|			ВременнаяТаблицаДокументПродажи.Соглашение.ГрафикОплаты = ЗНАЧЕНИЕ(Справочник.ГрафикиОплаты.ПустаяСсылка))
		|СГРУППИРОВАТЬ ПО
		|	ЭтапыГрафикаОплаты.Ссылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		...
		|	0                                                                 КАК Сдвиг,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаДокументПродажи.Соглашение.ГрафикОплаты = ЗНАЧЕНИЕ(Справочник.ГрафикиОплаты.ПустаяСсылка) ТОГДА 
		|			ВременнаяТаблицаДокументПродажи.Соглашение
		|		ИНАЧЕ
		|			ВременнаяТаблицаДокументПродажи.Соглашение.ГрафикОплаты
		|	КОНЕЦ                                                             КАК ГрафикОплаты,
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаДокументПродажи.Соглашение.ГрафикОплаты = ЗНАЧЕНИЕ(Справочник.ГрафикиОплаты.ПустаяСсылка) ТОГДА 
		|			ВременнаяТаблицаДокументПродажи.Соглашение.Календарь
		|		ИНАЧЕ
		|			ВременнаяТаблицаДокументПродажи.Соглашение.ГрафикОплаты.Календарь
		|	КОНЕЦ                                                             КАК Календарь
		|ИЗ
		|	ВременнаяТаблицаДокументПродажи
		|) КАК ВложенныйЗапрос
...
Процедура СформироватьЗапросКорректностьКоличестваЭтаповГрафикаОплаты(ТекстЗапроса, ПараметрыЗапроса)
		...
		ТекстЗапроса = ТекстЗапроса + "
			...
			|ВЫБРАТЬ
			|	ВЫБОР
			|		КОГДА СУММА(ВложенныйЗапрос.КоличествоЭтаповГрафика) <> 0
			|				И СУММА(ВложенныйЗапрос.КоличествоЭтаповДокумента) <> СУММА(ВложенныйЗапрос.КоличествоЭтаповГрафика)
			|				И МАКСИМУМ(ВложенныйЗапрос.ЕстьОшибкиЗаполненияЭтапыГрафикаОплаты) = ИСТИНА
			|			ТОГДА ИСТИНА
			|		ИНАЧЕ ЛОЖЬ
			|	КОНЕЦ КАК ЕстьОшибкиЗаполненияЭтапыГрафикаОплаты,
...
Процедура СформироватьЗапросНаличиеТоваровГИСМВДокументеПродажи(ТекстЗапроса)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|		ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродукцияМаркируемаяДляГИСМ),
...
Процедура СформироватьЗапросНаличиеАлкогольнойПродукцииВДокументеПродажи(ТекстЗапроса)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|	ВременнаяТаблицаТовары КАК ДокументТовары
		|ГДЕ
		|	ДокументТовары.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.АлкогольнаяПродукция)
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки ВОЗР
		|;
		|";
...
Процедура СообщитьОбОшибкахКорректностьЭтаповГрафикаОплаты(Знач Выборка,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Календарь) Тогда
			...
			Если Выборка.ВариантОплатыШаблона = Перечисления.ВариантыОплатыКлиентом.КредитПослеОтгрузки
				ИЛИ Выборка.ВариантОплатыШаблона = Перечисления.ВариантыОплатыКлиентом.КредитСдвиг Тогда
...
//      ИмяКолонкиУпаковка - Строка - имя колонки "Упаковка", если отличается.
//
// Возвращаемое значение:
// 		Строка - Текст запроса временной таблицы.
//
Функция ПолучитьТекстЗапросаВременнойТаблицыТоваров(ИмяТаблицы, ИмяКолонкиУпаковка = "Упаковка")
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяУпаковка", "Таблица." + ИмяКолонкиУпаковка);
...
// Рассчитывает сумму предоплаты по реализации в соответствии с графиком оплаты соглашения с клиентом.
//
// Параметры:
...
//
Функция СуммаПредоплатыПоСоглашению(Знач Объект, ЭтапыГрафикаОплаты, СуммаОплаты, СуммаЗалогаЗаТару = 0) Экспорт
	
	ЭтапыГрафикаОплатыДоИзменения = ЭтапыГрафикаОплаты.Выгрузить();
	...
	ЭтапыГрафикаОплаты.Загрузить(ЭтапыГрафикаОплатыДоИзменения);
...
Функция ОперацииИспользующиеДоговораКонтрагентов()
	...
	Операции.Добавить(Перечисления.ХозяйственныеОперации.СписаниеТоваровУХранителя);
