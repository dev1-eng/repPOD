...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета) Экспорт
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru='Не корректно заполнен кор. вид деятельности НДС для операции ""Возврат товаров между организациями"" для организации на УСН'"));
	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru='Не корректно заполнен тип записи в движениях документа ""Заявление о ввозе товаров из ЕАЭС""'"));
	...
	|		ИЛИ (Т.НДС <> 0 И Т.НДСУпр = 0 И ВЫРАЗИТЬ(Т.НДС * &КоэффициентВалютыУпр КАК ЧИСЛО(31,2)) <> 0))
...
Процедура СформироватьНачальныеОстаткиПартийСебестоимостьТоваров(ПараметрыРасчета, ПараметрыКорректировки)
	...
	|	ВЫРАЗИТЬ(ОстаткиСумм.СтоимостьУпр * Т.Количество / ОстаткиСумм.КоличествоИзПартий КАК ЧИСЛО (15,2)) 			 КАК СтоимостьУпр,
	|	ВЫРАЗИТЬ(ОстаткиСумм.ДопРасходыУпр * Т.Количество / ОстаткиСумм.КоличествоИзПартий КАК ЧИСЛО (15,2)) 			 КАК ДопРасходыУпр,
...
Процедура КорректировкаНачальныхОстатковРегистровДетализацииПартийНДС(ПараметрыРасчета)
	...
	Если ПараметрыРасчета.ФормироватьНачальныеОстаткиПартий22 Тогда
		СформироватьНачальныеОстаткиПартийНДСиУСН(ПараметрыРасчета, ПараметрыКорректировкиНДССредняя, ПараметрыКорректировкиНДСФИФО);
	КонецЕсли;
	...
	Если НЕ ПараметрыРасчета.ФормироватьНачальныеОстаткиПартий22 Тогда
		...
		// При переходе на ПУ 2.2 такая корректировка уже выполняется.
		КорректировкаДозаполнитьАналитикуУчетаПартийНДСиУСН2_4(ПараметрыРасчета, ПараметрыКорректировкиНДСФИФО);
		
		Если ПараметрыРасчета.УправленческийУчетОрганизаций Тогда
			Если РасчетСебестоимостиПовтИсп.ДатаНачалаВеденияУправленческогоУчетаОрганизаций() = ПараметрыРасчета.РасчетныйПериод.НачалоПериода Тогда
				КорректировкаЗаполнитьСуммыУпрУчетаПартииНДСиУСН(ПараметрыРасчета, ПараметрыКорректировкиНДССредняя, ПараметрыКорректировкиНДСФИФО);
			КонецЕсли;
		Иначе
			КорректировкаОчиститьСуммыУпрУчетаПартииНДСиУСН(ПараметрыРасчета, ПараметрыКорректировкиНДССредняя, ПараметрыКорректировкиНДСФИФО);
		КонецЕсли;
...
//
Процедура КорректировкаДозаполнитьАналитикуУчетаПартийНДСиУСН2_4(ПараметрыРасчета, ПараметрыКорректировки)
	
	РасчетСебестоимостиПрикладныеАлгоритмы.НачатьСледующуюКорректировкуОстатковРегистра(
		ПараметрыРасчета,
		ПараметрыКорректировки,
		Перечисления.ТипыЗаписейПартий.СлужебноеДозаполнениеАналитикиУчетаПартийНДСиУСН2_4);
	...
	|	ВТСебестоимость.ВидДеятельностиНДС КАК ВидДеятельностиНДС,
	...
	|			И Т.ВидДеятельностиНДС = ВТОтборСебестоимость.ВидДеятельностиНДС
	...
	|	Т.НДС КАК НДС,
	|	ВЫБОР
	|		КОГДА Т.ВидДеятельностиНДС В (ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС), ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД))
	|			ТОГДА Т.СтоимостьБезНДС + Т.НДС
	|		ИНАЧЕ Т.СтоимостьБезНДС
	|	КОНЕЦ КАК Знаменатель
	|ПОМЕСТИТЬ ВТНДС
	|ИЗ
	|	ВТРасчетныеНачальныеОстаткиДетализацияПартийТоваровДляНДСиУСН2_4 КАК Т
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Партия,
	...
	|			И Т.ВидДеятельностиНДС = ВТОтборНДС.ВидДеятельностиНДС
	...
	ПараметрыРаспределения = РасчетСебестоимостиУниверсальныеАлгоритмы.ИнициализироватьПараметрыРаспределенияМетодомУменьшаемогоОстатка(
		...
		"Организация, Партия, ВидДеятельностиНДС, НаправлениеДеятельности, ВидЦенности");
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.РаспределитьМетодомУменьшаемогоОстаткаСЗамеромВремени(
		...
		ПараметрыРаспределения,
		"РаспределитьМетодомУменьшаемогоОстатка",
		"КорректировкаДозаполнитьАналитикуУчетаПартийНДСиУСН2_4");
