...
Процедура Подключаемый_ВыполнитьКомандуИнтеграции_НоменклатурыКонтрагентов(Форма, Команда, Источник) Экспорт
	...
	ИнтеграцияС1СДокументооборотКлиент.ВыполнитьПодключаемуюКомандуИнтеграции(Команда, Форма, Источник);
...
//
Процедура ПоказатьНоменклатуруПартнера(Партнер, Ссылка, ТекущаяСтрока, ПредставлениеПартнера) Экспорт
	...
	Если Отказ Тогда
		...
		Если ТекущаяСтрока = Неопределено Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выберите строку таблицы ""Товары""'"), Ссылка, "Объект.Товары");
		ИначеЕсли Не ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
...
//
Процедура ОткрытьФормуВыбораНоменклатурыПартнера(ВладелецФормы, Партнер, ТекущаяСтрока, ПредставлениеНоменклатурыПартнера, ОповещениеОЗакрытие) Экспорт
...
//
// Параметры:
//  ПараметрыВыбора - ПараметрыВыбора             - параметры выбора строки.
...
// См. СопоставлениеНоменклатурыКонтрагентовКлиентПереопределяемый.ЭлементФормыПодсистемыОбработкаВыбора
