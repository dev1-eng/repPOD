...
Функция ПолучитьСценарийПоУмолчанию(Знач ТипПлана) Экспорт 
	...
	|					И СценарииПланирования.ИспользоватьВПланированииОстатков
	|				ТОГДА ИСТИНА
...
Процедура ПодготовитьТаблицуПериодовПередЗаполнением(Параметры) Экспорт 
	...
	ИспользоватьПланыОстатков = ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеОстатков")
								И Параметры.Сценарий.ИспользоватьВПланированииОстатков;
	...
	Если ИспользоватьПланыОстатков И Параметры.СмещениеПериода = 0 Тогда
		...
		Для каждого СтруктураПоля Из Поля Цикл
			...
			Если СоздаватьЭлемент Тогда
				...
				ИначеЕсли СтруктураПоля.ПрефиксРеквизитаКолонки = "Количество_" Тогда
					Элемент.Заголовок = СтрокаПериода.Заголовок;
				Иначе
					Элемент.Заголовок = СтруктураПоля.ПрефиксРеквизитаКолонки + СтрокаПериода.Заголовок;
				КонецЕсли;
...
//
// Параметры:
//  Таблица              - ТаблицаЗначений - кросс-таблица или основная таблица, в которой необходимо проверить реквизиты
//  ПутьКТаблице         - Путь к проверяемой таблице
//  СтруктураДействий    - Структура - поля группировки в таблице
//  Отказ                - Булево - Истина, если есть ошибки заполнения
...
Функция ЗаполнитьПоФормуле(Параметры) Экспорт 
	...
	Иначе
		...
		ЗаполняемаяТЧ.ЗаполнитьЗначения(0,"Количество");
...
//  Период			  - Строка таблицы Периоды - Описание текущего периода
//  ПредыдущийПериод  - Строка таблицы Периоды - Описание предыдущего периода
...
Процедура ВычислитьРассчитываемыеОперанды(СтрокаТоваров, Период, ПредыдущийПериод, НовыйПланОстатков, Разделитель, ОсновнойОперандПлана, ЗнакКорректировки) Экспорт
	...
	НакопленныйПланируемыйКонечныйДефицит = НакопленныйПланируемыйКонечныйДефицит
											...
											0)
											- ?(ОсновнойОперандПлана="" Или Не Период.Активная, 0, ЗнакКорректировки*СтрокаТоваров[ОсновнойОперандПлана+Разделитель+Период.ИмяКолонки]);
...
Функция ПоместитьДополнительныеПоляВХранилище(Форма, Смещение) Экспорт
	...
	ТаблицаОперандов = Форма.ДополнительныеПоля.Выгрузить();
	...
	Для Каждого ГруппаОперандов Из ИмяГруппыПредставление Цикл
		...
		Для Каждого Операнд Из ТаблицаОперандов.НайтиСтроки(Новый Структура("ВходитВОперанд", ГруппаОперандов.Идентификатор)) Цикл
			Если ОперандГруппа = Неопределено Тогда
				ОперандГруппа = РаботаСФормулами.ДобавитьОперанд(ДеревоОперандов, ГруппаОперандов.Идентификатор, ГруппаОперандов.Представление, 0);
			КонецЕсли;
			
			РаботаСФормулами.ДобавитьОперанд(ОперандГруппа, Операнд.Идентификатор, Операнд.Представление, Операнд.Значение);
		КонецЦикла;
...
// Заполняет движения документа "План остатков"
//
Процедура ОтразитьПланОстатков(ДополнительныеСвойства, Движения, Отказ) Экспорт

	Таблица = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаПланыОстатков;
	...
	Движения.ПланыОстатков.Записывать = Истина;
	Движения.ПланыОстатков.Загрузить(Таблица);
...
//	
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма плана.
//
...
// Вызывается в обработчике "ОбработкаПроверкиЗаполнения" при формировании отчета
//
Процедура СбалансированностьПлановОбработкаПроверкиЗаполнения(КомпоновщикНастроек, Отказ) Экспорт 
	...
	Для каждого ЭлементНастройки Из КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
		...
		Если Статус <> Неопределено И ЭлементНастройки.ИдентификаторПользовательскойНастройки = Статус.ИдентификаторПользовательскойНастройки Тогда
			...
			Иначе
				...
				СтатусыСценария.Добавить(Перечисления.СтатусыПланов.Отменен);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла; 
	...
	Если Сценарий <> Неопределено И ЗначениеЗаполнено(Сценарий.Значение) Тогда
		ПараметрыСценария = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Сценарий.Значение, "ИспользоватьВПланированииОстатков, Периодичность");
		...
		ИспользоватьРасчетПлановОстатков = ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеОстатков")
		...
		Если ИспользоватьРасчетПлановОстатков Тогда
			...
			ПараметрыРасчетаНачалаНакопленияОстатка.Вставить("НачалоПериода", Период.Значение.ДатаНачала);
			ПараметрыРасчетаНачалаНакопленияОстатка.Вставить("КонецПериода", Период.Значение.ДатаОкончания);
			...
			Если ПериодыПлановОстаткаЗаПериодБольшеНачалоПериода.Количество()>0 Тогда
				...
				Для Каждого ПериодПланаОстатков Из ПериодыПлановОстаткаЗаПериодБольшеНачалоПериода Цикл
					...
					КонецЕсли;
					СтрокаПериодыПланаОстатков = СтрокаПериодыПланаОстатков + " " + Формат(ПериодПланаОстатков, "ДФ= 'дд.ММ.гг'");
				КонецЦикла;
...
Процедура СбалансированностьПлановПриКомпоновкеРезультата(КомпоновщикНастроек, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка) Экспорт 
	...
	ПланыОстатковИспользование              = Ложь ИЛИ (ПланыОстатков <> Неопределено И ПланыОстатков.Использование);
	...
	ПланыСборкиКомплектыИспользование       = Ложь ИЛИ (ПланыСборкиКомплекты <> Неопределено И ПланыСборкиКомплекты.Использование);
	ПланыРазборкиКомплектыИспользование     = Ложь ИЛИ (ПланыРазборкиКомплекты <> Неопределено И ПланыРазборкиКомплекты.Использование);
	ПланыСборкиКомплектующиеИспользование   = Ложь ИЛИ (ПланыСборкиКомплектующие <> Неопределено И ПланыСборкиКомплектующие.Использование);
	ПланыРазборкиКомплектующиеИспользование = Ложь ИЛИ (ПланыРазборкиКомплектующие <> Неопределено И 	ПланыРазборкиКомплектующие.Использование);
	...
	Для каждого ЭлементНастройки Из КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
		...
		КонецЕсли;
		Если ПланыСборкиКомплекты <> Неопределено И ЭлементНастройки.ИдентификаторПользовательскойНастройки = ПланыСборкиКомплекты.ИдентификаторПользовательскойНастройки Тогда
			ПланыСборкиКомплектыИспользование = ЭлементНастройки.Использование;
		КонецЕсли;
		Если ПланыРазборкиКомплекты <> Неопределено И ЭлементНастройки.ИдентификаторПользовательскойНастройки = ПланыРазборкиКомплекты.ИдентификаторПользовательскойНастройки Тогда
			ПланыРазборкиКомплектыИспользование = ЭлементНастройки.Использование;
		КонецЕсли;
		Если ПланыСборкиКомплектующие <> Неопределено И ЭлементНастройки.ИдентификаторПользовательскойНастройки = ПланыСборкиКомплектующие.ИдентификаторПользовательскойНастройки Тогда
			ПланыСборкиКомплектующиеИспользование = ЭлементНастройки.Использование;
		КонецЕсли;
		Если ПланыРазборкиКомплектующие <> Неопределено И ЭлементНастройки.ИдентификаторПользовательскойНастройки = ПланыРазборкиКомплектующие.ИдентификаторПользовательскойНастройки Тогда
			ПланыРазборкиКомплектующиеИспользование = ЭлементНастройки.Использование;
		КонецЕсли;
	...
	Если Сценарий <> Неопределено И ЗначениеЗаполнено(Сценарий.Значение) Тогда
		ПараметрыСценария = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Сценарий.Значение, "ИспользоватьВПланированииЗакупок, 
			...
			|ПланированиеПоНазначениям, ИспользоватьВПланированииОстатков, Периодичность");
		Для каждого КлючИЗначение Из ПараметрыСценария Цикл
			КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, КлючИЗначение.Ключ,КлючИЗначение.Значение);
		КонецЦикла;
		...
		Если ИспользоватьРасчетПлановОстатков Тогда
			...
			КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ДатаНачалаНакопленияОстатка", Результат.ДатаНачалаНакопленияОстатка);
		Иначе
			КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ДатаНачалаНакопленияОстатка", Период.Значение.ДатаНачала);
		КонецЕсли;
	Иначе
	...
	УстановитьВыбранныеПоляПоОтборам(ВыбранныеПоля, 
		...
		ИспользоватьРасчетПлановОстатков, 
		ПланыПродажИспользование, 
		ПланыПроизводстваИспользование, 
		ПланыСборкиКомплектыИспользование,
		ПланыРазборкиКомплектыИспользование,
		ПланыСборкиКомплектующиеИспользование,
		ПланыРазборкиКомплектующиеИспользование,
...
Процедура УстановитьВыбранныеПоляПоОтборам(ВыбранныеПоля, 
		...
		ПланыПродаж, 
		ПланыПроизводства,
		ПланыСборкиКомплекты,
		ПланыРазборкиКомплекты,
		ПланыСборкиКомплектующие,
		ПланыРазборкиКомплектующие,
		ПланыМатериалов)
	...
	Для каждого ПолеВыбора Из ВыбранныеПоля.Элементы Цикл
		...
		Если ПолеВыбора.Поле = ПолеПланСборкиКомплекты Тогда
			ПолеВыбора.Использование = ПланыСборкиКомплекты;
		КонецЕсли; 
		Если ПолеВыбора.Поле = ПолеПланРазборкиКомплекты Тогда
			ПолеВыбора.Использование = ПланыРазборкиКомплекты;
		КонецЕсли;
		Если ПолеВыбора.Поле = ПолеПланСборкиКомплектующие Тогда
			ПолеВыбора.Использование = ПланыСборкиКомплектующие;
		КонецЕсли; 
		Если ПолеВыбора.Поле = ПолеПланРазборкиКомплектующие Тогда
			ПолеВыбора.Использование = ПланыРазборкиКомплектующие;
		КонецЕсли;
