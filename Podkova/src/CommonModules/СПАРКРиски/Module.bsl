...
Функция ИндексыСПАРККонтрагента(Контрагент, ПризнакЗагрузкиДанных = Ложь) Экспорт
	...
	Если ТипЗнч(Контрагент) = ТипСтрока Тогда
		...
		Если ИННСоответствуетТребованиям(КонтрагентИНН) Тогда
	...
	Если ТипЗнч(Контрагент) = ТипСтрока Тогда
		ТекстОшибки = "";
		Если ИННСоответствуетТребованиям(КонтрагентИНН, ТекстОшибки) = Истина Тогда
	...
	Иначе // Передали ссылку на контрагента.
		...
		Если Запись.Выбран() Тогда
			...
			// Индексы.
			Результат.Вставить("СводныйИндикатор",              Запись.СводныйИндикатор);
			Результат.Вставить("СводныйИндикаторГрадация",      ГрадацияИндекса(Запись.СводныйИндикатор, "СводныйИндикатор"));
			Результат.Вставить("ИндексДолжнойОсмотрительности", Запись.ИндексДолжнойОсмотрительности);
			Результат.Вставить("ИДОГрадация",                   ГрадацияИндекса(Запись.ИндексДолжнойОсмотрительности, "ИндексДолжнойОсмотрительности"));
			Результат.Вставить("ИндексФинансовогоРиска",        Запись.ИндексФинансовогоРиска);
			Результат.Вставить("ИФРГрадация",                   ГрадацияИндекса(Запись.ИндексФинансовогоРиска, "ИндексФинансовогоРиска"));
			Результат.Вставить("ИндексПлатежнойДисциплины",     Запись.ИндексПлатежнойДисциплины);
			Результат.Вставить("ИПДГрадация",                   ГрадацияИндекса(Запись.ИндексПлатежнойДисциплины, "ИндексПлатежнойДисциплины"));
			Результат.Вставить("Активен",                       Запись.Активен);
			Результат.Вставить("ОтображатьСтатус",              Запись.ОтображатьСтатус);
			Результат.Вставить("Статус",                        Запись.Статус);
			Результат.Вставить("СтатусНазвание",                Запись.СтатусНазвание);
	...
	Если (НеобходимоЗагрузитьИнформацию = Истина) Тогда
		...
		Если НЕ ПустаяСтрока(КонтрагентИНН) Тогда
			...
			ТекстОшибки = "";
			Если ИННСоответствуетТребованиям(КонтрагентИНН, ТекстОшибки) Тогда
	...
	Если Результат.Свойство("ИНН") И Не ИННСоответствуетТребованиям(Результат.ИНН) Тогда
		Результат.Вставить("ВидОшибки", Перечисления.ВидыОшибокСПАРКРиски.НекорректныйИНН);
	КонецЕсли;
...
Процедура ВсеОбновленияСПАРКРискиОбщее() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если МонопольныйРежим() Тогда
			...
			// Поэтому в монопольном режиме не следует выполнять никаких обновлений в подсистеме СПАРКРиски.
