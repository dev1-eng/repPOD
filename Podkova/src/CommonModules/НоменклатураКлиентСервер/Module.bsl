...
Процедура ПересчитатьСтатусУказанияСерийПриОбработке(ПараметрыУказанияСерий, ТекущийСтатус, СерииУказаныПолностью, КоличествоСерий, ВариантОбеспечения = Неопределено, ТекущиеДанные = Неопределено) Экспорт
	...
	ПрименятьСтатусМожноУказать = (ПараметрыУказанияСерий.ПодготовкаОрдера
									...
									Или ИспользоватьСтатусСерийМожноУказатьГОСИС)
...
Функция ПредставлениеСерии(ПараметрыШаблона, ЗначенияРеквизитов, ДобавитьСловоНовая = Ложь) Экспорт
	...
	Если ПараметрыШаблона.ИспользоватьПроизводителяВЕТИССерии Тогда
		ЧастиПредставления.Добавить(НСтр("ru='%ПроизводительВетИС%'"));
	КонецЕсли;
	
	Если ПараметрыШаблона.ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии Тогда
		ЧастиПредставления.Добавить(НСтр("ru='%ЗаписьСкладскогоЖурналаВетИС%'"));
	КонецЕсли;
	
	Если ПараметрыШаблона.ИспользоватьИдентификаторПартииВЕТИССерии Тогда
	...
	Если ПараметрыШаблона.ИспользоватьМРЦМОТПСерии Тогда
		ЧастиПредставления.Добавить(НСтр("ru = 'МРЦ %МаксимальнаяРозничнаяЦенаМОТП%'"))
	КонецЕсли;
	...
	Представление = СтрЗаменить(Представление, "%МаксимальнаяРозничнаяЦенаМОТП%", ЗначенияРеквизитов.МаксимальнаяРозничнаяЦенаМОТП);
