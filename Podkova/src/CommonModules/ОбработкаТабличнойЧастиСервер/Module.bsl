...
Процедура ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьКоличествоЕдиницВЕТИСВСтрокеТЧ(					ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьКоличествоЕдиницПоВЕТИСВСтрокеТЧ(				ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
...
Процедура ЗаполнитьСтавкуНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) 
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСтавкуНДС", СтруктураПараметровДействия) Тогда
		...
		Если СтруктураПараметровДействия.Свойство("ПоДатеОтгрузки") И 
			ЗначениеЗаполнено(ТекущаяСтрока.ДатаОтгрузки) Тогда
			
			СтруктураПараметровДействия.Вставить("Дата", ТекущаяСтрока.ДатаОтгрузки);
		КонецЕсли;
		
	КонецЕсли;
	
	УчетНДСУП.ЗаполнитьСтавкуНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
...
Процедура ПересчитатьСуммуНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) 
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуНДС", СтруктураПараметровДействия) Тогда
		УчетНДСУП.ПересчитатьСуммуНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураПараметровДействия, КэшированныеЗначения);
	КонецЕсли;
...
Процедура ПересчитатьСуммуНДССуффиксВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуНДССуффикс", СтруктураПараметровДействия) Тогда
		УчетНДСУП.ПересчитатьСуммуНДССуффиксВСтрокеТЧ(ТекущаяСтрока, СтруктураПараметровДействия, КэшированныеЗначения);
	КонецЕсли;
...
Процедура ЗаполнитьУсловияЗакупокВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий)
	...
	СтруктураПараметровОтбора = Новый Структура("Валюта, Дата, ВернутьМногооборотнуюТару, НалогообложениеНДС, Соглашение, ВидЦеныПоставщика, Номенклатура, Характеристика, Упаковка");
	ЗаполнитьЗначенияСвойств(СтруктураПараметровОтбора, ТекущаяСтрока);
	ЗаполнитьЗначенияСвойств(СтруктураПараметровОтбора, СтруктураПараметровДействия);
...
Процедура ЗаполнитьЦенуЗакупкиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	// Получение параметров запроса
	СтруктураПараметровОтбора = Новый Структура("Валюта, Дата, Соглашение, ВидЦеныПоставщика, НалогообложениеНДС, ВернутьМногооборотнуюТару, Номенклатура, Характеристика, Упаковка");
	ЗаполнитьЗначенияСвойств(СтруктураПараметровОтбора, ТекущаяСтрока);
	ЗаполнитьЗначенияСвойств(СтруктураПараметровОтбора, СтруктураПараметровДействия);
	Если ЗначениеЗаполнено(СтруктураПараметровОтбора.ВидЦеныПоставщика) Тогда
		Цена = ЗакупкиСервер.ПолучитьЦенуПоОтбору(СтруктураПараметровОтбора);
...
Процедура ПроверитьСопоставленнуюНоменклатуруПартнераВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ПроверитьСопоставленнуюНоменклатуруПартнера", СтруктураПараметровДействия) Тогда
		...
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(НоменклатураПартнеров.Ссылка) КАК КоличествоНоменклатурыПартнера,
		...
		|ГДЕ
		|	НоменклатураПартнеров.Владелец = &Партнер
		...
		|	И НоменклатураПартнеров.Упаковка = &Упаковка
		|	И НоменклатураПартнеров.Ссылка <> &НоменклатураПартнера
		...
		|	НоменклатураПартнеров.Характеристика,
		|	НоменклатураПартнеров.Упаковка");
...
Процедура ПроверитьЗаполнитьОбеспечениеВСтрокеТЧДокументаПродажи(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечениеВДокументеПродажи", ПараметрыДействия) Тогда
		...
		ВариантОбеспечения = ВариантыОбеспечения[ОбеспечениеКлиентСервер.ТипНоменклатурыСтрокой(Реквизиты.ТипНоменклатуры)];
...
Процедура ЗаполнитьСтатьюАналитикуРасходовПоУмолчанию(ТекущаяСтрока, СтруктураДействий)
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСтатьюАналитикуРасходовПоУмолчанию", СтруктураПараметровДействия) Тогда
		...
		Если Не СтруктураПараметровДействия = Неопределено Тогда
			...
			Если СтруктураПараметровДействия.Свойство("ИмяРеквизитаСтатьиРасходов") Тогда
				ИмяРеквизитаСтатьиРасходов = СтруктураПараметровДействия["ИмяРеквизитаСтатьиРасходов"];
			Иначе
				ИмяРеквизитаСтатьиРасходов = "СтатьяРасходов";
			КонецЕсли;
			
			Если СтруктураПараметровДействия.Свойство("ИмяРеквизитаАналитикаРасходов") Тогда
				ИмяРеквизитаАналитикаРасходов = СтруктураПараметровДействия["ИмяРеквизитаАналитикаРасходов"];
			Иначе
				ИмяРеквизитаАналитикаРасходов = "АналитикаРасходов";
			КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока[ИмяРеквизитаСтатьиРасходов]) Тогда
		
			СтатьяАналитика = ДоходыИРасходыСервер.СтатьяАналитикаРасходовПоУмолчанию(
														СтруктураПараметровДействия.ХозяйственнаяОперация,
														СтруктураПараметровДействия.Организация,
														СтруктураПараметровДействия.Подразделение,
														ТекущаяСтрока.Номенклатура);
			
			Если СтатьяАналитика.Следующий() Тогда
				ТекущаяСтрока[ИмяРеквизитаСтатьиРасходов]		= СтатьяАналитика.СтатьяРасходов;
				ТекущаяСтрока[ИмяРеквизитаАналитикаРасходов]	= СтатьяАналитика.АналитикаРасходов;
			КонецЕсли;
...
//
// Параметры:
// 		СтруктураРеквизитов - Структура - Структура с именами служебных реквизитов в качестве полей
...
Функция ДанныеЕдиницыИзмеренияВЕТИС(ЕдиницаИзмеренияВЕТИС, Номенклатура, КэшированныеЗначения) Экспорт
	...
	ДанныеЕдиницыИзмеренияВЕТИС = Справочники.УпаковкиЕдиницыИзмерения.КоэффициентЕдиницыИзмеренияПоВЕТИС(
...
Функция ШаблонПоляВыборкиПоКлючуДействия(КлючДействия, ТолькоПроверитьПоддерживаемыеКлючи = Ложь)
	...
	Если КлючДействия = "ЗаполнитьПризнакВедетсяУчетПоРНПТ" Тогда
		Возврат ",
		|	ЕСТЬNULL(втТаблицаНоменклатуры.%Ключ%.КодТНВЭД.ПрослеживаемыйТовар, ЛОЖЬ)
		|		И ЕСТЬNULL(втТаблицаНоменклатуры.%Ключ%.ВестиУчетПоГТД, ЛОЖЬ) КАК %Значение%";
	КонецЕсли;
