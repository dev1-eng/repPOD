...
Функция ПолучитьПоследнююДатуПоГрафику(Знач ДатаДокумента, Знач СоглашениеСПоставщиком) Экспорт
	...
	|		ИЛИ ЭтапыГрафикаОплаты.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыПоставщику.КредитСдвиг))
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПроверитьКорректностьЗаполненияДокументаЗакупки(ДокументЗакупки, Отказ) Экспорт
	...
	Для Каждого ТекЭлемент Из МассивПроверок Цикл
		...
		Иначе
			...
			УчетНДСУП.СформироватьЗапросыКорректностиУчетаНДСЗакупки(ТекЭлемент, ТекстЗапроса, ПараметрыЗапроса, ДокументЗакупки);
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		Иначе
			...
			ОписаниеДокументаЗакупки = Новый Структура;
			ОписаниеДокументаЗакупки.Вставить("ДокументЗакупки", ДокументЗакупки);
			ОписаниеДокументаЗакупки.Вставить("ИмяТЧ", "Товары");
			ОписаниеДокументаЗакупки.Вставить("ПредставлениеТЧ", НСтр("ru='Товары'"));
			
			УчетНДСУП.СообщитьОбОшибкахКорректностиУчетаНДСЗакупки(МассивПроверок[ТекИндекс], Выборка, ОписаниеДокументаЗакупки, Отказ);
...
Процедура ПроверитьКорректностьЭтаповГрафикаОплаты(Знач ДокументЗакупки,
	...
	Иначе
		...
		Для ТекИндекс = 0 По ДокументЗакупки.ЭтапыГрафикаОплаты.Количество()-1 Цикл
			...
			Если УчитыватьВариантОплаты И ОбщегоНазначения.ЕстьРеквизитОбъекта("ЖелаемаяДатаПоступления", ДокументЗакупки.Метаданные()) Тогда
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		ИначеЕсли ТекЭлемент = "КорректностьЭтаповГрафикаОплаты" Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Не Выборка.ЕстьОшибкиЗаполненияЭтапыГрафикаОплаты // В документе не заполнено соглашение или сумма документа равна 0
					ИЛИ Выборка.ЭтапШаблонаОтсутствует Тогда // График оплаты в соглашении не заполнен или не соответствует по количеству графику документа
...
//     "ПоляЗаполнения" - Строка - Строка с перечислением заполняемых полей в таблице. Значение по умолчанию: "Цена".
//                                 Дополнительные поля: "ВидЦеныПоставщика, СтавкаНДС"
...
Функция ЗаполнитьЦены(ТабличнаяЧасть, ВыделенныеСтроки, ПараметрыЗаполнения, СтруктураДействий = Неопределено, КэшированныеЗначения = Неопределено) Экспорт
	...
	Параметры.Вставить("ВидЦеныПоставщика", Справочники.ВидыЦенПоставщиков.ПустаяСсылка());
	Параметры.Вставить("ПоляЗаполнения", "Цена");
...
//  (тип договора). ДополнительныеПараметрыОтбораДоговоров - Структура - дополнительные параметры для отбора договоров
//  (см. функцию ДополнительныеПараметрыОтбораДоговоров).
//
// Возвращаемое значение:
	...
	Для Каждого СтрокаКДозаполнению Из ИсходныеСтрокиКДозаполнениюСОстатками Цикл
		...
		Для Каждого СтрокаОстатков Из СтрокиТаблицыОстатков Цикл
			...
			Иначе
				...
				ДобавляемыеРаспределяемыеПоСериямСтроки = Новый Структура;
				ДобавляемыеРаспределяемыеПоСериямСтроки.Вставить("Строка", ТекСтрока);
				ДобавляемыеРаспределяемыеПоСериямСтроки.Вставить("Серия", СтрокаОстатков.Серия);
				ДобавляемыеРаспределяемыеПоСериямСтроки.Вставить("Количество", ДобавляемоеКоличество);
				ДобавляемыеРаспределяемыеПоСериямСтроки.Вставить("КоличествоУпаковок", ДобавляемоеКоличествоУпаковок);
...
//	ПараметрыДляФормированияПредставления - Структура, возвращаемая функцией УчетНДСУП.СчетаФактурыПолученныеНаОсновании() 
//
// Возвращаемое значение:
...
// Инициализирует и возвращает структуру периода ввода номера таможенной декларации
// 
// Возвращаемое значение:
//  Структура:
//   * НачалоКорректногоПериода - Дата
//   * КонецКорректногоПериода - Дата
//
