...
Процедура СоздатьОбновитьЗаданиеТорговомуПредставителю(МобильноеЗадание, УзелОбмена, Сообщение, НастройкиЗадания) Экспорт
	...
	Для Каждого СтрокаТовара Из МобильноеЗадание.Товары Цикл
		...
		СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС",
			Новый Структура("НалогообложениеНДС, Дата", ЗаданиеОбъект.НалогообложениеНДС, ЗаданиеОбъект.Дата));
...
Процедура СоздатьОбновитьКлиента(МобильныйКлиент, УзелОбмена, Сообщение) Экспорт
	...
	Если НЕ Партнер.ПометкаУдаления = МобильныйКлиент.DeletionMark Тогда
		Партнер.ПометкаУдаления = МобильныйКлиент.DeletionMark;
	КонецЕсли;
	Если НЕ Партнер.Наименование = МобильныйКлиент.Description Тогда
		Партнер.Наименование = МобильныйКлиент.Description;
	КонецЕсли;
	Если НЕ Партнер.НаименованиеПолное = МобильныйКлиент.Description Тогда
		Партнер.НаименованиеПолное = МобильныйКлиент.Description;
	КонецЕсли;
	
	ПартнерРодитель = Справочники.Партнеры.ПолучитьСсылку(Новый УникальныйИдентификатор(МобильныйКлиент.Parent));
...
Процедура СоздатьОбновитьКонтрагента(МобильныйКонтрагент, Сообщение) Экспорт
	...
	Если ЕстьИзменения Тогда
		Если ПравоИзменения Тогда
			...
			Если ЭтоНовыйКонтрагент И НЕ СтруктураОтвета.Успешно Тогда
...
//  ИдентификаторМобильногоУстройства - Строка - не изменяемый код мобильного приложения, используется как код узла
//                                               плана обмена;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСКлиентами")
		...
		Иначе
			...
			Если МобильныеДанные.Товары.Количество() > 0 Тогда
				...
				ДокументОбъект.ЦенаВключаетНДС = ВидЦены.ЦенаВключаетНДС;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьСправочникСоглашенияПриИзменении(СоглашениеОбъект)
	...
	СоглашениеОбъект.ПорядокОплаты = 
		Перечисления.ПорядокОплатыПоСоглашениям.ПолучитьПорядокОплатыПоУмолчанию(СоглашениеОбъект.Валюта);
	СоглашениеОбъект.ГруппаФинансовогоУчета = 
		Справочники.ГруппыФинансовогоУчетаРасчетов.ПолучитьГруппуФинансовогоУчетаПоУмолчанию(
			СоглашениеОбъект.ПорядокОплаты, СоглашениеОбъект.ХозяйственнаяОперация);
...
Функция ИнформацияПоСтатусамЗаказовКлиентов(ЗаказыКлиентов)
	...
	Пока ВыборкаДокументов.Следующий()Цикл
		...
		Иначе
			...
			Структура.Вставить("ДатаСледующегоПлатежа", СледующаяДатаПлатежаПоЗаказу(ЗаказКлиентаИБ.Ссылка));
		КонецЕсли;
