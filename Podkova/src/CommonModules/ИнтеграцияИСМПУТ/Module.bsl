...
Процедура СформироватьТаблицуМаркируемойПродукцииДокумента(Документ, ТаблицаПродукции, ВидыМаркируемойПродукции) Экспорт
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.СборкаТоваров") Тогда
		...
		ТекстЗапроса = ТекстЗапроса +"
			...
			|
			|";
		ТекстЗапроса =ТекстЗапроса + ТекстЗапросаМаркируемойПродукции(Документ, "");
...
Функция ПриОпределенииНеобходимостиОбработкиКодовМаркировкиВДокументе(ДокументОбъект, ИмяТабличнойЧастиШтрихкодыУпаковокРасхождения) Экспорт
	...
	ПустоеЗначениеПризнан = ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("ВариантДействийПоРасхождениямКодовМаркировкиИСМП");
...
Процедура ОбработкаЗаполненияДокументаПеремаркировкаТоваровИСМП(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипОснования = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") Тогда
		ЗаполнитьПеремаркировкуТоваровИСМПНаОснованииВозвратаТоваровОтКлиента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ЗаполнитьКолонкуРасхожденияПоКодамМаркировки(Товары, ДанныеПоРасхождениям,
	...
	|	&СтрокаЗаменыПризнан   КАК Признан
	...
	Если ДоступноСогласованиеРасхождений Тогда
		СтрокаЗаменыПризнан = "Таблица.Признан";
	Иначе
		СтрокаЗаменыПризнан = "&Признан";
		Запрос.УстановитьПараметр("Признан", ВариантыДействийПоРасхождениямКодовМаркировкиИСМППризнать);
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СтрокаЗаменыПризнан", СтрокаЗаменыПризнан);
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокиТоваров = Товары.НайтиСтроки(Новый Структура("Номенклатура, Характеристика",
			...
			КоличествоСтрокТоваров = СтрокиТоваров.Количество();
			...
			Для Каждого ЗаполняемаяСтрока Из СтрокиТоваров Цикл
				...
				УчитыватьРасхожденияПоСтроке = ТекущийНомерСтроки < КоличествоСтрокТоваров;
				...
				Если ЗначениеЗаполнено(Выборка.Признан) Тогда
					Если (Выборка.Признан = ВариантыДействийПоРасхождениямКодовМаркировкиИСМПНеПризнать
						...
						ПредставлениеРасхождения = СформироватьПредставлениеНедостача(КоличествоНедостача, РасхождениеПоСтроке);
					ИначеЕсли
						...
						ПредставлениеРасхождения = СформироватьПредставлениеИзлишек(КоличествоИзлишек, РасхождениеПоСтроке);
						
					ИначеЕсли Не ДоступноСогласованиеРасхождений Тогда
						Если КоличествоНедостача И КоличествоИзлишек Тогда
							ПредставлениеРасхождения = СформироватьПредставлениеПересорт(КоличествоИзлишек, КоличествоНедостача, РасхождениеПоСтроке, ЗаполняемаяСтрока, УчитыватьРасхожденияПоСтроке);
						ИначеЕсли РасхождениеПоСтроке > 0 Тогда
							ПредставлениеРасхождения = СформироватьПредставлениеИзлишек(КоличествоИзлишек, РасхождениеПоСтроке, УчитыватьРасхожденияПоСтроке);
						Иначе
							ПредставлениеРасхождения = СформироватьПредставлениеНедостача(КоличествоНедостача, РасхождениеПоСтроке, УчитыватьРасхожденияПоСтроке);
						КонецЕсли;
					КонецЕсли;
				...
				Иначе
					Если ЗаполняемаяСтрока.Действие <> Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.НедостачаНеПризнана
						...
						ПредставлениеРасхождения = Новый ФорматированнаяСтрока("<требуется обработка КМ>", , ЦветаСтиля.ЦветТекстаПроблемаГосИС);
					КонецЕсли;
				КонецЕсли;
...
Процедура ОтразитьИзменениеКоличестваВТабличнойЧастиТоварыАктОРасхожденияхПриОтгрузке(ДокументОбъект, ТаблицаРасхожденийКодовМаркировки)
	...
	ТаблицаТовары.Свернуть("ТекстовоеОписание, Номенклатура,Характеристика,Серия, Назначение, Упаковка, ВидЦены, Цена,
	                       ...
	                       |КомментарийКлиента, КомментарийМенеджера, ДокументПоступления,"// Подразделение",!!! разница 4-13/5-6
...
Процедура УменьшитьКоличествоНедостача(СтрокаТаблицыТовары, ТаблицаТовары, СтруктураЗаполнения, КоличествоНедостачаНеПризнать, Действие = Неопределено)
	КоличествоУменьшить = Мин(СтрокаТаблицыТовары.КоличествоПоДокументу, КоличествоНедостачаНеПризнать);
	...
	ОбработатьСтрокуТабличнойЧасти(СтрокаТаблицыТовары, СтруктураЗаполнения.Изменение);
	КоличествоНедостачаНеПризнать = КоличествоНедостачаНеПризнать - КоличествоУменьшить;
	
КонецПроцедуры

Процедура УменьшитьКоличествоИзлишек(СтрокаТаблицыТовары, ТаблицаТовары, СтруктураЗаполнения, КоличествоИзлишекНеПризнать, Действие = Неопределено)
	КоличествоУменьшить = Мин(СтрокаТаблицыТовары.Количество, КоличествоИзлишекНеПризнать);
	...
	ОбработатьСтрокуТабличнойЧасти(СтрокаТаблицыТовары, СтруктураЗаполнения.Изменение);
	КоличествоИзлишекНеПризнать = КоличествоИзлишекНеПризнать - КоличествоУменьшить;
...
Функция ТекстЗапросаМаркируемойПродукции(ДокументСсылка, ИмяТабЧастиТовары = Неопределено,
	...
	Если ЕстьТабличнаяЧастьСерии Тогда
		...
		Если ДобавитьСвязьПоСкладуТабличнойЧасти Тогда
			ШаблонСвязьПоСкладу = "И ТаблицаТовары.%1 = ТаблицаСерии.%1";
			СвязьПоСкладу = СтрШаблон(ШаблонСвязьПоСкладу, ИмяПоляСклад);
		КонецЕсли;
		
		СвязьПоНазначению = "";
		Если ДобавитьСвязьПоНазначению Тогда
			СвязьПоНазначению = "И ТаблицаТовары.Назначение = ТаблицаСерии.Назначение";
		КонецЕсли;
...
Процедура ЗаполнитьТабличныеЧастиМаркировкиТоваровИСМП(
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ДокументОбъект.ВидПродукции)
	...
	ИначеЕсли ДокументОбъект.ВидПродукции = Перечисления.ВидыПродукцииИС.УпакованнаяВода
		...
		ДокументОбъект.ОперацияНанесения = Перечисления.ВидыОперацийИСМП.ОтчетОПечатиКМ;
	КонецЕсли;
...
Функция ТекстЗапросаЗаполненияМаркировкиТоваровИСМПИзОперацииВЕТИС(ИмяДокумента, ИмяПоляИдентификатор, ПустоеЗначениеИдентификатор)
	...
	|	ТоварыКОформлению.Номенклатура.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(ТоварыКОформлению.Количество) КАК Количество,
	|	СУММА(ТоварыКОформлению.Количество) КАК КоличествоУпаковок
	|ИЗ
	|	ТоварыКОформлению КАК ТоварыКОформлению
	...
	|	ТоварыКОформлению.Номенклатура.СтавкаНДС
	|ИМЕЮЩИЕ
	|	СУММА(ТоварыКОформлению.Количество) > 0
	|";
...
Процедура ЗаполнитьМаркировкаТоваровИСМПНаОснованииПроизводственнойОперацииВЕТИС(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	НЕ ДокументСсылка.Проведен                     КАК ЕстьОшибкиПроведен,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.УдалитьОтчетОПередачеКМНаПроизводственнуюЛинию) КАК ОперацияНанесения,
...
Процедура ЗаполнитьВыводИзОборотаИСМПНаОснованииРеализацииТоваровУслуг(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	|	ДокументСсылка.Организация      КАК Организация,
	|	НЕ ДокументСсылка.Проведен      КАК ЕстьОшибкиПроведен,
	|	ЕСТЬNULL(ФискальныеОперации.НомерЧекаККМ, ДокументСсылка.Номер) КАК НомерПервчиногоДокумента,
	|	ЕСТЬNULL(ФискальныеОперации.Дата        , ДокументСсылка.Дата)  КАК ДатаПервчиногоДокумента,
	|	ВЫБОР
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИСМП.ВыводИзОборотаПродажаПоОбразцамДистанционнаяПродажа)
...
Процедура ЗаполнитьВыводИзОборотаИСМПНаОснованииВнутреннегоПотребленияТоваров(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ЗаполнитьТабличныеЧастиВыводаИзОборотаИСМП(ДокументОбъект, ДанныеЗаполнения, "ВнутреннееПотреблениеТоваров", ОписаниеТабличнойЧасти);
...
Процедура ЗаполнитьТабличнуюЧастьПеремаркировкиТоваровИСМП(
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Счетчик = 1 По Выборка.Количество Цикл
			НоваяСтрока = ДокументОбъект.Товары.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
			НоваяСтрока.НоваяНоменклатура   = НоваяСтрока.Номенклатура;
			НоваяСтрока.НоваяХарактеристика = НоваяСтрока.Характеристика;
		КонецЦикла;
...
Функция ТекстЗапросаЗаполненияПеремаркировкиТоваровИСМПИзПрикладногоДокумента(ИмяДокумента, ТабличнаяЧастьШтрихкоды)
	...
	|		И ШтрихкодыУпаковокТоваров.Количество < 2
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
...
Процедура ЗаполнитьПеремаркировкуТоваровИСМПНаОснованииВозвратаТоваровОтКлиента(ДокументОбъект, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
...
//
Функция СформироватьПредставлениеНедостача(КоличествоНедостача, РасхождениеПоСтроке, УчитыватьРасхожденияПоСтроке = Истина) Экспорт
	...
	Если Недостача > 0 Тогда
		ПредставлениеНедостача = Новый ФорматированнаяСтрока("-" + Строка(Недостача));
		КоличествоНедостача = КоличествоНедостача - Недостача;
	КонецЕсли;
	Возврат ПредставлениеНедостача;
...
//   ФорматированнаяСтрока - представление расхождения по кодам маркировки.
//
Функция СформироватьПредставлениеИзлишек(КоличествоИзлишек, РасхождениеПоСтроке, УчитыватьРасхожденияПоСтроке = Истина) Экспорт
	...
	Если Излишек > 0 Тогда
		ПредставлениеИзлишек = Новый ФорматированнаяСтрока("+" + Строка(Излишек));
		КоличествоИзлишек = КоличествоИзлишек - Излишек;
	КонецЕсли;
	Возврат ПредставлениеИзлишек;
...
Функция СформироватьПредставлениеПересорт(КоличествоИзлишек, КоличествоНедостача,
	РасхождениеПоСтроке, ЗаполняемаяСтрока, УчитыватьРасхожденияПоСтроке) Экспорт
	...
	Если Излишек > 0 Тогда
		ПредставлениеПересорт = "+" + Строка(Излишек);
		КоличествоИзлишек = КоличествоИзлишек - Излишек;
	КонецЕсли;
	
	Если Недостача > 0 Тогда
		Если Излишек > 0 Тогда
			ПредставлениеПересорт = ПредставлениеПересорт + " / -" + Строка(Недостача);
		Иначе
			ПредставлениеПересорт = "-" + Строка(Недостача);
		КонецЕсли;
		КоличествоНедостача = КоличествоНедостача - Недостача;
	КонецЕсли;
