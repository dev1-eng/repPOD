...
Процедура ПредложенияИсточниковПоЗапросуКоммерческихПредложений(Знач СсылкаНаОбъект, Знач АнализируемыеИсточники, НайденныеПредложения) Экспорт
	...
	|	ЗапросКоммерческихПредложенийПоставщиковТовары.НоменклатураВСервисеПредставление КАК
	|		НоменклатураВСервисеПредставление,
	...
	|	ГотоваяВыборка.ДокументОснование КАК ДокументОснование,
	|	ГотоваяВыборка.Регистратор КАК Регистратор,
	|	ГотоваяВыборка.НоменклатураПоставщика КАК НоменклатураПоставщика,
	...
	ТоварыПредложений = РезультатыЗапроса[4].Выгрузить();
...
Процедура ЗаполнитьЗапросКоммерческихПредложенийПоставщиковНаОснованииЗаказаКлиента(Объект, ДанныеЗаполнения)
	...
	Для Каждого СтрокаЗаказа Из ТаблицаТовары Цикл
		...
		Если СтрокаЗаказа.ХарактеристикиИспользуются Тогда
			НоваяСтрока.НаименованиеСтрокой = НСтр(СтрШаблон("ru = '%1 (%2)'", НоваяСтрока.Номенклатура, 
...
Процедура КоммерческоеПредложениеПоставщикаОбработкаПроведения(ДокументОбъект, Отказ, РежимПроведения)
	...
	ИнициализироватьДанныеДокументаКоммерческоеПредложениеПоставщика(Ссылка, ДополнительныеСвойства);
	ПроведениеСерверУТ.ПодготовитьНаборыЗаписейКРегистрацииДвижений(ДокументОбъект);
...
Функция ТекстЗапросаТаблицаЦеныНоменклатурыПоставщиков(Запрос, ТекстыЗапроса)
	...
	Если НЕ ПроведениеСерверУТ.ТребуетсяТаблицаДляДвижений(ИмяРегистра, Неопределено) Тогда
		Возврат "";
	КонецЕсли; 
...
Функция ТекстЗапросаВременнаяТаблицаТовары(Запрос, ТекстыЗапроса)
	...
	|							ТОГДА КурсыВалюты.Курс * КурсыВалютыВидыЦеныПоставщика.Кратность / (КурсыВалютыВидыЦеныПоставщика.Курс * КурсыВалюты.Кратность)
	|						ИНАЧЕ 0
	|					КОНЕЦ
	|			ИНАЧЕ 1
	|		КОНЕЦ КАК ЧИСЛО(31, 2)) КАК Цена
...
Процедура ИнициализироватьДанныеДокументаКоммерческоеПредложениеПоставщика(ДокументСсылка, ДополнительныеСвойства)
