...
//     ОповещениеОРазблокировке - ОписаниеОповещения - Описание оповещения после разблокировки.
//     Объект                   - Неопределено - взять объект из реквизита формы "Объект",
//                              - ДанныеФормыСтруктура - по типу объекта.
//
Процедура РазрешитьРедактированиеРеквизитовОбъекта(Форма, Знач ИмяФормыРазблокировки = "", ОповещениеОРазблокировке = Неопределено, Объект = Неопределено) Экспорт
	
	ОписаниеОбъекта = ?(Объект = Неопределено, Форма.Объект, Объект);
	
	Если ЗначениеЗаполнено(ОписаниеОбъекта.Ссылка) Тогда
		...
		ОткрытьФорму(ИмяФормыРазблокировки, 
			Новый Структура("Объект", ОписаниеОбъекта.Ссылка),
			,
			,
			,
			,
...
Процедура КешироватьТекущуюСтроку(Форма, ИмяТабличнойЧасти) Экспорт
	...
	Иначе
		...
		Для Каждого КлючИЗначение Из СтруктураКеш Цикл
			СтруктураКеш[КлючИЗначение.Ключ] = Истина; // как в ОбщегоНазначенияУТ.ИнициализироватьКешТекущейСтроки()
		КонецЦикла;
...
//  РежимОткрытияОкнаФормы	 - Строка - значение по умолчанию "Независимый"
...
Процедура ОткрытьФормуПослеПроверкиПроведенностиДокумента(ИмяФормы, ПараметрыФормы, ФормаВладелец, РежимОткрытияФормы = "Независимый", ИмяПараметраСсылка = "") Экспорт
	...
	ДополнительныеПараметры.Вставить("ИмяПараметраСсылка", ИмяПараметраСсылка);
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'Неизвестное значение параметра РежимОткрытияФормы'");
...
//  ИмяРеквизита			 - Строка, Массив			 - имя обрабатываемого реквизита
...
//
Процедура ПродолжитьИзменениеРеквизита(Форма, ИмяРеквизита = "", Знач ДополнительныеПараметры = Неопределено, ТребуетсяВызовСервера = Ложь) Экспорт
	
	Если ДополнительныеПараметры <> Неопределено Тогда
		ДополнительныеПараметры = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ДополнительныеПараметры);
	Иначе
		ДополнительныеПараметры = Новый Структура;
	КонецЕсли; 
	ДополнительныеПараметры.Вставить("ТребуетсяВызовСервера", ТребуетсяВызовСервера);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("Подключаемый_ПриИзмененииРеквизитаЗавершение", Форма, ДополнительныеПараметры);
...
//  ТребуетсяВызовСервера	 - Булево					 - признак необходимости вызова сервера
//
Процедура ПродолжитьВыполнениеКоманды(Форма, ИмяКоманды = "", Знач ДополнительныеПараметры = Неопределено, ТребуетсяВызовСервера = Ложь) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("Подключаемый_ПриВыполненииКомандыЗавершение", Форма, ДополнительныеПараметры);
...
//  ЭтаФорма             - ФормаКлиентскогоПриложения - Управляемая форма, в которой происходит событие "Начало выбора"
