...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Период = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ПодборТоваровПоРеализациямИПередачам","ПериодПодбораТоваровПоРеализациям");
...
Процедура ПриСохраненииДанныхВНастройкахНаСервере(Настройки)
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПодборТоваровПоРеализациямИПередачам","ПериодПодбораТоваровПоРеализациям", Период);
...
Процедура ЗаполнитьТаблицуТоваров()
	...
	Если Не ВозвратОтРозничногоПокупателя
		...
		|	ДанныеДокументовРеализации.ДатаРеализацииПередачи КАК ДатаРеализацииПередачи,
		|	ДанныеДокументовРеализации.НомерРеализацииПередачи КАК НомерРеализацииПередачи,
		|	ДанныеДокументовРеализации.СтавкаНДС КАК СтавкаНДС,
		|	ДанныеДокументовРеализации.ДокументРеализацииПередачи КАК ДокументРеализацииПередачи,
		|	ДанныеДокументовРеализации.ДокументРеализацииПередачи.Валюта КАК ВалютаРеализации,
		|	ДанныеДокументовРеализации.ДокументРеализацииПередачи.ЦенаВключаетНДС КАК ЦенаВключаетНДС
		|ИЗ
		...
		|			И (ДанныеДокументовВозврата.ДокументРеализации = ДанныеДокументовРеализации.ДокументРеализацииПередачи)
		...
		|	ДанныеДокументовРеализации.ДатаРеализацииПередачи,
		|	ДанныеДокументовРеализации.НомерРеализацииПередачи,
		|	ДанныеДокументовРеализации.СтавкаНДС,
		|	ДанныеДокументовРеализации.ДокументРеализацииПередачи,
		|	ДанныеДокументовРеализации.ДокументРеализацииПередачи.Валюта,
		|	ДанныеДокументовРеализации.ДокументРеализацииПередачи.ЦенаВключаетНДС,
	...
	ИначеЕсли ВозвратОтРозничногоПокупателя Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЧекККМВозвратТовары.Ссылка КАК ДокументРеализацииПередачи,
		...
		|	ЧекККМВозвратТовары.Ссылка.Номер КАК НомерРеализацииПередачи,
		|	ЧекККМВозвратТовары.Ссылка.Валюта КАК ВалютаРеализацииПередачи,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЧекиККМ.ДокументРеализацииПередачи КАК ДокументРеализацииПередачи,
		...
		|	ЧекиККМ.ДатаРеализацииПередачи КАК ДатаРеализацииПередачи,
		|	ЧекиККМ.НомерРеализацииПередачи КАК НомерРеализацииПередачи,
		|	ЧекиККМ.ВалютаРеализации КАК ВалютаРеализации,
		|	ЧекиККМ.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
		|	ЧекиККМ.Партнер КАК Партнер,
		|	ЧекиККМ.ДокументРеализацииПередачи.Склад КАК Склад
		|ИЗ
		|	ЧекиККМ КАК ЧекиККМ
		|		ЛЕВОЕ СОЕДИНЕНИЕ ЧекиККМВозврат КАК ЧекиККМВозврат
		|		ПО ЧекиККМ.ДокументРеализацииПередачи = ЧекиККМВозврат.ДокументРеализацииПередачи.ЧекККМ
		...
		|		ПО ЧекиККМ.ДокументРеализацииПередачи = ДанныеВозврата.Ссылка.ЧекККМ
	...
	ИначеЕсли ВозвратМеждуОрганизациями Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаДокумента.ДокументРеализацииПередачи  КАК ДокументРеализацииПередачи,
		|	ТаблицаДокумента.Склад                       КАК Склад,
		|	ТаблицаДокумента.ДатаРеализацииПередачи      КАК ДатаРеализацииПередачи,
		|	ТаблицаДокумента.НомерРеализацииПередачи     КАК НомерРеализацииПередачи,
		|	ТаблицаДокумента.ВалютаРеализации            КАК ВалютаРеализации,
		|	ТаблицаДокумента.Номенклатура                КАК Номенклатура,
		|	ТаблицаДокумента.Характеристика              КАК Характеристика,
		|	ТаблицаДокумента.Серия                       КАК Серия,
		|	ТаблицаДокумента.Назначение                  КАК Назначение,
		...
		|		- ЕСТЬNULL(ТаблицаВозвратов.Количество, 0) КАК Количество,
		|	ВЫБОР
		|		КОГДА ТаблицаДокумента.Упаковка ЕСТЬ NULL 
		|				ИЛИ ТаблицаДокумента.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|			ТОГДА ТаблицаДокумента.Количество - ЕСТЬNULL(ТаблицаВозвратов.Количество, 0)
		|		ИНАЧЕ (ТаблицаДокумента.Количество - ЕСТЬNULL(ТаблицаВозвратов.Количество, 0)) / &ТекстЗапросаКоэффициентУпаковки
		|	КОНЕЦ                                        КАК КоличествоУпаковок,
		|	ТаблицаДокумента.ЦенаВключаетНДС             КАК ЦенаВключаетНДС,
		|	ВЫБОР
		|		КОГДА ТаблицаДокумента.Количество - ЕСТЬNULL(ТаблицаВозвратов.Количество, 0) = 0
		|			ТОГДА ТаблицаДокумента.Цена
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ТаблицаДокумента.Упаковка ЕСТЬ NULL 
		|						ИЛИ ТаблицаДокумента.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|					ТОГДА ТаблицаДокумента.Сумма / ТаблицаДокумента.Количество
		|				ИНАЧЕ ТаблицаДокумента.Сумма / (ТаблицаДокумента.Количество / &ТекстЗапросаКоэффициентУпаковки)
		|			КОНЕЦ
		|	КОНЕЦ КАК Цена,
		|	ТаблицаДокумента.Сумма
		|		- ЕСТЬNULL(ТаблицаВозвратов.Сумма, 0)    КАК Сумма,
		|	ТаблицаДокумента.СтавкаНДС                   КАК СтавкаНДС,
		|	ТаблицаДокумента.СуммаНДС
		|		- ЕСТЬNULL(ТаблицаВозвратов.СуммаНДС, 0) КАК СуммаНДС,
		|	ТаблицаДокумента.СуммаСНДС
		|		- ЕСТЬNULL(ТаблицаВозвратов.СуммаСНДС, 0) КАК СуммаСНДС
		|ИЗ
		|	ВТТоварыПередач КАК ТаблицаДокумента
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВозвраты КАК ТаблицаВозвратов
		...
		|ГДЕ
		|	ТаблицаДокумента.Количество
		|		- ЕСТЬNULL(ТаблицаВозвратов.Количество, 0) > 0
		|	И ТаблицаДокумента.Номенклатура.ТипНоменклатуры В
		|		(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
		|УПОРЯДОЧИТЬ ПО
		|	ДокументРеализацииПередачи УБЫВ";
	...
	ПараметрыНовогоКурса = Новый Структура("Валюта, Курс, Кратность",
											Валюта, КурсНовойВалюты.Курс, КурсНовойВалюты.Кратность);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Валюта <> Выборка.ВалютаРеализации Тогда
			...
			ПараметрыТекущегоКурса = Новый Структура("Валюта, Курс, Кратность",
													Выборка.ВалютаРеализации, КурсСтаройВалюты.Курс, КурсСтаройВалюты.Кратность);
			
			НоваяСтрока.СуммаСНДС = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(НоваяСтрока.СуммаСНДС,
...
Процедура ЗаполнитьНомераГТДПоРаспоряжениям()
	...
	НоменклатураСервер.ЗаполнитьНомераГТД(ТаблицаТоваров, "ПоРаспоряжениям", ПараметрыЗаполнения);
