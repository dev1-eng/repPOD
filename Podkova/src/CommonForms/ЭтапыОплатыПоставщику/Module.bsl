...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если УпрощенныйРежим Тогда
		...
		Иначе
			...
			Если ЭтапыГрафикаОплаты.Количество()>0 Тогда
				ДатаПредоплата = ЭтапыГрафикаОплаты[0].ДатаПлатежа;
			Иначе
...
Процедура ОК(Команда)
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ПорядокОплаты)
		...
		Если УпрощенныйРежим Тогда
			...
			Если НЕ ЭтоЗаказ ИЛИ ЭтоЗаказ И СуммаПлатежаПредоплата > 0 Тогда
...
Процедура ЭтапыГрафикаОплатыСдвигПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ЗначениеЗаполнено(ЖелаемаяДатаПоступления) Тогда
			ТекущиеДанные.ДатаПлатежа = ЖелаемаяДатаПоступления + 86400 * ТекущиеДанные.Сдвиг;
		Иначе
...
Процедура ЭтапыОплатыВариантОплатыПриИзменении(Элемент)
	ТекущиеДанные = Элементы.ЭтапыГрафикаОплаты.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		Если ТекущиеДанные.Сдвиг <> 0 И ТекущиеДанные.ВариантОплаты <> ПредопределенноеЗначение("Перечисление.ВариантыОплатыПоставщику.КредитСдвиг") Тогда
			ТекущиеДанные.Сдвиг = 0;
		ИначеЕсли ТекущиеДанные.ВариантОплаты = ПредопределенноеЗначение("Перечисление.ВариантыОплатыПоставщику.КредитСдвиг") Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ВариантыОплатыПоставщику.КредитСдвиг;
...
Процедура ЗаполнитьЭтапыОплатыПоСоглашениюСервер(ЗаполнятьФормуОплаты = Ложь)
	...
	ИначеЕсли НЕ ЭтоЗаказ И НЕ НакладнаяПоЗаказам Тогда
		ЭтапыОплатыСервер.СвернутьЭтапыОплаты(ЭтапыГрафикаОплаты, Истина);
	КонецЕсли;
...
Функция ЖелаемаяДатаОплатыЭтапаПоУмолчанию()
	...
	Иначе
		Возврат ?(ЗначениеЗаполнено(ЖелаемаяДатаПоступления), ЖелаемаяДатаПоступления, Дата);
	КонецЕсли;
...
Функция ЗаполнитьШаблоныГрафиков()
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		Пока ВыборкаГрафик.Следующий() Цикл
			...
			Если ВыборкаГрафик.ВариантОплаты = Перечисления.ВариантыОплатыПоставщику.КредитПослеПоступления
				ИЛИ ВыборкаГрафик.ВариантОплаты = Перечисления.ВариантыОплатыПоставщику.КредитСдвиг Тогда
...
Процедура РаспределитьСуммуПоЭтапамИЗаказамГрафикаОплаты()
	...
	ПараметрыЗаполнения.ЭтапыГрафикаОплаты        = ЭтапыГрафикаОплаты;
	ПараметрыЗаполнения.ПоЗаказам                 = НакладнаяПоЗаказам;
	ПараметрыЗаполнения.УпрощеннаяСхема           = ПолучитьФункциональнуюОпцию("ИспользоватьУпрощеннуюСхемуОплатыВЗакупках");
...
Процедура ЗаполнитьЭтапыГрафикаОплаты()
	...
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ЗаполнитьДату", УпрощенныйРежим И НЕ ЭтоЗаказ);
...
Процедура НастроитьЭлементыФормыПоПараметрам()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(
		Элементы, МассивЭлементов, "Видимость", ЭтоЗаказ);
	...
	Если НЕ ЭтоЗаказ Тогда
		...
		КонецЕсли;
		Элементы.ЭтапыГрафикаОплатыВариантОплаты.ТолькоПросмотр = Ложь;
	КонецЕсли;
...
Процедура ПреобразоватьТаблицуЭтаповВДанныеУпрощенногоРежима(ЭтапыОплаты)
	...
	Для Каждого ТекСтрока Из ЭтапыОплаты Цикл
		...
		Если ТекСтрока.ВариантОплаты = Перечисления.ВариантыОплатыПоставщику.КредитПослеПоступления 
			ИЛИ ТекСтрока.ВариантОплаты = Перечисления.ВариантыОплатыПоставщику.КредитСдвиг 
