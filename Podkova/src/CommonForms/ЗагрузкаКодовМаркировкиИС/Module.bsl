	...
	Для Индекс = 2 По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если ЗначениеЗаполнено(Штрихкод) Тогда
			...
			Если КолонкаКодУпаковкиИспользуется Тогда
				ПоляСтроки.Вставить("ШтрихкодУпаковки", СокрЛП(ТабличныйДокумент.Область(Индекс, НомерКолонкиКодУпаковки).Текст));
			Иначе
				ПоляСтроки.Вставить("ШтрихкодУпаковки", "");
			КонецЕсли;
			ПоляСтроки.Вставить("ФорматBase64", ФорматBase64);
...
// Проверяет наличие кодонки "Код маркировки" и хотя бы 1 заполненного значения в ней
// 
// Возвращаемое значение:
