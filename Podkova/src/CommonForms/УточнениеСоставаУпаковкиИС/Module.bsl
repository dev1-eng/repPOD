...
Процедура ОбработатьПереданныеПараметры()
	...
	Если Не ПараметрыСканирования.Свойство("ЗаполнятьДанныеВЕТИС")
		Или ПараметрыСканирования.ЗаполнятьДанныеВЕТИС = Неопределено Тогда
...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Для Каждого СтрокаТаблицы Из ДанныеДляУточнения Цикл
		...
		Если ПараметрыСканирования.Свойство("ЗаполнятьДанныеВЕТИС")
				И ПараметрыСканирования.ЗаполнятьДанныеВЕТИС = Истина Тогда
...
Процедура ОбновитьДанныеУпаковокПоАдресуДереваУпаковок()
	...
	Если (ПараметрыСканирования.Свойство("ЗаполнятьДанныеВЕТИС")
			И ПараметрыСканирования.ЗаполнятьДанныеВЕТИС <> Неопределено)
	...
	Для Каждого ВложеннаяСтрока Из СтрокаДерева.Строки Цикл
		...
		Иначе
			...
			ОбщиеДанные.Заполнено = ОбщиеДанные.Заполнено
				И ВложеннаяСтрока.ИдентификаторПроисхожденияВЕТИС = ЭлементДляПроверки.ИдентификаторПроисхожденияВЕТИС
				И ВложеннаяСтрока.ГоденДо = ЭлементДляПроверки.ГоденДо;
		КонецЕсли;
	КонецЦикла;
