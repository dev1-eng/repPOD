...
Процедура ОбработатьИПроверитьПереданныеПараметры(Отказ)
	...
	Если Не ПроверкаНеПоДокументу
		...
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'В обработку передан документ некорректного типа.'"),,,,Отказ);
...
Процедура ПоискПоШтрихкодуЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры <> Неопределено
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
			ДополнительныеПараметры, "ОбработкаДанныхТСД") Тогда
...
Процедура ОбработатьПолученныеДанныеТСД(Результат)
	...
	ВыполнитьСледующуюОперациюОбработкиДанныхТСД(СледующаяОперация);
...
&НаКлиенте
Процедура ВыполнитьСледующуюОперациюОбработкиДанныхТСД(СледующаяОперация)
	...
	ИначеЕсли СледующаяОперация = "МаркируемаяПродукция" Тогда 
		...
		ИначеЕсли ЗагрузкаДанныхТСД.МаркируемаяПродукция.Количество() <> ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента Тогда
			...
			Если РезультатОбработкиШтрихкода.ТребуетсяУточнениеДанных
				Или ШтрихкодированиеИСКлиент.ТребуетсяУточнениеДанныхУПользователя(ЭтотОбъект, РезультатОбработкиШтрихкода, ПараметрыСканирования) Тогда
				...
				Иначе
					...
					ДополнительныеПараметры = Новый Структура;
					ДополнительныеПараметры.Вставить("ОбработкаДанныхТСД", Истина);
					ОповещениеПриЗавершении = Новый ОписаниеОповещения(
						"ПоискПоШтрихкодуЗавершение", ЭтотОбъект, ДополнительныеПараметры);
