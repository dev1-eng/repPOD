...
Процедура СобытиеНажатиеКнопки(Команда)
	...
	КолГруз2=0;
	Кол=1;
		//++dev1 При добавлении товара срабатывает автоскидка, не срабатывает если уже действует скидка
	Если НЕ СейчасСкидка ИЛИ СейчасСкидкаРазовая ИЛИ СейчасКупон ИЛИ СкидкаИнетМаг тогда
		Автоскидка();
	КонецЕсли;
		//--dev1
...
Процедура СоздатьРеализацию(Наим, Ошибка, ЮЛ)
	...
	//++Сварка/Покраска черновик 
	Док.СуммаПокраскаЧерновик = СуммаПокраскаЧерновик;//СПЦ
	Док.СуммаСваркаЧерновик = СуммаСваркаЧерновик;//СПЦ
	//
...
Функция ЗаполнитьВизуальныйРасчет(СейчасСкидка,СкидкаИнетМаг,СейчасКупон)
	...
	ТипДиска="";
 	...
 	Для каждого СтрТЧ из Таб Цикл
		...
		//
		ИначеЕсли СтруктураПроф.ИспГрузики И СкидкаИнетМаг Тогда //ИСПОЛЬЗУЕТСЯ СКИДКА 10
		...
		ИначеЕсли СтруктураПроф.ИспГрузики И СейчасКупон Тогда //НЕ ИСПОЛЬЗУЕТСЯ
			...
			//
		ИначеЕсли СтруктураПроф.ИспГрузики И СейчасСкидкаРазовая Тогда // ИСПОЛЬЗУЕТСЯ СКИДКА 20 
		...
		Иначе //ИСПОЛЬЗУЕТСЯ БЕЗ груза СКИДКА/БЕЗ СКИДКИ  
			...
			//--dev1
				...
				Если 	СкидкаИнетМаг 			Тогда
					ПроцентСкидкиЧерновик = 10;
				ИначеЕсли СейчасСкидкаРазовая 	Тогда
					ПроцентСкидкиЧерновик = 20;
				ИначеЕсли СейчасСкидка 			Тогда
					ПроцентСкидкиЧерновик = 50;
				КонецЕсли;
	...
	Для Каждого СтрТаблВизВывода Из ТаблВизВывода Цикл
		...
		КонецЕсли; 
		//dev1
		Если СтрТаблВизВывода.Должность = Справочники.ШМ_Должности.НайтиПоНаименованию("Сварка") Тогда
			СуммаСваркаЧерновик = СтрТаблВизВывода.Сумма;
		КонецЕсли; 
		Если СтрТаблВизВывода.Должность = Справочники.ШМ_Должности.НайтиПоНаименованию("Покраска") Тогда
			СуммаПокраскаЧерновик = СтрТаблВизВывода.Сумма;
		КонецЕсли
		//
	КонецЦикла;
...
Функция ПолучитьТабДокЧерновика2020()
	...
	лТекст = "ВЫБРАТЬ
	         ...
	         |	РеализацияТоваровУслуг.СуммаСваркаЧерновик КАК СуммаСваркаЧерновик,
			 |	РеализацияТоваровУслуг.СуммаПокраскаЧерновик КАК СуммаПокраскаЧерновик
	         |ИЗ
	         |	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	         |ГДЕ
	...
	//++Безроднов Сварка/Покраска 
	
		//Покраска
		ОбластьСтрока.Параметры.Покраска = ВыбРеалЧерновик.СуммаПокраскаЧерновик;
		ОбластьСтрокаЖирн.Параметры.Покраска = ВыбРеалЧерновик.СуммаПокраскаЧерновик;
		ИтогПокраска = ИтогПокраска + ВыбРеалЧерновик.СуммаПокраскаЧерновик;  
		//Сварка 
		ОбластьСтрока.Параметры.Сварка = ВыбРеалЧерновик.СуммаСваркаЧерновик;
		ОбластьСтрокаЖирн.Параметры.Сварка = ВыбРеалЧерновик.СуммаСваркаЧерновик;
		ИтогСварка = ИтогСварка + ВыбРеалЧерновик.СуммаСваркаЧерновик;
	...
	ОбластьСтрокаИтог.Параметры.ИтогСварка = ИтогСварка;
	ОбластьСтрокаИтог.Параметры.ИтогПокраска = ИтогПокраска;
...
Функция ПолучитьТабДокДископравкаНаДату(ДатаСмены, ФормаВыбРабочиеМеста)
	...
	//++DEV1		
	Профессия = Справочники.ШМ_Должности.НайтиПоНаименованию("Правщик дисков");
	ПрофессияСварка = Справочники.ШМ_Должности.НайтиПоНаименованию("Сварка");
	ПрофессияПокраска = Справочники.ШМ_Должности.НайтиПоНаименованию("Покраска");
	
	МассивПрофессий = Новый Массив;
	МассивПрофессий.Добавить(Профессия);
	МассивПрофессий.Добавить(ПрофессияСварка);
	МассивПрофессий.Добавить(ПрофессияПокраска);
	//++DEV1 
	...
	ЗапросНоменклатураПрофессии.Текст = "ВЫБРАТЬ
	                                    ...
	                                    |	ЦеныНоменклатурыСрезПоследних.Цена КАК Цена,
	                                    |	ШМ_НоменклатураПрофессии.Дископравка КАК Дископравка,
	                                    |	РеализацияТоваровУслугТовары.Сумма - ШМ_НоменклатураПрофессии.МатериальныйВычет КАК СтоимостьУслуги,
	                                    |	ШМ_НоменклатураПрофессии.Профессия КАК Профессия
	                                    |ИЗ
	                                    ...
	                                    |	РеализацияТоваровУслугТовары.Сумма - ШМ_НоменклатураПрофессии.МатериальныйВычет,
	                                    |	ШМ_НоменклатураПрофессии.Профессия";
	...
	ВыборкаНоменклатураПрофессии = ЗапросНоменклатураПрофессии.Выполнить().Выбрать();
 	...
 	Пока ВыборкаНоменклатураПрофессии.Следующий() Цикл  
		...
		//++dev1 Добавление расчетов по сварке и покраске 29.05
		Сварка = Справочники.ШМ_Должности.НайтиПоНаименованию("Сварка");
		Покраска = Справочники.ШМ_Должности.НайтиПоНаименованию("Покраска");
		Если ВыборкаНоменклатураПрофессии.Профессия = Сварка Тогда
			СваркаВсего = СваркаВсего + ВыборкаНоменклатураПрофессии.СтоимостьУслуги;
		ИначеЕсли ВыборкаНоменклатураПрофессии.Профессия = Покраска тогда
			ПокраскаВсего = ПокраскаВсего + ВыборкаНоменклатураПрофессии.СтоимостьУслуги;
		КонецЕсли;
		СПЦВсего = СПЦВсего + ПокраскаВсего + СваркаВсего;
		//--dev1
	...
	//Сварка/Покраска	
	ОбластьПодвал.Параметры.СваркаВсего 		 = Строка(СваркаВсего);
	ОбластьПодвал.Параметры.ПокраскаВсего 		 = Строка(ПокраскаВсего);
...
Функция ПолучитьЗапросТоварныйЧек()
	...
	//++dev1
	ОбластьСкидок.Параметры.Пр = РеквизитФормыПроцентСкидки;
	//--dev1
...
Процедура ПриОткрытии(Отказ)
	...
	//++dev1 Автоскидка при открытии 14.05
	АвтоСкидка();
	//dev1
	Если СейчасСкидка Тогда
		СкидкаНаСервере();
	...
	//++dev1 Проверка активности скидки, если не включена то видимость скидки скрывается с формы 24.04
...
Процедура СкидкаНаСервере()
	...
	Для Каждого СтрТаб Из Таб Цикл
		...
		КонецЦикла;
		Если ПроцСк <> 0 тогда
			РеквизитФормыПроцентСкидки = ПроцСк;//
		КонецЕсли;
...
Процедура ОтменаСкидок(Команда)
	...
	//++dev1 14.05 Автоскидки не работают если клиент ЮР. лицо, иначе всегда работают
		Если НЕ Команда.Действие = "ОформитьЮЛ" Тогда
				Автоскидка();
		КонецЕсли;
	//--dev1
	ТаблицаПриОкончанииРедактированияНаСервере();
...
//++DEV1 Автоскидки на точках, если включен переключатель 14.05
&НаСервере
Процедура Автоскидка()
	//скидка 50 ложь
	СейчасСкидка=Ложь;
	Склад = ПолучитьСклад(ПользователиКлиентСервер.ТекущийПользователь());
	Если ЗначениеЗаполнено(Склад) Тогда
			Переключатель = Склад.Переключатель;
			//Автоскидка 10
	...
	Иначе
			...
			Сообщение.Текст = "Не удалось определить склад в персональных настройках пользователя,
			|обратитесь к Администратору информационной базы";
			Сообщение.Сообщить();
	КонецЕсли;
КонецПроцедуры
//--DEV1 Автоскидки на точках если включен переключатель
...
Функция ПолучитьЗапросДетализация()
	...
	ОбластьСуммыНДССтандарт  = Макет.ПолучитьОбласть("Строка|СуммаНДС");
	ОбластьСуммыСтандарт     = Макет.ПолучитьОбласть("Строка|Сумма");
//	ОбластьСуммыБезСкидокСтандарт = Макет.ПолучитьОбласть("Строка|СуммаБезСкидки");
	...
	ОбластьПомещение = Макет.ПолучитьОбласть("Итого|Помещение");
	ОбластьДанных    = Макет.ПолучитьОбласть("Итого|Данные");
	ОбластьСкидок    = Макет.ПолучитьОбласть("Итого|СуммаСкидки");
	ОбластьСуммы     = Макет.ПолучитьОбласть("Итого|Сумма");
	ОбластьСуммыНДС  = Макет.ПолучитьОбласть("Итого|СуммаНДС");
	ОбластьСуммыБезСкидок = Макет.ПолучитьОбласть("Итого|СуммаБезСкидки");
	//--
	...
	//	СуммаДокумента = Сумма + СуммаНДС;
	//Иначе
		СуммаДокБезСкидки = СуммаБезСкидки; 
		СуммаДокумента = Сумма;
		ВсегоСкидок = СуммаДокБезСкидки - СуммаДокумента;
	//КонецЕсли;
	//++dev1 Выводим сумму без скидок и сумму скидок
	ОбластьСуммыБезСкидок.Параметры.ВсегоБезСкидок = СуммаДокБезСкидки;
	ТабличныйДокумент.Присоединить(ОбластьСуммыБезСкидок);
...
Процедура ОформитьЮЛ(Команда)
	...
	Если ЗначениеЗаполнено(НомерМашины) Тогда
			//Если у нас нажата кнопка ЮЛ на форме, то мы отменяем все скидки
			ОтменаСкидок(Команда);
