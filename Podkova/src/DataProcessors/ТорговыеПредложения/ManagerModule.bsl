...
// Параметры:
//  ПараметрыПроцедуры - Структура - параметры выполнения процедуры.
//    * Заказы - ТаблицаЗначения - таблица заказов для отправки:
...
Процедура ОтправитьЗаказы(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	ЭлектронныеДокументы = БизнесСеть.ПолучитьУдалитьИзВременногоХранилища(АдресХранилища);
...
Процедура ОбновитьСтатистикуСостояний(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт 
	Перем ЗначениеОтбора, ЧастичныйПодсчетОпубликованных;
...
Процедура СформироватьКарточкуПубликуемогоТорговогоПредложения(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	ЭлектронноеВзаимодействиеПереопределяемый.ПолучитьДанныеЮрФизЛица(ВыборкаЗапросаПрайсЛисты.Организация, Сведения);
...
Процедура ПолучитьДанныеДиагностики(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	Перем Отбор, ПрайсЛист, ПозицииТорговыхПредложений;
	...
	СтрокаТорговогоПредложения = Новый Структура("ПрайсЛист, Номенклатура, Характеристика, Упаковка, Организация,
		|ПричинаИсключенияИзПубликации, ОписаниеОшибки");
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		Если НЕ ПустаяСтрока(ВыборкаДанных.КонтрольРеквизитов) Тогда
			...
			Для Каждого ПроверяемыйРеквизит Из ПроверяемыеРеквизиты Цикл
				СписокРеквизитов.Добавить(ПроверяемыйРеквизит, , Истина);
			КонецЦикла;
		КонецЕсли;
		...
		Если НЕ ПустаяСтрока(ВыборкаДанных.РеквизитыПредупреждений) Тогда
			...
			Для Каждого ПроверяемыйРеквизит Из ПроверяемыеРеквизиты Цикл
				СписокРеквизитов.Добавить(ПроверяемыйРеквизит, , Ложь);
			КонецЦикла;
		КонецЕсли;
		...
		Для Каждого Реквизит Из СписокРеквизитов Цикл
			...
			ЯвляетсяОшибкой     = Реквизит.Пометка И НЕ ДанныеОпубликованы;
			...
			ТорговыеПредложенияСлужебный.ОпределитьВладельцевНезаполненныхРеквизитов(ПроверяемыйРеквизит, СтрокаТорговогоПредложения, СсылкаНаОбъект, ПутьКДанным, ТекстСообщения);
			...
			Если Реквизит.Значение = "ОшибкаПубликацииСервиса" Тогда
			...
			НоваяСтрока.ЯвляетсяПредупреждением = НЕ ЯвляетсяОшибкой;
			...
			ИначеЕсли ЧастиИмениРеквизита[0] = "Товары" Тогда
				НоваяСтрока.ИмяРаздела = "СведенияОТоваре";
				ОбъектыСведенияОТоваре.Добавить(СсылкаНаОбъект);
			ИначеЕсли ЧастиИмениРеквизита[0] = "Свойства" Тогда
				НоваяСтрока.ИмяРаздела = "СведенияОТоваре";
				ОбъектыСведенияОТоваре.Добавить(СсылкаНаОбъект);
...
Процедура ДобавитьДанныеТаблицы(ДанныеТорговогоПредложения, ТаблицаРеквизитов, ИмяТаблицы, ВыборкаЗапроса, РежимРедактирования)
	...
	Для Каждого Реквизит Из Реквизиты Цикл
		...
		ИмяПараметры = СтрШаблон("%1_%2", Реквизит.ИмяТаблицы, Реквизит.ИмяРеквизита);
		ЗначениеПараметра = ВыборкаЗапроса[Реквизит.ИмяРеквизита];
		...
		Если РежимРедактирования И Реквизит.Доступность Тогда
			...
			ЗначениеПараметра.Вставить("Действие"               , "ОбработкаПрикладнойЛогикой");
...
Процедура УстановитьРедактированиеОбластей(ОбластьМакета, ТаблицаРеквизитов)
	...
	Для Каждого СтрокаРеквизита Из ТаблицаРеквизитов Цикл
		...
		Область.Картинка                      = БиблиотекаКартинок.Лупа;
...
Процедура ВывестиСвойстваСервиса(ТабличныйДокумент, Макет, ДанныеТорговогоПредложения, ТаблицаРеквизитов,
	...
	Для Каждого ДополнительныйРеквизит Из ДанныеНоменклатурыСервиса.ДополнительныеРеквизиты Цикл
		...
		ЭлектронноеВзаимодействиеСлужебный.ВывестиОбластьВТабличныйДокумент(ТабличныйДокумент, СвойстваСтрока,
			"СвойстваСтрока",, 1);
...
Процедура ВывестиИзображения(ТаблицаИзображений, ОбластьКартинка, ТабличныйДокумент, ДанныеИзображений)
	...
	Для Каждого ЭлементКоллекции Из ТаблицаИзображений Цикл
		...
		ДвоичныеДанныеИзображения = ТорговыеПредложенияСлужебный.ДвоичныеДанныеПоАдресуURL(ЭлементКоллекции, Ложь);
...
Процедура ОпределитьНоменклатуруСервиса(ДанныеТорговогоПредложения, РежимРедактирования)
	...
	ИдентификаторНоменклатурыСервиса = 
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторНоменклатурыСервиса);
...
Процедура ПроверитьСопоставлениеНоменклатурыКонтрагентов(Контрагент, ТоварыКонтрагента)
	...
	Для каждого СтрокаЗаказа Из ТоварыКонтрагента Цикл
		...
		Если ОбменСКонтрагентамиСлужебныйПовтИсп.ПодсистемаСопоставленияНоменклатурыСуществует() Тогда
			ОбменСКонтрагентамиСлужебныйПовтИсп.МодульСопоставлениеНоменклатурыКонтрагентовПереопределяемый().ПриОпределенииВладельцаНоменклатурыКонтрагента(
				Контрагент, ВладелецНоменклатуры);
		Иначе
			ОбменСКонтрагентамиПереопределяемый.ПриОпределенииВладельцаНоменклатурыКонтрагента(Контрагент, ВладелецНоменклатуры);
		КонецЕсли;
		...
		НоменклатураКонтрагента.СтавкаНДС = ОбменСКонтрагентамиПовтИсп.СтавкаНДСИзСоответствия(, СтрокаЗаказа.СтавкаНДС);
