...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТоварыХарактеристика.Видимость = ОбменСКонтрагентамиПовтИсп.ДополнительнаяАналитикаСправочникХарактеристикиНоменклатуры();
...
Процедура ИзменитьКоличествоВЗаказе(Идентификатор, Количество, ЭтоРазница = Ложь, ДополнительныеПараметры = Неопределено, Отказ = Ложь)
	...
	Иначе
		...
		Если КонтрагентыТоваров.Количество() = 0 Тогда
			СтрокаТовары.Контрагент = ЭлектронноеВзаимодействие.НайтиСсылку("Контрагенты",,
...
Процедура ЗаполнитьСтрокиПоНоменклатуреКонтрагента(СтрокаТовары = Неопределено)
	...
	Для каждого ЭлементКоллекцииТоваров Из СтрокиТоваров Цикл
		...
		Иначе
			ОбменСКонтрагентамиПереопределяемый.ПриОпределенииВладельцаНоменклатурыКонтрагента(ЭлементКоллекцииТоваров.Контрагент,
		...
		НайденноеЗначение = ОбменСКонтрагентами.НайтиСоответствиеНоменклатуры(Отбор, Истина);
...
Процедура ДлительнаяОперацияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПоставщикиСОшибкойСозданияЗаказа   = Новый Массив;
	ВыводитьВопросЧастичногоОформления = Ложь;
	...
	ИначеЕсли Параметры.Свойство("ТаблицаТоваров") Тогда
		...
		Для каждого СоставПозиции Из Параметры.ТаблицаТоваров Цикл
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.Контрагент) Тогда
				...
				Если КонтрагентыТоваров.Количество() = 0 Тогда
					НоваяСтрока.Контрагент = ЭлектронноеВзаимодействие.НайтиСсылку("Контрагенты",,
...
Процедура ЗаполнитьНоменклатуруПоИдентификаторамСервиса(ИдентификаторСтроки = Неопределено)
	...
	НоменклатураСсылки = РаботаСНоменклатурой.НоменклатураИХарактеристикиПоИдентификаторам(МассивИдентификаторов);
