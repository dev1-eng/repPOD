...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Перем ПараметрПозицииТорговыхПредложений, ТребуетсяВыполнитьПроверкуДанных;
	
	Параметры.Свойство("ПрайсЛист"                        , ПрайсЛист);
	Параметры.Свойство("ТребуетсяВыполнитьПроверкуДанных" , ТребуетсяВыполнитьПроверкуДанных);
	
	Если Параметры.Свойство("ПозицииТорговыхПредложений", ПараметрПозицииТорговыхПредложений)
		И ЗначениеЗаполнено(ПараметрПозицииТорговыхПредложений) Тогда
	...
	ДлительнаяОперацияОбновлениеДанных = ЗапуститьОбновлениеДанных(ПозицииТорговыхПредложений, ПрайсЛист, ТребуетсяВыполнитьПроверкуДанных, УникальныйИдентификатор);
...
&НаКлиенте
Процедура ДеревоОбъектовПриАктивизацииЯчейки(Элемент)
...
Процедура ДеревоОбъектовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ ТекущиеДанные.ОбработкаПричиныИсключения Тогда
		...
		Форма = ОткрытьФорму(ТекущиеДанные.ПолноеИмяМетаданных + ".ФормаОбъекта", ПараметрыОткрытияФормы, ЭтаФорма, , , , 
...
Процедура ПерезаполнитьДеревоНаСервере()
	...
	Если МножественныйРежим Тогда
		Для Каждого ЭлементМетаданных Из ДанныеДиагностики.ОбъектыМетаданных Цикл
			...
			Иначе
				НоваяСтрока.Представление = СтрШаблон("%1 (%2)", ЭлементМетаданных.Представление, НоваяСтрока.Строки.Количество());
			КонецЕсли;
	...
	ЕстьОшибки = ДанныеДиагностики.ТаблицаДиагностики.НайтиСтроки(Новый Структура("ЯвляетсяОшибкой", Истина)).Количество() > 0;
	ЕстьПредупреждения = ДанныеДиагностики.ТаблицаДиагностики.НайтиСтроки(Новый Структура("ЯвляетсяПредупреждением", Истина)).Количество() > 0;
