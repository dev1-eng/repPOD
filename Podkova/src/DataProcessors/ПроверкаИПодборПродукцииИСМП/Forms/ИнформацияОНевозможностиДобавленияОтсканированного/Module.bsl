...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДеревоОтсканированнойУпаковкиИндексКартинкиШтрихкод.КартинкаЗначений =
		ПроверкаИПодборПродукцииИС.КартинкаТипыУпаковкиПоВидуПродукции(Параметры.ВидПродукции);
...
Процедура СброситьРазмерыИПоложениеОкна()
	...
	Если ПравоДоступа("СохранениеДанныхПользователя", Метаданные) Тогда
		ХранилищеСистемныхНастроек.Удалить("Обработка.ПроверкаИПодборПродукцииИСМП.Форма.ИнформацияОНевозможностиДобавленияОтсканированного", "", ИмяПользователя);
	КонецЕсли;
...
Процедура ПоказатьИнформациюОПроблемахСМаркируемойПродукцией(ИнформацияВДереве)
	...
	Иначе
		...
		МаркируемаяПродукция = Параметры.ПредставлениеНоменклатуры;
	...
	ИначеЕсли ЗначениеЗаполнено(МаркируемаяПродукция) Тогда
		Префикс = СтрШаблон(ПрефиксМаркируемаяПродукцияИдентифицирована, МаркируемаяПродукция);
	Иначе
		Префикс = ПрефиксМаркируемаяПродукцияНеИдентифицирована;
	КонецЕсли;
	
	СоставСтроки = Новый Массив();
	СоставСтроки.Добавить(Префикс);
	...
	Элементы.ДекорацияОшибкаДобавленияМаркируемойПродукции.Заголовок =
		Новый ФорматированнаяСтрока(СоставСтроки);
