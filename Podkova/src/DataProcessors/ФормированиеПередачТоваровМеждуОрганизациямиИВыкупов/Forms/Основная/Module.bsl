...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ФормируемыеДокументы.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьПередачиТоваровМеждуОрганизациями")
		И ПолучитьФункциональнуюОпцию("ИспользоватьОтветственноеХранениеВПроцессеЗакупки");
...
Процедура ПериодАнализаПриИзменении(Элемент)
	Если Не ЗначениеЗаполнено(ПериодАнализа.ДатаНачала)
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"ПериодАнализа");
...
Процедура ДобавитьПоклажедателяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат)
		...
		НоваяСтрока = Объект.ВыбранныеОтправители.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Результат);
		НоваяСтрока.ТипЗапасов = ПредопределенноеЗначение("Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи");
		НоваяСтрока.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыкупПринятыхНаХранениеТоваров");
...
Процедура ПометитьНаУдалениеДокументы(Команда)
	ТекстВопроса = НСтр("ru = 'При возврате к настройкам созданные документы будут помечены на удаление. Продожить?'");
...
Функция ПроверитьОтправителей()
	...
	Если ТипЗапасов = Перечисления.ТипыЗапасов.Товар Тогда
		...
		ИначеЕсли ОформлятьПередачи(ФормируемыеДокументы) Тогда
			...
			Запрос.УстановитьПараметр("ОформлятьПередачи", ОформлятьПередачи(ФормируемыеДокументы));
			Запрос.УстановитьПараметр("ОформлятьВыкупы", Ложь);
			Запрос.УстановитьПараметр("ТипЗапасов",  ТипЗапасов);
			Запрос.Текст = Обработки.ФормированиеПередачТоваровМеждуОрганизациямиИВыкупов.ТекстЗапросаОтправителиПоНастройкамПередачи(Запрос);
			
			ТаблицаНастроекПередачи = Запрос.Выполнить().Выгрузить();
...
Процедура ИнициализироватьРежимИсправленияОстатков()
	...
	Для Каждого Источник Из Параметры.Источники Цикл
