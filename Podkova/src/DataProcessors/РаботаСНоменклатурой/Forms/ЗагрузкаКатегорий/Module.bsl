...
Процедура ПриОткрытии(Отказ)
	...
	Если ИнтернетПоддержкаПодключена Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ИдентификаторыКатегорийОграничения) Тогда
...
Процедура ОчиститьСтрокиПоискаКатегорий()
	...
	Если НЕ МожноПостроитьДеревоКатегорий(Идентификатор) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ТолькоКатегорииНациональногоКаталогаПриИзменении(Элемент)
	ОбработатьИзмнениеНастроекОтбораКатегорий();
КонецПроцедуры
...
Процедура ПроставитьПометкуВыбраннымКатегориям(ТекущаяВетка, МассивВыбранныхКатегорий, ПоследняяКатегорияИзСписка)
	...
	Для Каждого ЭлементКоллекции Из ДочерниеЭлементы Цикл
		...
		ИначеЕсли УстановленноДляВсех = Истина Тогда
			УстановленноДляВсех = (ЭлементКоллекции.Пометка = ПометкаЭталон);
		КонецЕсли;
	...
	Если ТипЗнч(ТекущаяВетка) = Тип("ДанныеФормыЭлементДерева") Тогда
		ТекущаяВетка.Пометка = ?(УстановленноДляВсех = Истина, ПометкаЭталон, 2);
	КонецЕсли;
...
Процедура ВычислениеКатегорийЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если СтатусЗадания <> Неопределено
		И СтатусЗадания <> "Выполнено" 
		И СтатусЗадания <> "Ошибка" Тогда
		Возврат;
	ИначеЕсли СтатусЗадания = "Выполнено" 
		И Результат.Свойство("АдресРезультата") Тогда 
		ОбработатьКатегорииТоварныхГрупп(
			Результат.АдресРезультата
			, ТекстВопроса
			, ИдентификаторыВыбранных
			, ДоступныеКатегории
			, ТекстНадписи
			, ЗаголовокВопроса
			, УникальныйИдентификатор
			, ТолькоКатегорииТоварнойГруппы);
		Если ДоступныеКатегории.Количество() Тогда
			ИдентификаторыКатегорийОграничения.ЗагрузитьЗначения(ДоступныеКатегории);
			Элементы.ТолькоКатегорииТоварнойГруппы.Видимость = Истина;
			Элементы.ТолькоКатегорииТоварнойГруппы.Заголовок = ТекстНадписи;
		КонецЕсли;
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ОбработатьКатегорииТоварныхГрупп(Знач АдресВременногоХранилища
	, ТекстВопроса
	, ИдентификаторыВыбранных
	, ДоступныеКатегории
	, КатегорииТоварнойГруппыЗаголовок
	, ЗаголовокВопроса
	, Знач УникальныйИдентификатор
	, ТолькоКатегорииТоварнойГруппы)
	
	КатегорииСервиса = РаботаСНоменклатурой.ДанныеВременногоХранилища(АдресВременногоХранилища);
	Если ТипЗнч(КатегорииСервиса) = Тип("Структура") Тогда
		...
		Если ДоступныеКатегории.Количество() Тогда
			...
			ИдентификаторыВыбранных          = ПересечениеМассивов(ИдентификаторыВыбранных, ДоступныеКатегории);
			КатегорииТоварнойГруппыЗаголовок = СтрШаблон(НСтр("ru = 'Показывать только категории по %1'"), КатегорииСервиса.Представление);
		КонецЕсли;
		Если КатегорииСервиса.ТоварныеГруппы.Количество() > 1 Тогда
			ЗаголовокВопроса = СтрШаблон(НСтр("ru = 'Рекомендуется выполнять сопоставление в рамках одной %1'"), КатегорииСервиса.ЗаголовокТоварныхГрупп);
...
&НаКлиенте
Процедура ОбработатьИзмнениеНастроекОтбораКатегорий()
...
Процедура ОбработатьОтветПоТоварнымГруппам(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТолькоКатегорииТоварнойГруппы Тогда
...
Процедура ВычислениеКэшаПутейЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	СформироватьДополнитьДеревоКатегорийНаСервере(Результат.АдресРезультата, ПараметрыЗаполненияДерева.ИдентификаторыПомеченных);
...
Функция ВосстановлениеДереваКатегорийИзКэшаНаСервере(ИдентификаторыПомеченных, РежимОтборПоСтроке, РежимТолькоВыбранные)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, ИмяМетода, 
		РаботаСНоменклатуройСлужебныйПовтИсп.КэшПутейКатегорий(), ИдентификаторыРодителей, Ложь);
...
&НаСервере
Процедура СформироватьДополнитьДеревоКатегорийНаСервере(АдресВременногоХранилища, ИдентификаторыКатегорий)
	...
	Если ТипЗнч(ПутиККорню) <> Тип("Соответствие") Тогда
		ПутиККорню = Новый Соответствие;
		Если НЕ УстановленОтборКатегорий Тогда
			РаботаСНоменклатуройСлужебныйКлиентСервер.УстановитьВидимостьОбластиКатегорий(Элементы, Ложь);
		КонецЕсли;
	КонецЕсли;
