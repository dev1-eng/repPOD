...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого ОбязательныйРеквизит Из КлючевыеРеквизиты Цикл
		...
		Если АльтернативныйРеквизит <> Неопределено Тогда
			...
			Продолжить;
		КонецЕсли;
		ОписаниеРеквизита = ОписаниеРеквизита(ФормализованныеРеквизиты, ИмяРеквизита, ПубликуемыеРеквизиты, ОбязательныйРеквизит.Значение);
	...
	Для каждого АльтернативныйРеквизит Из АльтернативныеРеквизиты Цикл
		...
		Пометка = (ВыгружаемыеРеквизиты = Неопределено ИЛИ НЕ ВыгружаемыеРеквизиты.Свойство("Реквизиты") ИЛИ ВыгружаемыеРеквизиты.Реквизиты.Найти(ИмяРеквизита) = Неопределено);
	...
	Для каждого ФормализованныйРеквизит Из ФормализованныеРеквизиты Цикл
		...
		Если КлючевыеРеквизиты.Свойство(ИмяРеквизита) Тогда
			Продолжить;
		КонецЕсли;
		ОписаниеРеквизита = ОписаниеРеквизита(ФормализованныеРеквизиты, ИмяРеквизита, ПубликуемыеРеквизиты, ФормализованныйРеквизит.Значение);
		Пометка = (ВыгружаемыеРеквизиты = Неопределено ИЛИ НЕ ВыгружаемыеРеквизиты.Свойство("Реквизиты") ИЛИ ВыгружаемыеРеквизиты.Реквизиты.Найти(ИмяРеквизита) = Неопределено);
		ОписаниеРеквизита.Вставить("Пометка", Пометка);
		ЗаполнитьЗначенияСвойств(ДеревоРеквизитов.Строки.Добавить(), ОписаниеРеквизита);
	КонецЦикла;
	...
	Для каждого ПубликуемыйРеквизит Из ПубликуемыеРеквизиты Цикл
		...
		ОписаниеРеквизита = Новый Структура("Свойство, Синоним", КлючРеквизита, ПубликуемыйРеквизит.Значение.Представление);
		Пометка = (ВыгружаемыеРеквизиты = Неопределено ИЛИ ВыгружаемыеРеквизиты.ДополнительныеРеквизиты.Найти(КлючРеквизита) = Неопределено);
		ОписаниеРеквизита.Вставить("Пометка", Пометка);
		ЗаполнитьЗначенияСвойств(ДеревоРеквизитов.Строки.Добавить(), ОписаниеРеквизита);
	КонецЦикла;
	...
	Если ИспользоватьДополнительныеРеквизиты ИЛИ ИспользоватьХарактеристикиНоменклатуры И ИспользоватьВидыНоменклатуры Тогда
		...
		Для каждого ПараметрДопРеквизита Из Новый Структура("Свойство, Синоним, ВидНоменклатуры, ИспользоватьХарактеристики") Цикл
			КолонкаДерева = ДеревоРеквизитов.Колонки[ПараметрДопРеквизита.Ключ];
			ДополнительныеРеквизиты.Колонки.Добавить(КолонкаДерева.Имя, КолонкаДерева.ТипЗначения);
		КонецЦикла;
		ДополнительныеРеквизиты.Колонки.Добавить("ВидНоменклатурыНаименование", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(200)));
		...
		Для каждого ВидНоменклатуры Из ВидыНоменклатуры.ВыгрузитьКолонку("ВидНоменклатуры") Цикл
			...
			Для каждого СтрокаВида Из СтрокиВида Цикл
				...
				Пометка = ВыгружаемыеРеквизиты = Неопределено ИЛИ ВыгружаемыеРеквизиты.ДополнительныеРеквизиты.Найти(ОписаниеДопРеквизита.Свойство) = Неопределено;
			...
			Если НЕ Родитель = Неопределено И СтрокиВида[0].ИспользоватьХарактеристики = Истина И ИспользоватьХарактеристикиНоменклатуры Тогда
				СтрокаХарактеристики = Новый Структура("Синоним, ВидНоменклатуры, ИспользоватьХарактеристики", "Характеристики");
				ЗаполнитьЗначенияСвойств(СтрокаХарактеристики, СтрокиВида[0],, "Синоним");
				Пометка = ВыгружаемыеРеквизиты = Неопределено ИЛИ ВыгружаемыеРеквизиты.ВидыНоменклатуры.Найти(СтрокаХарактеристики.ВидНоменклатуры) = Неопределено;
	...
	Если ИспользоватьХарактеристикиНоменклатуры = Истина Тогда
		Пометка = ?(ВыгружаемыеРеквизиты = Неопределено, Ложь, ВыгружаемыеРеквизиты.ВыгружатьИндивидуальныеХарактеристики);
		ЗаполнитьЗначенияСвойств(ДеревоРеквизитов.Строки.Вставить(ИндексВставки), Новый Структура("Синоним, ИспользоватьХарактеристики, Пометка", "Характеристики", Истина, Пометка));
	КонецЕсли;
...
Процедура СобратьВыгружаемыеРеквизитыИзДерева(ЭлементКоллекции, ВыгружаемыеРеквизиты)
	...
	Для Каждого ТекЭлемент Из ЭлементКоллекции.ПолучитьЭлементы() Цикл
		...
		Если ТекЭлемент.ИспользоватьХарактеристики = Истина Тогда
...
&НаСервереБезКонтекста
Процедура ЗаполнитьСлужебныеРеквизитыДерева(СтрокиДерева, ЗаполнитьПометки, УстановленноДляВсех = Истина)
	...
	Для каждого ВложеннаяСтрока Из СтрокиДерева.ПолучитьЭлементы() Цикл
		...
		Если УстановленноДляВсех = Истина И ПометкаДляВсех <> ВложеннаяСтрока.Пометка Тогда
			УстановленноДляВсех = Ложь;
		КонецЕсли;
