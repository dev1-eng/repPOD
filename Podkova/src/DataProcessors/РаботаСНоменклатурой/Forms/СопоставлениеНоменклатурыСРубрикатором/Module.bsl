...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СценарийИспользования <> "ВыгрузкаНоменклатуры" 
		И СценарийИспользования <> "УстранениеПроблемЗаполнения" Тогда
...
Процедура ПослеОбновленияПроблемЗаполнения(Результат, ДополнительныеПараметры) Экспорт
	...
	// обновить параметр динамического списка после завершения длительной операции
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Проблемы, "ДатаОткрытияФормы", ТекущаяДата());
...
&НаКлиенте
Процедура ПроверитьИзменитьОтборПоСсылке(ДинамическийСипсок, НовыйОбъект)
	
	Если ДинамическийСипсок.КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы.Количество() Тогда
		ПолеСсылка = Новый ПолеКомпоновкиДанных("Ссылка");
		Для каждого ЭлементОтбора Из ДинамическийСипсок.КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы Цикл
