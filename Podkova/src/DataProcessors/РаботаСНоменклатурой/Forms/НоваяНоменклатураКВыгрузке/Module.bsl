...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИмяТаблицыОрганизация = "" Тогда
		...
		НСтр("ru = 'Не удалось определить имя таблицы определяемого типа ""Организация"". Форма не может быть открыта.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	|				СостоянияВыгрузкиНоменклатуры.Организация = СправочникОрганизации.Ссылка
	|				И СостоянияВыгрузкиНоменклатуры.ХарактеристикаНоменклатуры = &ПустаяХарактеристика
	|				И СостоянияВыгрузкиНоменклатуры.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияВыгрузкиНоменклатуры.ОжидаетИсправления))";
	...
	Если Организации.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Новая номенклатура не найдена, форма не может быть открыта'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	Для каждого ОписаниеОрганизации Из Организации Цикл
		Элементы.Организация.СписокВыбора.Добавить(ОписаниеОрганизации.Организация, ОписаниеОрганизации.Представление,,
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокНовая, "ПустаяХарактеристика", ПустаяХарактеристика);
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(СписокОжидаетВыгрузки, "ПустаяХарактеристика", ПустаяХарактеристика);
...
Процедура УбратьИзВыгрузки(Команда)
	...
	НоменклатураДляУдаления = НоменклатураДляОбработки(Элементы.СписокНовая.ВыделенныеСтроки);
...
Процедура ОбновитьЗаголовкиЗакладок(ОбновитьВыгружается = Истина, ОбновитьИсключения = Истина)
	...
	Если ОбновитьВыгружается = Истина Тогда
		...
		ИначеЕсли КоличествоВыгружается > 0 Тогда
			ВыгружаетсяПредставление = СтрШаблон(НСтр("ru = ' (%1)'"), КоличествоВыгружается);
		КонецЕсли;
	...
	Если ОбновитьИсключения = Истина Тогда
		...
		ИначеЕсли КоличествоИсключения > 0 Тогда
			ИсключенияПредставление = СтрШаблон(НСтр("ru = ' (%1)'"), КоличествоИсключения);
		КонецЕсли;
...
Процедура ОбработатьВыделенные(Добавление = Истина)
	...
	Иначе 
		...
		Иначе 
			...
			Состояние = ПредопределенноеЗначение("Перечисление.СостоянияВыгрузкиНоменклатуры.Новая");
		КонецЕсли;
	КонецЕсли;
	
	ВыделенныеСтроки = НоменклатураДляОбработки(СписокИсточник.ВыделенныеСтроки);
	...
	Если Состояние = ПредопределенноеЗначение("Перечисление.СостоянияВыгрузкиНоменклатуры.Новая") Тогда
		ДлительнаяОперация = РаботаСНоменклатуройСлужебныйВызовСервера.ПроверитьГотовностьНоменклатурнойПозиции(УникальныйИдентификатор,
		Организация, ВыделенныеСтроки);
...
Функция ВыгрузитьНоменклатуру(Знач УникальныйИдентификатор, Знач Организация, Знач ДатаОткрытияФормы)
	...
	НаименованиеЗадания = НСтр("ru = 'Работа с номенклатурой. Ввыгрузка номенклатуры'");
...
&НаКлиенте
Функция НоменклатураДляОбработки(СтрокиСписка)

	Если ТипЗнч(СтрокиСписка) = ТипЗнч(ПустаяНоменклатура) Тогда
		Возврат ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтрокиСписка);
	КонецЕсли;
	
	НоменклатураДляОбработки = Новый Массив;
	Если ТипЗнч(СтрокиСписка) = Тип("Массив") Тогда
		Для каждого СтрокаСписка Из СтрокиСписка Цикл
			Если ТипЗнч(СтрокаСписка) = ТипЗнч(ПустаяНоменклатура) Тогда
				НоменклатураДляОбработки.Добавить(СтрокаСписка);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Возврат НоменклатураДляОбработки;
