...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОписаниеРаздела.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(ОписаниеШаблон, ГиперссылкаНаПромоСайтНоменклатура);
...
Процедура ЗапуститьОбновлениеОКПД2(РазделениеВключено, Отказ)
	...
	Если ЭлементыКлассификаторы.Количество() = 0 Тогда
		Сообщить(НСтр("ru = 'Ошибка обновления: в базе отсутствуют элементы классификатора.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;	
