...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		ОбновитьСписок(Элементы.ОбновитьСписок);
	КонецЕсли;
...
Процедура ЗакрытьЗаказы(Команда)
	...
	Если РезультатЗакрытия.ТребуетсяОбновлениеКлючаСессии Тогда
		ЗапроситьКлючСессииНачало("ЗакрытиеЗаказов");
	ИначеЕсли ЗначениеЗаполнено(РезультатЗакрытия.ТекстОшибки) Тогда
...
Процедура ЗаполнитьСписокОрганизаций()
	...
	Если Элементы.Организация.СписокВыбора.Количество() = 1 Тогда
		ЭтотОбъект.Организация = Элементы.Организация.СписокВыбора[0].Значение;
...
Процедура ЗапроситьКлючСессииНачало(ВыполняемаяОперация, Назначение = Неопределено)
	...
	Иначе
		...
		ПараметрыЗапроса = ИнтерфейсИСМПКлиентСервер.ПараметрыЗапросаКлючаСессииСУЗ(
...
Процедура ЗапроситьКлючСессииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ИначеЕсли ДополнительныеПараметры.ВыполняемаяОперация = "ЗакрытиеЗаказов" Тогда
			Результат = ЗакрытьЗаказыВСУЗ(ДополнительныеПараметры.ВызовПослеАвторизацииСУЗ);
...
Функция ЗагрузитьЗаказыИзСУЗ(ВызовПослеАвторизацииСУЗ = Ложь, ВызовПослеАвторизацииИСМП = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Строка Из ЗаказыСУЗ Цикл
			...
			Если Статус = Перечисления.СтатусыОбработкиЗаказовНаЭмиссиюКодовМаркировкиИСМП.Выполнен	Тогда
				Продолжить;
			КонецЕсли;
			...
			СтрокаСпискаЗаказов.ДатаЗаказа    = ИнтеграцияИС.ДатаИзСтрокиUNIX(Строка.createdTimestamp);
...
Функция ПолучитьДанныеИзИСМППоGTIN(МассивGTIN, Данные)
	...
	Результат = ИнтерфейсИСМП.ДанныеПродукцииПоШтрихкодуEAN(МассивGTIN);
...
&НаСервере
Функция ЗакрытьЗаказыВСУЗ(ВызовПослеАвторизации = Ложь)
	...
	ТаблицаОшибок.Колонки.Добавить("Индекс",      ОбщегоНазначения.ОписаниеТипаЧисло(6));
	ТаблицаОшибок.Колонки.Добавить("ТекстОшибки", ОбщегоНазначения.ОписаниеТипаСтрока(0));
	...
	Для Каждого СтрокаСпискаЗаказов Из ВыбранныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаСпискаЗаказов.Документ) Тогда
			...
			ДокументыКАрхивированию.Добавить(СтрокаСпискаЗаказов.Документ);
			
			Результат = РегистрыСведений.СтатусыДокументовИСМП.Архивировать(ДокументыКАрхивированию);
			
			Если Результат[0].ТекстОшибки <> "" Тогда
				
				Ошибка = ТаблицаОшибок.Добавить();
				Ошибка.Индекс      = СписокЗаказов.Индекс(СтрокаСпискаЗаказов);
				Ошибка.ТекстОшибки = Результат[0].ТекстОшибки;
		...
		Если Не Отказ Тогда
			...
			ПараметрыЗакрытияЗаказа.Вставить("ИдентификаторБизнесЗаказа", СтрокаСпискаЗаказов.Идентификатор);
			
			Результат = ИнтерфейсСУЗ.ЗакрытьПодзаказПоGTIN_V2(
				ПараметрыЗакрытияЗаказа, СтрокаСпискаЗаказов.ВидПродукции, НастройкаОбменаСУЗ);
			...
			Иначе
				Ошибка = ТаблицаОшибок.Добавить();
				Ошибка.Индекс      = СписокЗаказов.Индекс(СтрокаСпискаЗаказов);
				Ошибка.ТекстОшибки = Результат.ТекстОшибки;
			КонецЕсли;
	...
	Иначе
		АдресХранилищаЗакрытыеЗаказы = ПоместитьВоВременноеХранилище(ЗакрытыеЗаказы, ЭтотОбъект.УникальныйИдентификатор);
	КонецЕсли;
