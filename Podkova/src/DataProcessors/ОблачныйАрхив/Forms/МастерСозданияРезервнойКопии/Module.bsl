...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ru='Облачный архив. Сервис и регламент. Резервное копирование. Инициализация'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
Функция КоличествоСеансовИнформационнойБазы()
	...
	Возврат СоединенияИБ.КоличествоСеансовИнформационнойБазы(Ложь, Ложь); // АПК:278 Определение количества сеансов.
...
Процедура НачатьРезервноеКопирование()
	...
	ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИмяСобытия.
		НСтр("ru='Облачный архив. Сервис и регламент. Резервное копирование. Завершение'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИдентификаторШага.
	...
	СоединенияИБКлиент.ЗавершитьРаботуЭтогоСеанса(Ложь); // ВыводитьВопрос = Ложь. // АПК:278 Завершение работы системы для выполнения резервного копирования..
...
Процедура УстановитьБлокировкуСеансовНа2Минуты()
	...
	Блокировка.Сообщение = СоединенияИБ.СформироватьСообщениеБлокировки(
		...
		КодРазрешения); // АПК:277 Вызов метода родственной подсистемы.
...
#Область ПовторнаяИнициализацияАгентаРезервногоКопирования
	КодРезультата = 0;
	ОписаниеРезультата = "";
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхив.СозданиеФайлаРезервнойКопии.ЗапускРезервирования.ИзменениеНастроек.ПовторнаяИнициализацияАгентаРезервногоКопирования",
		НСтр("ru='Запуск резервного копирования, изменение настроек для Агента резервного копирования, повторная инициализация агента резервного копирования'"));
	...
	ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Команда повторной инициализации: %1
			|Код возврата: %2'"),
		СтрокаКоманды,
		КодВозврата);
