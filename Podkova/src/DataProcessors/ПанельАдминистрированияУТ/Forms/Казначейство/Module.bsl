...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПорядокЗачетаДокументов              = НаборКонстант.ПорядокЗачетаДокументовПоДатеОперации;
	...
	НастройкиСистемыЛокализация.ПриСозданииНаСервере_Казначейство(ЭтаФорма);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	НастройкиСистемыЛокализация.ПриЧтенииНаСервере_Казначейство(ЭтаФорма);
...
Процедура НоваяАрхитектураРасчетовПриИзменении(Элемент)
	...
	Если РежимРаспределенияВзаиморасчетов И НЕ НаборКонстант.НоваяАрхитектураВзаиморасчетов Тогда
		Ответ = Неопределено;
		ТекстВопроса = НСтр("ru='Включение новой архитектуры может занять продолжительное время, будет заблокировано проведение документов, участвующих в расчетах. Продолжить?'");
	...
	ИначеЕсли НЕ РежимРаспределенияВзаиморасчетов И НаборКонстант.НоваяАрхитектураВзаиморасчетов Тогда
		Ответ = Неопределено;
		ПервыйПериодЗаданий = РаспределениеВзаиморасчетовВызовСервера.НачалоРасчетов(ОбщегоНазначенияКлиент.ДатаСеанса());
		Если ЗначениеЗаполнено(ПервыйПериодЗаданий) Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='После отключения новой архитектуры будет необходимо выполнить распределение взаиморасчетов по документам,
					|и по необходимости выполнить закрытие месяца (с %1 г.). Продолжить?'"),
				Формат(ПервыйПериодЗаданий,"ДФ='MMMM гггг'"));
			ПоказатьВопрос(Новый ОписаниеОповещения("НоваяАрхитектураРасчетовОтключениеЗавершение", ЭтотОбъект), ТекстВопроса,РежимДиалогаВопрос.ДаНет);
		Иначе
			НоваяАрхитектураРасчетовОтключениеЗавершение(КодВозвратаДиалога.Да, Неопределено);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура НоваяАрхитектураРасчетовОтключениеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		
		Результат = ОчиститьРегистрыВзаиморасчетов();
		...
		Иначе
			...
			УдалитьЗаданияКРасчетуДвиженийВзаиморасчетов();
			НаборКонстант.НоваяАрхитектураВзаиморасчетов = Ложь;
			СохранитьЗначениеРеквизита("НаборКонстант.НоваяАрхитектураВзаиморасчетов");
			
			НаборКонстант.ПорядокЗачетаДокументовПоДатеОперации = 0;
			СохранитьЗначениеРеквизита("НаборКонстант.ПорядокЗачетаДокументовПоДатеОперации");
			ПорядокЗачетаДокументов = НаборКонстант.ПорядокЗачетаДокументовПоДатеОперации;
	...
	Иначе
		РежимРаспределенияВзаиморасчетов = Истина;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПорядокЗачетаДокументовПриИзменении(Элемент)
	НаборКонстант.ПорядокЗачетаДокументовПоДатеОперации = ПорядокЗачетаДокументов;
	СохранитьЗначениеРеквизита("НаборКонстант.ПорядокЗачетаДокументовПоДатеОперации");
КонецПроцедуры
...
Процедура Подключаемый_ПриИзмененииРеквизита(Элемент, ОбновлятьИнтерфейс = Истина, ВнешнееИзменение = Ложь)
	...
	Если НЕ ВнешнееИзменение Тогда
		НастройкиСистемыЛокализацияКлиент.ПриИзмененииРеквизита_Казначейство(
			Элемент,
			ЭтаФорма);
	КонецЕсли;
...
Процедура Подключаемый_ОбработкаНавигационнойСсылкиФормы(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	НастройкиСистемыЛокализацияКлиент.ОбработкаНавигационнойСсылкиФормы_Казначейство(Элемент,
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.НоваяАрхитектураВзаиморасчетов"
		...
		Элементы.АктуализироватьВзаиморасчетыВДокументах.Доступность = НЕ НаборКонстант.НоваяАрхитектураВзаиморасчетов;
		Элементы.АктуализироватьВзаиморасчетыВОтчетах.Доступность = НЕ НаборКонстант.НоваяАрхитектураВзаиморасчетов;
		ДоступностьВлюченияНовойАрхитектуры = ОбновлениеИнформационнойБазы.ОбъектОбработан("РасчетыСКлиентами").Обработан И 
			ОбновлениеИнформационнойБазы.ОбъектОбработан("РасчетыСПоставщиками").Обработан;
		Элементы.НоваяАрхитектураВзаиморасчетов.Доступность = ДоступностьВлюченияНовойАрхитектуры;
	КонецЕсли;
	Элементы.ПорядокЗачетаДокументов.Доступность = РежимРаспределенияВзаиморасчетов;
	Если РеквизитПутьКДанным = "РежимПечатиАвансовогоОтчета"
		Или РеквизитПутьКДанным = "" Тогда
	...
	НастройкиСистемыЛокализация.УстановитьДоступность_Казначейство(РеквизитПутьКДанным, ЭтаФорма);
...
&НаСервере
Функция ОчиститьРегистрыВзаиморасчетов()
	
	НаименованиеЗадания = НСтр("ru = 'Очистка регистров при отключении новой архитектуры расчетов'");
	
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		УникальныйИдентификатор,
		"ОперативныеВзаиморасчетыСервер.ОчиститьРегистрыПриВыключенииНовойАрхитектуры",
		,
		НаименованиеЗадания);
...
&НаСервере
Процедура УдалитьЗаданияКРасчетуДвиженийВзаиморасчетов()
	
	НаборЗаписей = РегистрыСведений.ЗаданияКЗакрытиюМесяца.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Операция.Установить(Перечисления.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоРасчетамСПартнерамиИПереоценкаРасчетов);
