...
Процедура ВывестиСообщениеСформированыДокументы(ИмяДокумента, ТипОперации = "", Обработаны = Ложь)
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%2%", ТипОперации);
	КонецЕсли;
...
Функция ДокументыВводаОстатковПодлежащиеОбработке(Действие)
	...
	Запрос.УстановитьПараметр("Комментарий_СкорректированСверткойБазы", "%"+НСтр("ru = 'Скорректирован обработкой свертки базы'")+"%");
	...
	|ГДЕ (Док.ДокументОснование ССЫЛКА Документ.ВводОстатков ИЛИ Док.ДокументОснование ССЫЛКА Документ.ПервичныйДокумент)
...
Процедура СформироватьДокументыВводаОстатков(ПовторныйЗапуск = Ложь) Экспорт
	...
	Если РасчетСебестоимостиПовтИсп.ПартионныйУчетВерсии22()
		...
		Пока ВыборкаОрганизации.Следующий() Цикл
			ПараметрыУчетнойПолитики = ЗначениеНастроекПовтИсп.ПараметрыУчетнойПолитики(ВыборкаОрганизации.Ссылка, КонецДня(ДатаСверткиИБ)); 
			
		    Если ПараметрыУчетнойПолитики.РаздельныйУчетТоваров 
...
Процедура СформироватьДокументыВводаОстатковРасчеты(ВидРасчетов)
	...
	Если ВидРасчетов = "РасчетыСКлиентами" Тогда
			...
			|	ЛЕВОЕ СОЕДИНЕНИЕ
			| 		РегистрНакопления.РасчетыСКлиентами.Обороты(,&ГраницаОст,День,) КАК ТабРегОбороты
		...
		ИначеЕсли ВидРасчетов = "РасчетыСПоставщиками" Тогда
			...
			|	ЛЕВОЕ СОЕДИНЕНИЕ 
			| 		РегистрНакопления.РасчетыСПоставщиками.Обороты(,&ГраницаОст,День) КАК ТабРегОбороты
			|		ПО 	ТабРег.АналитикаУчетаПоПартнерам = ТабРегОбороты.АналитикаУчетаПоПартнерам  И
		...
		КоличествоСформированныхДокументов = 1;
		// Для нумерации СФПолученных на аванс
		НомерСФПоПорядку = 1;
		Выборка = Результат.Выбрать();
			...
			Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
				Пока Выборка.СледующийПоЗначениюПоля("Партнер") Цикл
					...
					Пока Выборка.СледующийПоЗначениюПоля("ОбъектРасчетов") Цикл
						...
						КонецЕсли;
						ИзменилисьДанныеДляСФАванс = Ложь;
						Если ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам 
							Или ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамКлиентов Тогда
							Если ЗначениеЗаполнено(ДатаПлатежа) И ДатаПлатежа <> Выборка.ДатаПлатежа Тогда
								ИзменилисьДанныеДляСФАванс = Истина;
							ИначеЕсли ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамКлиентов
								И (ЗначениеЗаполнено(ДатаРасчетногоДокумента) И Выборка.ДатаРасчетногоДокумента <> ДатаРасчетногоДокумента
									Или ЗначениеЗаполнено(НомерРасчетногоДокумента) И Выборка.НомерРасчетногоДокумента <> НомерРасчетногоДокумента) Тогда
								ИзменилисьДанныеДляСФАванс = Истина;
							КонецЕсли;
							Если ИзменилисьДанныеДляСФАванс Тогда
								// СФ на аванс можно ввести только одну на основании одного документа ввода остатков.
								ДокОст.Записать(РежимЗаписиДокумента.Запись);
								СформироватьСчетФактуруНаАванс(ДокОст, Сумма, ДатаПлатежа, ДатаРасчетногоДокумента, НомерРасчетногоДокумента, НомерСФПоПорядку, Контрагент);
								КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
...
Процедура СформироватьСчетФактуруНаАванс(ДокОст, Сумма, ДатаПлатежа, ДатаРасчетногоДокумента, НомерРасчетногоДокумента, НомерСФПоПорядку, Контрагент)
	Если ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам 
		Или ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамКлиентов Тогда
		ЗапросАванс = Новый Запрос;
		ЗапросАванс.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		|НДСОборот КАК НДСОборот,
		|СтавкаНДС КАК СтавкаНДС
		|ИЗ РегистрНакопления.%ИмяРегистра%.Обороты(&НачалоПериода,&ДатаОст,,ВидЦенности = &ВидЦенности И %ИмяКонтрагент% = &Контрагент) КАК НДС
		|УПОРЯДОЧИТЬ ПО ДатаОплаты УБЫВ";
		ЗапросАванс.УстановитьПараметр("ДатаОст", КонецДня(ДатаСверткиИБ));
		Если НЕ ЗначениеЗаполнено(ДатаРасчетногоДокумента) ИЛИ КонецДня(ДатаСверткиИБ) < ДатаРасчетногоДокумента Тогда
			ЗапросАванс.УстановитьПараметр("НачалоПериода", НачалоГода(ДатаСверткиИБ));
		Иначе
			ЗапросАванс.УстановитьПараметр("НачалоПериода", НачалоГода(ДатаРасчетногоДокумента));
		КонецЕсли;
		ЗапросАванс.УстановитьПараметр("Контрагент", Контрагент);
		Если ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам  Тогда
			ЗапросАванс.Текст = СтрЗаменить(ЗапросАванс.Текст, "%ИмяРегистра%", "НДСЗаписиКнигиПокупок");
			ЗапросАванс.Текст = СтрЗаменить(ЗапросАванс.Текст, "%ИмяКонтрагент%", "Поставщик");
			ЗапросАванс.УстановитьПараметр("ВидЦенности", Перечисления.ВидыЦенностей.АвансыВыданные);
		Иначе
			ЗапросАванс.Текст = СтрЗаменить(ЗапросАванс.Текст, "%ИмяРегистра%", "НДСЗаписиКнигиПродаж"); 
			ЗапросАванс.Текст = СтрЗаменить(ЗапросАванс.Текст, "%ИмяКонтрагент%", "Покупатель");
			ЗапросАванс.УстановитьПараметр("ВидЦенности", Перечисления.ВидыЦенностей.АвансыПолученные);
		КонецЕсли;
		РезультатЗапроса = ЗапросАванс.Выполнить();
		Если РезультатЗапроса.Пустой() Тогда
			// Не формировалась СФ на аванс.
			Возврат;
		КонецЕсли;
		ВыборкаСуммы = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		ВыборкаСуммы.Следующий();

		Если ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам Тогда
			СФАванс = Документы.СчетФактураПолученныйАванс.СоздатьДокумент();
			СФАванс.ДатаСоставления = ?(ЗначениеЗаполнено(ДатаПлатежа), ДатаПлатежа, ДатаСверткиИБ);
		...
		Иначе
			...
			СФАванс.ДатаПлатежноРасчетногоДокумента = ДатаРасчетногоДокумента;
			СФАванс.НомерПлатежноРасчетногоДокумента = НомерРасчетногоДокумента;
			СФАванс.ДатаВыставления = ДатаПлатежа;
		...
		КонецЕсли;
		СтавкаНДСЧислом = 20/120;
		СтрокаСФ.СтавкаНДС = Перечисления.СтавкиНДС.НДС20_120;

		Если ЗначениеЗаполнено(ВыборкаСуммы.СтавкаНДС) Тогда
			Если ВыборкаСуммы.СтавкаНДС = Перечисления.СтавкиНДС.НДС18 Или
				ВыборкаСуммы.СтавкаНДС = Перечисления.СтавкиНДС.НДС18_118  Тогда
				СтавкаНДСЧислом = 18/118;
				СтрокаСФ.СтавкаНДС = Перечисления.СтавкиНДС.НДС18_118;

			ИначеЕсли ВыборкаСуммы.СтавкаНДС = Перечисления.СтавкиНДС.НДС10_110 Или
				ВыборкаСуммы.СтавкаНДС = Перечисления.СтавкиНДС.НДС10  Тогда
				СтавкаНДСЧислом = 10/110;
				СтрокаСФ.СтавкаНДС = Перечисления.СтавкиНДС.НДС10_110;
			КонецЕсли;
		КонецЕсли;

		СтрокаСФ.Сумма = Сумма;
		СтрокаСФ.СуммаНДС = Окр(Сумма * СтавкаНДСЧислом,2);
		СФАванс.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС;
		СФАванс.КодВидаОперации = "02";
		СФАванс.Сумма = СтрокаСФ.Сумма;
		СФАванс.СуммаНДС = СтрокаСФ.СуммаНДС;
		СФАванс.Дата = ДатаСверткиИБ;
		СФАванс.ДокументОснование = ДокОст.Ссылка;
		СФАванс.Организация = ДокОст.Организация;
		СФАванс.Комментарий = Комментарий_СформированСверткойБазы;
		СФАванс.Контрагент = Контрагент;
					...
					Пока Выборка.СледующийПоЗначениюПоля("НалогообложениеНДС") Цикл
						Пока Выборка.СледующийПоЗначениюПоля("ДокументПартии") Цикл
							...
							Если ЗначениеЗаполнено(Выборка.ДокументПартии) Тогда
								...
								ДокОст.Партия = ПреобразоватьСсылкуВПервичныйДокумент(Выборка.ДокументПартии, ДанныеПервичногоДокумента.Дата, ДанныеПервичногоДокумента.Номер);
							КонецЕсли;
				...
				Пока Выборка.СледующийПоЗначениюПоля("НалогообложениеНДС") Цикл
					Пока Выборка.СледующийПоЗначениюПоля("ДокументПартии") Цикл
						...
						Пока Выборка.Следующий() Цикл
							...
							ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
							НоваяСтрока.Количество = КоличествоПоСтроке;
							Если НоваяСтрока.Сумма = 0 И Выборка.СуммаПартии > 0 
...
Процедура ОбработатьСостоянияБлокировкиВычетаНДС()
	...
	|	СчетФактура.Дата КАК СчетФактураДата,
	|	СчетФактура.Номер КАК СчетФактураНомер,
	...
	Пока ВыборкаОрганизация.Следующий() Цикл
		...
		Пока ВыборкаСостояние.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				КонецЕсли;
				ПервичныйДокумент = ПреобразоватьСсылкуВПервичныйДокумент(Выборка.СчетФактура, 
									Выборка.СчетФактураДата, Выборка.СчетФактураНомер, ТипПервичногоДокумента,,,Истина);
...
Функция ПреобразоватьСсылкуВПервичныйДокумент(ДокументСсылка, ДокументДата, ДокументНомер, ТипПервичногоДокумента = Неопределено, Сумма = 0, СуммаРегл = 0, ТолькоПоиск = Ложь)
	...
	// Поиск и переназначения счета-фактуры выданного и полученного.
	...
	Если ТипПервичногоДокумента = Перечисления.ТипыПервичныхДокументов.ПриобретениеУПоставщика		
		...
		|ГДЕ СФ.ДокументОснование = &ДокСсылка
		|ОБЪЕДИНИТЬ ВСЕ
		|ВЫБРАТЬ РАЗЛИЧНЫЕ 
		|	СФ.Ссылка КАК Ссылка,
		|	ВЫБОР КОГДА НДСПродажи.Регистратор IS NULL ТОГДА
...
Функция СоздатьПервичныйДокументДляСтрокиРасчетов(ДокОст, ОбъектРасчетов, СтрокаРасчетов)
	...
	КонецЕсли;
	Если ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиКлиентов Тогда
		ТипПервичногоДокумента = Перечисления.ТипыПервичныхДокументов.РеализацияКлиенту;
	ИначеЕсли ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиПередПоставщиками Тогда
		ТипПервичногоДокумента = Перечисления.ТипыПервичныхДокументов.ПриобретениеУПоставщика;
	ИначеЕсли ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамКлиентов Тогда
		ТипПервичногоДокумента = Перечисления.ТипыПервичныхДокументов.ОплатаОтКлиента;
	ИначеЕсли ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам Тогда
	...
	Если ЗначениеЗаполнено(ОбъектРасчетов) И 
		НЕ (ТипЗнч(ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
		...
		Если СтруктураРеквизитовШапки.Свойство("Дата") И СтруктураРеквизитовШапки.Свойство("Номер") Тогда
			...
			Если ЗначениеЗаполнено(ПДСсылка) Тогда
				// Для авансов объекты расчетов типа "Заказ..." не меняем.
				Если НЕ (ТипЗнч(ОбъектРасчетов) = Тип("ДокументСсылка.ЗаказКлиента")
	...
	ПервичныйДок.ТипПервичногоДокумента = ТипПервичногоДокумента;
	ПервичныйДок.Дата = ДатаСверткиИБ;
	ЗаполнитьЗначенияСвойств(ПервичныйДок, ДокОст, "Контрагент, Организация, Партнер");
	Если ЗначениеЗаполнено(ДокОст.Контрагент) И НЕ ЗначениеЗаполнено(ДокОст.Партнер) Тогда
		ДокОст.Партнер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокОст.Контрагент, "Партнер");
	КонецЕсли;
	ЗаполнитьЗначенияСвойств(ПервичныйДок, СтрокаРасчетов, "Договор, СуммаРегл");
...
Процедура ЗаполнитьШапкуДокументаВводаОстатков(ДокОст, ПорядковыйНомерДокумента, Выборка, ЗаполнятьФлагиОтраженияВУчетах = Истина)
	...
	Попытка
		ДокОст.Менеджер = ДокОст.Ответственный;
	Исключение
	КонецПопытки; 
					...
					Пока Выборка.СледующийПоЗначениюПоля("ДокументПартии") Цикл
						Пока Выборка.СледующийПоЗначениюПоля("СчетФактура") Цикл
							...
							Если ЗначениеЗаполнено(ДокументПоступленияСсылка) Тогда
								...
								ДокОст.Партия = ПреобразоватьСсылкуВПервичныйДокумент(ДокументПоступленияСсылка, ДанныеПервичногоДокумента.Дата, ДанныеПервичногоДокумента.Номер);
							КонецЕсли;
								...
								Если ТаблицаОстатковПоГТД <> Неопределено И ТаблицаОстатковПоГТД.Количество() > 0 Тогда
									Для Каждого СтрТовары Из ДокОст.Товары Цикл
										...
										ИначеЕсли ЗначениеЗаполнено(СтрТовары.НомерГТД) Тогда
											...
											Продолжить;
										КонецЕсли;
										СтруктураПоиска = Новый Структура("Организация, Склад, Номенклатура, Характеристика",
												ДокОст.Организация, ДокОст.Склад, СтрТовары.Номенклатура, СтрТовары.Характеристика);
...
Процедура ОбработатьДвиженияНДСДоДатыСвертки()
	...
	Для Каждого РегистрКОбработке Из РегистрыКОбработке Цикл
		...
		Запрос.Текст = Запрос.Текст + "ИТОГИ ПО Регистратор";
		ЗаполнитьПараметрыЗапросаДатаСвертки(Запрос);
		...
		Пока ВыборкаРегистратор.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ПервичныйДокумент = Неопределено Тогда
					ПервичныйДокумент = ПреобразоватьСсылкуВПервичныйДокумент(Выборка.ДокПартии, 
									Выборка.ДокПартииДата, Выборка.ДокПартииНомер, , , , Истина);
