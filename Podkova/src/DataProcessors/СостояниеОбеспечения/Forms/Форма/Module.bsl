...
Функция СостояниеОбеспеченияЗаказов()
	...
	СторнируемыеЗаказы = Новый Массив();
	Если Параметры.ВызовИзФормыЗаказа Тогда
		...
		СторнируемыеЗаказы.Добавить(Документ.Ссылка);
		Если Документ.СторнируемыеДокументы.Количество() > 0 Тогда
			СторнируемыеЗаказы = Документ.СторнируемыеДокументы;
		КонецЕсли;
	...
	Состояние.Вставить("ГрафикПоступления", ТаблицаКорректировкиГрафикаПоступления(МенеджерВременныхТаблиц, СторнируемыеЗаказы));
...
Функция ТекстЗапросаЗаказыНоменклатуры(ДоступныеДокументы)
	...
	Если ДоступныеДокументы.Найти(Документы.ЗаказКлиента) <> Неопределено Или
		...
		ТекстЗапроса = ТекстЗапроса + РегистрыНакопления.ЗаказыКлиентов.ТекстЗапросаЗаказовНоменклатуры(
			ПолучатьНесогласованные,
			ДоступныеДокументы);
		ПервыйЗапрос = Ложь;
...
Функция ТекстЗапросаЗаказы(ДоступныеДокументы, ЕстьФильтр)
	...
	|	Заказы.НомерСтрокиПродукция КАК НомерСтрокиПродукция,
	|	Заказы.Продукция            КАК Продукция,
	|	Заказы.Этап                 КАК Этап,
	|	Заказы.ЭтапПорядок          КАК ЭтапПорядок,
	|	
	...
	|	
	|	Заказы.ВариантОбеспечения   КАК ВариантОбеспечения,
	|	Заказы.ДатаОтгрузки         КАК ДатаОтгрузки,
	|	Заказы.Упаковка             КАК Упаковка,
	|	Заказы.Количество           КАК Количество,
	|	
	|	Заказы.ЖелаемаяДатаОтгрузки КАК ЖелаемаяДатаОтгрузки,
	|	Заказы.НеОтгружатьЧастями   КАК НеОтгружатьЧастями,
	|	Заказы.Дата                 КАК Дата,
	|	Заказы.Партнер              КАК Партнер,
	|	Заказы.Статус               КАК Статус,
	|	Заказы.СтатусИспользуется   КАК СтатусИспользуется
	|ИЗ
	|	&ТекстНабораЗапросов КАК Заказы
	|
	|{ГДЕ
	|	Заказы.Заказ.*    КАК Заказ,
	|	Заказы.Склад.*    КАК Склад,
	...
	ТекстыЗаказов = Новый Массив;
	Для Каждого МенеджерОбъекта Из ДоступныеДокументы Цикл
		ТекстыЗаказов.Добавить(МенеджерОбъекта.ТекстЗапросаЗаказовКОбеспечению());
	КонецЦикла;
	
	ТекстНабораЗапросов = СтрСоединить(ТекстыЗаказов, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
...
Функция ЗаполнитьДатыОбеспеченияПотребностей(ОстаткиИОбороты, ГрафикПоступления, Итоги)
	...
	Для Каждого Заказ Из Товары.ПолучитьЭлементы() Цикл
		...
		Для Каждого Потребность Из Заказ.ПолучитьЭлементы() Цикл
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(ПараметрыОтбораОбособленные, Потребность);
				ПараметрыОтбораОбособленные.Назначение = ПустоеНазначение;
				
				ГрафикПоступленияТовара = ГрафикПоступления.НайтиСтроки(ПараметрыОтбораОбособленные);
	...
	ЗаполнитьДатыОбеспеченияОбособленныхПотребностей(
		ТаблицаОбособленныхПотребностей, СписокКолонокНовый, ПолностьюВНаличии, ОстаткиИОбороты, ГрафикПоступления);
...
&НаСервереБезКонтекста
Функция ТаблицаКорректировкиГрафикаПоступления(МенеджерВременныхТаблиц, СторнируемыеЗаказы)
	УстановитьПривилегированныйРежим(Истина);
	ЗапросГрафикаПоступлений = Новый Запрос();
	ЗапросГрафикаПоступлений.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	ЗапросГрафикаПоступлений.УстановитьПараметр("Ссылка", СторнируемыеЗаказы);
	ЗапросГрафикаПоступлений.Текст =
		...
		|		 И ГрафикПоступления.Назначение
		|			= ВЫБОР КОГДА Заказы.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Обособленно) ТОГДА
		|						Заказы.Назначение
		|					ИНАЧЕ
		...
		|	Заказы.Склад                                              КАК Склад,
		|	Заказы.Назначение                                         КАК Назначение,
		|	ЕСТЬNULL(ГрафикПоступления.ДатаСобытия, ДАТАВРЕМЯ(1,1,1)) КАК ДатаСобытия,
		|	
		|	СУММА(ГрафикПоступления.КоличествоИзЗаказов + ГрафикПоступления.КоличествоПодЗаказ) КАК Количество
		|	
		|ИЗ
		...
		|		 И ГрафикПоступления.Характеристика = Заказы.Характеристика
		|		 И ГрафикПоступления.Склад          = Заказы.Склад
		|		 И ГрафикПоступления.Назначение     = Заказы.Назначение
		|ГДЕ
		|	ГрафикПоступления.КоличествоИзЗаказов <> 0
		|		ИЛИ ГрафикПоступления.КоличествоПодЗаказ <> 0
		|СГРУППИРОВАТЬ ПО
		|	Заказы.Заказ, Заказы.Номенклатура, Заказы.Характеристика, Заказы.Склад, Заказы.Назначение, ГрафикПоступления.ДатаСобытия
		|УПОРЯДОЧИТЬ ПО
		|	Заказ, Номенклатура, Характеристика, Склад, Назначение, ДатаСобытия УБЫВ";
	
	ГрафикПоступлений = ЗапросГрафикаПоступлений.Выполнить().Выгрузить();
	ГрафикПоступлений.Индексы.Добавить("Заказ,Номенклатура,Характеристика,Склад,Назначение");
		...
		Для Индекс = 0 По ВсегоСтрок Цикл
			...
			Если ОбеспечениеКлиентСервер.ИзменилсяКлюч(Ключ, ТекущаяСтрока) Или Индекс = ВсегоСтрок Тогда
				...
				Если ПотребностиОднойАналитики.Количество() > 0 Тогда
					...
					ГрафикПоступленияТовара = ГрафикПоступления.НайтиСтроки(ПараметрыОтбораОбособленные);
					Если ГрафикПоступленияТовара.Количество() > 0 Тогда
						ТаблицаОстатков = СкорректироватьТаблицуОстатковГрафикомПоступления(ТаблицаОстатков, ГрафикПоступленияТовара);
					КонецЕсли;
...
Процедура ВыполнитьОбеспечение(Источник, Заказ, Параметр, ЖелаемаяДатаОтгрузки, НеОтгружатьЧастями)
	...
	Если Заказ = Неопределено Тогда
		...
		КонецЕсли;
		Если Параметры.ПараметрыВыполненияДействий.Свойство("ОтборПоИзмерениям") Тогда
			ПараметрыДокумента.Вставить("ОтборПоИзмерениям", Параметры.ПараметрыВыполненияДействий.ОтборПоИзмерениям);
		КонецЕсли;
...
Функция ТоварыОбособленныеПоВсемСкладам(ТоварыОбособленные)
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПеремещениемОбособленныхТоваров") Тогда
		
		ТоварыОбособленныеПоВсемСкладам = Новый ТаблицаЗначений;
		ТоварыОбособленныеПоВсемСкладам.Колонки.Добавить("Номенклатура",   Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
		ТоварыОбособленныеПоВсемСкладам.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
		ТоварыОбособленныеПоВсемСкладам.Колонки.Добавить("Назначение",     Новый ОписаниеТипов("СправочникСсылка.Назначения"));
		ТоварыОбособленныеПоВсемСкладам.Колонки.Добавить("Количество",     Новый ОписаниеТипов("Число"));
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВтОбеспечениеТекущее
		|ИЗ
		|	&Товары КАК Товары
		|;
		|
		...
		|		Остатки.Характеристика КАК Характеристика,
		|		Остатки.Назначение     КАК Назначение,
		|		Остатки.Остаток        КАК Количество
		|	ИЗ
		|		ВтОбеспечениеТекущее КАК Остатки) КАК Набор
		|СГРУППИРОВАТЬ ПО
		|	Набор.Номенклатура,
