...
Процедура ОбновитьДеревоЭД()
	...
	ПараметрыЭД = ОбменСКонтрагентамиСлужебный.ЗаполнитьПараметрыЭДПоИсточнику(ОбъектСсылка);
...
Процедура СформироватьДеревьяЭД()
	...
	ТекстыЗапросов.Добавить(
		"ВЫБРАТЬ
		|	СостоянияЭД.СсылкаНаОбъект КАК Ключ,
		|	СостоянияЭД.ЭлектронныйДокумент КАК Значение
		|ИЗ
		|	РегистрСведений.СостоянияЭД КАК СостоянияЭД
		|ГДЕ
		|	&УсловиеСостоянияЭД");
	
	Если ИспользоватьВнутренниеДокументыЭДО Тогда
		
		ТекстыЗапросов.Добавить(
			"ВЫБРАТЬ
			|	АктуальностьЭлектронныхДокументов.ОбъектУчета,
			|	АктуальностьЭлектронныхДокументов.ЭлектронныйДокумент
			|ИЗ
			|	РегистрСведений.АктуальностьЭлектронныхДокументов КАК АктуальностьЭлектронныхДокументов
			|ГДЕ
			|	&УсловиеАктуальностьЭлектронныхДокументов");

		
	КонецЕсли;
	
	ИмяТаблицыЭлектронныеДокументы = "";
	...
	Если ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий")
		...
		ТекстыЗапросов.Добавить(
			"ВЫБРАТЬ
			|	ЭлектронныеДокументы.Ссылка КАК Ключ,
			|	ЭлектронныеДокументы.Ссылка КАК Значение
			|ИЗ
			|	&ЭлектронныеДокументы КАК ЭлектронныеДокументы
			|ГДЕ
			|	ЭлектронныеДокументы.Ссылка = &Ссылка
			|	И ЭлектронныеДокументы.ВидЭД = ЗНАЧЕНИЕ(Перечисление.ВидыЭД.ПроизвольныйЭД)");
		
		ИмяТаблицыЭлектронныеДокументы = ОбщегоНазначения.ИмяТаблицыПоСсылке(ОбъектСсылка);
		
		УсловиеСостоянияЭД = "СостоянияЭД.ЭлектронныйДокумент = &Ссылка";
		УсловиеАктуальностьЭлектронныхДокументов =
			"	АктуальностьЭлектронныхДокументов.ОбъектУчета В
			|			(ВЫБРАТЬ
			|				АктуальностьЭлектронныхДокументов.ОбъектУчета
			|			ИЗ
			|				РегистрСведений.АктуальностьЭлектронныхДокументов КАК АктуальностьЭлектронныхДокументов
			|			ГДЕ
			|				АктуальностьЭлектронныхДокументов.ЭлектронныйДокумент = &Ссылка)";
		
	Иначе
		УсловиеСостоянияЭД = "СостоянияЭД.СсылкаНаОбъект = &Ссылка
			|И СостоянияЭД.ЭлектронныйДокумент <> НЕОПРЕДЕЛЕНО";
		УсловиеАктуальностьЭлектронныхДокументов = "АктуальностьЭлектронныхДокументов.ОбъектУчета = &Ссылка";
	КонецЕсли;
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеСостоянияЭД", УсловиеСостоянияЭД);
	
	Если ЗначениеЗаполнено(ИмяТаблицыЭлектронныеДокументы) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЭлектронныеДокументы", ИмяТаблицыЭлектронныеДокументы);
	КонецЕсли;
	
	Если ИспользоватьВнутренниеДокументыЭДО Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеАктуальностьЭлектронныхДокументов",
			УсловиеАктуальностьЭлектронныхДокументов);
	КонецЕсли;
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("Ссылка", ОбъектСсылка);
	ДанныеДерева = Запрос.Выполнить().Выгрузить();
	ДанныеДерева.Свернуть("Ключ, Значение");
	
	Если Не ЗначениеЗаполнено(ДанныеДерева) Тогда
		СтрокаДерева = ДанныеДерева.Добавить();
		СтрокаДерева.Ключ = ОбъектСсылка;
		СтрокаДерева.Значение = ОбъектСсылка;
	КонецЕсли;
	
	ДеревоОбъект = РеквизитФормыВЗначение("ДеревоПодчиненныеЭД");
	
	Если ЗначениеЗаполнено(НастройкиОбмена) И НастройкиОбмена.НаправлениеЭД = Перечисления.НаправленияЭД.Внутренний Тогда  
		ОбменСКонтрагентамиСлужебный.СформироватьДеревьяЭД(ДеревоОбъект, ДанныеДерева, Неопределено, Истина);
	Иначе
		ОбменСКонтрагентамиСлужебный.СформироватьДеревьяЭД(ДеревоОбъект, ДанныеДерева, НастройкиОбмена, Истина);
	КонецЕсли;
