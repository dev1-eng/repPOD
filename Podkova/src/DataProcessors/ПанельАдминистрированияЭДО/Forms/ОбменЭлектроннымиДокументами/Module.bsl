...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТребуетсяВзаимодействие = ОбщегоНазначения.ПодсистемаСуществует("КлиентЭДО");
...
&НаКлиенте
Процедура ОткрытьТекущиеДелаEDI(Команда)
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЭлектронноеВзаимодействие.СервисEDI") Тогда
		
		ОбщийМодульТекущиеДелаEDIКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ТекущиеДелаEDIКлиент");
		ОбщийМодульТекущиеДелаEDIКлиент.ОткрытьФормуТекущихДел();
...
Процедура ПрочитатьУстановитьДоступность(РеквизитПутьКДанным = "")
	...
	ЕстьСервисДоставки = Ложь;
	ОбщийМодуль = ОбщегоНазначения.ОбщийМодуль("СервисДоставкиСлужебный");
	Если ОбщийМодуль <> Неопределено Тогда
		ЕстьСервисДоставки = Истина;
		ОбщийМодуль.ПроверитьДоступностьСервисаДоставки(ЕстьСервисДоставки);
	КонецЕсли;
		
	ЕстьСервисДоставки = ЕстьСервисДоставки
		И ЕстьОбменБизнесСеть;
	
	ДоступныеСервисыБинесСеть = Новый Массив;
	
	Если ЕстьСервисEDI Тогда
		ДоступныеСервисыБинесСеть.Добавить(НСтр("ru = 'обмен и согласование документов без электронной подписи в сервисе 1С:EDI'"));
	КонецЕсли;
	
	Если ЕстьТорговыеПредложения Тогда
		ДоступныеСервисыБинесСеть.Добавить(НСтр("ru = 'публикация и поиск торговых предложений в сервисе 1С:Торговая площадка'"));
	КонецЕсли;
	
	Если ЕстьСервисДоставки Тогда
		ДоступныеСервисыБинесСеть.Добавить(НСтр("ru = 'оформление доставки в сервисах 1С:Доставка и 1С:Курьер'"));
	КонецЕсли;
	
	Если Не ДоступныеСервисыБинесСеть.Количество() = 0 Тогда
		
		ДоступныеСервисыБинесСеть[0] = ВРег(Лев(ДоступныеСервисыБинесСеть[0], 1)) + Сред(ДоступныеСервисыБинесСеть[0], 2);
		
		ТекстПодсказки = СтрСоединить(ДоступныеСервисыБинесСеть, ", ") + ".";
	...
	Если ЕстьОбменБизнесСеть
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппаНастройкиБизнесСетьEDI", "Доступность", Константа_ИспользоватьОбменБизнесСеть);
	...
	Если ЕстьСервисEDI И (РеквизитПутьКДанным = "Константа_ИспользоватьОбменEDI" ИЛИ РеквизитПутьКДанным = "") Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ОбработкаСервисEDIТекущиеДела", "Доступность", Константа_ИспользоватьОбменEDI);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппаНастройкиБизнесСетьEDIПраво", "Доступность", Константа_ИспользоватьОбменEDI);
...
Процедура ПриИзмененииНастроекEDI()
	...
	ТекстГиперссылкиНастройки = ?(ЕстьНастройкиОрганизаций, НСтр("ru = 'Настройки 1C:EDI'"), НСтр("ru = 'Настроить 1C:EDI'"));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
