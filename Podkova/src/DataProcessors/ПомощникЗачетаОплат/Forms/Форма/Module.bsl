...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если (Не ЕстьРасчетыСКлиентами И Не ЕстьРасчетыСПоставщиками)
	 ...
	 ИЛИ (ПоЗаказу И РасчетыПоЗаказам) 
	 ИЛИ НЕ РасчетыПоДоговорам И (ТипЗнч(Документ) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
...
Процедура ЗаполнитьТаблицуПоРасчетамСПартнерами()
	...
	АвансыРассчитаны = ?(НоваяАрхитектураВзаиморасчетов, Истина, НЕ ЕстьЗаданияКРаспределениюВзаиморасчетов(ЕстьРасчетыСКлиентами, КлючиАналитики));
...
Процедура ПолучитьРеквизитыРасчетовСПартнером()
	...
	Попытка
		Если ОбщегоНазначения.ВидОбъектаПоСсылке(Документ) = "Документ" Тогда
			...
			Проведен = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ,"Проведен");
	...
	Если СтруктураРеквизитов <> Неопределено Тогда
		...
		МассивРасчетыСПоставщиками.Добавить(Перечисления.ХозяйственныеОперации.СписаниеПринятыхТоваровНаРасходы);
		ЕстьРасчетыСПоставщиками = МассивРасчетыСПоставщиками.Найти(ХозяйственнаяОперация) <> Неопределено
			ИЛИ (ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПриемНаКомиссию И РасчетыПоДоговорам);
		...
		МассивРасчетыСКлиентами.Добавить(Перечисления.ХозяйственныеОперации.РеализацияОСсОтложеннымПереходомПрав);
		МассивРасчетыСКлиентами.Добавить(Перечисления.ХозяйственныеОперации.РеализацияОС);
		...
		МассивРасчетыСКлиентами.Добавить(Перечисления.ХозяйственныеОперации.ВыкупТоваровХранителем);
		МассивРасчетыСКлиентами.Добавить(Перечисления.ХозяйственныеОперации.РеализацияВнеоборотныхАктивов);
		Если Не КомиссионноеВознаграждение Тогда
		...
		ЕстьРасчетыСКлиентами = МассивРасчетыСКлиентами.Найти(ХозяйственнаяОперация) <> Неопределено
			ИЛИ (ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссию И РасчетыПоДоговорам);
...
Функция ТекстВременныеТаблицыПоРасчетамСПоставщиками()
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(ТаблицаРасшифровкаПлатежа.ОбъектРасчетов) В (&ТипыПлатежныхДокументов) 
	|		ТОГДА ТаблицаРасшифровкаПлатежа.ОбъектРасчетов
	|		ИНАЧЕ ДанныеДокумента.Ссылка
	|	КОНЕЦ                                          КАК Ссылка,
	|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(ТаблицаРасшифровкаПлатежа.ОбъектРасчетов) В (&ТипыПлатежныхДокументов) 
	|		ТОГДА ТаблицаРасшифровкаПлатежа.ОбъектРасчетов.Дата
	|		ИНАЧЕ ДанныеДокумента.Дата
	|	КОНЕЦ                                          КАК Дата,
	|	ТаблицаРасшифровкаПлатежа.ВалютаВзаиморасчетов КАК Валюта,
	|	СУММА(ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(ТаблицаРасшифровкаПлатежа.ОбъектРасчетов) В (&ТипыПлатежныхДокументов) 
	|		ТОГДА ТаблицаРасшифровкаПлатежа.ОбъектРасчетов.СуммаДокумента
	...
	|	СУММА(ДвиженияДенежныеСредстваКонтрагент.СуммаОплатыВВалютеВзаиморасчетов) КАК СуммаОплатыВВалютеВзаиморасчетов,
	|	СУММА(ДвиженияДенежныеСредстваКонтрагент.СуммаОплатыВВалютеПлатежа) КАК СуммаОплатыВВалютеПлатежа,
	|	СУММА(ДвиженияДенежныеСредстваКонтрагент.СуммаОплатыРегл) КАК СуммаОплатыРегл,
	|	СУММА(ДвиженияДенежныеСредстваКонтрагент.СуммаОплаты) КАК СуммаОплаты
	|	
	|ПОМЕСТИТЬ ВтДвиженияДенежныеСредстваКонтрагент
	|ИЗ РегистрНакопления.ДвиженияДенежныеСредстваКонтрагент КАК ДвиженияДенежныеСредстваКонтрагент
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		ТаблицаОстатков КАК ТаблицаОстатков
	|	ПО
	|		ДвиженияДенежныеСредстваКонтрагент.ОбъектРасчетов = ТаблицаОстатков.ЗаказПоставщику
	|		И ДвиженияДенежныеСредстваКонтрагент.ВалютаВзаиморасчетов = ТаблицаОстатков.Валюта
	|ГДЕ ДвиженияДенежныеСредстваКонтрагент.ХозяйственнаяОперация В (
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаПоставщику),
	...
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводНачальныхОстатковАвансовПоставщикам))
	|		И ДвиженияДенежныеСредстваКонтрагент.СуммаОплатыВВалютеВзаиморасчетов > 0
	|СГРУППИРОВАТЬ ПО
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтДвиженияДенежныеСредстваКонтрагент КАК ДвиженияРасчеты
	|	ПО
	|		РасчетыСПоставщиками.Регистратор = ДвиженияРасчеты.Регистратор
	...
	|					ИЛИ (НАЧАЛОПЕРИОДА(ЕСТЬNULL(ТаблицаДокументов.Дата, ДАТАВРЕМЯ(3000,1,1)), ДЕНЬ) >= &ДатаДокумента))))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// (3) Документ приобретения только регл.
	|ВЫБРАТЬ
	|	РасчетыСПоставщиками.ЗаказПоставщику                                КАК Заказ,
	|	РасчетыСПоставщиками.Валюта                                         КАК ВалютаВзаиморасчетов,
	|	РасчетыСПоставщиками.ЗаказПоставщику                                КАК Документ,
	|	Аналитика.КлючАналитики                                             КАК КлючАналитики,
	|	Аналитика.Организация                                               КАК Организация,
	|	Аналитика.Партнер                                                   КАК Партнер,
	|	Аналитика.Контрагент                                                КАК Контрагент,
	...
	|	РасчетыСПоставщиками.СуммаПриход - РасчетыСПоставщиками.СуммаРасход КАК ДоступноКЗачету,
	|	РасчетыСПоставщиками.СуммаПриход - РасчетыСПоставщиками.СуммаРасход КАК СуммаВВалютеПлатежа,
	|	0                                                                   КАК СуммаРегл,
	|	0                                                                   КАК СуммаУпр,
	...
	|		ТаблицаДокументов КАК ТаблицаДокументов
	|	ПО
	|		РасчетыСПоставщиками.ЗаказПоставщику = ТаблицаДокументов.Ссылка
	|	
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(РасчетыСПоставщиками.ЗаказПоставщику) = ТИП(Документ.ПриобретениеТоваровУслуг)
	|	И &ЭтоУправленческаяОрганизация
	|	И ТаблицаДокументов.Валюта = &ВалютаВзаиморасчетов
	|;";
...
Функция ТекстЗапросаПоРасчетамСПоставщиками()
	...
	|	КОНЕЦ                                                            КАК СуммаУпр,
	|	
	|	ВЫБОР КОГДА (НЕ ТаблицаДокументов.Ссылка ЕСТЬ NULL ИЛИ РасчетыСПоставщиками.Регистратор = &Документ) 
	|		И НЕ ТИПЗНАЧЕНИЯ(РасчетыСПоставщиками.Регистратор) = ТИП(Документ.ВводОстатков)
	|		И НЕ ТИПЗНАЧЕНИЯ(РасчетыСПоставщиками.Регистратор) = ТИП(Документ.ВозвратТоваровПоставщику)
	//Зачтено
	|		И ВЫБОР 
	|				КОГДА РасчетыСПоставщиками.ЗаказПоставщику <> &ОбъектРасчетов 
	|					ТОГДА РасчетыСПоставщиками.СуммаРасход
...
Функция ТекстЗапросаАвансыПоРасчетамСПоставщиками(Запрос)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		|	И ТИПЗНАЧЕНИЯ(РасчетыСПоставщиками.ЗаказПоставщику) <> ТИП(Документ.ПриобретениеТоваровУслуг)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	АналитикаУчетаПоПартнерам,
		|	ЗаказПоставщику,
	...
	Иначе
		...
		|	РасчетыСПоставщиками.АналитикаУчетаПоПартнерам         КАК АналитикаУчетаПоПартнерам,
		|	РасчетыСПоставщиками.АналитикаУчетаПоПартнерам.Партнер КАК Партнер,
		|	РасчетыСПоставщиками.ОбъектРасчетов                    КАК ЗаказПоставщику,
		|	РасчетыСПоставщиками.Валюта                            КАК Валюта,
		|	РасчетыСПоставщиками.ПредоплатаОстаток                 КАК СуммаАванса,
		|	РасчетыСПоставщиками.ПредоплатаРеглОстаток             КАК СуммаРегл,
		|	РасчетыСПоставщиками.ПредоплатаУпрОстаток              КАК СуммаУпр,
		...
		|		ТаблицаДокументов КАК ТаблицаДокументов
		|	ПО
		|		РасчетыСПоставщиками.ОбъектРасчетов = ТаблицаДокументов.Ссылка
		|	
		|ГДЕ
		|	РасчетыСПоставщиками.ПредоплатаОстаток > 0
		|	И РасчетыСПоставщиками.ОбъектРасчетов <> НЕОПРЕДЕЛЕНО
		|ИНДЕКСИРОВАТЬ ПО
		|	АналитикаУчетаПоПартнерам,
		|	ЗаказПоставщику,
		|	Валюта
	...
	|		ИЛИ Аналитика.Договор <> &Договор
	|		ИЛИ НЕ (ТИПЗНАЧЕНИЯ(&ОбъектРасчетов) В (&ТипыНакладных)
	|				ИЛИ НАЧАЛОПЕРИОДА(ЕСТЬNULL(ТаблицаОстатковПоДокументам.Дата, ДАТАВРЕМЯ(3000,1,1)), ДЕНЬ) >= &ДатаДокумента))
	|	И НЕ (&ЭтоУправленческаяОрганизация И ТаблицаОстатковПоДокументам.ЗаказПоставщику = Неопределено)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстВременныеТаблицыПоРасчетамСКлиентами()
	...
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводНачальныхОстатковАвансовКлиентов))
	|		И ДвиженияРасчеты.СуммаОплатыВВалютеВзаиморасчетов > 0
	...
	//Остатки по объекту расчетов платежке
	|		ИЛИ (ТИПЗНАЧЕНИЯ(РасчетыСКлиентами.ЗаказКлиента) В (&ТипыПлатежныхДокументов)
	|				И Аналитика.Договор = &Договор
	|				И Аналитика.НаправлениеДеятельности = &НаправлениеДеятельности
	|				И РасчетыСКлиентами.Валюта = &ВалютаВзаиморасчетов 
	//Зачесть можно если объект расчетов накладная или если платежка позже объекта расчетов.
	|				И (ТИПЗНАЧЕНИЯ(&ОбъектРасчетов) В (&ТипыНакладных)
	|					ИЛИ (НАЧАЛОПЕРИОДА(ЕСТЬNULL(ТаблицаДокументов.Дата, ДАТАВРЕМЯ(3000,1,1)), ДЕНЬ) >= &ДатаДокумента))))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// (3) Документы реализации только регл.
	|ВЫБРАТЬ
	|	РасчетыСКлиентами.ЗаказКлиента                                КАК Заказ,
	|	РасчетыСКлиентами.Валюта                                      КАК ВалютаВзаиморасчетов,
	|	РасчетыСКлиентами.ЗаказКлиента                                КАК Документ,
	|	Аналитика.КлючАналитики                                       КАК КлючАналитики,
	|	Аналитика.Организация                                         КАК Организация,
	|	Аналитика.Партнер                                             КАК Партнер,
	|	Аналитика.Контрагент                                          КАК Контрагент,
	...
	|	ТаблицаДокументов.СуммаДокумента                              КАК СуммаДокумента,
	|	ТаблицаДокументов.Валюта                                      КАК Валюта,
	|	РасчетыСКлиентами.СуммаРасход - РасчетыСКлиентами.СуммаПриход КАК ДоступноКЗачету,
	|	РасчетыСКлиентами.СуммаРасход - РасчетыСКлиентами.СуммаПриход КАК СуммаВВалютеПлатежа,
	|	0                                                             КАК СуммаРегл,
	|	0                                                             КАК СуммаУпр,
	|	ИСТИНА                                                        КАК ДоступноРедактирование,
	|	ИСТИНА                                                        КАК РасшифровкаОбъектаРасчетов
	...
	|		ТаблицаДокументов КАК ТаблицаДокументов
	|	ПО
	|		РасчетыСКлиентами.ЗаказКлиента = ТаблицаДокументов.Ссылка
	|
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(РасчетыСКлиентами.ЗаказКлиента) = ТИП(Документ.РеализацияТоваровУслуг)
	|	И &ЭтоУправленческаяОрганизация
	|	И ТаблицаДокументов.Валюта = &ВалютаВзаиморасчетов
	|
	|;";
...
Функция ТекстЗапросаПоРасчетамСКлиентами()
	...
	|	КОНЕЦ                                                            КАК СуммаУпр,
	|
	|	ВЫБОР КОГДА (НЕ ТаблицаДокументов.Ссылка ЕСТЬ NULL ИЛИ РасчетыСКлиентами.Регистратор = &Документ) 
	|		И НЕ ТИПЗНАЧЕНИЯ(РасчетыСКлиентами.Регистратор) = ТИП(Документ.ВводОстатков)
	|		И НЕ ТИПЗНАЧЕНИЯ(РасчетыСКлиентами.Регистратор) = ТИП(Документ.ВозвратТоваровОтКлиента)
	//Зачтено
	|		И ВЫБОР 
	|				КОГДА РасчетыСКлиентами.ЗаказКлиента <> &ОбъектРасчетов 
	|					ТОГДА РасчетыСКлиентами.СуммаПриход
...
Функция ТекстЗапросаАвансыПоРасчетамСКлиентами(Запрос)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		|	И ТИПЗНАЧЕНИЯ(РасчетыСКлиентами.ЗаказКлиента) <> ТИП(Документ.РеализацияТоваровУслуг)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	АналитикаУчетаПоПартнерам,
		|	ЗаказКлиента,
	...
	Иначе
		...
		|		ТаблицаДокументов КАК ТаблицаДокументов
		|	ПО
		|		РасчетыСКлиентами.ОбъектРасчетов = ТаблицаДокументов.Ссылка
		|	
		|ГДЕ
		|	РасчетыСКлиентами.ПредоплатаОстаток > 0
...
Функция ЗаблокироватьДокументыДляРедактирования(МассивСтрок, ИмяТаблицы)
	...
	Для Каждого ИдентификаторСтроки Из МассивСтрок Цикл
		...
		Попытка
			Если РасчетыПоЗаказам ИЛИ РасчетыПоДоговорам ИЛИ НЕ ЗначениеЗаполнено(СтрокаТаблицы.Заказ) ИЛИ СтрокаТаблицы.Дата >= ДатаДокумента Тогда
...
Процедура ОтразитьЗачетОплатыВДокументе(ДокументСсылка, ДанныеДокумента, СтруктураСумм, ПолеОшибки, Отказ, ОбъектРасчетовЗачета = Неопределено)
	...
	Если ТаблицаРасшифровкаПлатежа.Количество() = 0 И Метаданные.ОпределяемыеТипы.ОбъектРасчетовНакладная.Тип.СодержитТип(ТипЗнч(ДокументСсылка)) Тогда
		ЕстьСуммаВзаиморасчетов = НЕ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ОтчетКомиссионераОСписании")
			И НЕ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ОтчетКомитентуОСписании");
