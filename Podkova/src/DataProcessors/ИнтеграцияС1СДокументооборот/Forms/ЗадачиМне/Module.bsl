...
Процедура ВыполнитьЗадачуПервая(Команда)
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessIssuesSolution" И ТочкаМаршрута = "РассмотрениеИнициатором" Тогда
		ТребоватьРезультатВыполнения = Истина;
	КонецЕсли;
...
Процедура ПостроитьПодменюСозданияНаОсновании()
	...
	ЗапросИсточникиПолучатели = Новый Запрос("
		...
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	Справочник.ПравилаИнтеграцииС1СДокументооборотом.ПравилаЗаполненияРеквизитовДО КАК Реквизиты
		|ПО
