	...
	Иначе // нет подходящих правил
		...
		Если ПравоДоступа("Добавление", Метаданные.Справочники.ПравилаИнтеграцииС1СДокументооборотом) Тогда
			...
			Строки.Добавить(Новый ФорматированнаяСтрока(
				НСтр("ru = 'Настроить.'"),,,, МетаданныеПотребителя.ПолноеИмя()));
			Элементы.ДекорацияИнфо.Заголовок = Новый ФорматированнаяСтрока(Строки);
		Иначе
...
Процедура ОбработатьФормуСогласноВерсииСервиса()
	...
	Иначе // версия известна
		...
		Иначе
			...
			ДоступенФункционалПочта = ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.8.1.CORP");
			...
			ДоступенФункционалХронометраж = ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.3.2.3.CORP");
...
Процедура СоздатьФайлСДискаПослеПомещенияФайла(ПомещенныеФайлы, ПараметрыОповещения) Экспорт
	...
	Если НастройкиДокументооборот.НужноИзвлечьТекст Тогда
		ИзвлеченныйТекст = ИнтеграцияС1СДокументооборотКлиент.ИзвлечьТекст(ПараметрыОповещения.ПолноеИмяФайла);
...
Процедура ЗаполнитьСтрокуДерева(СтрокиДерева, СтрокаXDTO)
	...
	Если ДокументТип = "DMCorrespondent" Или ДокументТип = "DMOutgoingEMail" Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(СтрокаXDTO, "sent") Тогда
