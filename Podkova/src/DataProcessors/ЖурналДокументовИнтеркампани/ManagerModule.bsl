...
Функция ТекстЗапросаОформляемыеОтчетыПоКомиссии(ПараметрыЗапроса = Неопределено) Экспорт
	...
	Если ЭтоРасчетГиперссылки Тогда
		ТекстЗапросаВТОтборыКОформлению = СтрЗаменить(ТекстЗапросаВТОтборыКОформлению, "ВЫБРАТЬ", "ВЫБРАТЬ ПЕРВЫЕ 1");
		ТекстЗапросаВТОтборыКОформлению = СтрЗаменить(ТекстЗапросаВТОтборыКОформлению, "ПОМЕСТИТЬ ВТОтборыКОформлению", "");
	КонецЕсли;
	
	ТекстЗапроса = ТекстЗапроса + ОбщегоНазначенияУТ.РазделительЗапросовВПакете() + ТекстЗапросаВТОтборыКОформлению;
	...
	Если Не ЭтоРасчетГиперссылки Тогда 
		...
		|		ИНАЧЕ ЕСТЬNULL(УчетныеПолитики.НалогообложениеНДСПоУмолчанию, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС))
