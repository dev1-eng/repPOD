...
#КонецОбласти
	...
	Элементы.ПроизводительВЕТИС.Видимость = НастройкиИспользованияСерий.ИспользоватьПроизводителяВЕТИССерии;
	Элементы.ОстаткиСерийПроизводительВЕТИС.Видимость = НастройкиИспользованияСерий.ИспользоватьПроизводителяВЕТИССерии;
	
	Элементы.ЗаписьСкладскогоЖурналаВЕТИС.Видимость = НастройкиИспользованияСерий.ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии;
	...
	Элементы.ИдентификаторПартииВЕТИС.Видимость = НастройкиИспользованияСерий.ИспользоватьИдентификаторПартииВЕТИССерии;
	...
	Элементы.МаксимальнаяРозничнаяЦенаМОТП.Видимость = НастройкиИспользованияСерий.ИспользоватьМРЦМОТПСерии;
...
&НаКлиенте
Процедура ПроизводительВЕТИСПриИзменении(Элемент)
	НайтиСерию();
	УстановитьЗаголовокДекорацииСерия();
КонецПроцедуры

&НаКлиенте
Процедура ЗаписьСкладскогоЖурналаВЕТИСПриИзменении(Элемент)
	НайтиСерию();
	УстановитьЗаголовокДекорацииСерия();
КонецПроцедуры

&НаКлиенте
Процедура ИдентификаторПартииВЕТИСПриИзменении(Элемент)
	НайтиСерию();
	УстановитьЗаголовокДекорацииСерия();
КонецПроцедуры

&НаКлиенте
Процедура МаксимальнаяРозничнаяЦенаМОТППриИзменении(Элемент)
	НайтиСерию();
	УстановитьЗаголовокДекорацииСерия();
КонецПроцедуры
...
Процедура НайтиСерию()
	...
	|			И СерииНоменклатуры.ПроизводительВЕТИС = &ПроизводительВЕТИС
	|			И СерииНоменклатуры.ЗаписьСкладскогоЖурналаВЕТИС = &ЗаписьСкладскогоЖурналаВЕТИС
	|			И СерииНоменклатуры.ИдентификаторПартииВЕТИС = &ИдентификаторПартииВЕТИС
	|			И СерииНоменклатуры.МаксимальнаяРозничнаяЦенаМОТП = &МаксимальнаяРозничнаяЦенаМОТП
	...
	Если НастройкиИспользованияСерий.ИспользоватьПроизводителяВЕТИССерии Тогда
		Запрос.УстановитьПараметр("ПроизводительВЕТИС", ПроизводительВЕТИС);
	Иначе
		Запрос.УстановитьПараметр("ПроизводительВЕТИС", Справочники.ПредприятияВЕТИС.ПустаяСсылка());
	КонецЕсли;
	
	Если НастройкиИспользованияСерий.ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии Тогда
		Запрос.УстановитьПараметр("ЗаписьСкладскогоЖурналаВЕТИС", ЗаписьСкладскогоЖурналаВЕТИС);
	Иначе
		Запрос.УстановитьПараметр("ЗаписьСкладскогоЖурналаВЕТИС", Справочники.ЗаписиСкладскогоЖурналаВЕТИС.ПустаяСсылка());
	КонецЕсли;
	
	Если НастройкиИспользованияСерий.ИспользоватьИдентификаторПартииВЕТИССерии Тогда
		Запрос.УстановитьПараметр("ИдентификаторПартииВЕТИС", ИдентификаторПартииВЕТИС);
	Иначе
		Запрос.УстановитьПараметр("ИдентификаторПартииВЕТИС", "");
	КонецЕсли;
	
	Если НастройкиИспользованияСерий.ИспользоватьМРЦМОТПСерии Тогда
		Запрос.УстановитьПараметр("МаксимальнаяРозничнаяЦенаМОТП", МаксимальнаяРозничнаяЦенаМОТП);
	Иначе
		Запрос.УстановитьПараметр("МаксимальнаяРозничнаяЦенаМОТП", 0);
	КонецЕсли;
...
Функция ОбязательныеРеквизитыЗаполнены(ВыводитьСообщения = Истина, ЭтоСписываемаяМетка = Ложь)
	...
	Если НастройкиИспользованияСерий.ИспользоватьДатуПроизводстваСерии
		...
		Если ВыводитьСообщения Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"ДатаПроизводства");
		КонецЕсли;
	КонецЕсли;
	...
	Если НастройкиИспользованияСерий.ИспользоватьПроизводителяВЕТИССерии
		И Не ЗначениеЗаполнено(ПроизводительВЕТИС) Тогда
		Отказ = Истина;
		Если ВыводитьСообщения Тогда
			ТекстСообщения = НСтр("ru = 'Поле ""Производитель (ВетИС)"" не заполнено'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"ПроизводительВЕТИС");
		КонецЕсли;
	КонецЕсли;
	
	Если НастройкиИспользованияСерий.ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии
		И Не ЗначениеЗаполнено(ЗаписьСкладскогоЖурналаВЕТИС) Тогда
		Отказ = Истина;
		Если ВыводитьСообщения Тогда
			ТекстСообщения = НСтр("ru = 'Поле ""Запись складского журнала (ВетИС)"" не заполнено'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"ЗаписьСкладскогоЖурналаВЕТИС");
		КонецЕсли;
	КонецЕсли;
	
	Если НастройкиИспользованияСерий.ИспользоватьИдентификаторПартииВЕТИССерии
		И Не ЗначениеЗаполнено(ИдентификаторПартииВЕТИС) Тогда
		Отказ = Истина;
		Если ВыводитьСообщения Тогда
			ТекстСообщения = НСтр("ru = 'Поле ""Идентификатор партии (ВетИС)"" не заполнено'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"ИдентификаторПартииВЕТИС");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СерияДляЗаполнения) Тогда
		...
		Если ЗначениеЗаполнено(СерияСсылка) Тогда
			РеквизитыСерии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СерияСсылка,
				"Наименование,Номер,ГоденДо,НомерКИЗГИСМ,ДатаПроизводства,ПроизводительЕГАИС,Справка2ЕГАИС,ПроизводительВЕТИС,
				|ЗаписьСкладскогоЖурналаВЕТИС,ИдентификаторПартииВЕТИС,МаксимальнаяРозничнаяЦенаМОТП,
				|RFIDUser,RFIDEPC,EPCGTIN,RFIDTID,RFIDМеткаНеЧитаемая");
