...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли Не Объект.РезультатВзятьИзПула
		И Не ЭтоОшибкиОписанияТовара
		И ЗначениеЗаполнено(Объект.КодМаркировки)
		...
		КодыМаркировки.Добавить(Объект.КодМаркировки);
		КодыМаркировки.Добавить(Объект.НовыйКодМаркировки);
		ЗначенияШтрихкодов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(КодыМаркировки, "ЗначениеШтрихкода");
		
		ПредставлениеКодаМаркировки = ЗначенияШтрихкодов[Объект.КодМаркировки];
		РезультатРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
		...
		ПредставлениеКодаМаркировки = ЗначенияШтрихкодов[Объект.НовыйКодМаркировки];
		РезультатРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
			ПредставлениеКодаМаркировки,
			Объект.ВидПродукции);
		
		Если РезультатРазбора <> Неопределено Тогда
			GTINНовый = РезультатРазбора.СоставКодаМаркировки.GTIN;
		Иначе
...
Процедура КодТНВЭДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормИСМПКлиентПереопределяемый.ПриНачалеВыбораКодТНВЭД(Элемент, Объект, СтандартнаяОбработка);
...
Функция ОбработатьШтрихкодКодаМаркировки(ДанныеШтрихкода)
	...
	Если СтруктураПолей.ЭтоНовыйКод
		И ЗначениеЗаполнено(Объект.КодМаркировки)
		И ЗначениеЗаполнено(Объект.ПричинаПеремаркировки)
		И Объект.ПричинаПеремаркировки <> ПредопределенноеЗначение(
		"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ОшибкиОписанияТовара") Тогда
...
Процедура КодМаркировкиПриИзмененииЗначения()
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		ЗначенияРеквизиов = ПолучитьЗначенияРеквизитовКодаМаркировки(ВыбранноеЗначение);
		
		Если Объект[СтруктураПолей.Номенклатура] <> ЗначенияРеквизиов.Номенклатура Тогда
			ФлагИзмененияНоменклатуры = 1;
		ИначеЕсли Объект[СтруктураПолей.Характеристика] <> ЗначенияРеквизиов.Характеристика Тогда
			ФлагИзмененияНоменклатуры = 2;
		КонецЕсли;
...
Функция ПолучитьЗначенияРеквизитовКодаМаркировки(КодМаркировки)
	...
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			КодМаркировки, "Номенклатура, Характеристика");
...
Процедура ОбновитьПредставленияНоменклатуры(Форма)
	...
	Иначе
		ТолькоПросмотрНовойНоменклатуры = ЗначениеЗаполнено(Форма.Объект.КодМаркировки)
			И Форма.Объект.ПричинаПеремаркировки <> ПредопределенноеЗначение(
			"Перечисление.ПричиныПеремаркировкиТоваровИСМП.ОшибкиОписанияТовара");
	КонецЕсли;
