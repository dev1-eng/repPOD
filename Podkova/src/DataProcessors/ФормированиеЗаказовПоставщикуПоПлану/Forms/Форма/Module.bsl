...
Процедура ВосстановитьНастройки()
	...
	Если ТипЗнч(ЗначениеНастроек) = Тип("Структура") Тогда
		...
		Если ЗначениеНастроек.Свойство("ПериодПоступления") Тогда
			Объект.РежимРасчетаПотребностейПоПлану = 1;
			Объект.ПериодНачало = ЗначениеНастроек.ПериодПоступления.ДатаНачала;
			Объект.ПериодОкончание = ЗначениеНастроек.ПериодПоступления.ДатаОкончания;
		КонецЕсли;
		
		Если ЗначениеНастроек.Свойство("ПериодЗаказа") Тогда
			Объект.РежимРасчетаПотребностейПоПлану = 0;
			Объект.ПериодНачало = ЗначениеНастроек.ПериодЗаказа.ДатаНачала;
			Объект.ПериодОкончание = ЗначениеНастроек.ПериодЗаказа.ДатаОкончания;
		КонецЕсли;
...
Процедура СоздатьЗаказыНаСервере()
	...
	|	ТаблицаЗаказов.Цена,
	|	ВЫБОР КОГДА ТаблицаЗаказов.ДатаПоступления>&ТекущаяДатаСеанса ТОГДА ТаблицаЗаказов.ДатаПоступления ИНАЧЕ &ТекущаяДатаСеанса КОНЕЦ КАК ДатаПоступления,
