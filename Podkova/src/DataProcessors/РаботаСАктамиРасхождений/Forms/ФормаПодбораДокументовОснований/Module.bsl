...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ПеренестиВДокумент Тогда
		...
		Если ВыбранныеОснования.Количество() > 1
			И Не ДокументыВыбраныКорректноСУчетомУчастияВЭДО(ВыбранныеОснования) Тогда
...
&НаСервере
Функция ДокументыВыбраныКорректноСУчетомУчастияВЭДО(ВыбранныеОснования)
	
	ДокументыУчаствующиеВЭДО = Новый Массив;
	
	Для Каждого ВыбранноеОснование Из ВыбранныеОснования Цикл
		
		Если РасхожденияСервер.ДокументУчаствуетВЭДО(ВыбранноеОснование) Тогда
	...
	Если ДокументыУчаствующиеВЭДО.Количество() > 0 Тогда
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Выбор нескольких документов-оснований доступен только если ни один из них не учавствует в электронном документообороте.
