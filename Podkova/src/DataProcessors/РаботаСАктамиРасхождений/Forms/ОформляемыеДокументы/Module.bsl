...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьПравоОбменаЭДО = ОбменСКонтрагентамиСлужебныйВызовСервера.ЕстьПравоВыполненияОбмена();
...
Процедура ЗаполнитьТаблицуДействий(АктОРасхождениях, РезультатЗапроса);
	...
	Пока ВыборкаОснования.Следующий() Цикл
		...
		Если Метаданные.ОпределяемыеТипы.ОснованияЭлектронныхДокументов.Тип.ПривестиЗначение(ВыборкаОснования.Основание) = ВыборкаОснования.Основание Тогда
...
Функция ТекстЛегенды(ТипДокумента, ХозяйственнаяОперация = Неопределено)
	...
	ИначеЕсли ТипДокумента = "ЭлектронныйДокументПодтверждение" Тогда
		Возврат НСтр("ru = 'Необходимо подтвердить полученный электронный документ, так как для отражения расхождения оформляется документ ""Корректировка поступления"".'");
	ИначеЕсли ТипДокумента = "ЭлектронныйДокументОтклонение" Тогда
...
Функция СтруктурыОписанияВыводимогоВОтчетДействия()
	...
	СтруктураОписания = Новый Структура(
	                 ...
	                 |СпособОтражения, Склад, ВыводитьСклад, ДокументЭДО, СтатусЭДО");
