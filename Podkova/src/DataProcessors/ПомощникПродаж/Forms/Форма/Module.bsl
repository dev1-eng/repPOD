...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоСоглашениюИспользуютсяДоговорыКонтрагентов = ЗначениеЗаполнено(Объект.Соглашение)
	                      И Обработки.ПомощникПродаж.ЗначениеРеквизитаОбъектаТипаБулево(Объект.Соглашение, "ИспользуютсяДоговорыКонтрагентов");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "СчитанаКартаЛояльности"
		...
		ПараметрыОповещения =  Новый Структура("ИмяСобытия, Источник, Параметр", ИмяСобытия, Источник, Параметр);
		ОписаниеОповещения  = Новый ОписаниеОповещения("ОбработкаОповещенияЗавершение", ЭтотОбъект, ПараметрыОповещения);
...
#Область ПроверкиРеквизитовШапки
	...
	Если Не ВариантСозданияНеКоммерческоеПредложение Тогда
		...
		ТаблицаКонтактнойИнформации = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Объект.Менеджер, Справочники.ВидыКонтактнойИнформации.EmailПользователя, ТекущаяДатаСеанса(), Ложь);
		Если ТаблицаКонтактнойИнформации.Количество() = 0
			Или ПустаяСтрока(ТаблицаКонтактнойИнформации[0].Представление) Тогда
			
			ТекстОшибки = НСтр("ru = 'У менеджера не указан адрес электронной почты.'");
			
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ,"Менеджер", "Объект", Отказ);
	...
	Если НаправлениеДеятельностиОбязательно 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.НаправлениеДеятельности", , Отказ);
...
#Область ПроверкиТабличнойЧастиТовары
	...
	ИначеЕсли Объект.Товары.Итог("КоличествоУпаковок") = 0
	...
	Для ТекИндекс = 0 По Объект.Товары.Количество()-1 Цикл
		...
		Если Не ЗначениеЗаполнено(Объект.Товары[ТекИндекс].КоличествоУпаковок)
			И НЕ ЗначениеЗаполнено(Объект.Товары[ТекИндекс].КоличествоУпаковокКВозврату) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Товары[ТекИндекс].Цена)
			И ?(Объект.ВернутьМногооборотнуюТару
				...
				Объект.Товары[ТекИндекс].ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.МногооборотнаяТара,
				Истина) Тогда
		...
		Если Объект.Товары[ТекИндекс].СкладОбязателен
			И Не ЗначениеЗаполнено(Объект.Товары[ТекИндекс].Склад)
			И ВариантСозданияНеКоммерческоеПредложение Тогда
		...
		Если (Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиента
			...
			Если Объект.Товары[ТекИндекс].ДатаОтгрузкиОбязательна
				И Не ЗначениеЗаполнено(Объект.Товары[ТекИндекс].ДатаОтгрузки)
	...
	Если (Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиента
			Или Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг)
		И ДатаОтгрузкиОбязательна
		И Объект.НеОтгружатьЧастями
		И Не ЗначениеЗаполнено(Объект.ДатаОтгрузки) Тогда
		
		ТекстОшибки = НСтр("ru = 'Поле ""Дата отгрузки"" не заполнено'");
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.ДатаОтгрузки", , ЕстьОшибкиТовары);
...
Процедура НадписьЭтапыОплатыНажатие(Элемент, СтандартнаяОбработка)
	...
	ОплатаУказываетсяВРеализации = ОплатаУказываетсяВРеализации(Объект.ВариантОформленияДокументов,
																Объект.ПорядокРасчетов);
	
	УпрощенныйРежим = Объект.ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг");
	
	ТребуетсяЗалогЗаТару         = Объект.ВернутьМногооборотнуюТару
									И Объект.ТребуетсяЗалогЗаТару;
	...
	ЭтоНакладная = Объект.ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг")
				ИЛИ Объект.ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг");
	...
	ПараметрыФормы.Вставить("ЭтоНакладная",                   ЭтоНакладная);
	...
	ПараметрыФормы.Вставить("УпрощенныйРежим",                ИспользоватьУпрощеннуюСхемуОплаты ИЛИ УпрощенныйРежим);
	ПараметрыФормы.Вставить("РежимРеализации",                ПараметрыФормы.УпрощенныйРежим И ИспользоватьСтатусыРеализацийТоваровУслуг И ОплатаУказываетсяВРеализации 
																И НЕ Объект.ВариантОформленияДокументов = ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг"));
	АдресВоВременномХранилище = ПоместитьЭтапыОплатыВоВременноеХранилищеНаСервере();
	...
	ОткрытьФорму("ОбщаяФорма.ЭтапыОплатыКлиентом", ПараметрыФормы, ЭтаФорма, , , , Оповещение,
...
Процедура ТоварыПриАктивизацииСтроки(Элемент)
	...
	ТекстСтатистики = НСтр("ru='продаже'");
	ТекстПродажи    = НСтр("ru='продажа'");
...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не ОтменаРедактирования Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ДатаОтгрузки)
			И Объект.НеОтгружатьЧастями
			И (КешСтроки = Неопределено
				Или КешСтроки.ДатаОтгрузки <> ТекущиеДанные.ДатаОтгрузки) Тогда
...
Процедура УправлениеСвязямиПараметровВыбораРеквизитовФормы()
	...
	ПараметрВыбора = Новый СвязьПараметраВыбора("ПоСоглашениюИспользуютсяДоговорыКонтрагентов",
												"ПоСоглашениюИспользуютсяДоговорыКонтрагентов",
...
Функция ЗаписатьКоммерческоеПредложение(МассивНайденныхДокументов, Отказ)
	...
	ДокументОбъект.Клиент                              = Объект.Партнер;
	ДокументОбъект.ВариантУказанияСрокаПоставки        = Перечисления.ВариантыСроковПоставкиКоммерческихПредложений.НеУказывается;
	ДокументОбъект.ПрочаяДополнительнаяИнформацияТекст = Объект.ДополнительнаяИнформация;
	ДокументОбъект.Налогообложение                     = КоммерческиеПредложенияДокументыКлиентСерверУТ.НалогообложениеКоммерческихПредложенийПоНалогообложениюНДС(Объект.НалогообложениеНДС);
	ДокументОбъект.Статус                              = Объект.СтатусКоммерческогоПредложения;
	ДокументОбъект.УсловияДоставкиТекст                = Объект.ДополнительнаяИнформацияПоДоставке;
	ДокументОбъект.ХозяйственнаяОперация               = КоммерческиеПредложенияДокументыКлиентСерверУТ.ХозяйственнаяОперацияДокументаКоммерческоеПредложениеКлиенту(Объект.ХозяйственнаяОперация);
	ДокументОбъект.МожетВыкупатьсяЧастично             = Не Объект.НеМожетВыкупатьсяЧастично;
	ДокументОбъект.Статус                              = Объект.СтатусКоммерческогоПредложения;
...
Функция ЗаписатьЗаказ(МассивНайденныхДокументов, Отказ, БезПроведения)
	...
	ДокументОбъект = НайтиСоздатьДокумент(Документы.ЗаказКлиента,
	...
	Попытка
		...
		Если СозданНовыйДокумент Тогда
			...
			ДобавитьСтрокуВТаблицуСформированныхДокументов(ДокументОбъект.Ссылка,
															ДокументОбъект.СуммаДокумента,
															ДокументОбъект.Валюта,
															2,
		...
		ИначеЕсли СтрокаТаблицы <> Неопределено Тогда
			...
			ОбновитьСтрокуВТаблицуСформированныхДокументов(СтрокаТаблицы,
															ДокументОбъект.СуммаДокумента,
															ДокументОбъект.Валюта,
															2,
															?(ДокументОбъект.Проведен,0,2));
...
Функция ЗаписатьЗаявкуНаВозврат(МассивНайденныхДокументов, Склад, Отказ)
	...
	СозданНовыйДокумент = Ложь;
	СтрокаТаблицы = Неопределено;
	ЭтоОперацияПередачиТоваров = Ложь;
	
	ДокументОбъект = НайтиСоздатьДокумент(Документы.ЗаявкаНаВозвратТоваровОтКлиента,
	...
	Если ЭтоОперацияПередачиТоваров Тогда
...
Процедура ЗаписатьТранспортнуюНакладную(МассивНайденныхДокументов, ОснованиеОбъект, Отказ)
	...
	ДокументОбъект = НайтиСоздатьДокумент(Документы.ТранспортнаяНакладная,
	...
	Документы.ТранспортнаяНакладная.СоздатьИлиЗаполнитьТранспортнуюНакладную(МассивРаспоряжений, ЗначенияЗаполнения, ,
		ДокументОбъект);
...
&НаСервере
Процедура ДобавитьСтрокуВТаблицуСформированныхДокументов(ДокументСсылка, Сумма, Валюта, Порядок, Состояние = 0, Печать = Ложь)
...
Функция ПровестиОтменитьПроведениеДокументовСервер(ВыделенныеДокументы, Проведение = Истина)
	...
	Попытка
		...
		Если Отказ Тогда
			...
			ИмяСобытия = НСтр("ru = 'Проведение документов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ОбновитьТекстИнформационнойНадписиФормируемыеДокументы()
	...
	ИначеЕсли Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиента Тогда
		...
		ДобавитьТекстНадписиВМассив(МассивНадписей, НСтр("ru='Заказ'"), "%СуммаИтог%", СуммаЗаказа, СоздаватьСчетНаоплату);
	...
	ИначеЕсли ЭтоРеализация(Объект.ВариантОформленияДокументов) Тогда
		...
		Если Объект.Товары.Количество() = 0 Тогда
			Если Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг Тогда
				...
				ДобавитьТекстНадписиВМассив(МассивНадписей, НСтр("ru='Заказ'"), "%СуммаЗаказа%", СуммаЗаказа,
					СоздаватьСчетНаоплату);
			Иначе
				СоздаватьСчетНаоплату = Объект.СоздаватьСчетНаОплату
										И Объект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным;
				
				ДобавитьТекстНадписиВМассив(МассивНадписей, НСтр("ru='Реализация'"), "%СуммаЗаказа%", СуммаЗаказа,
					СоздаватьСчетНаоплату);
			КонецЕсли;
		КонецЕсли;
		...
		Если ТребуетсяОформлениеЗаказа Тогда
			...
			ДобавитьТекстНадписиВМассив(МассивНадписей, НСтр("ru='Заказ'"), "%СуммаЗаказа%", СуммаЗаказа, СоздаватьСчетНаоплату);
		КонецЕсли;
		...
		Если ТребуетсяОформлениеРеализации Тогда
			...
			ДобавитьТекстНадписиВМассив(МассивНадписей, НСтр("ru='Реализация'"), "%СуммаРеализации%", СуммаРеализации,
				СоздаватьСчетНаоплату);
		КонецЕсли;
		...
		Если ТребуетсяОформлениеАкта Тогда
			...
			ДобавитьТекстНадписиВМассив(МассивНадписей, НСтр("ru='Акт'"), "%СуммаАкта%", СуммаАкта, СоздаватьСчетНаоплату);
		КонецЕсли;
...
Процедура ЗаполнитьУсловияПродажПоУмолчанию()
	...
	ИначеЕсли Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.КоммерческоеПредложение Тогда
...
Процедура ПерезаполнитьЭтапыОплатыПриИзмененииСоглашения(ПредыдущийГрафикОплаты)
	...
	ГрафикЗаполнен           = ИспользоватьГрафикиОплаты
								И ЗначениеЗаполнено(Объект.ГрафикОплаты)
								И ПредыдущийГрафикОплаты <> Объект.ГрафикОплаты;
	ГрафикСоглашенияЗаполнен = ПродажиВызовСервера.ГрафикСоглашенияЗаполнен(Объект.Соглашение);
	
	Если ГрафикЗаполнен
		Или ГрафикСоглашенияЗаполнен Тогда
...
Процедура ЗаполнитьЭтапыОплатыСервер(ГрафикСоглашенияЗаполнен, ГрафикЗаполнен)
	...
	ЭтапыОплатыСервер.ЗаполнитьЭтапыОплатыДокументаПродажи(Объект,
	...
	Если Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.ЗаказКлиентаРеализацияТоваровУслуг
		И Объект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным
		ИЛИ Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг Тогда
		
		ЕстьПредоплата = ИспользоватьСтатусыРеализацийТоваровУслуг
						И Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг;
		
		ЭтапыОплатыСервер.СвернутьЭтапыОплаты(Объект.ЭтапыГрафикаОплаты, ЕстьПредоплата);
...
Процедура ВариантОформленияДокументовПриИзмененииСервер()
	...
	Элементы.ОформленныеДокументыПечать.Видимость = 
	         Объект.ВариантОформленияДокументов <> Перечисления.ВариантыОформленияДокументовПродажи.КоммерческоеПредложение;
...
Процедура УстановитьВариантОформленияДокументовПродажиПоУмолчанию()
	...
	ИначеЕсли Объект.СоздаватьКоммерческоеПредложение
...
Процедура СоглашениеПриИзмененииСервер()
	...
	ПоСоглашениюИспользуютсяДоговорыКонтрагентов = ЗначениеЗаполнено(Объект.Соглашение) И Обработки.ПомощникПродаж.ЗначениеРеквизитаОбъектаТипаБулево(Объект.Соглашение, "ИспользуютсяДоговорыКонтрагентов");
...
Функция СтруктураОснованияДляПечати()
	...
	Если ЭтоОперацияПередачи(Объект.ХозяйственнаяОперация) Тогда
		ТекстПередачи = НСтр("ru='на комиссию'");
		
		Если ЗначениеЗаполнено(СтруктураОснования.Основание) Тогда
			СтруктураОснования.Основание = СтруктураОснования.Основание + ", " + НСтр("ru='передача %ТекстПередачи%'");
		Иначе
			СтруктураОснования.Основание = НСтр("ru='Передача %ТекстПередачи%'");
		КонецЕсли;
		
		СтруктураОснования.Основание = СтрЗаменить(СтруктураОснования.Основание, "%ТекстПередачи%", ТекстПередачи);
	КонецЕсли;
...
Процедура УправлениеСозданиемТранспортныхНакладных()
	...
	СоздаватьТранспортнуюНакладную = ИспользоватьТТН
										И Объект.СоздаватьТранспортнуюНакладнуюПоУмолчанию;
...
Процедура УправлениеТранспортнымиНакладными()
	...
	Если СтрокаТранспортнаяНакладная <> Неопределено
		И (НЕ СоздаватьТранспортнуюНакладную
			ИЛИ НЕ ДокументПроведен
			ИЛИ ДокументПомеченНаУдаление) Тогда
		
		Объект.Документы.Удалить(СтрокаТранспортнаяНакладная);
		
	ИначеЕсли СтрокаТранспортнаяНакладная = Неопределено
		И ДокументОснование <> Неопределено
		И СоздаватьТранспортнуюНакладную
		И ДокументПроведен
		И НЕ ДокументПомеченНаУдаление Тогда
		
		ЗаписатьТранспортнуюНакладную(Новый Массив, ДокументОснование, Ложь);
...
Функция ОплатаУказываетсяВРеализации(ВариантОформления, ПорядокРасчетов)
	...
	Возврат ВариантОформления <> ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.КоммерческоеПредложение")
...
Процедура УстановитьЗаголовокРеквизитовПечати()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАктыНаПередачуПрав") Тогда
		ЗаголовокЭлемента = НСтр("ru = 'Реквизиты печати ТОРГ-12 / Акта на передачу прав'");
		//++ НЕ НЕ УТ
		ЗаголовокЭлемента = ?(Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи,
								НСтр("ru = 'Реквизиты печати ТОРГ-12'"),
								НСтр("ru = 'Реквизиты печати ТОРГ-12 / Акта на передачу прав'"));
		//-- НЕ НЕ УТ
		
		Элементы.РеквизитыПечатиТОРГ12.Заголовок = ЗаголовокЭлемента;
	КонецЕсли;
...
Процедура СкрытьНедоступныеДополнительныеРеквизиты()
	...
	Если НЕ ЕстьДопРеквизитыЗаказа Тогда
		...
		МассивЭлементов.Добавить("ДополнительныеРеквизитыЗаказКлиента2");
	КонецЕсли;
...
Процедура УстановитьДоступностьКомандОбеспечения()
	...
	Элементы.ЗаполнитьДатуОтгрузки.Видимость                  = ВариантОформленияНеКоммерческоеПредложение;
