...
Процедура ЗаполнитьМесяцИсправления() Экспорт
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Пока ТекМесяц <= КонецТекущегоМесяца Цикл
			...
			ТекстЗапросаПериод = СтрЗаменить(ТекстЗапросаПериод, "ШаблонПериод", "Период" + Формат(ТекМесяц, "ДФ=yyyyMM"));
...
Процедура СформироватьНовыеРезервы(ПоНовымНастройкам = Ложь) Экспорт
	...
	Для Каждого Организация Из Организации Цикл
		...
		ДокументОбъект.Вставить("Движения", Новый Массив);
		ДокументОбъект.Движения.Добавить("ТоварыОрганизаций");
		ДокументОбъект.Движения.Добавить("РезервыТоваровОрганизаций");
		...
		ДокументОбъект.ДополнительныеСвойства.ДляПроведения.Вставить("СтруктураВременныеТаблицы", Новый Структура);
		ДокументОбъект.ДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.Вставить("МенеджерВременныхТаблиц",
		...
		ДокументОбъект.ДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаТоварыОрганизаций",
																			ТаблицаТоварыОрганизаций);
		...
		Попытка
			...
			Если ДокументОбъект.ДополнительныеСвойства.Свойство("ТаблицаРезервыТоваровОрганизаций")
				И ДокументОбъект.ДополнительныеСвойства.ТаблицаРезервыТоваровОрганизаций.Количество() Тогда
				...
				КорректировкаРегистров.Комментарий = СтрЗаменить(ШаблонКомментария,
																"%ТекущаяДатаСеанса%",
				...
				Набор.Загрузить(ДокументОбъект.ДополнительныеСвойства.ТаблицаРезервыТоваровОрганизаций);
...
Процедура ЗаполнитьТребуетсяПереформированиеРезервов() Экспорт
	...
	ЗапросОрганизацииИсточники				= ЗапросОрганизацииИсточники(МенеджерВременныхТаблиц,
																		Истина,
																		Истина,
																		Истина,
																		Ложь);
	...
	Пока ВыборкаОрганизацииСОтрицательнымОстатком.Следующий() Цикл
		...
		ЗапросОрганизацииИсточники.УстановитьПараметр("Организация",
														ВыборкаОрганизацииСОтрицательнымОстатком.Организация);
		ЗапросОрганизацииИсточники.УстановитьПараметр("ТипЗапасов",
														ВыборкаОрганизацииСОтрицательнымОстатком.ТипЗапасов);
...
Процедура СформироватьРезультатКонтроляОформленияДокументовТовародвижения(АдресДопПараметров, СДетализациейПоПозициям) Экспорт
	...
	КомпоновкаДанныхКлиентСервер.УстановитьПараметр(
		ОтчетКонтроля.КомпоновщикНастроек, "РежимПомощникаИсправленияОстатковТоваровОрганизаций", Истина);
	...
	ДопПараметры = Новый Структура(
		"Схема, ДанныеРасшифровки", 
		СхемаРезультатаКонтроляОформления, 
		ДанныеРасшифровкиРезультатаКонтроляОформления);
...
Процедура ДобавитьОтборыКонтроляОформленияДокументов(КомпоновщикНастроек)
	...
	ГруппаИли = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(
		КомпоновщикНастроек.Настройки.Отбор.Элементы, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
	Для Каждого Стр Из ОтборыДляФормированияОтчетаКонтроля Цикл
		ГруппаИ = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(
			ГруппаИли.Элементы, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
...
Процедура ЗаполнитьОтборыДляФормированияОтчетаКонтроля()
	...
	Если РезультатЗапроса.Пустой() Тогда
		ЗаполнитьЗначенияСвойств(ОтборыДляФормированияОтчетаКонтроля.Добавить(),
			Новый Структура(
				"Номенклатура, Характеристика, 	Серия, 			Склад, 			Назначение",
...
Функция ЗапросОрганизацииИсточники(
	...
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст, "ВЫБРАТЬ ПЕРВЫЕ 1", ?(ТолькоПервыйИсточник, "ВЫБРАТЬ ПЕРВЫЕ 1", "ВЫБРАТЬ"));
...
Функция ЗапросИсточникиПокрытияРазвернутогоСальдо(МенеджерВременныхТаблиц)
	...
	Запрос.УстановитьПараметр("ТипыЗапасовВнешнийВладелец",
								Перечисления.ТипыЗапасов.ТипыЗапасовВнешнийВладелец());
