...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Константы.КонтролироватьОстаткиТоваровОрганизаций.Получить() 
		И Не ПраваПользователяПовтИсп.ПропускКонтроляТоваровОрганизацийПриОтменеПриходов() Тогда
		СтандартнаяОбработка = Ложь;
		Отказ = Истина;
		ТекстСообщения = 
			НСтр("ru = 'Для работы с помощником требуется роль ""%ПропускКонтроляТоваровОрганизацийПриОтменеПриходов%"" или отключение функциональной опции ""%КонтролироватьОстаткиТоваровОрганизаций%"". Обратитесь к администратору.'");
		ТекстСообщения = СтрЗаменить(
			ТекстСообщения, "%ПропускКонтроляТоваровОрганизацийПриОтменеПриходов%", 
			Метаданные.Роли.ПропускКонтроляТоваровОрганизацийПриОтменеПриходов.Синоним);
		ТекстСообщения = СтрЗаменить(
...
Процедура ИнициализироватьПомощник()
	...
	АдресДополнительныхПараметровРезультатаКонтроляОформления = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
...
Процедура ОткрытьСписокСформированныхКорректировок(Команда)
	...
	Для Каждого Стр Из Объект.КорректировкиРезервов Цикл
		ПараметрыОткрытияФормы.Отбор.Ссылка.Добавить(Стр.Документ);
	КонецЦикла;
...
Процедура ОформитьОприходование(Команда)
	...
	Для Каждого СтрДанныеЗаполнения Из СтрокиДанныеЗаполнения Цикл
		...
		ПараметрыОткрытияФормы.Основание.Товары.Добавить(СтрТовары);
	КонецЦикла;
	ОткрытьФорму(
...
Процедура НастроитьЭлементыФормы()
	...
	БылаПроблема = Объект.КоличествоПозицийОтрицательныйОстатокБыло
		Или Объект.КоличествоПозицийРазвернутоеСальдоБыло;
	...
	Если НомерСтраницы = 0 Тогда
		...
		Элементы.НавигацияПерейтиВНачало.Заголовок = "Начать";
	...
	Если НомерСтраницы = 4 Тогда
		...
		Элементы.ГруппаШаг4СостояниеПроблема.Видимость = Объект.ТребуетсяИсправлениеРазвернутогоСальдо;
		Элементы.ГруппаШаг4СостояниеНетПроблемы.Видимость = Не Объект.ТребуетсяИсправлениеРазвернутогоСальдо;
		Элементы.ДекорацияШаг4Пояснение1.Видимость = Не Объект.ТребуетсяИсправлениеРазвернутогоСальдо;
		Элементы.ДекорацияШаг4Пояснение2.Видимость = Объект.ТребуетсяИсправлениеРазвернутогоСальдо;
	...
	Если НомерСтраницы = 7 Тогда
		...
		ЗаголовокОтчета = 
			СтрЗаменить(ЗаголовокОтчета, "%МесяцИсправления%" ,НРег(Формат(Объект.МесяцИсправления, "ДФ='MMMM yyyy'")));
...
Процедура ЗавершитьФормированиеРезультатаКонтроляОформления(
	...
	ДополнительныеПараметры = ПолучитьИзВременногоХранилища(АдресДопПараметров);
