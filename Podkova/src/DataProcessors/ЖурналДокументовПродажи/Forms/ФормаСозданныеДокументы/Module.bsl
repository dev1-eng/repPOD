...
Функция УдалитьСозданныеДокументы()
	...
	ПрепятствующиеУдалению = Неопределено;
	...
	Попытка
		УдалитьОбъекты(МассивПомеченныхНаУдаление, Истина, ПрепятствующиеУдалению,,,МассивИсключений);
	Исключение
	...
	Если ПрепятствующиеУдалению.Колонки.Количество() < 3 Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок,
			НСтр("ru = 'Не удалось выполнить удаление документов.'"),
			ОписаниеОшибки(), 
			Неопределено);
	...
	Для Каждого СтрокаТаблицы Из ПрепятствующиеУдалению Цикл
		...
		Если МассивИсключений.Найти(СтрокаТаблицы[2]) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
			
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрСообщения, СокрЛП(СтрокаТаблицы[0]), СокрЛП(СтрокаТаблицы[1]));
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок,
			"Объект.СписокСозданныеДокументы",
			ТекстОшибки, 
			Неопределено);
			
		Индекс = МассивПомеченныхНаУдаление.Найти(СтрокаТаблицы[0]);
