...
Процедура ОформитьПродажуНаКлиентеЗавершение(Результат, ДопПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		Если ДопПараметры = Неопределено Тогда // фоновое задание отменено.
			Возврат;
		КонецЕсли;
	...
	Если ПараметрыСозданныхДокументов.Свойство("Параметры") Тогда
		...
		КонецЕсли;
		Если ПараметрыСозданныхДокументов.Параметры.Свойство("ЕстьОшибкиСозданияДокументов")
			И ПараметрыСозданныхДокументов.Параметры.ЕстьОшибкиСозданияДокументов Тогда
			ЕстьОшибкиСозданияДокументов = Истина;
		ИначеЕсли ПараметрыСозданныхДокументов.Параметры.Свойство("Основание") 
				  И ПараметрыСозданныхДокументов.Параметры.Основание.Свойство("ЕстьОшибкиСозданияДокументов")
				  И ПараметрыСозданныхДокументов.Параметры.Основание.ЕстьОшибкиСозданияДокументов = Истина Тогда
			ЕстьОшибкиСозданияДокументов = Истина;
		КонецЕсли; 
	КонецЕсли;
	...
	ИначеЕсли ПараметрыСозданныхДокументов.Свойство("ИмяФормы") И
			(НЕ НеОткрыватьФормуСозданногоДокумента ИЛИ 
			(НеОткрыватьФормуСозданногоДокумента И ЕстьОшибкиСозданияДокументов)) Тогда
	...
	ИначеЕсли ПараметрыСозданныхДокументов.Свойство("РежимПечатиДокументов") Тогда
		...
		Для Каждого ТекСтрока Из СозданныеДокументы Цикл
			...
			Если НЕ (ТекЭлемент.Проведен И НеОткрыватьФормуСозданногоДокумента) Тогда
...
Функция ОформитьРядНакладныхПоТаблицам(СтруктураПараметров)
	...
	// не удается передавать адрес в хранилище, поэтому передаем исходую таблицу значений и заботимся о том,
	// чтобы она не попала обратно на клиент.
	
	УдалитьИзВременногоХранилища(АдресХранилища);
	СтруктураПараметровДляПередачиВФон = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(СтруктураПараметров);
...
Функция ОформитьЧерезФормуПерезаполнения(ДокументыПоТипамНакладных, ПоОрдерам, СписокОшибок)
	...
	МетаданныеНакладной = МетаданныеНакладнойПоОперации(ВыделеннаяСтрока.ХозяйственнаяОперация);
	...
	Иначе
		...
		Если СоздаватьДокументПродажи Тогда
			...
			Для каждого Элемент Из СозданныеДокументы Цикл
				Если Элемент.Значение.Свойство("ЕстьОшибки") И Элемент.Значение.ЕстьОшибки = Истина Тогда
					ЕстьОшибки = Истина;
					Прервать;
				КонецЕсли; 
			КонецЦикла; 
			...
			Если НеОткрыватьФормуСозданногоДокумента И НЕ ЕстьОшибки Тогда
...
&НаСервере
Функция МетаданныеНакладнойПоОперации(ОперацияРаспоряжения)
