...
Процедура ОформитьАктОРасхождениях(Команда)
	...
	Если МассивТиповДокументПоступления.Найти(ТипЗнч(ТекущиеДанные.Ссылка)) = Неопределено Тогда
...
Процедура ОформитьПоПриемке(Команда)
	...
	Если ТекущиеДанные.СостояниеОрдера = 1 
		И ХозОперацииПоступленияТоваров.Найти(ТекущиеДанные.ХозяйственнаяОперацияПриемки) = Неопределено 
		И ТипЗнч(ТекущиеДанные.Ссылка) <> Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
		
		Основание = Новый Структура("ДокументОснование, СкладПоступления", ТекущиеДанные.Ссылка, ТекущиеДанные.Склад);
	...
	ИначеЕсли ХозОперацииПоступленияТоваров.Найти(ТекущиеДанные.ХозяйственнаяОперацияПриемки) <> Неопределено Тогда
		...
		Если ОпределитьНаличиеОформленныхНакладных(ТекущиеДанные.Ссылка,
			ТекущиеДанные.Склад,
		...
		Иначе
			...
			ОткрытьФорму("Документ.ПоступлениеТоваровНаСклад.ФормаОбъекта", ПараметрыЗаполнения, ЭтаФорма);
		КонецЕсли;
...
Процедура ОформитьПоступлениеТоваров(Команда)
	...
	ИначеЕсли ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.ПоступлениеТоваровНаСклад") Тогда
...
Процедура ОформитьПриобретениеТоваровПоРаспоряжению(Команда)
	...
	Если ТипЗнч(ТекущиеДанные.Ссылка) = Тип("СправочникСсылка.СоглашенияСПоставщиками")
		...
		Если ТекущиеДанные.СостояниеПоступленияПриобретения = 3
				И ОпределитьНаличиеОформленныхНакладных(ТекущиеДанные.Ссылка,
					ТекущиеДанные.Склад,
					ТекущиеДанные.ХозяйственнаяОперация,
					Ложь,
					Ложь,
					Истина) Тогда
			ОткрытьФормуПереоформлениеНакладных(ТекущиеДанные.Ссылка, ТекущиеДанные.Склад, ТекущиеДанные.ХозяйственнаяОперация, Истина);
...
Функция ОпределитьНаличиеОформленныхНакладных(РаспоряжениеСсылка,
	...
	|	И ТИПЗНАЧЕНИЯ(ТаблицаЗаказы.Регистратор) В (
	...
	|		ИЛИ ДокументПоступления.Договор = &РаспоряжениеСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
&НаКлиенте
Процедура ОткрытьФормуПереоформлениеНакладных(РаспоряжениеСсылка, Склад, ХозяйственнаяОперация, ЭтоОформлениеПриобретенияПоПоступлениям = Ложь)
	...
	Если ТипЗнч(РаспоряжениеСсылка) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		...
		ИначеЕсли ХозОперацииПоступленияТоваров.Найти(ХозяйственнаяОперация) <> Неопределено Тогда
			СформироватьДанныеЗаполненияПоступленияТоваров(МассивСсылок, РеквизитыШапки, ХозяйственнаяОперация, ЭтоОформлениеПриобретенияПоПоступлениям);
		Иначе
			ЗакупкиВызовСервера.СформироватьДанныеЗаполненияПоступления(МассивСсылок, "ПриобретениеТоваровУслуг", РеквизитыШапки);
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(РаспоряжениеСсылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		РеквизитыШапки = РеквизитыШапкиДоговора(РаспоряжениеСсылка);
	Иначе
...
Функция РеквизитыШапкиДоговора(РаспоряжениеСсылка)
	...
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РаспоряжениеСсылка, ОсновныеРеквизитыДоговора);
	
	ХозОперацииПоступленияТоваров = ХозОперацииПоступленияТоваров();
	Если ХозОперацииПоступленияТоваров.Найти(РеквизитыДоговора.ХозяйственнаяОперация) <> Неопределено Тогда
		ПарыОпераций = ЗакупкиСервер.ПарыОперацийРаздельнойЗакупки(Истина);
		РеквизитыДоговора.ХозяйственнаяОперация = ПарыОпераций[РеквизитыДоговора.ХозяйственнаяОперация];
	КонецЕсли;
...
&НаСервере
Процедура СформироватьДанныеЗаполненияПоступленияТоваров(МассивСсылок,
	РеквизитыШапки,
	ХозяйственнаяОперация,
	ЭтоОформлениеПриобретенияПоПоступлениям = Ложь)
	
	ЗакупкиВызовСервера.СформироватьДанныеЗаполненияПоступления(МассивСсылок, "ПриобретениеТоваровУслуг", РеквизитыШапки);
	Если Не ЭтоОформлениеПриобретенияПоПоступлениям Тогда
		ПарыОпераций = ЗакупкиСервер.ПарыОперацийРаздельнойЗакупки(Истина);
		РеквизитыШапки.ХозяйственнаяОперация = ПарыОпераций[ХозяйственнаяОперация];
	КонецЕсли;
...
Функция НастройкиФормыПереоформленияНакладных(РаспоряжениеСсылка,
	...
	ХозОперацииПоступленияТоваров = ХозОперацииПоступленияТоваров();
	Если ХозОперацииПоступленияТоваров.Найти(ХозяйственнаяОперация) <> Неопределено
		И Не ЭтоОформлениеПриобретенияПоПоступлениям Тогда
	...
	Если ТипЗнч(РаспоряжениеСсылка) = Тип("СправочникСсылка.СоглашенияСПоставщиками")
		Или ТипЗнч(РаспоряжениеСсылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		НастройкиФормы.ДанныеЗаполнения =  Новый Структура("ДокументОснование, СкладПоступления", РаспоряжениеСсылка, Склад);
	КонецЕсли;
