...
Процедура ЗаполнитьПоЗаказамПоПриемке(ДанныеОтбора, Товары, СкладПоступления, ПоОрдеру,
	...
	Пока (ИндексСтрокиТовары+1) <= Товары.Количество()-1 Цикл
		...
		ИсходныеДанныеСтроки = Новый Структура();
		
		ИсходныеДанныеСтроки.Вставить("Количество", ТекущаяСтрокаТовары.Количество);
		ИсходныеДанныеСтроки.Вставить("КоличествоУпаковок", ТекущаяСтрокаТовары.КоличествоУпаковок);
		ИсходныеДанныеСтроки.Вставить("СуммаНДС", ТекущаяСтрокаТовары.СуммаНДС);
		ИсходныеДанныеСтроки.Вставить("Сумма", ТекущаяСтрокаТовары.Сумма);
		ИсходныеДанныеСтроки.Вставить("СуммаСНДС", ТекущаяСтрокаТовары.СуммаСНДС);
		ИсходныеДанныеСтроки.Вставить("СуммаРучнойСкидки", ТекущаяСтрокаТовары.СуммаРучнойСкидки);
		...
		Для каждого ПринятаяСтрока из НайденныеСтроки Цикл
			...
			Если ПринятаяСтрока.Количество > 0 Тогда
				...
				Если ЗначениеЗаполнено(СтрокаТовары.ЗаказПоставщику) 
					И СтрокаТовары.КоличествоВЗаказе >= ПринятаяСтрока.Количество
					И СтрокаТоварыКоличество = ПринятаяСтрока.Количество 
					И СтрокаТовары.ТипНоменклатуры = ПринятаяСтрока.ТипНоменклатуры Тогда
				...
				Если ПринятаяСтрока.Количество <= СтрокаТоварыКоличество Тогда
					...
					СтрокаТовары.КоличествоВЗаказе = ПринятаяСтрока.Количество;
				Иначе
				...
				ИсходныеДанныеСтроки.Количество = ИсходныеДанныеСтроки.Количество - СтрокаТовары.Количество;
				ИсходныеДанныеСтроки.КоличествоУпаковок = ИсходныеДанныеСтроки.КоличествоУпаковок - СтрокаТовары.КоличествоУпаковок;
				ИсходныеДанныеСтроки.СуммаНДС = ИсходныеДанныеСтроки.СуммаНДС - СтрокаТовары.СуммаНДС;
				ИсходныеДанныеСтроки.Сумма = ИсходныеДанныеСтроки.Сумма - СтрокаТовары.Сумма;
				ИсходныеДанныеСтроки.СуммаСНДС = ИсходныеДанныеСтроки.СуммаСНДС - СтрокаТовары.СуммаСНДС;
				ИсходныеДанныеСтроки.СуммаРучнойСкидки = ИсходныеДанныеСтроки.СуммаРучнойСкидки - СтрокаТовары.СуммаРучнойСкидки;
				...
				Если СтрокаТоварыКоличество > 0 И НайденныеСтроки.Найти(ПринятаяСтрока) <> НайденныеСтроки.ВГраница() Тогда
					...
					ЗаполнитьЗначенияСвойств(СтрокаТовары, ИсходныеДанныеСтроки);
				Иначе
					Прервать;
				КонецЕсли;	
...
Процедура ПерезаполнитьКоличествоПодобрано(СЗаполнением = Ложь, ТоварыНакладной = Неопределено)
	...
	СоставИсключаемыхПолей = "Количество, КоличествоУпаковок, Сумма, СуммаСНДС, СуммаНДС";
