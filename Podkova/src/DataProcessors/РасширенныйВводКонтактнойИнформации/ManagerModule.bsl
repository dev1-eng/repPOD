...
Функция ПредставлениеНаселенногоПункта(ОбъектАдреса) Экспорт
	...
	ИначеЕсли ЭтоМуниципальныйАдрес Тогда
		СписокПолейАдреса = "Area,MunDistrict,Settlement,City,CityDistrict,Locality,Territory";
	Иначе
	...
	Для каждого ИмяПоля Из СписокПолей Цикл
		...
		Если СтрСравнить(ИмяПоля, "MunDistrict") = 0 Тогда
			ОбъектАдреса[ИмяПоля] = РаботаСАдресамиКлиентСервер.КраткоеНаписаниеМуниципальныхРайонов(ОбъектАдреса[ИмяПоля], ОбъектАдреса[ИмяПоля + "Type"]);
		КонецЕсли;
		
		Если СтрСравнить(ИмяПоля, "District") = 0 Тогда
			ОбъектАдреса[ИмяПоля] = РаботаСАдресамиКлиентСервер.КраткоеНаписаниеПоселений(ОбъектАдреса[ИмяПоля], ОбъектАдреса[ИмяПоля + "Type"]);
		КонецЕсли;
		
		Если ОбъектАдреса.Свойство(ИмяПоля) И ЗначениеЗаполнено(ОбъектАдреса[ИмяПоля]) Тогда
			
			ПредставлениеУровня = УправлениеКонтактнойИнформациейКлиентСервер.СоединитьНаименованиеИТипАдресногоОбъекта(
				ОбъектАдреса[ИмяПоля], ОбъектАдреса[ИмяПоля + "Type"], СтрСравнить(ИмяПоля, "Area") = 0);
