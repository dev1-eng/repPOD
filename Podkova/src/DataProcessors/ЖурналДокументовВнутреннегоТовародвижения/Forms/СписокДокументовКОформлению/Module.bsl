...
Процедура ОформитьАктОРасхождениях(Команда)
	...
	Если ЗначениеЗаполнено(Накладные) Или ЗначениеЗаполнено(Заказы) Тогда
		...
		ПараметрыСозданныхДокументов = ОформитьАктОРасхожденияхНаСервере(Накладные, Заказы,
			ПараметрыФормыСозданныхДокументов(Истина), СписокОшибок, "Объект.СписокРаспоряженияНаОформление");
		НакладныеКлиент.ОткрытьФормуСозданныхДокументов(ПараметрыСозданныхДокументов, ЭтаФорма);
...
Функция ОформитьАктОРасхожденияхНаСервере(Накладные, Заказы, ПараметрыФормыСозданныхДокументов, СписокОшибок, ПолеОшибки)
...
Процедура СформироватьТекстыЗапросов()
	...
	ТекстЗапросаСпискаРаспоряжений =
		...
		|//&УсловиеНеобязательностиПриемки
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Если Не ДоступностьАктовОРасхождениях Тогда
		ТекстЗапросаСпискаРаспоряжений = СтрЗаменить(ТекстЗапросаСпискаРаспоряжений, "//&УсловиеНеобязательностиПриемки",
		"ГДЕ
		|	(НЕ ОстаткиСостояния.Ссылка ССЫЛКА Документ.ЗаказНаПеремещение
		|			И НЕ ОстаткиСостояния.Ссылка ССЫЛКА Документ.ПеремещениеТоваров)
...
Функция ПараметрыФормыСозданныхДокументов(ОформляетсяАктОРасхождении = Ложь)
	...
	ПараметрыФормы.СобытияОбновления.Добавить("Запись_ВнутреннееПотреблениеТоваров");
