...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ПараметрыФормы.Свойство("Ошибки", Ошибки) Тогда
		...
		Если ВидыОшибок.Количество() = 1 И ВидыОшибок[0] = ОбменСКонтрагентамиДиагностикаКлиентСервер.ВидОшибкиРаботаСФайлами() Тогда
			Элементы.ПояснениеПравая.Заголовок = НСтр("ru = 'работы с файлами'");
		Иначе 
			Элементы.ПояснениеПравая.Заголовок = СтрШаблон(НСтр("ru = 'с %1'"), ОбменСКонтрагентамиДиагностикаКлиентСервер.ПредставлениеОшибок(Ошибки));
		КонецЕсли;
		Если Не ЭтоКомплекснаяДиагностика И Ошибки.Количество()  > 0 Тогда
			УчетныеЗаписиМассив = ЭлектронноеВзаимодействиеОбработкаОшибокКлиентСервер.ЗначенияСвойствОшибок(Ошибки, "УчетнаяЗапись");
	...
	ДлительнаяОперация = ОбменСКонтрагентамиДиагностикаВызовСервера.ВыполнитьДиагностику(ПараметрыВыполненияДиагностики);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "МастерДиагностикиКриптографии_ПроверкаСертификата" Тогда
		
		Если ОбменСКонтрагентамиДиагностикаКлиент.ВсеПроверкиСертификатаВыполненыУспешно(Параметр.РезультатПроверки) Тогда
...
Процедура ДекорацияОшибкиСертификатаКриптографияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НРег(НавигационнаяСсылкаФорматированнойСтроки) = "посмотреть"
		...
		ОбменСКонтрагентамиДиагностикаКлиент.ОткрытьОшибкиСертификата(ЭтаФорма, ОшибкиСертификатов[0].Сертификат, ОшибкиСертификатов[0].Ошибки);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПояснениеНепривязанныеСертификатыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ГруппаРезультатНаличиеСертификатовРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ПосмотретьКорневойСертификатГУЦНажатие(Элемент)
	...
	ОбменСКонтрагентамиДиагностикаКлиент.ПосмотретьСертификат(ЕдинственныйСертификатГУЦ());
...
Процедура УстановитьКорневойСертификатГУЦНажатие(Элемент)
	...
	ОбменСКонтрагентамиДиагностикаКлиент.УстановитьКорневойСертификатГУЦ(ЕдинственныйСертификатГУЦ(), ЕдинственныйОтпечатокСертификатаГУЦ());
...
Процедура УчетнаяЗаписьКомплекснаяДиагностикаНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ОтмеченныеУчетныеЗаписи.Количество() Тогда
		ПоказатьЗначение(, ОтмеченныеУчетныеЗаписи[0].КлючЗаписи);
	КонецЕсли;
...
Процедура УчетныеЗаписиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФорму("РегистрСведений.УчетныеЗаписиЭДО.Форма.УчетнаяЗапись",
		Новый Структура("Ключ", Элемент.ТекущиеДанные.КлючЗаписи));
...
Процедура ОшибкиСертификатовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОбменСКонтрагентамиДиагностикаКлиент.ОшибкиСертификатовВыбор(ЭтаФорма, Элемент, Поле, Элементы.ГруппаОшибкиСертификатовКриптография);
...
Процедура КорневыеСертификатыГУЦВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.КорневыеСертификатыГУЦУстановить Тогда
		ОбменСКонтрагентамиДиагностикаКлиент.УстановитьСертификат(Элемент.ТекущиеДанные.АдресВоВременномХранилище);
	ИначеЕсли Поле = Элементы.КорневыеСертификатыГУЦПосмотреть Тогда
		ОбменСКонтрагентамиДиагностикаКлиент.ПосмотретьСертификат(Элемент.ТекущиеДанные.АдресВоВременномХранилище);
...
Процедура ОшибкиТестаАутентификацииВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ОшибкиТестаАутентификацииУчетнаяЗапись Тогда
		СтрокиТаблицыУчетныеЗаписи = УчетныеЗаписи.НайтиСтроки(Новый Структура("Идентификатор", Элемент.ТекущиеДанные.Идентификатор));
		Если СтрокиТаблицыУчетныеЗаписи.Количество() Тогда
			ПоказатьЗначение(, СтрокиТаблицыУчетныеЗаписи[0].КлючЗаписи);
		КонецЕсли;
...
Процедура ПовторитьДействие(Команда)
	...
	ЭтоПрограммноеЗакрытиеФормы = Истина;
	Закрыть(ОбменСКонтрагентамиДиагностикаКлиент.ДействиеПовторитьДействие());
...
Процедура ИзменитьПутьККаталогуВременныхФайлов(Команда)
	...
	#Если Не МобильныйКлиент Тогда
		...
		Если Не ЗначениеЗаполнено(ПутьККаталогуВременныхФайлов) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Не заполнен путь к каталогу временных файлов'"));
			Возврат;
		КонецЕсли;
		Оболочка = Новый COMОбъект("Wscript.Shell");
		Результат = Оболочка.RegWrite("HKEY_CURRENT_USER\Environment\TMP", ПутьККаталогуВременныхФайлов, "REG_EXPAND_SZ");
...
Процедура ПрименитьОтборПоУчетнымЗаписям(Команда)
	...
	Если КоличествоОтмеченныхУчетныхЗаписей(УчетныеЗаписи) = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не отмечено ни одной учетной записи для проведения диагностики'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьПомощникРегистрацииСертификатов(РегистрацияНепривязанных = Истина) 
	...
	Если РегистрацияНепривязанных Тогда
		ПараметрыФормы.Вставить("Операция", "РегистрацияНепривязанныхСертификатов");
		ПараметрыФормы.Вставить("Сертификаты", РезультатДиагностики.Результаты.Криптография.Результаты.НепривязанныеСертификаты.Сертификаты);
	Иначе 
		ПараметрыФормы.Вставить("ПрофилиНастроек", РезультатДиагностики.Результаты.Криптография.УчетныеЗаписиБезСертификатов);
		Если ПараметрыФормы.ПрофилиНастроек.Количество() = 0 Тогда
			ПараметрыФормы.ПрофилиНастроек = РезультатДиагностики.ДоступныеУчетныеЗаписи;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ПараметрыФормы.ПрофилиНастроек) Тогда
			Оповещение = Новый ОписаниеОповещения("ПослеВопросаОСозданииУчетнойЗаписи", ЭтотОбъект);
			ПоказатьВопрос(Оповещение, НСтр("ru = 'Отсутствуют доступные учетные записи, создать учетную запись?'"), РежимДиалогаВопрос.ДаНетОтмена);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	ОткрытьФорму("РегистрСведений.СертификатыУчетныхЗаписейЭДО.Форма.ПомощникРегистрацииСертификатов", ПараметрыФормы);
...
Процедура ПослеОткрытия()
	...
	ПараметрыВыполненияДиагностикиКопия.ОповещениеОЗавершении = Новый ОписаниеОповещения("ПослеВыполненияДиагностики", ЭтотОбъект);
	ПараметрыВыполненияДиагностикиКопия.ДлительнаяОперация    = ДлительнаяОперация;
	ОбменСКонтрагентамиДиагностикаКлиент.ПродолжитьВыполнениеДиагностики(ПараметрыВыполненияДиагностикиКопия);
...
Процедура ПослеВыполненияДиагностики(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатДиагностики.Результат И РезультатДиагностики.Результаты.Количество() = 1 Тогда
		...
		КонецЦикла;
		СоответствиеВидовДиагностикиВидамОшибок = ОбменСКонтрагентамиДиагностикаКлиент.СоответствиеВидовДиагностикиВидамОшибок();
...
Процедура ЗаполнитьУчетныеЗаписи(ИдентификаторыЭДО) 
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ЗначенияКлюча = Новый Структура("ИдентификаторЭДО", ВыборкаДетальныеЗаписи.ИдентификаторЭДО);
		КлючЗаписи = РегистрыСведений.УчетныеЗаписиЭДО.СоздатьКлючЗаписи(ЗначенияКлюча);
...
Процедура УправлениеФормой(РезультатДиагностики)
	...
	Элементы.ГруппаПояснениеКомплекснаяДиагностика.Видимость = ЭтоКомплекснаяДиагностика И КоличествоОтмеченныхУчетныхЗаписей > 0;
	...
	Для каждого КлючИЗначение Из РезультатДиагностики.Результаты Цикл
			...
			Если СвертываемаяГруппа <> Неопределено Тогда
				...
				Иначе 
					...
					Для каждого КлючИЗначениеВидДиагностики Из РезультатыВидаДиагностики Цикл
						...
						КонецЕсли;
					КонецЦикла;
					СвертываемаяГруппа.ЗаголовокСвернутогоОтображения = СвертываемаяГруппа.Заголовок + СтрШаблон(" (%1)", КоличествоОшибок);
				КонецЕсли;
			КонецЕсли;
		...
		Иначе 
			...
			Для каждого РезультатВидаДиагностики Из РезультатыВидаДиагностики Цикл
				...
				// Криптография начало
				Если ВидДиагностики = ОбменСКонтрагентамиДиагностикаКлиентСервер.ВидДиагностикиКриптография() Тогда
			...
			Если ВидДиагностики = ОбменСКонтрагентамиДиагностикаКлиентСервер.ВидДиагностикиИнтернетПоддержка() Тогда
			...
			Если ВидДиагностики = ОбменСКонтрагентамиДиагностикаКлиентСервер.ВидДиагностикиТестАутентификации() Тогда
...
Процедура УправлениеФормойКриптография(Знач РезультатыВидаДиагностики)
	...
	Для каждого РезультатВидаДиагностики Из РезультатыВидаДиагностики Цикл
		Если РезультатВидаДиагностики.Ключ <> "ПроверкиСертификатов" И РезультатВидаДиагностики.Значение.Результат = Ложь Тогда
	...
	Для каждого РезультатВидаДиагностики Из РезультатыВидаДиагностики Цикл
		Если РезультатВидаДиагностики.Ключ <> "НеполученныеСертификаты" И РезультатВидаДиагностики.Значение.Результат = Ложь Тогда
	...
	Если Не ОбменСКонтрагентамиДиагностика.ПараметрыКриптографии().ЕстьКриптографияНаКлиенте Тогда
		Элементы.ГруппаНаличиеКорневогоСертификатаГУЦ.Видимость = Ложь;
	КонецЕсли;
	
	Если ЕстьПроверкиКоторыеНеВыполнялись И Не ЕстьОшибкиПроверок И Не ЕстьОшибкиКриптографииКромеПроверокСертификатов Тогда
...
Процедура УправлениеФормойИнтернетСоединение(ВидПроверки, РезультатВидаДиагностики)
	...
	ДоступнаУстановкаСертификатов = ОбменСКонтрагентамиДиагностика.ДоступнаУстановкаСертификатов();
	...
	Если РезультатВидаДиагностики.Значение.Свойство("Сервисы", Сервисы) Тогда
		...
		Если ВыводитьСервисВЗаголовкеГруппы И КоличествоОшибок = 1 Тогда
			...
			ЭлементЗаголовокВидПроверки.Заголовок = СтрШаблон(ЗаголовкиПроверок[ВидПроверки].ЕдинственноеЧисло,
				ОбменСКонтрагентамиДиагностика.ПредставлениеСервисаЭДО(ПолныйАдресСервиса));
		Иначе 
			...
			Для каждого Сервис Из Сервисы Цикл
				Если Не Сервис.Результат Тогда
					...
					ГруппаСервис = Элементы.Добавить("ГруппаСервисы" + ПостфиксЭлементаФормы, Тип("ГруппаФормы"), ГруппаДляДобавленияСервисов);
					...
					ДекорацияКартинка = Элементы.Добавить("ДекорацияКартинкаСервис" + ПостфиксЭлементаФормы, Тип("ДекорацияФормы"), ГруппаСервис);
					...
					ДекорацияТекст = Элементы.Добавить("ДекорацияТекстСервис" + ПостфиксЭлементаФормы, Тип("ДекорацияФормы"), ГруппаСервис);
					...
					Если ВидПроверки = "НаличиеКорневыхСертификатов" И ДоступнаУстановкаСертификатов
						...
						ДекорацияТекст = Элементы.Добавить("УстановитьСертификат" + ПостфиксЭлементаФормы, Тип("ДекорацияФормы"), ГруппаСервис);
						ДекорацияТекст.Вид = ВидДекорацииФормы.Надпись;
						ДекорацияТекст.Заголовок = "Установить";
						ДекорацияТекст.Гиперссылка = Истина;
						ДекорацияТекст.Шрифт = Новый Шрифт(ДекорацияТекст.Шрифт,,, Истина);
						ДекорацияТекст.УстановитьДействие("Нажатие", "Подключаемый_УстановитьСертификатНажатие");
						
						ДекорацияТекст = Элементы.Добавить("ПосмотретьСертификат" + ПостфиксЭлементаФормы, Тип("ДекорацияФормы"), ГруппаСервис);
						ДекорацияТекст.Вид = ВидДекорацииФормы.Надпись;
						...
						Для каждого ДанныеСертификата Из Сервис.Сертификаты Цикл
							АдресСертификата = ПоместитьВоВременноеХранилище(ДанныеСертификата.Сертификат, УникальныйИдентификатор);
							Прервать;
						КонецЦикла;
...
Процедура УправлениеФормойРаботаСФайлами(РезультатДиагностики)
	...
	Если РезультатДиагностики.Результаты.Количество() = 1
		И РезультатДиагностики.Результаты.Свойство(ОбменСКонтрагентамиДиагностикаКлиентСервер.ВидДиагностикиРаботаСФайлами()) Тогда
...
Процедура УправлениеФормойТестАутентификации(РезультатыВидаДиагностики) 
	...
	КоличествоУчетныхЗаписейСНевыполненнымТестом = ОшибкиТестаАутентификации.НайтиСтроки(Новый Структура("ТестВыполнялся", Ложь)).Количество();
...
Процедура Подключаемый_УстановитьСертификатНажатие(Элемент) 
	...
	ОбменСКонтрагентамиДиагностикаКлиент.УстановитьСертификат(СертификатыСервиса(Элемент));
...
Процедура Подключаемый_ПосмотретьСертификатНажатие(Элемент) 
	...
	ОбменСКонтрагентамиДиагностикаКлиент.ПосмотретьСертификат(СертификатыСервиса(Элемент));
...
Процедура ПовторитьДиагностикуВыполнение() 
	...
	Если РезультатДиагностикиКриптографии <> Неопределено Тогда
		ПараметрыВыполненияДиагностикиКопия.СертификатыСУстановленнымиПаролями = РезультатДиагностикиКриптографии.СертификатыСУстановленнымиПаролями;
	КонецЕсли;
	...
	ОбменСКонтрагентамиДиагностикаКлиент.ВыполнитьДиагностику(ПараметрыВыполненияДиагностикиКопия);
