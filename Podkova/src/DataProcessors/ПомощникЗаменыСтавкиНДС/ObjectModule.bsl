...
Функция ТекстЗапроса(ТипыИзменяемыхОбъектов, ОграничитьВыборку = Ложь) Экспорт
	...
	Возврат СтрСоединить(Результат, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
	...
	Попытка
		...
		Для Каждого Ссылка Из ОбрабатываемыеОбъекты Цикл
			...
			Попытка
				...
				УчетНДСУП.СкорректироватьСтавкуНДС(ИзменяемыйОбъект.СтавкаНДС, '20190101');
