...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("Откуда",		ОтборОтправительАдресПредставление);
	Параметры.Свойство("Куда",			ОтборПолучательАдресПредставление);
...
Процедура СписокЗаказовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "СписокДокументОснованиеПредставление" 
		И ЗначениеЗаполнено(ТекущиеДанные.ДокументОснование) Тогда 
			...
			ТипДокумента = ТипЗнч(ТекущиеДанные.ДокументОснование);
			
			ПараметрыОткрытияФормы = Новый Структура();
			ПараметрыОткрытияФормы.Вставить("Ключ", ТекущиеДанные.ДокументОснование);
			
			ОткрытьФорму(СтроковоеПредставлениеТипа(ТипДокумента) + ".ФормаОбъекта",
...
Функция СтроковоеПредставлениеТипа(Тип)
	Возврат СтрЗаменить(ОбщегоНазначения.СтроковоеПредставлениеТипа(Тип), "Ссылка", "");
КонецФункции
...
Процедура УстановитьЗначенияПоУмолчанию()
	...
	Если Настройки.Получить("ОтборРольОрганизации") = Неопределено Тогда
		ОтборРольОрганизации = 0;
	Иначе
		ОтборРольОрганизации = Настройки.Получить("ОтборРольОрганизации");
	КонецЕсли;
	
	Если Настройки.Получить("ОтборСостояние") = Неопределено Тогда
		ОтборСостояние = 0;
	Иначе
		ОтборСостояние = Настройки.Получить("ОтборСостояние");
	КонецЕсли;
	
	Если Настройки.Получить("ПериодДатаСоздания") = Неопределено Тогда
		ПериодДатаСоздания.Вариант = ВариантСтандартногоПериода.Месяц;
	Иначе
		ПериодДатаСоздания = Настройки.Получить("ПериодДатаСоздания");
	КонецЕсли;
	
	Если Настройки.Получить("РазмерСтраницы") = Неопределено Тогда
		РазмерСтраницы = 20;
	Иначе
		РазмерСтраницы = Настройки.Получить("РазмерСтраницы");
	КонецЕсли;
...
Процедура ОбновитьНавигационноеМеню()
	...
	КолвоСтраницНадпись = ?(Страницы.КоличествоСтраниц=1,1,Страницы.КоличествоСтраниц);
	Элементы.НавигацияСтраницаТекущаяСтраница.Заголовок =
		СтрШаблон(
			НСтр("ru = 'Страница %1 из %2'"),
				Страницы.Страница, КолвоСтраницНадпись);
...
Процедура СоздатьЗаказНаДоставку()
	...
	ПараметрыОткрытияФормы.Вставить("ДокументОснование");
	ПараметрыОткрытияФормы.Вставить("ТипГрузоперевозки", ТипГрузоперевозки);
...
Процедура ОткрытьФормуЗаказа(ДанныеПоЗаказу)
	...
	ПараметрыОткрытияФормы.Вставить("ДокументОснование", ДанныеПоЗаказу.ДокументОснование);
	ПараметрыОткрытияФормы.Вставить("ТипГрузоперевозки", ТипГрузоперевозки);
