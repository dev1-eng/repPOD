...
Процедура Загрузить(Команда)
	...
	Если ЕстьОтмеченныеПечатныеФормы() Тогда 
...
Процедура НастроитьФормуПоТипуГрузоперевозки()
	...
	Если ТипГрузоперевозки = 1 Тогда
		Заголовок = НСтр("ru = '1C:Доставка: Печатные формы'");
	Иначе
		Заголовок = НСтр("ru = '1C:Курьер: Печатные формы'");
	КонецЕсли;
...
Функция ЕстьОтмеченныеПечатныеФормы()
	...
	Для Каждого ПечатнаяФорма Из Список Цикл
...
Процедура ВыполнитьЗапросЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		СостояниеВыполненияЗапроса = НСтр("ru = 'Выберите требуемые печатные формы'");
	КонецЕсли;
...
Функция ПараметрыЗапросаПолучитьФайлыПечатныхФорм(ПараметрыОперации, Отказ)
	...
	ПараметрыЗапроса.ИдентификаторЗаказа = ИдентификаторЗаказа;
	...
	Для Каждого ТекущаяСтрока Из Список Цикл
		Если ТекущаяСтрока.Пометка Тогда
			...
			ПараметрыЗапроса.Список.Добавить(ПараметрыФормы);
		КонецЕсли;
	КонецЦикла;
			...
			Если Результат.Свойство("Список") Тогда
				Если ТипЗнч(Результат.Список) = Тип("Массив") Тогда
					Для Каждого ТекущийФайл Из Результат.Список Цикл
						ИмяФайла = ИдентификаторЗаказа + " " + ТекущийФайл.Наименование + "." + ТекущийФайл.Расширение;
						...
						Если НРег(ТекущийФайл.Кодировка) = "base64" Тогда
							...
							Исключение
								...
								ОбщегоНазначенияКлиент.СообщитьПользователю(
									СтрШаблон(НСтр("ru = 'Ошибка извлечения данных файла: %1'"),ИмяФайла));
...
Процедура УстановитьПометкуДляВсехЭлементовСписка(Пометка)
	...
	Для Каждого ТекущаяПечатнаяФорма Из Список Цикл
...
&НаКлиенте
Процедура ВыбратьКаталогЗагрузки()
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыбратьКаталогЗагрузкиЗавершение", ЭтотОбъект);
...
Процедура КопированиеФайлаЗавершение(ДополнительныеПараметры) Экспорт
	...
	Если ОткрытьПечатныеФормыПослеЗагрузки Тогда
		ФайловаяСистемаКлиент.ОткрытьФайл(ДополнительныеПараметры.ПутьКФайлу, ОписаниеОповещения);
	КонецЕсли;
