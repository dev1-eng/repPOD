...
Функция БалансовыеРегистры(ВключаяОнлайнРасчеты = Ложь) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей") ИЛИ ПолучитьФункциональнуюОпцию("БазоваяВерсия") Тогда
		...
		Исключения.Вставить("РезервыПоСомнительнымДолгам", Истина);
	КонецЕсли;
...
//
// Параметры:
//  ОбъектДокумента - ДокументОбъект - объект документа для которого необходимо сформировать и записать движения упр. баланса
//
...
Процедура ДобавитьТекстЗапросаРасчетыСКлиентамиПоСрокам(Запрос, ТаблицаДвижений)
	...
	|		ИНАЧЕ -СУММА(ВложенныйЗапрос.Сумма)
	|	КОНЕЦ КАК Сумма,
	...
	|		КОНЕЦ КАК Статья,
	|		ЗНАЧЕНИЕ(Перечисление.СтатьиБезАналитики.ПустаяСсылка) КАК Аналитика,
	|		СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.ДолгУпр ИНАЧЕ Т.ДолгУпр КОНЕЦ) КАК Сумма,
	|		СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.ДолгУпр ИНАЧЕ Т.ДолгУпр КОНЕЦ) КАК СуммаБаланса
	|
	|	ИЗ
	|		ПервичныеДвижения КАК Т
	...
	|		КОНЕЦ КАК Статья,
	|		ЗНАЧЕНИЕ(Перечисление.СтатьиБезАналитики.ПустаяСсылка) КАК Аналитика,
	|		СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА -Т.ПредоплатаУпр ИНАЧЕ Т.ПредоплатаУпр КОНЕЦ) КАК Сумма,
...
Процедура ДобавитьТекстЗапросаРасчетыСПоставщикамиПоСрокам(Запрос, ТаблицаДвижений)
	...
	|		КОНЕЦ КАК Статья,
	|		ЗНАЧЕНИЕ(Перечисление.СтатьиБезАналитики.ПустаяСсылка) КАК Аналитика,
	|		СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) ТОГДА -Т.ДолгУпр ИНАЧЕ Т.ДолгУпр КОНЕЦ) КАК Сумма,
	...
	|		КОНЕЦ КАК Статья,
	|		ЗНАЧЕНИЕ(Перечисление.СтатьиБезАналитики.ПустаяСсылка) КАК Аналитика,
	|		СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.ПредоплатаУпр ИНАЧЕ Т.ПредоплатаУпр КОНЕЦ) КАК Сумма,
	|		СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.ПредоплатаУпр ИНАЧЕ Т.ПредоплатаУпр КОНЕЦ) КАК СуммаБаланса
	|
	|	ИЗ
	|		ПервичныеДвижения КАК Т
...
Процедура ДобавитьТекстЗапросаУслугиКОформлениюОтчетовПринципалу(Запрос, ТаблицаДвижений)
	...
	|ИЗ
	|	втПериодыУслугиКОформлению КАК Периоды
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
	|		ПО Периоды.Валюта = КурсыВалют.Валюта
	|			И Периоды.Период = КурсыВалют.Период
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалютУпр
	...
	Запрос.Параметры.ТаблицыДвиженийНаправлений.Добавить(ИмяТаблицы);
	Запрос.Параметры.ВсеТаблицы.Добавить("втДанныеРегистраУслугиКОформлению");
	Запрос.Параметры.ВсеТаблицы.Добавить("втПериодыУслугиКОформлению");
	Запрос.Параметры.ВсеТаблицы.Добавить("втКурсыВалютУслугКОформлению");
	Запрос.Параметры.ВсеТаблицы.Добавить(ИмяТаблицы);
...
Процедура ВыгрузитьРасчетыПоСрокам(ДокументРегистратор, ТаблицыДвижений)
	...
	Пока ВидыРасчетов.Следующий() Цикл
		...
		Движения = РегистрыНакопления[ИмяРегистра].СоздатьНаборЗаписей().Выгрузить();
