...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВесЗаказа.Заголовок = СтрШаблон(НСтр("ru = 'Вес (%1)'"), ЕдиницаИзмеренияВеса);
	
	ВалютаУправленческогоУчета = Константы.ВалютаУправленческогоУчета.Получить();
	Элементы.СуммаЗаказа.Заголовок = СтрШаблон(НСтр("ru = 'Сумма (%1)'"), ВалютаУправленческогоУчета);
	...
	Если Параметры.Свойство("ОтборПоТипуОбеспечения") Тогда
...
&НаКлиенте
Процедура ЗаполнитьИсточникОбеспечения(Команда)
...
Процедура РассчитатьСтрокуТаблицыЗаказыКОбеспечениюНаКлиенте(СпособОбеспечения)
	...
	Отбор = Новый Структура("Номенклатура, Характеристика, Склад, Назначение, СпособОбеспечения,
		|Заказ, ДатаОтгрузки, Требуется, ПодразделениеПолучатель, ДатаОтгрузкиЗаказа, СкладОтгрузкиЗаказа");
	ЗаполнитьЗначенияСвойств(Отбор, Поставка);
	Отбор.СпособОбеспечения = СпособОбеспечения;
...
Процедура СпособОбеспеченияОчистка(Поставка, ВозможныРаботы)
	...
	Если Поставка.Отметка Тогда
		...
		ОтразитьПоставкуВКорзине(ЭтаФорма, Поставка, 0, ВозможныРаботы);
	...
	КоличествоКЗаказу = Поставка.Округлено * ?(Поставка.Отметка, 1, 0);
	ОтразитьПоставкуВКорзине(ЭтаФорма, Поставка, КоличествоКЗаказу, ВозможныРаботы);
...
Функция ВидимостьГруппаНастройкаФоновыхОпераций(Таблица)
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Если (Строка.Отметка И Строка.КЗаказу > 0 Или Строка.Требуется = Строка.Обеспечено)
			И Строка.РазмещатьЗаказыНаОтгрузку
...
Процедура СохранитьНастройки()
	...
	Настройки = Новый Структура("ФильтрНеобеспеченные, РазместитьЗаказыНаОтгрузкуВСформированныхЗаказахНаПоставку");
