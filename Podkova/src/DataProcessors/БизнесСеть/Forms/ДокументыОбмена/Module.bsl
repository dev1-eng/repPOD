...
Процедура ОткрытьПредставление(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		МассивДанныхДокументов = БизнесСетьВызовСервера.ПолучитьДанныеДокументаСервиса(
			ОтборОрганизация, МассивИдентификаторовДокументов, Не РежимИсходящихДокументов, УникальныйИдентификатор, Истина);
...
Процедура НастроитьПолеВыбораОрганизации()
	...
	Элементы.Организация.Видимость = ЭлектронноеВзаимодействиеСлужебный.ИспользуетсяНесколькоОрганизаций();
...
Процедура ВывестиЭДНаПросмотр(СтрокаДанных)
	...
	ПараметрыОткрытия.Организация              = ОтборОрганизация;
	...
	Для каждого СтрокаЗагрузки Из ЗагружаемыеДокументы Цикл
		...
		Для каждого ЭлементКоллекции Из НоменклатураСопоставленияПоДокументу Цикл
			...
			Если НоменклатураСопоставленияДокументов.НайтиСтроки(Отбор).Количество() = 0 Тогда 
				...
				НоваяЗапись.ДанныеНоменклатурыКонтрагента = ЭлементКоллекции.Значение;
