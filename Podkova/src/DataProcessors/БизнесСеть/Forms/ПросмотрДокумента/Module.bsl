	...
	Для Каждого ЭлементКоллекции Из СоответствиеНоменклатуры Цикл
		Если ЗначениеЗаполнено(ЭлементКоллекции.НоменклатураИБ) Тогда
			ОбменСКонтрагентами.УстановитьСоответствиеНоменклатуры(
				...
				ЭлементКоллекции.ИспользоватьХарактеристики);
		КонецЕсли;	
	КонецЦикла;
...
Процедура ОбновитьИнформациюОКонтрагенте(Контрагент, УчастникИНН, УчастникКПП, ДатаДокумента)
	...
	ОбменСКонтрагентамиПереопределяемый.СсылкаНаОбъектПоИННКПП("Контрагенты", УчастникИНН, УчастникКПП, Контрагент,, ДатаДокумента)
...
Процедура СформироватьДокументИБ(ДокументСсылка, ТекстСообщения, Записывать = Ложь,
	...
	Если Не Отказ Тогда
		БизнесСеть.УстановитьСтатусЗагружен(Организация, Идентификатор, ДокументСсылка, Отказ);
	КонецЕсли;
