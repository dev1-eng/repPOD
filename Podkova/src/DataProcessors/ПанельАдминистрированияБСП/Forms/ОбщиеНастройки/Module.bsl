...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьЭлектронныеПодписи" 
		...
		МассивВключенныхПодсистем = Новый Массив();
		
		Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП")
			И НаборКонстант.ВестиУчетМаркируемойПродукцииИСМП Тогда
			МассивВключенныхПодсистем.Добавить(НСтр("ru = '""Интеграция с ИС МП (обувь, одежда, табак...)""'"));
		КонецЕсли;
		
		Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ГИСМ")
			И НаборКонстант.ВестиУчетМаркировкиПродукцииВГИСМ Тогда
			МассивВключенныхПодсистем.Добавить(НСтр("ru = '""Интеграция с ГИСМ (изделия из меха)""'"));
		КонецЕсли;
		
		ЗависимостьЭлектронныеПодписи = (МассивВключенныхПодсистем.Количество() > 0);
		Элементы.ИспользоватьЭлектронныеПодписи.Доступность          = Не ЗависимостьЭлектронныеПодписи;
		Элементы.КомментарийИспользоватьЭлектроннуюПодпись.Видимость = ЗависимостьЭлектронныеПодписи;
		Элементы.ДекорацияИспользоватьЭлектроннуюПодпись.Видимость   = ЗависимостьЭлектронныеПодписи;
		
		Если ЗависимостьЭлектронныеПодписи Тогда
			
			СтрокиОбщийТекст = Новый Массив();
			СтрокиОбщийТекст.Добавить(НСтр("ru = 'Невозможно отключение электронной подписи, потому что включена'"));
			СтрокиОбщийТекст.Добавить(" ");
			СтрокиОбщийТекст.Добавить(НСтр("ru = 'маркировка товаров контрольными (идентификационными) знаками.'"));
			СтрокиОбщийТекст.Добавить(Символы.ПС);
			СтрокиОбщийТекст.Добавить(НСтр("ru = 'Отключить использование можно в'"));
			СтрокиОбщийТекст.Добавить(" ");
			Если МассивВключенныхПодсистем.Количество() > 1 Тогда
				СтрокиОбщийТекст.Добавить(НСтр("ru = 'разделах:'"));
			Иначе
				СтрокиОбщийТекст.Добавить(НСтр("ru = 'разделе:'"));
			КонецЕсли;
			СтрокиОбщийТекст.Добавить(" ");
			СтрокиОбщийТекст.Добавить(СтрСоединить(МассивВключенныхПодсистем, ", "));
			Элементы.КомментарийИспользоватьЭлектроннуюПодпись.Заголовок = Новый ФорматированнаяСтрока(СтрокиОбщийТекст);
