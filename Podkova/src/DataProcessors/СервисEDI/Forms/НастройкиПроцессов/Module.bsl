...
Процедура ИнициализироватьДанныеФормы()
	...
	Если ИспользуютсяСклады И ЭтоПолнаяИнтеграция Тогда
		...
		Если ДоступноАвтоматическоеЗаполнениеСклада Тогда
			СписокВыбораЗаполнениеСклада.ДоБавить(Перечисления.ВариантыОпределенияСкладаВДокументахEDI.ОпрелятьАвтоматически, ПредставлениеНастройкиАвтоматическогоЗаполненияСклада);
...
#Область СкладЗаказКлиента
	...
	ИначеЕсли Форма.ВариантОпределенияСкладаЗаказКлиента = ПредопределенноеЗначение("Перечисление.ВариантыОпределенияСкладаВДокументахEDI.ОпрелятьАвтоматически") Тогда
	...
	Если ТекущиеНастройкиИнтеграции.ПрефиксНомеровДокументовСервис <> ПрефиксНомеровДокументовСервис
		...
		Если РезультатОбновленияНастройки.Ошибка Тогда 
			...
			Иначе
				...
				ОтветственныйПоУмолчаниюДляНовыхЗаказовКлиента = РезультатПолученияНастроек.Данные.ОтвественныйЗаНовыеЗаказыКлиентовПоУмолчанию;
			КонецЕсли;
		Иначе
...
Процедура ОпределитьПодключенныеОрганизации()
	...
	Запрос.Текст = "
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СУММА(1) КАК КоличествоОрганизаций,
	|	СУММА(ВЫБОР
...
Процедура ЗаполнитьСписокВыбораОрганизаций()
	...
	Запрос.Текст              = СтрЗаменить(Запрос.Текст, "И &ТекстУсловияПоОрганзации", ТекстУсловияПоОрганизации);
