...
Процедура ОбновитьОтображениеФормы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НаименованиеАдресаДоставкидресаДоставки",
	...
	// Видимость даты поставки в шапке и табичной части
...
&НаСервере
Процедура ПриПолученииДанныхДокументаНаСервере(АдресРезультата)
	...
	ИначеЕсли ОтветСервиса.Ошибка Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаСтраницыФормы",
			"ТекущаяСтраница", Элементы.СтраницаОшибкаПолученияДанных);
		ОбщегоНазначения.СообщитьПользователю(ОтветСервиса.ТекстОшибки);
		Возврат;
	КонецЕсли;
...
Процедура ПриПолученииДанныхДокумента(РезультатВыполнения, ДополнительныеПараметры) Экспорт 
	...
	ПриПолученииДанныхДокументаНаСервере(РезультатВыполнения.АдресРезультата);
...
Процедура ОтправитьСообщениеWSS(ТекстЗаголовкаСообщения, ТекстСообщения)
	...
	СторокаСообщения = Новый Структура;
	
	СторокаСообщения = ТабличнаяЧастьСообщений.Добавить();
	СторокаСообщения.ИдентификаторСообщения   = ДанныеЗапроса.MessageID;
	СторокаСообщения.ИдентификаторОтправителя = ИдентификаторОрганизации;
	СторокаСообщения.Дата                     = ТекущаяДата();
	СторокаСообщения.Заголовок                = ДанныеЗапроса.Header;
	СторокаСообщения.Текст                    = ДанныеЗапроса.Text;
	СторокаСообщения.Статус                   = ДокументыEDIИнтеграцияКлиент.ИдентификаторСтатусаДоставкиОжидает();
	
	ВывестиСообщениеHTML(СторокаСообщения);
...
&НаКлиенте
Процедура ПослеЗагрузкиДокумента(РзультатЗагрузки, ДополнительныеПараметры) Экспорт
