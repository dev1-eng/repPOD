...
&НаСервере
Процедура УстановитьОтборыФормы() Экспорт
...
Процедура ПриОжиданииРезультатовОтправки()
	...
	Для Каждого ЭлементСоответствия Из РезультатОтправки Цикл
		...
		Если ДанныеСинхронизации.СостояниеВыполнения = ПредопределенноеЗначение("Перечисление.СостоянияСинхронизацииДокументовEDI.Выполняется") Тогда
			КоличестоВыполняется = КоличестоВыполняется + 1;
			Продолжить;
