...
Процедура ТаблицаДоступныхДействийТекущаяВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТаблицаДоступныхДействийТекущаяСтатусУказанияСерий"
		...
		ТекущееКоличествоПодобрано = ТекущиеДанные.Количество;
...
Процедура РаспределитьОстаток(Команда)
	ТекстСообщения = РаспределитьОстатокНаСервере();
...
Процедура ОбновитьКоличество()
	...
	Иначе
		...
		Если ТекущиеДанные <> Неопределено Тогда
			Количество = Макс(Количество, КоличествоПодобрано - ТекущиеДанные.Количество);
		...
		Если ТекущиеДанные <> Неопределено Тогда
			ТекущиеДанные.Количество = Мин(Количество + ТекущиеДанные.Количество  - КоличествоПодобрано,
				ТекущиеДанные.ДоступноТекущее + ТекущиеДанные.Количество);
...
Процедура НастроитьВидимостьНаСервере()
	...
	СтраницаСерии = (Элементы.СтраницыТоварыСерии.ТекущаяСтраница = Элементы.СтраницаСерии);
...
Процедура ЗаполнитьПараметрыПолученияТаблицыДоступныхДействий(ПараметрыПолученияТаблицыДоступныхДействий, ТипНоменклатуры)
	...
	Если СписокВыбора <> Неопределено Тогда
		...
		КонецЕсли;
		Если СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.ИзЗаказов) = Неопределено Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.ИзЗаказов);
		КонецЕсли;
		Если СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.Требуется) = Неопределено Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.Требуется);
		КонецЕсли;
		Если СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.СоСклада) = Неопределено Тогда
			ПараметрыПолученияТаблицыДоступныхДействий.ИсключаемыеВарианты.Добавить(Перечисления.ВариантыОбеспечения.СоСклада);
		КонецЕсли;
		Если СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.НеТребуется) = Неопределено Тогда
		...
		КонецЕсли;
		Если СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.Обособленно) = Неопределено Тогда
	...
	Если ОграничиватьВариантыОбеспечения Тогда
		...
		ТипНоменклатурыСтрока = ОбеспечениеКлиентСервер.ТипНоменклатурыСтрокой(ТипНоменклатуры);
			...
			Если ТолькоОбособленно И Отбор.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар Тогда
				Для Каждого Строка Из ТаблицаДоступныхДействийВызовИзФормыЗаказа Цикл
					Если Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно Или
						...
						Если ОтгружатьЕслиПоступилоПодНазначениеПолностью
							И Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно
							И Строка.Доступно < КоличествоТребуется Тогда
							Продолжить;
						КонецЕсли;
...
Процедура ДополнитьТаблицуДоступныхДействий()
	...
	Если ПодборВариантовОбеспечения
		...
		И (Не ТипЗнч(СписокВыбора) = Тип("СписокЗначений")
...
Процедура ОбновитьСписокВыбораДоступныхДействий()
	...
	Если СписокВыбора = Неопределено
		Или СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.Обособленно) <> Неопределено
		Или СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.Требуется) <> Неопределено
		Или СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.СоСклада) <> Неопределено
		Или СписокВыбора.НайтиПоЗначению(Перечисления.ВариантыОбеспечения.ИзЗаказов) <> Неопределено Тогда
...
Процедура ТаблицаДоступныхДействийТекущаяПодобратьКоличество(ИдентификаторТекущейСтроки)
	...
	Если (ТекущиеДанные.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно")
		...
		Если Не ПодборТоваров
			...
			Или ТекущиеДанные.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить)
			И ТекущийСклад = ТекущиеДанные.Склад Тогда
...
Процедура ОбновитьПодобранноеКоличесто()
	...
	Если Элементы.СтраницыТоварыСерии.ТекущаяСтраница = Элементы.СтраницаСерии Тогда
		КоличествоСерийПодобрано = 0;
		Для Каждого Строка Из ОстаткиСерийТекущие Цикл
			КоличествоСерийПодобрано = КоличествоСерийПодобрано + Строка.Количество;
		КонецЦикла;
		КоличествоПодобрано = КоличествоПодобрано - ТекущееКоличествоПодобрано + КоличествоСерийПодобрано; 
	КонецЕсли;
...
Функция РаспределитьОстатокНаСервере()
	...
	Если КоличествоКРаспределению <= 0 Тогда
		Возврат НСтр("ru = 'Все количество товара уже распределено по сериям.'");
	КонецЕсли;
	...
	Если КоличествоКРаспределению = 0 Тогда
		Возврат "";
	Иначе
		Возврат НСтр("ru = 'Распределено все доступное количество в выделенных строках, но его не достаточно для распределения всего количества товара.
