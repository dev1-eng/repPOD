...
Процедура ЗапуститьОперациюСбербанк(Отказ)
	...
	Если НЕ ЗапускалосьОбновлениеСертификатовБанка
			...
			И ОбменСБанкамиСлужебныйКлиент.ЗначениеИзКэшаСбербанк("ПолученКриптопрофильВСессии") <> Истина Тогда
...
Процедура ОбработатьРезультатЗапросаВыписки(РезультатЗадания,
	...
	Если РезультатЗадания.Статус = "Ошибка" Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, РезультатЗадания.ПодробноеПредставлениеОшибки, РезультатЗадания.КраткоеПредставлениеОшибки, "ОбменСБанками", НастройкаОбмена);
...
Процедура ПолучитьИзвещениеОСостоянииЭД(Результат,
	...
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, Результат.ПодробноеПредставлениеОшибки, Результат.КраткоеПредставлениеОшибки, "ОбменСБанками", НастройкаОбмена);
		ЗакрытьФормуНаКлиенте(Истина);
		Возврат;
	КонецЕсли;
...
Функция СохранитьВыписку(Знач ДанныеВыписки, Знач НастройкаОбмена, Знач ДатаНачала, Знач ДатаОкончания)
	...
	СтруктураРазбораФайла = ОбменСБанкамиСлужебный.СформироватьДеревоРазбора(
		Перечисления.ВидыЭДОбменСБанками.ВыпискаБанка, ДанныеВыписки.Выписка);
...
Процедура ПослеОтправкиЗапросаСтатусаЗапросаСостоянияСбербанк(Ответ,
	...
	Иначе
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ТекстОшибки, ТекстСообщения, "ОбменСБанками", НастройкаОбмена);
			ПриОшибкеТекущейОперацииСбербанк();
			ЗакрытьФормуНаКлиенте(Истина);
			Возврат;
		КонецПопытки;
...
Процедура ПолучитьКонечныеСтатусыЗапросовВыпискиПослеПроверкиПодписиСбербанк(
	...
	ЭлектронноеВзаимодействиеСлужебныйКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОжиданияКонечногоСтатусаЗапросовВыпискиСбербанк);
...
Функция ВыпискиПолученыСбербанк(Знач НастройкаОбмена, Знач МассивЗапросов,
	...
	Для каждого ЭлементКоллекции Из СтатусыЗапросов Цикл
		Если ЭлементКоллекции.Значение = Перечисления.СтатусыОбменСБанками.Обработан Тогда
			СтруктураВозврата.КоличествоВыписок = СтруктураВозврата.КоличествоВыписок
				+ 1;
		Иначе
...
Процедура ПослеПолученияИдентификатораКриптопрофиляСбербанк(Результат,
	...
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, Результат.ПодробноеПредставлениеОшибки, ТекстСообщения, "ОбменСБанками", НастройкаОбмена);
	Иначе // выполнено
...
Процедура ОбработатьОшибкуПолученияТомаДокументаЧерезТокенСбербанк(ИнформацияОбОшибке,
	...
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, ТекстСообщения, "ОбменСБанками", ДополнительныеПараметры.НастройкаОбмена);
...
&НаКлиенте
Процедура ПодписатьСообщенияОбменаРекурсивноСбербанк(Оповещение,
		СообщенияОбмена, ПроверятьНаличиеПодписей = Ложь)
...
&НаКлиенте
Процедура ПослеПодписанияЭДСбербанк(Результат,
		ДополнительныеПараметры) Экспорт
	...
	Если Результат.Успех Тогда
		...
		ОбменСБанкамиСлужебныйВызовСервера.ДобавитьПодпись(СообщениеОбмена, ДвоичныеДанныеЭП, Сертификат, ДанныеПроверкиПодписи);

		КоличествоПодписанных = КоличествоПодписанных + 1;
		ПодписатьСообщенияОбменаРекурсивноСбербанк(ДополнительныеПараметры.ОповещениеПослеПодписания, ДополнительныеПараметры.СообщенияОбмена, ДополнительныеПараметры.ПроверятьНаличиеПодписей);
	Иначе
...
Процедура ПослеПолученияОтветаНаЗапросСертификатовСбербанк(Ответ, ДополнительныеПараметры) Экспорт
	...
	ОбменСБанкамиСлужебныйКлиент.ЗакэшироватьПараметрСбербанка("ПолученКриптопрофильВСессии", Истина);
...
Процедура ОбработатьРезультатПробнойОтправкиЗапросаВыпискиВБанк(РезультатЗадания,
	...
	Если РезультатЗадания.Статус = "Ошибка" Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, РезультатЗадания.ПодробноеПредставлениеОшибки, РезультатЗадания.КраткоеПредставлениеОшибки, "ОбменСБанками");
		ЗакрытьФормуНаКлиенте(Истина);
	Иначе // выполнено
		Результат = ПолучитьИзВременногоХранилища(РезультатЗадания.АдресРезультата);
