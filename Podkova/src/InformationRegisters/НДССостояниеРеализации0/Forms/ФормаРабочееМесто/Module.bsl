...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СвойстваСписка.ОсновнаяТаблица = "РегистрСведений.НДССостояниеРеализации0";
...
Процедура ПодтверждениеСтавкиНДССервер(ПараметрыПодтверждения)
	...
	ИначеЕсли ТекущееСостояние = Перечисления.НДССостоянияРеализация0.НеПодтвержденаРеализация0 Тогда
		СтавкаНДС = ?(СтавкаНДС = Неопределено, УчетНДСУП.СтавкаНДСПоУмолчанию(НачалоПериода), СтавкаНДС);
		ДатаПодтверждения = КонецДня(КонецПериода);;
	Иначе
		СтавкаНДС = Перечисления.СтавкиНДС.ПустаяСсылка();
