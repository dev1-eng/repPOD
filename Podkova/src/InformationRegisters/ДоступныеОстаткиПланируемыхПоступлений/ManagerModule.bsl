...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("bc91a96e-034f-47c6-9824-cfb7b2c9f429");
	...
	Обработчик.Комментарий = НСтр("ru = 'Актуализирует движения по регистру сведений Доступные остатки планируемых поступлений по документам Заказ поставщику, Приходный ордер на товары и Акт о расхождениях после поступления.'");
	...
	Читаемые.Добавить(Метаданные.РегистрыСведений.ДоступныеОстаткиПланируемыхПоступлений.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ДоступныеОстаткиПланируемыхПоступлений.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.ЗаказПоставщику.Товары КАК ТоварыДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтДублиПоДоговорам КАК Дубли
	|			ПО ТоварыДокумента.Ссылка.Договор = Дубли.Распоряжение
	|			ИЛИ ТоварыДокумента.Ссылка.Соглашение = Дубли.Распоряжение
	|ГДЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтДублиПоДоговорам КАК Дубли
	|			ПО ТоварыДокумента.Ссылка.Распоряжение = Дубли.Распоряжение
	|ГДЕ
	|	НЕ Дубли.Распоряжение ЕСТЬ NULL
	|	И ТоварыДокумента.Ссылка.Проведен
	|	И ТоварыДокумента.Номенклатура.ТипНоменклатуры В(
	...
	|	ТоварыДокумента.Склад КАК Склад
	|ИЗ
	|	Документ.АктОРасхожденияхПослеПриемки.Товары КАК ТоварыДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтДублиПоДоговорам КАК Дубли
	|			ПО ТоварыДокумента.Ссылка.Договор = Дубли.Распоряжение
	|			ИЛИ ТоварыДокумента.Ссылка.Соглашение = Дубли.Распоряжение
	|ГДЕ
