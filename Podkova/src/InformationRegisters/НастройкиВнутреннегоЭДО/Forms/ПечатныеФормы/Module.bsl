...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого ПечатнаяФорма Из ПечатныеФормыМассив Цикл
		ЗаполнитьЗначенияСвойств(ПечатныеФормы.Добавить(), ПечатнаяФорма);
	...
	РасширенныйРежим = РазличныеЗначенияМаршрутов.Количество() > 1;
	ПриИзмененииРасширенногоРежима(ЭтаФорма);
	
	УстановитьУсловноеОформление();
	
	ОбменСКонтрагентамиСлужебныйКлиентСервер.УстановитьСвойстваПоляМаршрутПодписанияДляВнутреннегоЭДО(Элементы.ПечатныеФормыМаршрутПодписания,
		ВидЭлектроннойПодписи, Организация, Истина);
