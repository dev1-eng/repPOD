...
//                     * Номенклатура   - номенклатура товара,
//                     * Характеристика - характеристика товара.
//
// Возвращаемое значение:
//  Строка - Текст запроса запроса временной таблицы "ТопологияСкладов" с полями:
//           * Номенклатура   - номенклатура товара,
//           * Характеристика - характеристика товара,
//           * Центр          - склад, с которого товары перемещаются на другие склады,
//           * Склад          - склад на который перемещаются товары из центра, а также сам центр распределительной сети.
//
Функция ВременнаяТаблицаТопологияСкладов(ИмяТаблицыТовары) Экспорт
	
	Текст = ВременнаяТаблицаСпособыОбеспеченияВсехСкладов(Перечисления.ТипыОбеспечения.Перемещение);
	...
	Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ТоварыИСпособыОбеспечения.Номенклатура                                                  КАК Номенклатура,
		|	ТоварыИСпособыОбеспечения.Характеристика                                                КАК Характеристика,
		|	ТоварыИСпособыОбеспечения.Склад                                                         КАК Склад,
		|	ТоварыИСпособыОбеспечения.СпособОбеспеченияПотребностей.ИсточникОбеспеченияПотребностей КАК Центр
		|ПОМЕСТИТЬ ТопологияСкладов
		|ИЗ
		|	ВтСпособыОбеспечения КАК ТоварыИСпособыОбеспечения
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТоварыИСпособыОбеспечения.Номенклатура                                                  КАК Номенклатура,
		|	ТоварыИСпособыОбеспечения.Характеристика                                                КАК Характеристика,
		|	ТоварыИСпособыОбеспечения.СпособОбеспеченияПотребностей.ИсточникОбеспеченияПотребностей КАК Склад,
		|	ТоварыИСпособыОбеспечения.СпособОбеспеченияПотребностей.ИсточникОбеспеченияПотребностей КАК Центр
		|ИЗ
		|	ВтСпособыОбеспечения КАК ТоварыИСпособыОбеспечения";
	ТекстыЗапроса.Добавить(Текст);
	...
	ТекстыЗапроса.Добавить("УНИЧТОЖИТЬ ВтФорматыскладов");
	ТекстыЗапроса.Добавить("УНИЧТОЖИТЬ ВтСпособыОбеспечения");
	
	Текст = СтрСоединить(ТекстыЗапроса, ОбщегоНазначения.РазделительПакетаЗапросов());
	Возврат Текст;
...
//
// Параметры:
//  ИмяТаблицыТовары - Строка - имя временной таблицы содержащей ключи (уникальность не требуется):
//                     * Номенклатура   - номенклатура товара,
//                     * Характеристика - характеристика товара,
//                     * Склад          - склад.
...
//           * Номенклатура   - номенклатура товара,
//           * Характеристика - характеристика товара,
//           * Назначение - назначение товара, либо Неопределено,
//           * Склад          - склад.
//
