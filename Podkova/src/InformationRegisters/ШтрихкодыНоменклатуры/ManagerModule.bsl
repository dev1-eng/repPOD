...
Процедура ПолучитьДанныеПоШтрихкодам(КэшированныеЗначения, Штрихкоды) Экспорт
	...
	|				ТОГДА ВЫРАЗИТЬ(УпаковкиНоменклатуры.Числитель / УпаковкиНоменклатуры.Знаменатель КАК ЧИСЛО(15, 7))
	|			ИНАЧЕ 0
	|		КОНЕЦ = 1
	|		И
	...
	ОпределениеПризнакаМаркируемаяПродукция = ИнтеграцияИСУТ.ОпределениеПризнакаМаркируемаяПродукцияТекстаЗапроса("ШтрихкодыНоменклатуры.Номенклатура");
...
Функция СформироватьШтрихкодEAN13() Экспорт
	...
	Если Диапазоны = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияНеНастроеныДоступныеДиапазоныШтучныхШтрихкодов());
...
Функция ПолучитьМаксимальноеЗначениеКодаВесовыхШтрихкодов() Экспорт
	...
	Если РезультатЗапроса.Количество()=0 и МаксимальныйКодВесовогоТовара>0 Тогда
