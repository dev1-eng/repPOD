...
Процедура УстановитьВидимость()
	...
	Если ИспользоватьСоглашенияСКлиентами Тогда
		ИспользуютсяДоговорыКонтрагентов = ЗаполнятьСоглашение И ЗначениеЗаполнено(Объект.Соглашение) 
			И Документы.ПланПродаж.ЗначениеРеквизитаОбъектаТипаБулево(Объект.Соглашение, "ИспользуютсяДоговорыКонтрагентов");
	Иначе
...
Функция ЗагрузитьИзExcelНаСервере(МассивСтруктурТовары, ЕстьДобавленныеСтроки)
	...
	Если ПланированиеПоНазначениямВТЧ И ЕстьДобавленныеСтроки Тогда
		...
		Если ВидПлана.ЗаполнятьПартнера И НЕ ВидПлана.ЗаполнятьПартнераВТЧ Тогда
			ДетализацияПоОбщемуКлиенту = Истина;
		Иначе
			ДетализацияПоОбщемуКлиенту = Ложь;
		КонецЕсли;
		...
		Для Каждого СтруктураТовар Из МассивСтруктурТовары Цикл
			...
			Если ДетализацияПоОбщемуКлиенту Тогда
				...
				Если СтруктураТовар.Свойство("ЗагружаемоеСоглашение") И ВидПлана.ЗаполнятьСоглашениеВТЧ Тогда
					...
					Если ЗначениеЗаполнено(СтруктураТовар.ЗагружаемоеСоглашение) Тогда
						...
						ПроверяемоеСоглашение = Справочники.СоглашенияСКлиентами.НайтиПоНаименованию(СтруктураТовар.ЗагружаемоеСоглашение);
				...
				Если СтруктураТовар.Свойство("ЗагружаемыйПартнер") И ВидПлана.ЗаполнятьПартнераВТЧ Тогда
					...
					Если ЗначениеЗаполнено(СтруктураТовар.ЗагружаемыйПартнер) Тогда
						...
						Если ЗначениеЗаполнено(ПроверяемыйПартнер) Тогда
							СтруктураТовар.Вставить("Партнер", ПроверяемыйПартнер.Ссылка); // Клиент
						КонецЕсли;
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ЗаполнятьПартнера, ЗаполнятьСоглашение, ЗаполнятьСклад,
		|ЗаполнятьМенеджера, ЗаполнятьФорматМагазина, ПланированиеПоНазначениям, ЗаполнятьНазначениеВТЧ");
