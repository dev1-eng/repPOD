...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	ТранспортнаяНакладнаяЛокализация.ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры);
...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	ТранспортнаяНакладнаяЛокализация.ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры);
...
// Если распоряжение было распроведено или помечено на удаление, то оно удалится из ТЧ "ДокументыОснования" в
// транспортной накладной Если после удаления распоряжения в ТЧ "ДокументыОснования" она оказалась пуста, то вся
// транспортная накладная помечается на удаление.
//
// Параметры:
...
Процедура ДобавитьКомандыПечати(КомандыПечати, Порядок = Неопределено) Экспорт
	...
	Если ПараметрыТТН.ИспользоватьТТН
		...
		КомандаПечати.Обработчик = "УправлениеПечатьюУТКлиентЛокализация.ПечатьТТН";
	...
	ТранспортнаяНакладнаяЛокализация.ДобавитьКомандыПечати(КомандыПечати, Порядок = Неопределено);
...
Процедура ЗаполнитьРеквизитыТранспортнойНакладной(МассивРаспоряжений, РеквизитыТранспортнойНакладной, ЗначенияЗаполнения = Неопределено, ЗаданиеНаПеревозку = Неопределено)
	...
	|	ЗаданиеНаПеревозкуМаршрут.Ссылка.УдостоверениеНомер КАК УдостоверениеНомер,
	|	ЗаданиеНаПеревозкуМаршрут.Ссылка.УдостоверениеСерия КАК УдостоверениеСерия,
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ДокументРаспоряжениеТовары.ЗаказКлиента.СпособДоставки В (&СпособыДоставкиСНашимУчастием)
	|						ТОГДА ЕСТЬNULL(ДанныеЗаданияНаПеревозкуЗаказы.УдостоверениеНомер, """")
	|					ИНАЧЕ """"
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ДокументРаспоряжение.СпособДоставки В (&СпособыДоставкиСНашимУчастием)
	|					ТОГДА ЕСТЬNULL(ДанныеЗаданияНаПеревозкуНакладные.УдостоверениеНомер, """")
	|				ИНАЧЕ """"
	|			КОНЕЦ
	|	КОНЕЦ КАК УдостоверениеНомер,
	|	ВЫБОР
	|		КОГДА ДокументРаспоряжение.РеализацияПоЗаказам
	|			ТОГДА ВЫБОР
	|					КОГДА ДокументРаспоряжениеТовары.ЗаказКлиента.СпособДоставки В (&СпособыДоставкиСНашимУчастием)
	|						ТОГДА ЕСТЬNULL(ДанныеЗаданияНаПеревозкуЗаказы.УдостоверениеСерия, """")
	|					ИНАЧЕ """"
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ДокументРаспоряжение.СпособДоставки В (&СпособыДоставкиСНашимУчастием)
	|					ТОГДА ЕСТЬNULL(ДанныеЗаданияНаПеревозкуНакладные.УдостоверениеСерия, """")
	|				ИНАЧЕ """"
	|			КОНЕЦ
	|	КОНЕЦ КАК УдостоверениеСерия
	|ИЗ
	...
	|		И (ЗаданияНаПеревозкуРаспоряжения.Ссылка = &ЗаданиеНаПеревозку
	|			ИЛИ &НетЗаданияНаПеревозку
	|			ИЛИ НЕ &ИспользоватьРасширенныеВозможностиЗаказаКлиента)) КАК ТаблицаМассДата";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
								"&ТекстЗапросаВесУпаковки",
								Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаВесУпаковки("ТаблицаТоваров.Упаковка",
																							"ТаблицаТоваров.Номенклатура",
																							Ложь));
		
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
								"&ТекстЗапросаВесНоменклатуры",
								Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаВесУпаковки("ТаблицаТоваров.Номенклатура.ЕдиницаИзмерения",
																							"ТаблицаТоваров.Номенклатура",
																							Ложь));
