...
Процедура ПродажиСуммаБезНДСПриИзменении(Элемент)
	...
	ТекущаяСтрока = Элементы.Продажи.ТекущиеДанные;
	СтавкаНДСПроцентом = УчетНДСУПКлиентСервер.ПолучитьСтавкуНДС(ТекущаяСтрока.СтавкаНДС) / 100;
...
Процедура УстановитьПериодичность()
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		Период = ?(Объект.Исправление, Объект.СводнаяСправкаОснование.Дата, Объект.Дата);
		УчетнаяПолитикаОрганизации = РегистрыСведений.УчетнаяПолитикаОрганизаций.УчетныеПолитикиОрганизаций(Объект.Организация, Период, Истина);
...
Процедура ЗаполнитьТаблицуПродажСервер()
	...
	СтруктураПолученияДанных.Вставить("ИсправляемыйПериод",
		?(ВидСправки = 1, КонецМесяца(Объект.СводнаяСправкаОснование.Дата), Неопределено));
...
Процедура ОбработатьИзменениеСправкиОснования(ВыбраннаяСправка)
	...
	Объект.Организация = ВыбраннаяСправка.Организация;
