...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоСоглашениюИспользуютсяДоговорыКонтрагентов = ЗначениеЗаполнено(Объект.Соглашение) И Документы.ЗаказПоставщику.ЗначениеРеквизитаОбъектаТипаБулево(Объект.Соглашение, "ИспользуютсяДоговорыКонтрагентов");
	...
	ПараметрыEDIПриСоздании.МестоРазмещенияКоманд = Элементы.ПодменюEDI;
		...
		ИначеЕсли СуммаЗаказано <> СуммаЭтаповОплаты Или СуммаЗалогаЗаТару <> СуммаЗалогаПоЭтапам Тогда
			...
			Если КоличествоЭтапов = 0 Тогда
				...
				Если Не ГрафикСоглашенияЗаполнен Тогда
					...
					ПараметрыДляДобавления.ВариантОплаты = ПредопределенноеЗначение("Перечисление.ВариантыОплатыПоставщику.КредитПослеПоступления");
...
Процедура ПартнерПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Соглашение) И Объект.Товары.Итог("Цена")<>0 Тогда
		ЦенообразованиеКлиент.ЗадатьВопросПересчетаЦеныПриИзмененииДаты(Объект,
...
Функция ЗаполнитьПодчиненныеСвойстваПоСтатистике(ИмяРеквизитаРодителя)
	Возврат ЗаполнениеСвойствПоСтатистикеСервер.ЗаполнитьПодчиненныеСвойства(Объект, ИмяРеквизитаРодителя);
КонецФункции
...
Функция СкорректироватьЦеныПоПоступлениямСервер()
	...
	|							ТоварыПоступления.Цена / ТоварыПоступления.Курс * ТоварыПоступления.Кратность
	|						ИНАЧЕ ТоварыПоступления.Цена * ТоварыПоступления.Курс / ТоварыПоступления.Кратность
	|					КОНЕЦ
	...
	|					ВЫБОР
	|						КОГДА ТоварыПоступления.Валюта = &ВалютаРегУчета ТОГДА
	|							ТоварыПоступления.СуммаРучнойСкидки / ТоварыПоступления.Курс * ТоварыПоступления.Кратность
	|						ИНАЧЕ ТоварыПоступления.СуммаРучнойСкидки * ТоварыПоступления.Курс / ТоварыПоступления.Кратность
	|					КОНЕЦ
	...
	|					ВЫБОР
	|						КОГДА ТоварыПоступления.Валюта = &ВалютаРегУчета ТОГДА
	|							ТоварыПоступления.Сумма / ТоварыПоступления.Курс * ТоварыПоступления.Кратность
	|						ИНАЧЕ ТоварыПоступления.Сумма * ТоварыПоступления.Курс / ТоварыПоступления.Кратность
	|					КОНЕЦ
...
Процедура ЗаполнитьПустыеДатыПоступления()
	...
	Если ПроверитьПустуюДатуПоступленияКлиент() Тогда
...
Процедура НадписьЭтапыОплатыНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Соглашение",                Объект.Соглашение);
	ПараметрыФормы.Вставить("Дата",                      Объект.Дата);
	ПараметрыФормы.Вставить("ЖелаемаяДатаПоступления",   ?(ЗначениеЗаполнено(Объект.ЖелаемаяДатаПоступления),Объект.ЖелаемаяДатаПоступления,Объект.ДатаПоступления));
	ПараметрыФормы.Вставить("Валюта",                    Объект.Валюта);
...
&НаКлиенте
Функция ПроверитьПустуюДатуПоступленияКлиент()
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	СтруктураДействийДобавленныеСтроки.Вставить("ПриИзмененииТипаНоменклатуры", Новый Структура("ЕстьРаботы, ЕстьОтменено", Истина, Истина));
