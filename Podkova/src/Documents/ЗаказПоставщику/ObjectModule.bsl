...
Процедура ЗаполнитьЭтапыГрафикаОплаты(ТолькоРаспределять = Ложь) Экспорт
	...
	Если ПолучитьКоличествоЗаказанныхСтрок() <> 0 И ПорядокРасчетов <> Перечисления.ПорядокРасчетов.ПоНакладным Тогда
...
Функция УстановитьСтатус(НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	ЗакрыватьТолькоБезИменений = Ложь;
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ДополнительныеПараметры.Свойство("ЗакрыватьТолькоБезИзменений")
		И ДополнительныеПараметры.ЗакрыватьТолькоБезИзменений  Тогда
		
		ЗакрыватьТолькоБезИменений = Истина;
	...
	Если Товары.Количество() > 0 Тогда
		...
		Если Не ЗакрыватьТолькоБезИменений
	...
	Если ДополнительныеПараметры <> Неопределено Тогда
		...
		Если ЗакрыватьТолькоБезИменений
			И ЗаказИзменен Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказамНакладным Тогда
...
Процедура ЗаполнитьДокументНаОснованииКоммерческогоПредложенияПоставщика(Знач КоммерческоеПредложение)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСоглашенияСКлиентами") Тогда
		...
		РеквизитыДляОтбора.Вставить("Партнер",         Партнер);
		РеквизитыДляОтбора.Вставить("Валюта",          Валюта);
		РеквизитыДляОтбора.Вставить("ЦенаВключаетНДС", ЦенаВключаетНДС);

		ЗаполнениеСвойствПоСтатистикеСервер.ПолучитьЗначенияСвойств(
			Ссылка, ЗаполняемыеРеквизиты, РеквизитыДляОтбора);

		Если ЗначениеЗаполнено(ЗаполняемыеРеквизиты.Соглашение) Тогда
			Соглашение = ЗаполняемыеРеквизиты.Соглашение;
		КонецЕсли;
		
	КонецЕсли;
	
	Параметры = ЗакупкиСервер.ДополнительныеПараметрыОтбораДоговоров();
	Параметры.ВалютаВзаиморасчетов = Валюта;
	
	Договор = ЗакупкиСервер.ПолучитьДоговорПоУмолчанию(ЭтотОбъект, ХозяйственнаяОперация, Параметры);
...
Процедура ЗаполнитьДокументНаОснованииИзмененияАссортимента(Знач ИзменениеАссортимента)
	...
	Пока ВыборкаТоварыОснования.Следующий() Цикл
		...
		СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС", Новый Структура("НалогообложениеНДС, Дата",
			Метаданные.Документы.ЗаказПоставщику.Реквизиты.НалогообложениеНДС.ЗначениеЗаполнения, Дата));
...
Функция СкорректироватьСтрокиЗаказа(СтруктураПараметров) Экспорт
	...
	Таблица = ТаблицаТовары.Выгрузить(,"Номенклатура, Характеристика, Склад, Количество, КодСтроки, Отменено");
