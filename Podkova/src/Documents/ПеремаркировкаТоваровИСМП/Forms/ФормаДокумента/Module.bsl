...
Процедура ОбновитьСтатусИСМП()
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			Если СтатусПроверкиИПодбора <> Перечисления.СтатусыПроверкиИПодбораИС.Выполняется Тогда
				...
				БлокировкаКодов = Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ЗаблокируйтеКодыМаркировки;
				Если СтатусИСМП = Перечисления.СтатусыОбработкиПеремаркировкиТоваровИСМП.Черновик
					И ДопустимыеДействия.Найти(БлокировкаКодов) <> Неопределено
					И ДальнейшееДействие.Найти(БлокировкаКодов) = Неопределено Тогда
					ДальнейшееДействие.Вставить(0, БлокировкаКодов);
				КонецЕсли;
			КонецЕсли;
	...
	ИнтеграцияИСКлиентСервер.УправлениеДоступностьюЭлементовФормы(ЭтотОбъект,
...
Процедура ОбработкаРезультатаПодбораНоменклатуры(ВыбранноеЗначение)
	...
	ПараметрыЗаполнения.ПересчитатьКоличествоЕдиниц   = Ложь;
	ПараметрыЗаполнения.ПересчитатьКоличествоУпаковок = Ложь;
	ПараметрыЗаполнения.ЗаполнитьКодТНВЭД             = Ложь;
...
Процедура ОткрытьФормуВводаКодовМаркировки(ИсходныеДанныеСтроки)
	...
	Если ТипЗнч(ИсходныеДанныеСтроки) = Тип("Структура") Тогда
		...
		ПараметрыОткрытия.Вставить("АдресДанныеШтрихкода",
			ПоместитьВоВременноеХранилище(ИсходныеДанныеСтроки.ДанныеШтрихкода, ЭтотОбъект.УникальныйИдентификатор));
