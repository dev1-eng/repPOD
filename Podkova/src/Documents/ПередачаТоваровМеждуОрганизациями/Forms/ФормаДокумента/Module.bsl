...
Процедура АктуализироватьПередачуПодДеятельность(Заполнить = Истина)
	...
	Если Заполнить Тогда
		УчетНДСУП.ЗаполнитьВидДеятельностиНДС(Объект.ПередачаПодДеятельность, ПараметрыЗаполнения,
			УчетНДСКэшированныеЗначенияПараметров);
	КонецЕсли;
...
Процедура ОбновитьСостояниеЭлементовУправленияИтоговПоРасчетам()
	...
	Иначе
		РасчетыПолучатель = НСтр("ru='Оплачено'") + ": " + Формат(СуммаОплатыПолучатель, "ЧДЦ=2; ЧН=") + " " + Объект.ВалютаВзаиморасчетов + "  " + ПроцентОплатыПолучатель + "%";
		РасчетыОтправитель = НСтр("ru='Получено'") + ": " + Формат(СуммаОплатыОтправитель, "ЧДЦ=2; ЧН=") + " " + Объект.ВалютаВзаиморасчетов + "  " + ПроцентОплатыОтправитель + "%";
	КонецЕсли;
