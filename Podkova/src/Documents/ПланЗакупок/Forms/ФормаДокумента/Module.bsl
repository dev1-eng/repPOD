...
Процедура СкопироватьИзКроссТаблицы(КэшированныеЗначения)
	...
	Если Объект.КроссТаблица
		...
		Планирование.ЗаполнитьТаблицуИзКроссТаблицы(Объект.Товары, ТоварыПоПериодам, ПараметрыПреобразования, 
...
&НаСервере
Функция ЗагрузитьИзExcelНаСервере(МассивСтруктурТовары, ЕстьДобавленныеСтроки)
	...
	ВидПлана = Объект.ВидПлана; // СправочникСсылка.ВидыПланов
	
	ПланированиеПоНазначениямВТЧ = ВидПлана.ЗаполнятьНазначениеВТЧ;
	Если ПланированиеПоНазначениямВТЧ И ЕстьДобавленныеСтроки Тогда
		
		ИспользоватьХарактеристикиНоменклатуры = ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристикиНоменклатуры");
		
		Если ВидПлана.ЗаполнятьПартнера И НЕ ВидПлана.ЗаполнятьПартнераВТЧ Тогда
			ДетализацияПоОбщемуПоставщику = Истина;
		Иначе
			ДетализацияПоОбщемуПоставщику = Ложь;
		КонецЕсли;
		
		// Обработка строк загружаемых товаров с учетом назначений и сопоставлением с данными базы данных
		Для Каждого СтруктураТовар Из МассивСтруктурТовары Цикл
			
			ПроверяемаяНоменклатура = Неопределено;
			Если СтруктураТовар.Свойство("ЗагружаемаяНоменклатура") И ЗначениеЗаполнено(СтруктураТовар.ЗагружаемаяНоменклатура) Тогда
				ПроверяемаяНоменклатура = Справочники.Номенклатура.НайтиПоНаименованию(СтруктураТовар.ЗагружаемаяНоменклатура);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ПроверяемаяНоменклатура) Тогда
				
				СтруктураТовар.Вставить("Номенклатура", ПроверяемаяНоменклатура.Ссылка);
				
				Если СтруктураТовар.Свойство("ЗагружаемаяУпаковка") Тогда
					
					ПроверяемаяУпаковка = Справочники.УпаковкиЕдиницыИзмерения.НайтиПоНаименованию(СтруктураТовар.ЗагружаемаяУпаковка);
					Если ЗначениеЗаполнено(ПроверяемаяУпаковка) Тогда
						СтруктураТовар.Вставить("Упаковка", ПроверяемаяУпаковка.Ссылка);
					КонецЕсли;
					ПроверяемаяУпаковка = Неопределено;
					
				КонецЕсли;
				
				Если ИспользоватьХарактеристикиНоменклатуры Тогда
					
					Если СтруктураТовар.Свойство("ЗагружаемаяХарактеристика") И ЗначениеЗаполнено(СтруктураТовар.ЗагружаемаяХарактеристика) Тогда
						
						ПроверяемаяХарактеристика = Справочники.ХарактеристикиНоменклатуры.НайтиПоНаименованию(СтруктураТовар.ЗагружаемаяХарактеристика);
						Если ЗначениеЗаполнено(ПроверяемаяХарактеристика) Тогда
							СтруктураТовар.Вставить("Характеристика", ПроверяемаяХарактеристика.Ссылка);
						КонецЕсли;
				...
				Если СтруктураТовар.Свойство("ЗагружаемоеНазначение") Тогда
					
					Если ВидПлана.ЗаполнятьНазначениеВТЧ И ЗначениеЗаполнено(СтруктураТовар.ЗагружаемоеНазначение) Тогда
						
						ПроверяемоеНазначение = Справочники.Назначения.НайтиПоНаименованию(СтруктураТовар.ЗагружаемоеНазначение);
						Если ЗначениеЗаполнено(ПроверяемоеНазначение) Тогда
							СтруктураТовар.Вставить("Назначение", ПроверяемоеНазначение.Ссылка);
						КонецЕсли;
						ПроверяемоеНазначение = Неопределено;
						
					КонецЕсли;
					
				КонецЕсли;
				
				Если СтруктураТовар.Свойство("ЗагружаемыйСклад") Тогда
					
					Если ВидПлана.ЗаполнятьСкладВТЧ И ЗначениеЗаполнено(СтруктураТовар.ЗагружаемыйСклад) Тогда
						
						ПроверяемыйСклад = Справочники.Склады.НайтиПоНаименованию(СтруктураТовар.ЗагружаемыйСклад);
						Если ЗначениеЗаполнено(ПроверяемыйСклад) Тогда
							СтруктураТовар.Вставить("Склад", ПроверяемыйСклад.Ссылка);
						КонецЕсли;
						ПроверяемыйСклад = Неопределено;
			...
			Если ДетализацияПоОбщемуПоставщику Тогда
				
				Если СтруктураТовар.Свойство("ЗагружаемоеСоглашение") И ВидПлана.ЗаполнятьСоглашениеВТЧ Тогда
					
					Если ЗначениеЗаполнено(СтруктураТовар.ЗагружаемоеСоглашение) Тогда
						
						ПроверяемоеСоглашение = Справочники.СоглашенияСПоставщиками.НайтиПоНаименованию(СтруктураТовар.ЗагружаемоеСоглашение);
						Если ЗначениеЗаполнено(ПроверяемоеСоглашение) И ПроверяемоеСоглашение.Партнер = Объект.Партнер Тогда
							СтруктураТовар.Вставить("Соглашение", ПроверяемоеСоглашение.Ссылка);
						КонецЕсли;
						ПроверяемоеСоглашение = Неопределено;
			...
			Иначе
				...
				Если СтруктураТовар.Свойство("ЗагружаемыйПартнер") И ВидПлана.ЗаполнятьПартнераВТЧ Тогда
					
					Если ЗначениеЗаполнено(СтруктураТовар.ЗагружаемыйПартнер) Тогда
						
						ПроверяемыйПартнер = Справочники.Партнеры.НайтиПоНаименованию(СтруктураТовар.ЗагружаемыйПартнер);
						Если ЗначениеЗаполнено(ПроверяемыйПартнер) Тогда
							СтруктураТовар.Вставить("Партнер", ПроверяемыйПартнер.Ссылка); // Поставщик
						КонецЕсли;
					...
					Если СтруктураТовар.Свойство("ЗагружаемоеСоглашение") И ВидПлана.ЗаполнятьСоглашениеВТЧ Тогда
						...
						Если ЗначениеЗаполнено(СтруктураТовар.ЗагружаемоеСоглашение) Тогда
							...
							ПроверяемоеСоглашение = Справочники.СоглашенияСПоставщиками.НайтиПоНаименованию(СтруктураТовар.ЗагружаемоеСоглашение);
							Если ЗначениеЗаполнено(ПроверяемоеСоглашение) И ПроверяемоеСоглашение.Партнер = ПроверяемыйПартнер Тогда
								СтруктураТовар.Вставить("Соглашение", ПроверяемоеСоглашение.Ссылка);
							КонецЕсли;
							ПроверяемоеСоглашение = Неопределено;
					...
					ПроверяемыйПартнер = Неопределено;
...
&НаСервере
Функция ОпределитьНаличиеНазначенийВТЧ(ВидПлана)
	
	Возврат ВидПлана.ЗаполнятьНазначениеВТЧ;
...
Функция ЗагрузитьИзExcelНаКлиентеПродолжение(ДанныеЗагрузки)
	...
	Если ЕстьДобавленныеСтроки Тогда
		...
		ПланированиеПоНазначениямВТЧ = ОпределитьНаличиеНазначенийВТЧ(Объект.ВидПлана);
		Если ПланированиеПоНазначениямВТЧ Тогда
			ЗагрузитьИзExcelНаСервере(ДанныеЗагрузки, ЕстьДобавленныеСтроки);
		Иначе
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	Реквизиты = "ИспользоватьВПланированииОстатков, ИспользоватьВПланированииПродаж, ИспользоватьВПланированииЗакупок, ИспользоватьВПланированииСборкиРазборки";
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Закупки прошедшего периода с учетом остатка'"), 
			"Макс([ФактыЗакупок], [ПланыЗакупок])" +
			?(ИспользоватьПланыОстатков," - [ПланОстатков]",?(ПланированиеПоНазначениям," - [ФактическиеНачальныеОстатки]"," - [СвободныеОстатки]")));
	
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='Пополнение остатка'"),
			?(ИспользоватьПланыОстатков,"?([ПланОстатков] = 0 И ",
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ЗаполнятьПартнера,
		|ЗаполнятьСоглашение, ЗаполнятьСклад, ПланированиеПоНазначениям, ЗаполнятьНазначениеВТЧ");
