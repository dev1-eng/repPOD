...
Процедура ПриОткрытии(Отказ)
	...
	Если ИмяПользователя()="Сотрудник" Тогда
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ПоСоглашениюИспользуютсяДоговорыКонтрагентов = ЗначениеЗаполнено(Соглашение) И Документы.РеализацияТоваровУслуг.ЗначениеРеквизитаОбъектаТипаБулево(Соглашение, "ИспользуютсяДоговорыКонтрагентов");
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиентуРеглУчет Тогда
		...
		Объект.РеализацияПоЗаказам = Ложь;
		ПриИзмененииРеализацииПоНесколькимЗаказам();
		НаправленияДеятельностиСервер.ПриИзмененииНаправленияДеятельностиСервер(ЭтотОбъект);
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуре()
	...
	// Проверка и подбор ИСМП
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ПараметрыИнтеграцииГосИС")
		И ЭтотОбъект["ПараметрыИнтеграцииГосИС"].Получить("ИСМП")<>Неопределено Тогда
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ПараметрыИнтеграцииГосИС")
		И ЭтотОбъект["ПараметрыИнтеграцииГосИС"].Получить("ИСМП")<>Неопределено Тогда
		ПроверкаИПодборПродукцииИСМП.ПрименитьКешШтрихкодовУпаковок(ЭтотОбъект);
	КонецЕсли;
	// Проверка и подбор ИСМП
...
Функция ПредварительноРассчитанныеСкидкиНаценкиПоЗаказу()
	...
	СкидкиНаценкиЗаказа = ПакетЗапросов[4].Выгрузить();
	
	ВыборкаТовары = ПакетЗапросов[3].Выбрать();
	Пока ВыборкаТовары.Следующий() Цикл
		...
		СуммаКРаспределению = Окр(ВыборкаТовары.ЗаказСуммаАвтоматическойСкидки * (ВыборкаТовары.РеализацияКоличество / ВыборкаТовары.ЗаказКоличество) * ВыборкаТовары.ВалютаКоэффициентПересчета, 2);
...
Процедура УстановитьВидимостьОпераций()
	...
	Иначе
		...
		ВозможнаРеализацияБезПереходаПраваСобственности = 
			Документы.РеализацияТоваровУслуг.ЗначениеРеквизитаОбъектаТипаБулево(Объект.Соглашение, "ВозможнаРеализацияБезПереходаПраваСобственности");
...
Функция ПоместитьТоварыВоВременноеХранилищеНаСервере()
	...
	Возврат ПоместитьВоВременноеХранилище(Объект.Товары.Выгрузить(,"КодСтроки, Номенклатура, ЗаказКлиента, СуммаСНДС, СуммаВзаиморасчетов"), 
...
Функция ШтрихкодыУпаковокЗаполнены()
	...
	Возврат ПродажиСервер.ШтрихкодыУпаковокЗаполнены(Объект);
	...
	Если Объект.ХозяйственнаяОперация <> ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию") Тогда
		...
		ИначеЕсли СуммаВсего <> СуммаЭтаповОплаты + СуммаЗалогаЗаТару Или СуммаЗалогаЗаТару <> СуммаЗалогаПоЭтапам Тогда
			...
			Если Объект.ЭтапыГрафикаОплаты.Количество() = 0 Тогда
				ЗаполнитьЭтапыОплатыСервер();
			Иначе
				Если Объект.РеализацияПоЗаказам И Объект.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоЗаказамНакладным") Тогда
...
&НаСервере
Процедура ПечатьЗаказНарядаНаСервере()
...
Функция ПолучитьЗапросТоварныйЧек()
	...
	ДокРеал=Объект.Ссылка;
	ВыбраноАвтоЮЛ=ДокРеал.ВыбраноАвтоЮЛ;
	НомерМашины=СокрЛП(ДокРеал.Контрагент.Наименование);
...
Процедура ПечатьЗаказНаряда(Команда)
	...
	ТабДок.Показать();
	ПечатьЗаказНарядаНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ДетализацияНаККМ(Команда)
...
Функция МК_СформироватьТекстКопииЧека()
	...
	ТекстСкидки = "";
	//Если ОбЪект.ПроцентСкидкиЧерновик=10 Тогда
	//	ТекстСкидки = "Ск.10% ";
	//ИначеЕсли ОбЪект.ПроцентСкидкиЧерновик=20 Тогда
	//	ТекстСкидки = "Ск.20% ";
	//ИначеЕсли ОбЪект.ПроцентСкидкиЧерновик=50 Тогда
	//	ТекстСкидки = "Ск.50% ";
	//КонецЕсли;
	...
	Для Каждого СтрокаТаб Из Объект.Товары Цикл
		...
		КонецЕсли;
		//Если ((СтрокаТаб.Цена*СтрокаТаб.Количество)-СтрокаТаб.Сумма)>0 Тогда
		...
		КонецЦикла;
		//ТекстВыводаСуммы=ТекстВыводаСуммы+Формат(СтрокаТаб.Цена-(СтрокаТаб.СуммаСкидки/СтрокаТаб.Количество),"ЧГ=0,ЧДЦ=2");
		ТекстВыводаСуммы=Формат((СтрокаТаб.Сумма/СтрокаТаб.Количество),"ЧГ=0,ЧДЦ=2");
		...
		СуммаСкидкиПечать=СуммаСкидкиПечать+((СтрокаТаб.Цена*СтрокаТаб.Количество)-СтрокаТаб.Сумма);
	...
	ТекстРеализация = "№ Реал. "+Объект.Номер+" от "+Объект.Дата;
	ТекстКопииЧека = ТекстКопииЧека +
