...
Функция ДанныеДляПроверкиВводаНаОсновании(Распоряжения, ХозяйственнаяОперация = Неопределено) Экспорт
	...
	ТекстыЗапросов.Добавить(РегистрыНакопления.ЗаказыНаСборку.ТекстЗапросаОстатки("ВтОстаткиКОформлению",
		Новый Структура("ЗаказНаСборку", "Распоряжения"), "КОформлению <> 0"));
	ТекстыЗапросов.Добавить(
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Функция РаспределитьСтоимостьКомплектующихНаКомплекты(ТаблицаКомплектов, ТаблицаКомплектующих, Реквизиты)
	...
	ПараметрыУчетнойПолитики = ЗначениеНастроекПовтИсп.ПараметрыУчетнойПолитики(Реквизиты.Организация, НачалоМесяца(Реквизиты.Период)); 
	ФИФОСкользящаяОценка 	 = (ПараметрыУчетнойПолитики.МетодОценкиСтоимостиТоваров = Перечисления.МетодыОценкиСтоимостиТоваров.ФИФОСкользящаяОценка);
...
Функция ПолучитьДанныеДляПечатнойФормыОтборРазмещениеТоваров(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА &ТекстЗапросаИспользоватьОрдернуюСхему
	|				И СборкаТоваров.Дата >= &ТекстЗапросаДатаНачалаОрдернойСхемы
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	ТипОрдернойСхемы = ?(ПараметрыПечати.ИмяФормы = "ЗаданиеНаОтбор" , "ПриОтгрузке", "ПриПоступлении");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаИспользоватьОрдернуюСхему",
		"СборкаТоваров.Склад.ИспользоватьОрдернуюСхему" + ТипОрдернойСхемы);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаДатаНачалаОрдернойСхемы",
		"СборкаТоваров.Склад.ДатаНачалаОрдернойСхемы" + ТипОрдернойСхемы);
