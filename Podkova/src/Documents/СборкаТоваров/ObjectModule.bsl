...
Функция ВременныеТаблицыДанныхДокумента() Экспорт
	...
	|		ПО ТаблицаТоваров.Заказ = ЗаказНаСборку.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК НазначениеЗаказа
	|		ПО ЗаказНаСборку.Назначение = НазначениеЗаказа.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК ЗаказКакНазначение
	|		ПО ЗаказНаСборку.НазначениеТовары = ЗаказКакНазначение.Ссылка
	|
	|ГДЕ
...
Процедура РаспределитьНомераГТД() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		ДанныеАналитикУчета.Вставить(Выборка.КлючАналитики,
									Новый Структура("УчетПоГТД, ШтучнаяЕдиница",
													Выборка.УчетПоГТД, Выборка.ШтучнаяЕдиница));
	КонецЦикла;
	...
	Пока Индекс >= 0 Цикл
		...
		Для Каждого ЗапасСписание Из СписываемыеЗапасы Цикл
			...
			ЗапасОприходование.Количество		= Окр(ОсталосьРаспределить * ЗапасСписание.Количество / ВсегоКоличество,
														Разрядность,
														1);
			ЗапасОприходование.КоличествоПоРНПТ	= ?(ВсегоКоличествоПоРНПТ <> 0,
													ОсталосьРаспределить * ЗапасСписание.КоличествоПоРНПТ
														/ ВсегоКоличествоПоРНПТ * ВсегоКоличество
														/ ЗапасСписание.Количество,
													0);
