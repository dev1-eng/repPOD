...
// Заполняет табличную часть УпаковкиВЕТИС документа базовыми упаковками продукции
//
// Параметры:
//   Объект - ДокументОбъект.ИсходящаяТранспортнаяОперацияВЕТИС - заполняемый документ
//
Процедура ЗаполнитьПотребительскиеУпаковки(Объект) Экспорт
	...
	|	ЗНАЧЕНИЕ(Перечисление.УровниУпаковокВЕТИС.ПотребительскийУровень) КАК УровеньУпаковки,
	|	Товары.Продукция.ФасовкаЕдиницаИзмерения.Коэффициент КАК КоэффициентЕдиницыУпаковки,
...
//
// Возвращаемое значение:
//  см. ИнтеграцияИС.ПараметрыУказанияСерий - параметры указания серий.
//
