...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ЗаполнитьПредставлениеПериодовПоСтроке(ЭтаФорма, Элементы.Товары.ТекущиеДанные);
...
&НаСервере
Процедура ЗаполнитьФасовкуПоПродукции(ВыбраннаяПродукция = Неопределено)
	...
	Если ВыбраннаяПродукция = Неопределено Тогда
		...
		Фасовка.Очистить();
	Иначе
		Если Фасовка.НайтиСтроки(Новый Структура("Продукция", ВыбраннаяПродукция)).Количество() Тогда
			Возврат;
		КонецЕсли;
...
Процедура ОбновитьКоличествоУпаковокПоФасовке(ИдентификаторСтрокиТовары, ИзмененаПродукция = Ложь)
	...
	ИначеЕсли УпаковкиСтроки.Количество() = 0 Тогда
		...
		СтрокаУпаковка.УровеньУпаковки = Перечисления.УровниУпаковокВЕТИС.ПотребительскийУровень;
