...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьСлужебныеРеквизиты(Объект.ДанныеВнешнейСистемы);
	ЗаполнитьСлужебныеРеквизиты(Объект.РезультатыСверки);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ОбщегоНазначенияУТ.ИнициализироватьКешТекущейСтроки(ЭтотОбъект, "ДанныеВнешнейСистемы");
...
Процедура ДанныеВнешнейСистемыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ОбщегоНазначенияУТКлиент.КешироватьТекущуюСтроку(ЭтотОбъект, "ДанныеВнешнейСистемы");
...
Процедура ЗаполнитьСлужебныеРеквизиты(Таблица)
	...
	Если Таблица = Объект.РезультатыСверки Тогда
		ПараметрыЗаполненияРеквизитов.Вставить(
			"ЗаполнитьПризнакВедетсяУчетПоГТД",
			Новый Структура("Номенклатура", "ВедетсяУчетПоГТД"));
	КонецЕсли;
	
	НоменклатураСервер.ЗаполнитьСлужебныеРеквизитыПоНоменклатуреВКоллекции(Таблица, ПараметрыЗаполненияРеквизитов);
...
Функция ПроверитьНеобходимостьФормированияПодчиненныхДокументов()
	...
	Пока Выборка.Следующий() Цикл
		СтруктураДокументы[Выборка.ТипДокумента].МассивДокументов.Добавить(Выборка.Ссылка);
...
Процедура ВыполнитьДействиеНавигационнойСсылки(Команда)
	...
	Если Команда = "СоздатьВводОстатковСебестоимости" ИЛИ ОбновитьВсе Тогда
		СоздатьПодчиненныйДокумент(СтруктураДокументы, "ВводОстатковСебестоимости", Документы.ВводОстатков);
	КонецЕсли;
	
	Если Команда = "СоздатьВводОстатковКоличества" ИЛИ ОбновитьВсе Тогда
		СоздатьПодчиненныйДокумент(СтруктураДокументы, "ВводОстатковКоличества", Документы.ВводОстатков);
	КонецЕсли;
	
	Если Команда = "СоздатьОприходованиеПосле" ИЛИ ОбновитьВсе Тогда
		СоздатьПодчиненныйДокумент(СтруктураДокументы, "ОприходованиеПосле", Документы.ОприходованиеИзлишковТоваров);
	КонецЕсли;
	
	Если Команда = "СоздатьСписаниеПосле" ИЛИ ОбновитьВсе Тогда
