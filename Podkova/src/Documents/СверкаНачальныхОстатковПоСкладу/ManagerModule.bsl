...
Функция ЗаполнитьВводОстатковТоваровНаОснованииСверки(ЗаполняемыйДокумент, ДанныеЗаполнения, ИнтерактивноеЗаполнение = Ложь) Экспорт
	...
	КоэффициентПересчетаИзВалютыУпрВРегл = РаботаСКурсамиВалютУТ.ПолучитьКоэффициентПересчетаИзВалютыВВалюту(ВалютаУправленческогоУчета, ВалютаРегламентированногоУчета, КонецМесяца(ДанныеШапки.Дата));
	КоэффициентПересчетаИзВалютыРеглВУпр = РаботаСКурсамиВалютУТ.ПолучитьКоэффициентПересчетаИзВалютыВВалюту(ВалютаРегламентированногоУчета, ВалютаУправленческогоУчета, КонецМесяца(ДанныеШапки.Дата));
	
	ЗаполнитьОбщиеРеквизитыШапкиПодчиненныхДокументов(ДанныеЗаполнения, ДанныеШапки, ЗаполняемыйДокумент);
	
	ЗаполняемыйДокумент.ТипОперации 			  = Перечисления.ТипыОперацийВводаОстатков.ОстаткиСобственныхТоваров;
	...
	ДанныеСверки = РезультатЗапроса[1].Выбрать();
...
Функция ЗаполнитьОприходованиеИзлишковТоваровНаОснованииСверки(ЗаполняемыйДокумент, ДанныеЗаполнения, ИнтерактивноеЗаполнение = Ложь) Экспорт
	...
	Иначе
		ЗаполняемыйДокумент.Валюта = Константы.ВалютаРегламентированногоУчета.Получить();
	КонецЕсли;
...
Процедура СопоставитьЗагружаемыеДанные(АдресЗагружаемыхДанных, АдресТаблицыСопоставления, СписокНеоднозначностей, ПолноеИмяТабличнойЧасти, ДополнительныеПараметры) Экспорт
	...
	Остатки				= ПолучитьИзВременногоХранилища(АдресТаблицыСопоставления);
