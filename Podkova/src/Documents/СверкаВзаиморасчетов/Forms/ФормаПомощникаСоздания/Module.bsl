...
Процедура ВыполнитьПереходПоСтраницам(Команда)
	...
	Если Команда.Имя = "Далее"
		...
		Если НЕ (РасчетыСКлиентами ИЛИ РасчетыСПоставщиками ИЛИ РасчетыСКредитором ИЛИ РасчетыСДебитором ИЛИ РасчетыСЛизингодателем) Тогда
			ТекстСообщения = НСтр("ru = 'Не выбран тип расчетов для сверки.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "РасчетыСКлиентами");
			Возврат;
		КонецЕсли;
...
Функция ПараметрыЗаполнены()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаТаблицаРасчетов Тогда
		...
		Если БудетСоздано = 0 И БудетПерезаполнено = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не отмечена ни одна строка для создания или перезаполнения'"),,"ТаблицаРасчетов");
			Отказ = Истина;
		КонецЕсли;
...
Процедура УстановитьОсновныеОтборы()
	...
	Если РасчетыСЛизингодателем Тогда
		СписокРасчетов.Добавить(Перечисления.ТипыРасчетовСПартнерами.РасчетыСЛизингодателем);
	КонецЕсли;
...
Процедура УстановитьТипРасчетовНаФорме()
	...
	ФлагиОтборов.Вставить(Перечисления.ТипыРасчетовСПартнерами.РасчетыСЛизингодателем, "РасчетыСЛизингодателем");
