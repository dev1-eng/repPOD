...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ (ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыКредитовИДепозитов")
				ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьЛизинг")) Тогда
	...
	СписокВыбора.Добавить(Перечисления.ТипыРасчетовСПартнерами.РасчетыСЛизингодателем);
...
Процедура ГруппировкиРасчетыОбъектРасчетовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РасчетыСКлиентами = ТекущиеДанные.ТипРасчетов = ПредопределенноеЗначение("Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом");
	
	НастройкиВыбора = ФинансыКлиент.ПараметрыВыбораДокументаРасчетов();
	НастройкиВыбора.РедактируемыйДокумент = Объект.Ссылка;
	НастройкиВыбора.ЭтоРасчетыСКлиентами  = РасчетыСКлиентами;
...
Процедура ГруппировкиРасчетыОбъектРасчетовОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		ТекущиеДанные.ОбъектРасчетов = ВыбранноеЗначение.Заказ;
...
Процедура РаспределениеПоДокументамЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	ТабличнаяЧасть = Объект.ГруппировкиРасчеты;
	НазваниеТабличнойЧасти = НСтр("ru = 'Взаиморасчеты'");
	Если Объект.ДанныеКонтрагента.Количество() > 0 Тогда
		ТабличнаяЧасть = Объект.ДанныеКонтрагента;
		НазваниеТабличнойЧасти = НСтр("ru = 'Задолженность по данным контрагента'");
	ИначеЕсли Объект.ГруппировкиРасчеты.Количество() = 0 
		И Объект.ГруппировкиФинансовыеИнструменты.Количество() > 0 Тогда
		ТабличнаяЧасть = Объект.ГруппировкиФинансовыеИнструменты;
		НазваниеТабличнойЧасти = НСтр("ru = 'Расчеты по финансовым инструментам'");
	КонецЕсли;
...
Процедура РассчитатьСуммуВзаиморасчетов(ТекущиеДанные, Суффикс = "")
	...
	Долг = ТекущиеДанные["СуммаДолг" + Суффикс];
	Аванс = ТекущиеДанные["СуммаАванс" + Суффикс];
	ТекущиеДанные["СуммаВзаиморасчетов" + Суффикс] = Макс(Долг, -Долг) + Макс(Аванс, -Аванс);
	
	ИменаПолейИтогов = "СуммаДолг,СуммаАванс,СуммаАвансРегл";
...
Функция РасчетныйДокументОбработкаВыбораНаСеврере(ВыбранноеЗначение)
	...
	ОписаниеДокумента = Документы.СверкаВзаиморасчетов.ОписаниеРасчетногоДокумента(ДанныеДокумента.Ссылка, ДанныеДокумента.Номер, ДанныеДокумента.Дата);
...
Функция ДоговорОбработкаВыбораНаСеврере(ВыбранноеЗначение)
	...
	ИначеЕсли РеквизитыДоговора.ХарактерДоговора = Перечисления.ХарактерыДоговоровФинансовыхИнструментов.Лизинг Тогда
		ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСЛизингодателем;
...
#Область ДетальныеЗаписиФинансовыхИнструментов
	...
	#Область ЛизинговыйПлатежКонтрагент
	Элемент = УсловноеОформление.Элементы.Добавить();

	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ДетальныеЗаписиЛизингЛизинговыйПлатежКонтрагент.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ДетальныеЗаписиФинансовыеИнструменты.ЛизинговыйПлатежКонтрагент");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ДетальныеЗаписиФинансовыеИнструменты.ЛизинговыйПлатеж");

	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветНедоступногоТекста);
	#КонецОбласти
	
	#Область ВыкупПредметаЛизингаКонтрагент
	Элемент = УсловноеОформление.Элементы.Добавить();

	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ДетальныеЗаписиЛизингВыкупПредметаЛизингаКонтрагент.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ДетальныеЗаписиФинансовыеИнструменты.ВыкупПредметаЛизингаКонтрагент");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ДетальныеЗаписиФинансовыеИнструменты.ВыкупПредметаЛизинга");
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ДетальныеЗаписиФинансовыеИнструменты.ЛизинговыйПлатеж");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = 0;
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ДетальныеЗаписиФинансовыеИнструменты.ВыкупПредметаЛизинга");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = 0;
...
Процедура УстановитьВидимость()
	...
	ЕстьВыкупПредметаЛизинга = ЕстьСуммы("ДетальныеЗаписиФинансовыеИнструменты", "ВыкупПредметаЛизинга");
	...
	Иначе
		...
		Иначе
			...
			Элементы.ИтогВыкупПредметаЛизинга.Видимость = ЕстьВыкупПредметаЛизинга;
		КонецЕсли;
	КонецЕсли;
	...
	Элементы.ДетальныеЗаписиЛизингЛизинговыйПлатеж.ОтображатьВШапке = Объект.ЕстьРасхождения;
	Элементы.ДетальныеЗаписиЛизингЛизинговыйПлатежКонтрагент.ОтображатьВШапке = Объект.ЕстьРасхождения;
	Элементы.ДетальныеЗаписиЛизингВыкупПредметаЛизинга.ОтображатьВШапке = Объект.ЕстьРасхождения;
	Элементы.ДетальныеЗаписиЛизингВыкупПредметаЛизингаКонтрагент.ОтображатьВШапке = Объект.ЕстьРасхождения;
	
	УстановитьВидимостьОтветственногоЛица();
	...
	СтруктураПараметров.Вставить("ЕстьВыкупПредметаЛизинга", ЕстьВыкупПредметаЛизинга);
	//++ Локализация
	//СтараяСверка
...
&НаСервере
Процедура УстановитьВидимостьОтветственногоЛица()
...
Процедура ЗаполнитьПоОстаткамСервер()
	...
	ДокументОбъект.ПерезаполненыОстатки = Ложь;
	ДокументОбъект.ДанныеКонтрагента.Очистить();
...
Процедура ЗаполнитьСтрокуПоРасчетномуДокументуСервер(ДокументСсылка, ОписаниеДокумента, ВалютаВзаиморасчетов)
	...
	Если ЗначениеЗаполнено(ЗначенияРеквизитов.Номер) Тогда
		...
		ОписаниеДокумента = Документы.СверкаВзаиморасчетов.ОписаниеРасчетногоДокумента(ДокументСсылка,
			ЗначенияРеквизитов.Номер, ЗначенияРеквизитов.Дата);
...
Функция ПараметрыОтбораДетализации(ВидРасчетов)
	...
	ПараметрыОтбора.Вставить("ТабличнаяЧасть", "ДетальныеЗаписи" + ВидРасчетов);
	ПараметрыОтбора.Вставить("ТаблицаФормы", "ДетальныеЗаписи" + ВидРасчетов);
	...
	ПараметрыОтбора.Вставить("ПолеТипаРасчетов", "ТипРасчетовОбъектаРасчетов");
	ПараметрыОтбора.Вставить("ПоляИтогов", "СуммаДолг,СуммаАванс,СуммаАвансРегл");
	ПараметрыОтбора.Вставить("ТипРасчетов", Объект.ТипРасчетов);
	...
	Если НЕ ЭтоРасчеты Тогда
		...
		ПараметрыОтбора.Вставить("ПолеТипаРасчетов", "ТипРасчетовДоговора");
		ПараметрыОтбора.Вставить("ПоляИтогов", "ОсновнойДолг,Проценты,Комиссия");
	КонецЕсли;
	
	ИмяТЧГруппировки = "Группировки" + ВидРасчетов;
	Если Элементы[ИмяТЧГруппировки].ТекущиеДанные = Неопределено Тогда
		Элементы[ИмяТЧГруппировки].ТекущаяСтрока = Объект[ИмяТЧГруппировки][0].ПолучитьИдентификатор();
	КонецЕсли;
	ПараметрыОтбора.Вставить("ТаблицаГруппировки", ИмяТЧГруппировки);
	ТекущиеДанные = Элементы[ИмяТЧГруппировки].ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		ИмяРеквизита = "ПоказатьГруппировки" + ВидРасчетов;
		ЭтаФорма[ИмяРеквизита] = НЕ ЭтаФорма[ИмяРеквизита];
...
Процедура УстановитьОтборДетализации(ПараметрыОтбора)
	...
	ЗаполнитьЗначенияСвойств(ДеталиОбъекта, ПараметрыОтбора);
...
Процедура УстановитьСледующийОбъект(ТекущийОтбор, ВОбратномНаправлении = Ложь)
	...
	Если НовыеДанные <> Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ТипРасчетов) Тогда
			ТекущийОтбор.ТипРасчетов = НовыеДанные.ТипРасчетов;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Объект.Партнер) Тогда
			ТекущийОтбор.Партнер = НовыеДанные.Партнер;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Объект.Договор) Тогда
			ТекущийОтбор.Договор = НовыеДанные.Договор;
		КонецЕсли;
...
Процедура ЕстьРасхожденияПриИзмененииНаСервере()
	...
	Для Каждого Строка Из Объект.ДетальныеЗаписиФинансовыеИнструменты Цикл
		...
		Строка["ЛизинговыйПлатежКонтрагент"] = ?(Объект.ЕстьРасхождения, Строка["ЛизинговыйПлатеж"], 0);
		Строка["ВыкупПредметаЛизингаКонтрагент"] = ?(Объект.ЕстьРасхождения, Строка["ВыкупПредметаЛизинга"], 0);
	КонецЦикла;
...
Процедура УстановитьВидимостьУстаревшейСверки()
	...
	Элементы.СтраницаФинансовыеИнструменты.Видимость = 
		...
		И (ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыКредитовИДепозитов")
			ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьЛизинг"));
