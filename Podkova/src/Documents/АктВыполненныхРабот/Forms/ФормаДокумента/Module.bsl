...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоСоглашениюИспользуютсяДоговорыКонтрагентов = ЗначениеЗаполнено(Объект.Соглашение) И Документы.АктВыполненныхРабот.ЗначениеРеквизитаОбъектаТипаБулево(Объект.Соглашение, "ИспользуютсяДоговорыКонтрагентов");
...
Процедура НадписьЭтапыОплатыНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("НакладнаяПоЗаказам",                      СписокЗаказов.Количество() > 0 
		И НЕ Объект.ПорядокРасчетов = ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоНакладным"));
	...
	ПараметрыФормы.Вставить("УпрощенныйРежим",                         НЕ ПараметрыФормы.НакладнаяПоЗаказам ИЛИ ГрафикИсполненияВДоговоре
		ИЛИ ИспользоватьУпрощеннуюСхемуОплатыВПродажах И НЕ ИспользоватьАктыВыполненныхРаботПоНесколькимЗаказам);
...
Процедура НадписьЭтапыОплатыНажатиеЗавершение(ПараметрыОплаты, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыОплаты <> Неопределено Тогда
		...
		ИначеЕсли Объект.ЭтапыГрафикаОплаты.Количество() > 0 Тогда
			Объект.ЭтапыГрафикаОплаты[0].ДатаПлатежа = ПараметрыОплаты.ДатаПлатежа;
		Иначе
			НовСтр = Объект.ЭтапыГрафикаОплаты.Добавить();
			НовСтр.ДатаПлатежа = ПараметрыОплаты.ДатаПлатежа;
			НовСтр.СуммаПлатежа = СуммаВсего;
...
&НаКлиенте
Процедура УслугиСуммаВзаиморасчетовПриИзменении(Элемент)
...
Процедура УстановитьВидимостьРеквизитовОплаты()
	...
	УстановитьВидимостьПоляПартнер(ЭтотОбъект);
	Элементы.ЗаголовокОплата.Видимость = Элементы.ЗачетОплатыФорма.Видимость ИЛИ Элементы.Расчеты.Видимость;
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	ИначеЕсли Элемент.Имя = "УслугиСуммаВзаиморасчетов" Тогда
		УслугиСуммаВзаиморасчетовПриИзменении(Элемент);
	Иначе
...
Функция ПоместитьЭтапыОплатыВоВременноеХранилищеНаСервере()
	...
	УникальныйИдентификаторЭтапов = Новый УникальныйИдентификатор;
	Возврат ПоместитьВоВременноеХранилище(Объект.ЭтапыГрафикаОплаты.Выгрузить(),УникальныйИдентификаторЭтапов);
...
Функция ПоместитьТоварыВоВременноеХранилищеНаСервере()
	...
	Возврат ПоместитьВоВременноеХранилище(Объект.Услуги.Выгрузить(,"КодСтроки, Номенклатура, ЗаказКлиента, СуммаСНДС, СуммаВзаиморасчетов"), 
...
Процедура РаспределитьСуммуПоЭтапамГрафикаОплаты()
	...
	СуммаДокумента = ЦенообразованиеКлиентСервер.ПолучитьСуммуДокумента(Объект.Услуги, Объект.ЦенаВключаетНДС);
	
	Если СуммаДокумента = 0 И Объект.ЭтапыГрафикаОплаты.Количество() > 0 Тогда
		Объект.ЭтапыГрафикаОплаты.Очистить();
	ИначеЕсли СуммаДокумента <> Объект.ЭтапыГрафикаОплаты.Итог("СуммаПлатежа") Тогда
