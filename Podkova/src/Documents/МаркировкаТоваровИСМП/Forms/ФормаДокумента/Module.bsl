...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотИмпортСФТС
		И (СтатусИСМП = Перечисления.СтатусыОбработкиМаркировкиТоваровИСМП.КодыАТКСформированы 
				...
				И Объект.ВариантЗаполненияДекларации = Перечисления.ВариантыЗаполненияДекларацииИСМП.КодыМаркировки) Тогда
				
				Если Не ЗначениеЗаполнено(Объект.РегистрационныйНомерДекларации) Тогда
					ТекстСообщения = Нстр("ru = 'Регистрационный номер декларации не заполнен'");
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,, "Объект.РегистрационныйНомерДекларации", Отказ);
				КонецЕсли;
				
				Если Не ЗначениеЗаполнено(Объект.ДатаДекларации) Тогда
					ТекстСообщения = Нстр("ru = 'Дата декларации не заполнена'");
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,, "Объект.ДатаДекларации", Отказ);
				КонецЕсли;
...
Процедура ОперацияНанесенияПриИзменении(Элемент)
	...
	ТребуетсяОчиститьКодыМаркировки = ЗначениеЗаполнено(Объект.ОперацияНанесения) И Не ЗначениеЗаполнено(ПредыдущаяОперация);
	...
	Если ТребуетсяОчиститьКодыМаркировки И Объект.ШтрихкодыУпаковок.Количество() Тогда
		...
		ТекстВопроса = НСтр(
			"ru = 'При указании операции нанесения табличная часть Штрихкоды упаковок будет очищена. Продолжить?'");
		Кнопки = РежимДиалогаВопрос.ДаНет; 
		ПоказатьВопрос(ОписаниеОповещенияОЗавершении, ТекстВопроса, Кнопки);
		Возврат;
