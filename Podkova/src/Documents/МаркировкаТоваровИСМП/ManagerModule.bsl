...
Функция СтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ВидыОперацийИСМП.ПолучениеРезультатаОбработкиДокумента
		...
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры, "ЕстьЗависимыеСообщения", Ложь)
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(Шапка.ВидПродукции, Ложь, Истина)
		...
		Иначе
			...
			Настройки = Новый Структура(
				"ЗапрашиватьСтатусыИСМП, СсылкаНаОбъект",
				Ложь, ДокументСсылка);
...
#Область РазборКодовМаркировки
	...
	Если Шапка.ОтчетПроизводственнойЛинии Тогда
		...
		Для Каждого СтрокаДанныхОтчета Из ДанныеОтчета Цикл
			...
			ЭлементыСтрокиДанныхОтчета = Новый Массив;
			ЭлементыСтрокиДанныхОтчета.Добавить(СтрокаДанныхОтчета.ЗначениеШтрихкода);
			Если ЗначениеЗаполнено(СтрокаДанныхОтчета.ЗначениеШтрихкодаУпаковки) Тогда
				ЭлементыСтрокиДанныхОтчета.Добавить(СтрокаДанныхОтчета.ЗначениеШтрихкодаУпаковки);
			КонецЕсли;
			
			Для Каждого ЭлементСтрокиДанныхОтчета Из ЭлементыСтрокиДанныхОтчета Цикл
				
				ДанныеРазбораШтрихкода = ГрупповаяОбработкаШтрихкодовИС.ВидУпаковкиИПредставлениеШтрихкода(
					ЭлементСтрокиДанныхОтчета, Шапка.ВидПродукции, КешДанныхРазбора,
				...
				Если ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(Шапка.ВидПродукции) Тогда
					Если ДанныеРазбораШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
						...
						ДанныеРазбораИПримечание = КешДанныхРазбора[ЭлементСтрокиДанныхОтчета];
						...
						НормализованныйКодМаркировки = РазборКодаМаркировкиИССлужебныйКлиентСервер.НормализоватьКодМаркировки(
							ДанныеРазбораИПримечание.ДанныеРазбора, Шапка.ВидПродукции, ПараметрыНормализацииТабакМаркируемыйТовар);
						
					КонецЕсли;
				Иначе
					НормализованныйКодМаркировки = ДанныеРазбораШтрихкода.НормализованныйШтрихкод;
				КонецЕсли;
				
				Если ДанныеРазбораШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
					Или ДанныеРазбораШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
					
					ДобавитьКодКПолучению(ПолныеКодыМаркировки, НормализованныйКодМаркировки);
	...
	Иначе
		...
		Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.МаркированныеТовары Цикл
			...
			Если СтрокаТЧМаркированныеТовары.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
				ДобавитьКодКПолучению(ПолныеКодыМаркировки, СтрокаТЧМаркированныеТовары.Штрихкод, ЭтоМолочнаяПродукция, СтрокаТЧМаркированныеТовары);
				ШтрихкодированиеИСМП.ДобавитьКодМаркировкиВТаблицуДляПоискаВПуле(
			...
			Если СтрокаРодитель <> Неопределено Тогда
				Если СоответствиеБлоков[СтрокаРодитель.Штрихкод] = Неопределено Тогда
					...
					Если СтрокаРодитель.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
						ДобавитьКодКПолучению(ПолныеКодыМаркировки, СтрокаРодитель.Штрихкод, ЭтоМолочнаяПродукция, СтрокаРодитель);
						ШтрихкодированиеИСМП.ДобавитьКодМаркировкиВТаблицуДляПоискаВПуле(
...
#КонецОбласти
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(Шапка.ВидПродукции, Ложь, Истина)
		...
		ИначеЕсли Шапка.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотПроизводствоВнеЕАЭС Тогда
			...
			ПроверитьЗаполнениеДатыДекларации(СообщениеJSON, Шапка);
			ПроверитьЗаполнениеИНН(СообщениеJSON, РеквизитыОрганизации, Шапка);
			ПроверитьЗаполнениеКодаТНВЭДШапки(СообщениеJSON, Шапка);
	...
	ИначеЕсли ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетОтчетыОНанесенииКодовМаркировки(Шапка.ВидПродукции)
		...
		ЗаполнитьСоответствиеПолныеКодыМаркировки(ПолныеКодыМаркировки, РезультатПоискаВПуле, ЭтоМолочнаяПродукция);
...
Функция АгрегацияJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Если ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетОтчетыОНанесенииКодовМаркировки(Шапка.ВидПродукции)
...
Процедура ЗаполнитьДанныеОбъектаФормыПоДеревуУпаковок(Форма, ДеревоУпаковок, ПараметрыЗаполнения,
	...
	ЭтоМолочнаяПродукция = Форма.Объект.ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС;
...
Процедура ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыЗаполнения)
	...
	Если (ИнтеграцияИСМПКлиентСервер.ОперацииНанесенияКодовМаркировки().Найти(Форма.Объект.Операция) <> Неопределено
			...
			И ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(Форма.Объект.ВидПродукции) Тогда
		СпецификаОбработки = Форма.Объект.ВидПродукции;
	КонецЕсли;
	...
	Если СпецификаОбработки = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС Тогда
...
Процедура ОтразитьРезультатыПроверкиИПодбора(ПараметрыОкончанияПроверки) Экспорт
	...
	СпецификаОперацииНанесения = Неопределено;
	Если ИнтеграцияИСМПКлиентСервер.ОперацииНанесенияКодовМаркировки().Найти(ДокументОбъект.Операция) <> Неопределено
			Или ЗначениеЗаполнено(ДокументОбъект.ОперацияНанесения) Тогда
		СпецификаОперацииНанесения = ДокументОбъект.ВидПродукции;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеДатыДекларации(СообщениеJSON, Шапка)
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из ДанныеПоШтрихкодам.МаркированныеТовары Цикл
		...
		Если ТребуетсяНоменклатура Тогда
			...
			Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(Шапка.ВидПродукции) Тогда
				Отбор.Вставить("СрокГодности", СтрокаТЧМаркированныеТовары.ГоденДо);
...
Процедура ЗаполнитьСведенияОТоварахПоДаннымОтчета(
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из ДанныеОтчета Цикл
		...
		РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
			СтрокаТЧМаркированныеТовары.ЗначениеШтрихкода, Шапка.ВидПродукции, Ложь);
...
Функция ПроверитьИРазбитьНаГруппыПолныеКодыМаркировки(СообщенияJSON, СообщениеJSON, ГруппыКодов,
	...
	ЕстьОшибки = Ложь;
	Для Каждого ГруппаИдентификаторВЕТИС Из ПолныеКодыМаркировки Цикл
		Для Каждого ГруппаГоденДо Из ГруппаИдентификаторВЕТИС.Значение Цикл
			...
			ОписаниеПакета.Вставить("КлючИдентификаторПроисхожденияВЕТИС", ГруппаИдентификаторВЕТИС.Ключ);
			ОписаниеПакета.Вставить("КлючГоденДо", ГруппаГоденДо.Ключ);
			ГруппыКодов.Добавить(ОписаниеПакета);
			Для Каждого КлючИЗначение Из ГруппаГоденДо.Значение Цикл
				...
				Если ПолныйКодМаркировки = Неопределено Тогда
					...
					ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
						СообщениеJSON,
						СтрШаблон(НСтр("ru = 'Не найден полный код маркировки для кода %1.'"), КодМаркировки));
				Иначе
...
Процедура ДобавитьКодКПолучению(ПолныеКодыМаркировки, НормализованныйШтрихкод, МолочнаяПродукция = Ложь, ДанныеШтрихкодовУпаковок = Неопределено)
	...
	ГруппаИдентификаторВЕТИС = ПолныеКодыМаркировки.Получить(ИдентификаторПроисхожденияВЕТИС);
	Если ГруппаИдентификаторВЕТИС = Неопределено Тогда
		ГруппаИдентификаторВЕТИС = Новый Соответствие;
	КонецЕсли;
	ГруппаГоденДо = ГруппаИдентификаторВЕТИС.Получить(ГоденДо);
	Если ГруппаГоденДо = Неопределено Тогда
		ГруппаГоденДо = Новый Соответствие;
	КонецЕсли;
	
	ГруппаГоденДо.Вставить(НормализованныйШтрихкод);
	ГруппаИдентификаторВЕТИС.Вставить(ГоденДо, ГруппаГоденДо);
	ПолныеКодыМаркировки.Вставить(ИдентификаторПроисхожденияВЕТИС, ГруппаИдентификаторВЕТИС);
	
КонецПроцедуры

Процедура ЗаполнитьСоответствиеПолныеКодыМаркировки(ПолныеКодыМаркировки, РезультатПоискаВПуле, Знач МолочнаяПродукция = Ложь)
	
	МолочнаяПродукция = МолочнаяПродукция И РезультатПоискаВПуле.Колонки.Найти("ГоденДо") <> Неопределено;
	
	КлючиМолочнойПродукции = Новый Структура("ИдентификаторПроисхожденияВЕТИС, ГоденДо", "", "");
	Для Каждого СтрокаТаблицы Из РезультатПоискаВПуле Цикл
		
		Если МолочнаяПродукция Тогда
			ЗаполнитьЗначенияСвойств(КлючиМолочнойПродукции, СтрокаТаблицы);
			Если КлючиМолочнойПродукции.ИдентификаторПроисхожденияВЕТИС = Неопределено Тогда
				КлючиМолочнойПродукции.ИдентификаторПроисхожденияВЕТИС = "";
			КонецЕсли;
		КонецЕсли;
		
		ГруппаИдентификаторВЕТИС = ПолныеКодыМаркировки.Получить(КлючиМолочнойПродукции.ИдентификаторПроисхожденияВЕТИС);
		Если ГруппаИдентификаторВЕТИС = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ГруппаГоденДо = ГруппаИдентификаторВЕТИС.Получить(КлючиМолочнойПродукции.ГоденДо);
		Если ГруппаГоденДо = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ГруппаГоденДо.Вставить(СтрокаТаблицы.КодМаркировки, СтрокаТаблицы.ПолныйКодМаркировки);
