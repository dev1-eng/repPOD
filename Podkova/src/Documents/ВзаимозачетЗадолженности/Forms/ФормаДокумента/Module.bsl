...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	УстановитьСтатусФискальнойОперации();
...
Процедура ТекстДокументыНаОснованииОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ФискальнаяОперацияОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
...
&НаСервере
Процедура УстановитьСтатусФискальнойОперации()

	Документы.ВзаимозачетЗадолженности.УстановитьСтатусФискальнойОперации(ЭтотОбъект, Объект);
	
КонецПроцедуры

&НаКлиенте
Процедура ФискальнаяОперацияОбработкаНавигационнойСсылки(НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "КонтрагентПоставщикПробитьЧек" Тогда
		...
		ПараметрыФормы.Вставить("Партнер",        Объект.КонтрагентДебитор);
		ПараметрыФормы.Вставить("ТорговыйОбъект", Неопределено);
		ПараметрыФормы.Вставить("ВидЧека",        "КонтрагентПоставщик");
	...
	ИначеЕсли НавигационнаяСсылка = "КонтрагентДебиторПробитьЧек" Тогда
		...
		ПараметрыФормы.Вставить("Партнер",        Объект.КонтрагентДебитор);
		ПараметрыФормы.Вставить("ТорговыйОбъект", Неопределено);
		ПараметрыФормы.Вставить("ВидЧека",        "КонтрагентДебитор");
	...
	ИначеЕсли НавигационнаяСсылка = "КонтрагентКредиторПробитьЧек" Тогда
		...
		ПараметрыФормы.Вставить("ДокументСсылка", Объект.Ссылка);
		ПараметрыФормы.Вставить("Организация",    Объект.Организация);
		ПараметрыФормы.Вставить("Партнер",        Объект.КонтрагентКредитор);
		ПараметрыФормы.Вставить("ТорговыйОбъект", Неопределено);
		ПараметрыФормы.Вставить("ВидЧека",        "КонтрагентКредитор");
	...
	ИначеЕсли НавигационнаяСсылка = "ОрганизацияПробитьЧек" Тогда
		...
		ПараметрыФормы.Вставить("Партнер",        Объект.КонтрагентДебитор);
		ПараметрыФормы.Вставить("ТорговыйОбъект", Неопределено);
		ПараметрыФормы.Вставить("ВидЧека",        "ОрганизацияДебитор");
	...
	ИначеЕсли НавигационнаяСсылка = "ОрганизацияКредиторПробитьЧек" Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ДокументСсылка", Объект.Ссылка);
		ПараметрыФормы.Вставить("Организация",    Объект.ОрганизацияКредитор);
		ПараметрыФормы.Вставить("Партнер",        Объект.КонтрагентДебитор);
		ПараметрыФормы.Вставить("ТорговыйОбъект", Неопределено);
		ПараметрыФормы.Вставить("ВидЧека",        "ОрганизацияКредитор");
...
Процедура ПолучитьДебиторскуюЗадолженностиИзХранилища(АдресПлатежейВХранилище)
	...
	ДебиторскаяЗадолженность.Свернуть("ТипРасчетов, Партнер, Заказ, ВалютаВзаиморасчетов, Организация","Сумма, НашДолг, ДолгПартнера");
...
Процедура ПолучитьКредиторскуюЗадолженностиИзХранилища(АдресПлатежейВХранилище)
	...
	КредиторскаяЗадолженность.Свернуть("ТипРасчетов, Партнер, Заказ, ВалютаВзаиморасчетов, Организация","Сумма, НашДолг, ДолгПартнера");
...
Функция ОстаткиЗадолженностиПоОбъектамРасчетов(ПараметрыАналитики, МассивЗаказов)
	...
	Иначе
		...
		|ВЫБРАТЬ
		|	Аналитика.Партнер                                       КАК Партнер,
		|	Остатки.ОбъектРасчетов                                  КАК Заказ,
		...
		|ИЗ РегистрНакопления.РасчетыСКлиентамиПоСрокам.Остатки(&Граница, ОбъектРасчетов В (&МассивЗаказов) И АналитикаУчетаПоПартнерам В (
		|																ВЫБРАТЬ Аналитика.АналитикаУчетаПоПартнерам
		|																ИЗ ВтАналитика КАК Аналитика)) КАК Остатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтАналитика КАК Аналитика
		|			ПО Аналитика.АналитикаУчетаПоПартнерам = Остатки.АналитикаУчетаПоПартнерам
		...
		|	Остатки.ДолгРеглОстаток + Остатки.ПредоплатаРеглОстаток КАК СуммаРегл,
		|	Остатки.ДолгУпрОстаток + Остатки.ПредоплатаУпрОстаток   КАК СуммаУпр
		|ИЗ РегистрНакопления.РасчетыСПоставщикамиПоСрокам.Остатки(&Граница, ОбъектРасчетов В (&МассивЗаказов) И АналитикаУчетаПоПартнерам В (
		|																ВЫБРАТЬ Аналитика.АналитикаУчетаПоПартнерам
		|																ИЗ ВтАналитика КАК Аналитика)) КАК Остатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтАналитика КАК Аналитика
		|			ПО Аналитика.АналитикаУчетаПоПартнерам = Остатки.АналитикаУчетаПоПартнерам
