...
Функция ТекстЗапросаРасчетыСКлиентами()
	...
	|			ТОГДА НАЧАЛОПЕРИОДА(ВЫРАЗИТЬ(ТабличнаяЧасть.Ссылка КАК Документ.ВзаимозачетЗадолженности).Дата,ДЕНЬ)
	|		ИНАЧЕ ДАТАВРЕМЯ(1,1,1)
	|	КОНЕЦ                                          КАК ДатаПлатежа,
	|	ВЫРАЗИТЬ(ТабличнаяЧасть.Ссылка КАК Документ.ВзаимозачетЗадолженности).Дата   КАК ДатаРегистратора,
	|	ВЫРАЗИТЬ(ТабличнаяЧасть.Ссылка КАК Документ.ВзаимозачетЗадолженности).Номер  КАК НомерРегистратора,
...
Процедура УстановитьСтатусФискальнойОперации(Форма, Источник) Экспорт
	
	Организация = Источник.Организация;
	ОрганизацияКредитор = Источник.ОрганизацияКредитор;
	
	КонтрагентДебитор = Источник.КонтрагентДебитор;
	КонтрагентКредитор = Источник.КонтрагентКредитор;
	...
	Если ЗначениеЗаполнено(КонтрагентДебитор) Тогда
		ТипКонтрагентДебитор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтрагентДебитор, "ЮрФизЛицо");
	КонецЕсли;
	
	ТипКонтрагентКредитор = Неопределено;
	Если ЗначениеЗаполнено(КонтрагентКредитор) Тогда
		ТипКонтрагентКредитор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтрагентКредитор, "ЮрФизЛицо");
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ОрганизацияКредитор)
		...
		ФискальнаяОперацияДанныеЖурнала = МенеджерОборудованияВызовСервера.ДанныеФискальнойОперации(Источник.Ссылка, , , ТипРасчета);
		
		Если ФискальнаяОперацияДанныеЖурнала = Неопределено Тогда
			ПредставлениеПробитьЧек = НСтр("ru = 'пробить чек'");
			
			МассивПредставлений.Добавить(
					Новый ФорматированнаяСтрока(
						ПредставлениеПробитьЧек,,
						ЦветаСтиля.ЦветГиперссылки,,
						"КонтрагентПоставщикПробитьЧек"));
		...
		Иначе
			...
			МассивПредставлений.Добавить(
				Новый ФорматированнаяСтрока(
					...
					ЦветаСтиля.ЦветГиперссылки,,
					"КонтрагентПоставщикОткрытьЗаписьФискальнойОперации"));
		КонецЕсли;
			...
			МассивПредставлений.Добавить(
				Новый ФорматированнаяСтрока(
					СтрШаблон(
						...
						ФискальнаяОперацияДанныеЖурнала.НомерЧекаККМ),,
					ЦветаСтиля.ЦветГиперссылки,,
					"КонтрагентДебиторОткрытьЗаписьФискальнойОперации"));
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ОрганизацияКредитор)
		...
		ФискальнаяОперацияДанныеЖурнала = МенеджерОборудованияВызовСервера.ДанныеФискальнойОперации(Источник.Ссылка);
			...
			Иначе
				...
				МассивПредставлений.Добавить(
						Новый ФорматированнаяСтрока(
							ПредставлениеПробитьЧек,,
							ЦветаСтиля.ЦветГиперссылки,,
							"КонтрагентКредиторПробитьЧек"));
				
			КонецЕсли;
		
		Иначе
			
			ИмяГиперссылки = "КонтрагентДебиторОткрытьЗаписьФискальнойОперации";
			Если ТипКонтрагентДебитор <> Перечисления.ЮрФизЛицо.ФизЛицо Тогда
				ИмяГиперссылки = "КонтрагентКредиторОткрытьЗаписьФискальнойОперации";
			КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ОрганизацияКредитор)
		...
		МассивПредставлений.Добавить("" + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтрагентДебитор, "Наименование") + ": ");
		...
		МассивПредставлений.Добавить("" + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтрагентКредитор, "Наименование") + ": ");
			...
			МассивПредставлений.Добавить(
				Новый ФорматированнаяСтрока(
					СтрШаблон(
						...
						ФискальнаяОперацияДанныеЖурнала.НомерЧекаККМ),,
					ЦветаСтиля.ЦветГиперссылки,,
					"КонтрагентКредиторОткрытьЗаписьФискальнойОперации"));
	...
	Если ЗначениеЗаполнено(ОрганизацияКредитор)
		...
		И ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.КлиентаМеждуОрганизациями
		И ТипКонтрагентДебитор = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		
		МассивПредставлений.Добавить("" + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "Наименование") + ": ");
			...
			МассивПредставлений.Добавить(
				Новый ФорматированнаяСтрока(
					СтрШаблон(
						...
						ФискальнаяОперацияДанныеЖурнала.НомерЧекаККМ),,
					ЦветаСтиля.ЦветГиперссылки,,
					"ОрганизацияОткрытьЗаписьФискальнойОперации"));
		...
		МассивПредставлений.Добавить("" + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОрганизацияКредитор, "Наименование") + ": ");
			...
			МассивПредставлений.Добавить(
				Новый ФорматированнаяСтрока(
					СтрШаблон(
						...
						ФискальнаяОперацияДанныеЖурнала.НомерЧекаККМ),,
					ЦветаСтиля.ЦветГиперссылки,,
					"ОрганизацияКредиторОткрытьЗаписьФискальнойОперации"));
			
		КонецЕсли;
		
	КонецЕсли;
	
	Форма.ТекстДокументыНаОснованииПодвал = Новый ФорматированнаяСтрока(МассивПредставлений);
...
//
Функция ПараметрыОперацииФискализацииЧека(ДокументСсылка, СуммаПредоплатыКорректировка = Неопределено, ВерсияФФД = Неопределено, ВидЧека = Неопределено) Экспорт
	
	ПараметрыФискализацииЧека = Неопределено;
	
	Если ВидЧека = "КонтрагентПоставщик" Тогда
		ПараметрыФискализацииЧека = ПараметрыОперацииФискализацииЧекаКонтрагентПоставщик(ДокументСсылка, ВерсияФФД);
	ИначеЕсли ВидЧека = "КонтрагентДебитор" Тогда
		ПараметрыФискализацииЧека = ПараметрыОперацииФискализацииЧекаКонтрагентДебитор(ДокументСсылка, ВерсияФФД);
	ИначеЕсли ВидЧека = "КонтрагентКредитор" Тогда
		ПараметрыФискализацииЧека = ПараметрыОперацииФискализацииЧекаКонтрагентКредитор(ДокументСсылка, ВерсияФФД);
	ИначеЕсли ВидЧека = "ОрганизацияДебитор" Тогда
		ПараметрыФискализацииЧека = ПараметрыОперацииФискализацииЧекаОрганизацияДебитор(ДокументСсылка, ВерсияФФД);
	ИначеЕсли ВидЧека = "ОрганизацияКредитор" Тогда
		ПараметрыФискализацииЧека = ПараметрыОперацииФискализацииЧекаОрганизацияКредитор(ДокументСсылка, ВерсияФФД);
	КонецЕсли;
...
Функция ПолучитьДанныеШапкиДокументаДляПараметровФискальногоЧека(ДокументСсылка)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВзаимозачетЗадолженности.Организация,
	|	ВзаимозачетЗадолженности.ОрганизацияКредитор,
	|	ВзаимозачетЗадолженности.КонтрагентДебитор,
	|	ВзаимозачетЗадолженности.КонтрагентКредитор
	|ИЗ
...
Функция ПолучитьДанныеПоЗадолженностиПередПоставщикамиДляДляПараметровФискальногоЧека(ДокументСсылка)
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	РасчетыСПоставщикамиПоСрокам.РасчетныйДокумент КАК ОбъектРасчетов,
	...
	Иначе
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ВзаимозачетЗадолженностиКредиторскаяЗадолженность.Заказ КАК ОбъектРасчетов,
...
Функция ПолучитьДанныеПоДебиторскойЗадолженностиДляДляПараметровФискальногоЧека(ДокументСсылка)
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	РасчетыСКлиентамиПоСрокам.РасчетныйДокумент КАК ОбъектРасчетов,
		|	СУММА(РасчетыСКлиентамиПоСрокам.ПредоплатаРегл + РасчетыСКлиентамиПоСрокам.ДолгРегл) КАК Сумма
		|ИЗ
	...
	Иначе
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ВзаимозачетЗадолженностиДебиторскаяЗадолженность.Заказ КАК ОбъектРасчетов,
		|	СУММА(ВзаимозачетЗадолженностиДебиторскаяЗадолженность.СуммаРегл) КАК Сумма
		|ИЗ
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ВзаимозачетЗадолженностиДебиторскаяЗадолженность.Заказ";
		Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
...
Процедура ЗаполнитьШапкуПараметровОперацииФискализацииЧека(ПараметрыФискализацииЧека, ДокументСсылка, Организация, Контрагент, ТипРасчета)
	
	СведенияОбОрганизации = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(Организация, ТекущаяДатаСеанса());
	
	СведенияОПокупателе = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(Контрагент, ТекущаяДатаСеанса());
	ТипКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ЮрФизЛицо");
	
	ПараметрыФискализацииЧека.ДокументОснование      = ДокументСсылка;
	
	ПараметрыФискализацииЧека.ТипРасчета             = ТипРасчета;
	ПараметрыФискализацииЧека.СистемаНалогообложения = РозничныеПродажи.СистемаНалогообложенияФискальнойОперации(Организация);
	
	ПараметрыФискализацииЧека.Организация            = Организация;
	ПараметрыФискализацииЧека.ОрганизацияНазвание    = СведенияОбОрганизации.ПолноеНаименование;
	ПараметрыФискализацииЧека.ОрганизацияИНН         = СведенияОбОрганизации.ИНН;
	ПараметрыФискализацииЧека.ОрганизацияКПП         = СведенияОбОрганизации.КПП;
	
	ПараметрыФискализацииЧека.АдресМагазина          = СведенияОбОрганизации.ФактическийАдрес;
	ПараметрыФискализацииЧека.НаименованиеМагазина   = СведенияОбОрганизации.СокращенноеНаименование;
	
	ПараметрыФискализацииЧека.ТорговыйОбъект         = СведенияОбОрганизации.СокращенноеНаименование;
...
Процедура ЗаполнитьПозицииПараметровОперацииФискализацииЧекаОплатойКредита(ПараметрыФискализацииЧека, Сумма)
	...
	СтрокаПозицииЧека.Наименование = НСтр("ru = 'Оплачено'");
	
	СтрокаПозицииЧека.Количество   = 1;
	
	СтрокаПозицииЧека.Цена         = Сумма;
	
	СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС20_120");
	СтрокаПозицииЧека.СтавкаНДС = РозничныеПродажиКлиентСервер.СтавкаНДСФискальнойОперации(СтавкаНДС, Истина);
	
	СтрокаПозицииЧека.Сумма          = Сумма;
	СтрокаПозицииЧека.ЦенаСоСкидками = Сумма;
...
Процедура ЗаполнитьПозицииПараметровОперацииФискализацииЧекаПоОбъектуРасчетов(ПараметрыФискализацииЧека, ПризнакСпособаРасчета, ОбъектРасчетов, Сумма, ВерсияФФД)
	
	ПолноеИмя = ОбъектРасчетов.Метаданные().ПолноеИмя();
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
	
	ПараметрыОперацииФискализацииЧекаПоОбъектуРасчетов = МенеджерОбъекта.ПараметрыОперацииФискализацииЧека(
		ОбъектРасчетов,
		,
		ВерсияФФД);
	...
	Иначе
		...
		ОбщаяСуммаЧека = 0;
		Для Каждого СтрокаПозицииЧека Из ПараметрыОперацииФискализацииЧекаПоОбъектуРасчетов.ПозицииЧека Цикл
			Если СтрокаПозицииЧека.ПризнакСпособаРасчета = Перечисления.ПризнакиСпособаРасчета.ПредоплатаПолная Тогда
				СтрокаПозицииЧека.ПризнакСпособаРасчета = Перечисления.ПризнакиСпособаРасчета.ПредоплатаЧастичная;
			КонецЕсли;
			ОбщаяСуммаЧека = ОбщаяСуммаЧека + СтрокаПозицииЧека.Сумма;
		КонецЦикла;
		
		ФорматноЛогическийКонтрольКлиентСервер.ПривестиСуммыПозицийЧекаКСуммеТаблицыОплат(
			ПараметрыОперацииФискализацииЧекаПоОбъектуРасчетов, ОбщаяСуммаЧека, 0, 0, Ложь, "");
	КонецЕсли;
...
Процедура ЗаполнитьПозицииПараметровОперацииФискализацииЧекаВозвратомАванса(ПараметрыФискализацииЧека, Сумма)
	...
	СтрокаПозицииЧека.Наименование = НСтр("ru = 'Выплачено'");
...
Процедура ЗаполнитьТаблицуОплатПараметровОперацииФискализацииЧека(ПараметрыФискализацииЧека)
	
	ИтогоПоПозициямЧека = 0;
	Для Каждого СтрокаПозицииЧека Из ПараметрыФискализацииЧека.ПозицииЧека Цикл
		ИтогоПоПозициямЧека = ИтогоПоПозициямЧека + СтрокаПозицииЧека.Сумма; 
	КонецЦикла;
	
	СтрокаОплаты = Новый Структура();
	СтрокаОплаты.Вставить("ТипОплаты", Перечисления.ТипыОплатыККТ.ВстречноеПредоставление);
	СтрокаОплаты.Вставить("Сумма"    , ИтогоПоПозициямЧека);
	ПараметрыФискализацииЧека.ТаблицаОплат.Добавить(СтрокаОплаты);
	
КонецПроцедуры

Функция ПараметрыОперацииФискализацииЧекаКонтрагентПоставщик(ДокументСсылка, ВерсияФФД)
	
	ПараметрыФискализацииЧека = МенеджерОборудованияКлиентСервер.ПараметрыОперацииФискализацииЧека();
	
	// Шапка
	Шапка = ПолучитьДанныеШапкиДокументаДляПараметровФискальногоЧека(ДокументСсылка);
	
	Организация = Шапка.Организация;
	Контрагент  = Шапка.КонтрагентКредитор;
	ТипРасчета  = Перечисления.ТипыРасчетаДенежнымиСредствами.РасходДенежныхСредств;
	
	ЗаполнитьШапкуПараметровОперацииФискализацииЧека(ПараметрыФискализацииЧека, ДокументСсылка, Организация, Контрагент, ТипРасчета);
	
	// Позиции чека
	ТаблицаСуммыВзаимозачетовПоОбъектамРасчетов = ПолучитьДанныеПоЗадолженностиПередПоставщикамиДляДляПараметровФискальногоЧека(ДокументСсылка);
	
	Для Каждого ОбъектРасчетовССуммойВзаимозачета Из ТаблицаСуммыВзаимозачетовПоОбъектамРасчетов Цикл
		
		Попытка
			
			ПризнакСпособаРасчета = Перечисления.ПризнакиСпособаРасчета.ОплатаКредита;
			ОбъектРасчетов = ОбъектРасчетовССуммойВзаимозачета.ОбъектРасчетов;
			Сумма = ОбъектРасчетовССуммойВзаимозачета.Сумма;
			
			ЗаполнитьПозицииПараметровОперацииФискализацииЧекаПоОбъектуРасчетов(ПараметрыФискализацииЧека, ПризнакСпособаРасчета, ОбъектРасчетов, Сумма, ВерсияФФД);
			
		Исключение
			
			ЗаполнитьПозицииПараметровОперацииФискализацииЧекаОплатойКредита(ПараметрыФискализацииЧека, ОбъектРасчетовССуммойВзаимозачета.Сумма);
	...
	// Таблица оплат
	ЗаполнитьТаблицуОплатПараметровОперацииФискализацииЧека(ПараметрыФискализацииЧека);
	
	Возврат ПараметрыФискализацииЧека;
	
КонецФункции

Функция ПараметрыОперацииФискализацииЧекаКонтрагентДебитор(ДокументСсылка, ВерсияФФД)
	...
	Организация = Шапка.Организация;
	Контрагент  = Шапка.КонтрагентДебитор;
	ТипРасчета  = Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств;
	
	ЗаполнитьШапкуПараметровОперацииФискализацииЧека(ПараметрыФискализацииЧека, ДокументСсылка, Организация, Контрагент, ТипРасчета);
	
	// Позиции чека
	ТаблицаСуммыВзаимозачетовПоОбъектамРасчетов = ПолучитьДанныеПоДебиторскойЗадолженностиДляДляПараметровФискальногоЧека(ДокументСсылка);
...
Функция ПараметрыОперацииФискализацииЧекаКонтрагентКредитор(ДокументСсылка, ВерсияФФД)
	...
	// Позиции чека
	ТаблицаСуммыВзаимозачетовПоОбъектамРасчетов = ПолучитьДанныеПоКредиторскойЗадолженностиДляДляПараметровФискальногоЧека(ДокументСсылка);
	...
	Для Каждого ОбъектРасчетовССуммойВзаимозачета Из ТаблицаСуммыВзаимозачетовПоОбъектамРасчетов Цикл
		...
		Попытка
			...
			ЗаполнитьПозицииПараметровОперацииФискализацииЧекаПоОбъектуРасчетов(ПараметрыФискализацииЧека, Неопределено, ОбъектРасчетов, Сумма, ВерсияФФД);
			
		Исключение
			
			ЗаполнитьПозицииПараметровОперацииФискализацииЧекаВозвратомАванса(ПараметрыФискализацииЧека, ОбъектРасчетовССуммойВзаимозачета.Сумма);
...
Функция ПараметрыОперацииФискализацииЧекаОрганизацияДебитор(ДокументСсылка, ВерсияФФД)
...
Функция ПараметрыОперацииФискализацииЧекаОрганизацияКредитор(ДокументСсылка, ВерсияФФД)
	...
	Организация = Шапка.ОрганизацияКредитор;
	Контрагент  = Шапка.КонтрагентДебитор;
	ТипРасчета  = Перечисления.ТипыРасчетаДенежнымиСредствами.ВозвратДенежныхСредств;
