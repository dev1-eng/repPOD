...
Процедура ОперацияПриИзменении(Элемент)
	...
	ОчиститьЗависимыеРеквизиты(ОперацияВыводИзОборота, ОперацияГосКонтракт);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	НастроитьДоступностьОперацииГосКонтракта();
	НастроитьТипКонтрагентаПоУмолчанию();
...
&НаСервере
Процедура НастроитьДоступностьОперацииГосКонтракта()
	
	Если Объект.Ссылка.Пустая()
		ИЛИ Объект.Операция <> Перечисления.ВидыОперацийИСМП.ОтгрузкаПриобретениеГосПредприятием Тогда
		
		Используется275ФЗ = Ложь;
		ИнтеграцияИСМППереопределяемый.ИспользуетсяПоддержкаПлатежейВСоответствииС275ФЗ(Используется275ФЗ);
		Если Не Используется275ФЗ Тогда
			УдаляемыйЭлемент = Элементы.Операция.СписокВыбора.НайтиПоЗначению(Перечисления.ВидыОперацийИСМП.ОтгрузкаПриобретениеГосПредприятием);
			Если УдаляемыйЭлемент <> Неопределено Тогда
				Элементы.Операция.СписокВыбора.Удалить(УдаляемыйЭлемент);
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Процедура НастроитьТипКонтрагентаПоУмолчанию()
	...
	Если Не ИспользуетсяПередачаТоваровМеждуОрганизациями Тогда
		...
		Элементы.Контрагент.ДоступныеТипы = Метаданные.ОпределяемыеТипы.КонтрагентГосИС.Тип;
	КонецЕсли;
...
&НаКлиенте
Процедура ОчиститьЗависимыеРеквизиты(ОперацияВыводИзОборота, ОперацияГосКонтракт)
...
Процедура КонтрольДатыВыводаИзОборота()
	...
	Если Объект.ДатаВыводаИзОборота < Объект.ДатаПервичногоДокумента Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Объект.ДатаВыводаИзОборота");
	КонецЕсли;
