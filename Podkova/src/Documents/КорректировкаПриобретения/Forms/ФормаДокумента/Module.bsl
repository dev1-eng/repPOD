...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УчетПрослеживаемыхТоваровЛокализация.УстановитьВидимостьКоличестваРНПТ(ЭтаФорма, Объект.Дата, "РасхожденияГруппаКоличествоПоРНПТ");
...
Процедура РасхожденияПриАктивизацииЯчейки(Элемент)
	...
	Если Элемент.ТекущийЭлемент <> Неопределено
		...
		Если СтрокаТаблицы.КоличествоУпаковок > 0 Тогда
			Если Не ОснованиеАктОРасхожденияхПослеПриемки И ЭтоТовар Тогда
		...
		ИначеЕсли СтрокаТаблицы.КоличествоУпаковок < 0 Тогда
			...
			КонецЕсли;
		ИначеЕсли СтрокаТаблицы.КоличествоУпаковок = 0 И СтрокаТаблицы.СуммаСНДС > 0 Тогда
			Если ЭтоТовар И Не ОснованиеАктОРасхожденияхПослеПриемки Тогда
				СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОтраженияКорректировокПоступлений.СписатьНаРасходы"));
			КонецЕсли;	
...
Функция ЗаполнитьЦеныПоСоглашениюСервер()
	...
	ПараметрыЗаполненияЦены.Вставить("ВидЦеныПоставщика", Объект.Соглашение);
