...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЗначениеЗаполнено(Соглашение) ИЛИ 
		 НЕ Документы.КорректировкаПриобретения.ЗначениеРеквизитаОбъектаТипаБулево(Соглашение, "ИспользуютсяДоговорыКонтрагентов") Тогда
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если ИспользоватьРаздельноеОформлениеЗакупок
		...
		ВидыЗапасов.Свернуть("АналитикаУчетаНоменклатуры, ВидЗапасов, НомерГТД, ВариантОтражения",
...
Функция ВременныеТаблицыДанныхДокумента(ХозяйственнаяОперация, Отбор = Неопределено)
	...
	Запрос.УстановитьПараметр("ТаблицаТоваров",		 Расхождения.Выгрузить(, "НомерСтроки, Номенклатура, Характеристика,
																			|Назначение, Серия, АналитикаУчетаНоменклатуры,
																			|ВидЗапасов, НомерГТД, СтавкаНДС, Количество,
																			|КоличествоПоРНПТ, СуммаСНДС, СуммаНДС, Склад,
																			|СтатусУказанияСерий, ВариантОтражения"));
	Запрос.УстановитьПараметр("ТаблицаВидыЗапасов",	 ВидыЗапасов.Выгрузить(, "НомерСтроки, АналитикаУчетаНоменклатуры,
																			|ВидЗапасов, ВидЗапасовПолучателя, НомерГТД,
																			|ВариантОтражения, Количество, КоличествоПоРНПТ"));
	...
	Запрос.УстановитьПараметр("ИспользоватьРаздельныйУчетПоНалогообложению",
								РегистрыСведений.УчетнаяПолитикаОрганизаций.РаздельныйУчетТоваровПоНалогообложениюНДС(
									Организация, Дата));
...
Процедура ПроверитьЗаполнениеКолонкиСтроки(ИмяКолонки, ЗначениеРеквизитовДокументаОснования, СоответствиеНоменклатураТип, ТекущаяСтрока, Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ТекущаяСтрока[ИмяКолонки]) Тогда
		...
		ТекстОшибки = НСтр("ru='Не заполнена колонка """ + ИмяКолонки + """ в строке %НомерСтроки% списка ""Товары""'");
