...
Функция ВременныеТаблицыДанныхДокумента(ТаблицаВидыЗапасов, УчетПоНомерамГТД = Ложь) Экспорт
	...
	|	ТаблицаТоваров.НомерГТДОтсутствует			КАК НомерГТДОтсутствует,
	|	ВЫБОР
	|		КОГДА ТаблицаТоваров.НомерГТДОтсутствует
	...
	|
	|ГДЕ
	|	ТаблицаТоваров.НомерГТДОтсутствует = (НЕ &УчетПоНомерамГТД)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПараметрыУказанияСерий = НоменклатураСервер.ПараметрыУказанияСерий(ЭтотОбъект,
																		Документы.ВводОстатковПрослеживаемыхИмпортныхТоваров);
...
Процедура ПроверитьКорректностьЗаполнениеТабличнойЧасти(Отказ)
	...
	|		ИСТИНА								КАК ПрослеживаемыйТовар,
	|		ЛОЖЬ								КАК ПустоеКоличествоПоРНПТ,
	|		ЕСТЬNULL(ТаблицаНоменклатуры.ВестиУчетПоГТД, ЛОЖЬ)
	|			И ТаблицаТоваров.ПустойНомерГТД	КАК ПустойНомерГТД,
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов() Экспорт
	...
	КолонкиГруппировки  = "АналитикаУчетаНоменклатуры, НомерГТДОтсутствует, НомерГТД";
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		СтруктураПоиска.НомерГТД = ?(СтрокаТоваров.НомерГТДОтсутствует,
									Справочники.НомераГТД.ПустаяСсылка(),
									СтрокаТоваров.НомерГТД);
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			НомерГТДОприходования	= ?(ЗначениеЗаполнено(СтрокаЗапасов.НомерГТД),
										СтрокаЗапасов.НомерГТД,
										СтрокаТоваров.НомерГТД);
			...
			НоваяСтрока.НомерГТДОприходования	= НомерГТДОприходования;
			
			СтрокаЗапасов.Количество			= СтрокаЗапасов.Количество - НоваяСтрока.Количество;
			СтрокаЗапасов.КоличествоПоРНПТ		= КоличествоТоваровПоРНПТ - НоваяСтрока.КоличествоПоРНПТ;
			СтрокаЗапасов.НомерГТДОприходования	= НомерГТДОприходования;
