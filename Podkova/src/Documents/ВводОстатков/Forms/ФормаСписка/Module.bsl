...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступныеТипыОпераций = Документы.ВводОстатков.ДоступныеТипыОпераций(Истина);
	СписокДоступныхТиповОпераций.ЗагрузитьЗначения(ДоступныеТипыОпераций);
	...
	Если ЭтоБазоваяВерсия Тогда
		...
		Элементы.ОтражатьВОперативномУчете.Видимость = Ложь;
		Элементы.ОтражатьСебестоимость.Видимость = Ложь;
	КонецЕсли;
...
Процедура ОткрытьФормуСпискаФизическиеЛица(Команда)
	...
	Возврат; // в УТ 11 процедура не должна быть пустой
	...
	Пока ВыборкаРазделы.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ИначеЕсли ВыборкаДетальныеЗаписи.ОперацияВводаОстатков = Перечисления.ТипыОперацийВводаОстатков.ОстаткиТоваровПолученныхНаКомиссию Тогда
				НоваяСтрока.Пояснение = НСтр("ru='Вводятся остатки товаров, принятых на комиссию'")
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если НЕ Копирование Тогда
		...
		Если ОтборПоТипамОпераций.Количество() = 1 Тогда
			ТипОперации = ОтборПоТипамОпераций[0].Значение;
			Если ТипОперации = ПредопределенноеЗначение("Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасчетовМеждуОрганизациямиПоАвансам")
				ИЛИ ТипОперации = ПредопределенноеЗначение("Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасчетовМеждуОрганизациямиПоРеализациям") Тогда
				ТекстСообщения = НСтр("ru='Начиная с версии 2.4 для ввода остатков по расчетам между организациями следует использовать операции ввода остатков расчетов с партнерами.
									 |При этом в поле ""Партнер"" выбрать значение ""Наше предприятие"", а в поле ""Контрагента"" организацию, задолженность которой необходимо ввести.
									 |Остатки по каждой организации вводятся отдельно.'");
