...
#Область ОбработчикиТабличнойЧастиКорректировкаОстатков
...
Процедура КорректировкаОстатковАлкогольнаяПродукцияПриИзмененииНаСервере(Идентификатор)
	...
	СобытияФормЕГАИС.ОкруглитьКоличествоДляУпакованнойПродукции(Элементы.КорректировкаОстатков.ТекущиеДанные,
...
Процедура ВыполнитьОчереднойЗапрос()
	...
	Если ДанныеТекущегоЗапроса <> Неопределено Тогда
		...
		Если Не ДанныеТекущегоЗапроса.ЗапросОтправлен Тогда
			...
			Если ОсталосьДоОтправкиЗапроса > 0 Тогда
				СостояниеЗапросаМарок = Новый ФорматированнаяСтрока(
					СтрШаблон(
						НСтр("ru = 'Запрос марок готовится к отправке через %1 секунд'"),
						ОсталосьДоОтправкиЗапроса));
		...
		Иначе
			...
			// Ожидали ответ 12 минут, но он не пришел
			Если ДанныеТекущегоЗапроса.Ссылка <> Неопределено
				И ТекущаяДата() - ДанныеТекущегоЗапроса.ДатаЗапроса >= 12*60 Тогда
			...
			// 12 минут с момента отправки запроса еще не прошло, ждем ответа.
			ИначеЕсли ДанныеТекущегоЗапроса.Ссылка <> Неопределено Тогда
				
				ТребуетсяПроверитьОтветНаЗапросМарок = Ложь;
				Если ЗначениеЗаполнено(ДанныеТекущегоЗапроса.ДатаПроверкиМарок)
					И ТекущаяДата() - ДанныеТекущегоЗапроса.ДатаПроверкиМарок >= 30 Тогда
					ТребуетсяПроверитьОтветНаЗапросМарок = Истина;
				ИначеЕсли Не ЗначениеЗаполнено(ДанныеТекущегоЗапроса.ДатаПроверкиМарок)
					И ТекущаяДата() - ДанныеТекущегоЗапроса.ДатаЗапроса >= 30 Тогда
					ТребуетсяПроверитьОтветНаЗапросМарок = Истина;
				КонецЕсли;
				
				Если ТребуетсяПроверитьОтветНаЗапросМарок Тогда
					ИнтеграцияЕГАИСКлиент.ВыполнитьОбмен(
						Объект.ОрганизацияЕГАИС,
						Новый ОписаниеОповещения("ПроверитьОтветНаЗапросМарок", ЭтотОбъект));
				Иначе
...
Процедура ПроверитьОтветНаЗапросМарок(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеТекущегоЗапроса.ДатаЗапроса) Тогда
		...
		Если ЗначениеЗаполнено(ДанныеТекущегоЗапроса.ДатаЗапроса) Тогда
...
Функция МинимальнаяДатаОтправкиЗапросовОстатковАкцизныхМарок()
	...
	Запрос.УстановитьПараметр("НачалоПериода",          ТекущаяДата() - 24*60*60);
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Возврат МестноеВремя(Выборка.ДатаМодификацииУниверсальная, ЧасовойПоясСеанса()) + 12*60;
	КонецЦикла;
