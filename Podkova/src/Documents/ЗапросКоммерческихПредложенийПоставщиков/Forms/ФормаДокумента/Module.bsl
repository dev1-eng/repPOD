...
Процедура ЗаполнитьПредставлениеКатегории(ТекущиеДанные)
	...
	Если Не ПустаяСтрока(ТекущиеДанные.ИдентификаторЗадания) Тогда 
		ОтменитьОперациюНаСервере(ТекущиеДанные.ИдентификаторЗадания);
...
Функция ПолучитьСсылкуНаЕдиницуИзмерения(КодЕдИзм, РеквизитыБазовойЕдиницыИзмерения)
	...
	Возврат ЭлектронноеВзаимодействие.НайтиСсылку("ЕдиницыИзмерения", Строка(КодЕдИзм),
							РеквизитыБазовойЕдиницыИзмерения);
...
Процедура ПроверитьРегистрациюОрганизаций()
	...
	ОрганизацияЗарегистрирована = БизнесСеть.ОрганизацияПодключена(Объект.Организация);
	ТребуетсяРегистрацияОрганизации = Не ОрганизацияЗарегистрирована;
	Элементы.ГруппаРегистрацииБизнесСеть.Видимость = ТребуетсяРегистрацияОрганизации;
...
Процедура ОтменитьФоновоеЗаданиеПоискаТорговыхПредложений()
	...
	ОтменитьОперациюНаСервере(ИдентификаторЗадания);
	
	Элементы.ГруппаЗагрузкаВыбораПредложений.ТекущаяСтраница = Элементы.ГруппаЗагрузкаВыбораПредложенийВыполнено;
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ОтменитьОперациюНаСервере(ИдентификаторЗадания)
...
Процедура НастроитьЭлементыФормы()
	...
	Если Не ЗначениеЗаполнено(Объект.Организация) Тогда
		Объект.Организация = НастройкиУчета.ЕдинственнаяОрганизация;
	КонецЕсли;
	
	Элементы.Валюта.Видимость = Не НастройкиУчета.ИспользуетсяЕдинственнаяВалюта;
	Если Не ЗначениеЗаполнено(Объект.Валюта) Тогда
		Объект.Валюта = НастройкиУчета.ЕдинственнаяВалюта;
	КонецЕсли;
