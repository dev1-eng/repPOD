...
Процедура НайтиСоздатьЗапросКоммерческихПредложений(Знач ДанныеВходящегоДокумента, УчетныйДокумент) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(УчетныйДокумент) Тогда
			ДокументОбъект = ЭлектронноеВзаимодействиеСлужебный.ОбъектПоСсылкеДляИзменения(УчетныйДокумент);
		Иначе
		...
		ДокументОбъект.Валюта = ЭлектронноеВзаимодействие.НайтиСсылку("Валюты",
		...
		Организация = ЭлектронноеВзаимодействие.НайтиСсылку("Организации",,РеквизитыОрганизации);
		...
		Если Не КоммерческиеПредложенияСлужебный.МетаданныеПоОпределяемомуТипу("Партнер") = Неопределено Тогда
...
Процедура ЗаполнитьПараметрыЭлектронногоДокумента(Источник, Параметры) Экспорт
	...
	Параметры.Вставить("НаправлениеЭД", Перечисления.НаправленияЭД.Входящий);
...
Функция ПечатьЗапросаКоммерческихПредложений(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ЭлектронноеВзаимодействиеПереопределяемый.ПолучитьДанныеЮрФизЛица(Шапка.Организация, Сведения);
