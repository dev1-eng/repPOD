...
Процедура ОтображениеДекорацииЕстьТоварыКОформлениюСкладскихАктовИнвентаризационныеОписи()
	...
	Если ЗначениеЗаполнено(Объект.Склад)
		...
		Если ПравоДоступа("Изменение", Метаданные.Документы.ИнвентаризационнаяОпись) Тогда 
			ПараметрыФормирования = УправлениеПечатьюУТВызовСервераЛокализация.ПолучитьПараметрыФормирования(Объект.Ссылка);
...
&НаСервере
Процедура ПечатьОписиНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

Функция ПолучитьМакетОписи()
	Возврат РеквизитФормыВЗначение("Объект").ПолучитьМакет("Опись");
КонецФункции
...
Функция ПолучитьОпись()
	
	ДокОписи=Объект.Ссылка;
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_Опись";
	Макет = ПолучитьМакетОписи();
	
	ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
	ОбластьМакета.Параметры.НомерДок = ДокОписи.Номер;
	ОбластьМакета.Параметры.ДатаДок = ДокОписи.Дата;	
	...
	СуммаТекИтого=0;
	СуммаФактИтого=0;
	ОтклонениеСуммаНедостачи=0;
	ОтклонениеСуммаИзлишки=0;
	
	Для Каждого СтрокаДокОписи Из ДокОписи.Товары Цикл
		Если СтрокаДокОписи.Количество>СтрокаДокОписи.КоличествоФакт Тогда
			ОбластьМакета = Макет.ПолучитьОбласть("СтрокаНедостача");
			ОбластьМакета.Параметры.Материал=СтрокаДокОписи.Номенклатура.Наименование;
			ОбластьМакета.Параметры.Цена = ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			ОбластьМакета.Параметры.ОстатокТек = СтрокаДокОписи.Количество;
			ОбластьМакета.Параметры.СуммаТек = СтрокаДокОписи.Количество*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			ОбластьМакета.Параметры.ОстатокФакт = СтрокаДокОписи.КоличествоФакт;
			ОбластьМакета.Параметры.СуммаФакт = СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			ОбластьМакета.Параметры.ОтклонениеКол = СтрокаДокОписи.Количество-СтрокаДокОписи.КоличествоФакт;
			ОбластьМакета.Параметры.ОтклонениеСумма = (СтрокаДокОписи.Количество*ПолучитьЦену(СтрокаДокОписи.Номенклатура))-(СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура));
			ТабличныйДокумент.Вывести(ОбластьМакета);
			СуммаТекИтого=СуммаТекИтого+СтрокаДокОписи.Количество*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			СуммаФактИтого=СуммаФактИтого+СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			ОтклонениеСуммаНедостачи=ОтклонениеСуммаНедостачи+(СтрокаДокОписи.Количество*ПолучитьЦену(СтрокаДокОписи.Номенклатура))-(СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура));
		ИначеЕсли СтрокаДокОписи.Количество<СтрокаДокОписи.КоличествоФакт Тогда
			ОбластьМакета = Макет.ПолучитьОбласть("СтрокаИзлишки");
			ОбластьМакета.Параметры.Материал=СтрокаДокОписи.Номенклатура.Наименование;
			ОбластьМакета.Параметры.Цена = ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			ОбластьМакета.Параметры.ОстатокТек = СтрокаДокОписи.Количество;
			ОбластьМакета.Параметры.СуммаТек = СтрокаДокОписи.Количество*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			ОбластьМакета.Параметры.ОстатокФакт = СтрокаДокОписи.КоличествоФакт;
			ОбластьМакета.Параметры.СуммаФакт = СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			ОбластьМакета.Параметры.ОтклонениеКол = СтрокаДокОписи.КоличествоФакт-СтрокаДокОписи.Количество;
			ОбластьМакета.Параметры.ОтклонениеСумма = (СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура))-(СтрокаДокОписи.Количество*ПолучитьЦену(СтрокаДокОписи.Номенклатура));
			ТабличныйДокумент.Вывести(ОбластьМакета);
			СуммаТекИтого=СуммаТекИтого+СтрокаДокОписи.Количество*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			СуммаФактИтого=СуммаФактИтого+СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			ОтклонениеСуммаИзлишки=ОтклонениеСуммаИзлишки+(СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура))-(СтрокаДокОписи.Количество*ПолучитьЦену(СтрокаДокОписи.Номенклатура));
		Иначе
			ОбластьМакета = Макет.ПолучитьОбласть("СтрокаНедостача");
			ОбластьМакета.Параметры.Материал=СтрокаДокОписи.Номенклатура.Наименование;
			ОбластьМакета.Параметры.Цена = ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			...
			ОбластьМакета.Параметры.ОстатокФакт = СтрокаДокОписи.КоличествоФакт;
			ОбластьМакета.Параметры.СуммаФакт = СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			ОбластьМакета.Параметры.ОтклонениеКол = 0;
			ОбластьМакета.Параметры.ОтклонениеСумма = 0;
			ТабличныйДокумент.Вывести(ОбластьМакета);
			СуммаТекИтого=СуммаТекИтого+СтрокаДокОписи.Количество*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
			СуммаФактИтого=СуммаФактИтого+СтрокаДокОписи.КоличествоФакт*ПолучитьЦену(СтрокаДокОписи.Номенклатура);
		КонецЕсли;
	...
	ОбластьМакета = Макет.ПолучитьОбласть("СтрокаИтог");
	ОбластьМакета.Параметры.СуммаТекИтого=СуммаТекИтого;
	ОбластьМакета.Параметры.СуммаФактИтого=СуммаФактИтого;
	ОбластьМакета.Параметры.ОтклонениеСуммаНедостачи=ОтклонениеСуммаНедостачи;
	ОбластьМакета.Параметры.ОтклонениеСуммаИзлишки=ОтклонениеСуммаИзлишки;
	ТабличныйДокумент.Вывести(ОбластьМакета);
...
&НаКлиенте
Процедура ПечатьОписи(Команда)
	ТабДок = ПолучитьОпись();
	ТабДок.Показать();
	ПечатьОписиНаСервере();
КонецПроцедуры
