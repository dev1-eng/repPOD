...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СтатьиРасходовСформироватьДокументы.Доступность = ДоступноДобавление;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_РаспределениеПрочихЗатрат"
		...
		Если Строки.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Состояние) И Состояние <> Строки[0].СостояниеУпр
				И Состояние <> Строки[0].СостояниеРегл Тогда
				ОбновитьДанныеФормы();
			КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Состояние) 
				...
				И Состояние <> СостояниеРегл Тогда
				ОбновитьДанныеФормы();
			КонецЕсли;
...
Процедура ОбновитьДанныеФормы()
	...
	ЗапуститьФоновоеЗадание("ЗаполнитьСтатьиРасходов");
...
Процедура ОбработатьРезультатФормированияДокументов(АдресХранилища)
	...
	Для Каждого РезультатПоОрганизации Из Результат Цикл
		...
		Если ДетализацияРезультата.ТекстыОшибок.Количество() > 0 Тогда
			Для Каждого Ошибка Из ДетализацияРезультата.ТекстыОшибок Цикл
...
Функция ОпределитьПараметрыОткрытияФормыДокумента(ТекущиеДанные, ПараметрыОткрытия)
	...
	Если ТекущиеДанные.ДокументРегл = ТекущиеДанные.ДокументУпр
		И ЗначениеЗаполнено(ТекущиеДанные.ДокументУпр) Тогда
		Возврат ПараметрыФормы;
	КонецЕсли;
	
	ТекущееПоле = Элементы.СтатьиРасходов.ТекущийЭлемент;
	Если ТекущееПоле.Имя = "СтатьиРасходовСостояниеРегл"
		Или ТекущиеДанные.СостояниеУпр = ПредопределенноеЗначение("Перечисление.СостоянияРаспределенияРасходов.НеТребуется") Тогда
	...
	Если ТекущееПоле.Имя = "СтатьиРасходовСостояниеУпр"
		Или ТекущиеДанные.СостояниеРегл = ПредопределенноеЗначение("Перечисление.СостоянияРаспределенияРасходов.НеТребуется") Тогда
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.ДокументРегл)
		И Не ЗначениеЗаполнено(ТекущиеДанные.ДокументУпр) Тогда
		...
		ПараметрыФормы.НастройкаРаспределения = ПолучитьНастройкиРаспределенияСтатейРасходов(
			ПолучитьПараметрыРасходов(ТекущиеДанные.ПолучитьИдентификатор()))[0];
		
		Если ПараметрыФормы.НастройкаРаспределения.ПравилоРаспределенияРегл = ПараметрыФормы.НастройкаРаспределения.ПравилоРаспределенияУпр
			И ПараметрыФормы.НастройкаРаспределения.ВариантРаспределенияРегл = ПараметрыФормы.НастройкаРаспределения.ВариантРаспределенияУпр
	...
	ВариантыОтвета.Добавить(ПредопределенноеЗначение("Перечисление.РазрезыУчета.Регламентированный"), 
		НСтр("ru = 'Для регламентированного учета'"));
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ОбработатьОтветПользователя", ЭтотОбъект, ПараметрыФормы), 
		НСтр("ru = 'Правила распределения в упр. и регл. учете различаются. 
