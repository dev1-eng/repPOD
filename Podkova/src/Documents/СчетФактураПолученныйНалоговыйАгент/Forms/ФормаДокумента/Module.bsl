...
#Область ОбработчикиСобытийТаблицыФормыДокументыОснования
...
Процедура РасшифровкаСуммыНалоговаяБазаПриИзменении(Элемент)
	...
	СтавкаНДСПроцентом = УчетНДСУПКлиентСервер.ПолучитьСтавкуНДС(ТекущаяСтрока.СтавкаНДС);
...
&НаСервере
Процедура ПересчитатьНалоговуюБазу(ИдентфикаторСтроки = Неопределено)
	
	Итератор = Объект.ДокументыОснования;
	Если Не ИдентфикаторСтроки = Неопределено Тогда
		Итератор = Новый Массив;
		
		НайденнаяСтрока = Объект.ДокументыОснования.НайтиПоИдентификатору(ИдентфикаторСтроки);
	...
	Для Каждого ТекущаяСтрока Из Итератор Цикл
		...
		ТекущаяСтрока.НалоговаяБаза = ?(Объект.Валюта = ВалютаРегламентированногоУчета, ТекущаяСтрока.Сумма,
			ПересчитатьВВалюту(ТекущаяСтрока.Сумма, Объект.Валюта, ВалютаРегламентированногоУчета, Объект.ДатаСоставления)) *
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ОсновнаяСтавкаНДС = УчетНДСУП.СтавкаНДСПоУмолчанию(Объект.ДатаСоставления, Истина);
	ПроцентНДС = УчетНДСУПКлиентСервер.ПолучитьСтавкуНДС(ОсновнаяСтавкаНДС);
...
&НаСервереБезКонтекста
Функция ПересчитатьВВалюту(Сумма, СтараяВалюта, НоваяВалюта, Период)
	
	Возврат РаботаСКурсамиВалют.ПересчитатьВВалюту(Сумма, СтараяВалюта, НоваяВалюта, Период);
