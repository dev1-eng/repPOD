...
Процедура ЗаполнитьЭтапыГрафикаОплаты(СуммаКорректировки = 0, ТолькоРаспределять = Ложь) Экспорт
	...
	Если ЭтапыГрафикаОплаты.Количество() = 0 Тогда
		...
		ПараметрыДляДобавления.ЖелаемаяДата = ЖелаемаяДатаОтгрузки;
		...
		ПараметрыДляДобавления.СуммаОтклоненияМерныхТоваров = СуммаКорректировки;
...
Процедура ЗаполнитьУсловияПродажПоСоглашению() Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КоммерческоеПредложениеКлиенту")
...
Функция УстановитьСтатус(НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры <> Неопределено Тогда
		...
		Если ДополнительныеПараметры.Свойство("ЗакрыватьТолькоБезИзменений")
			И  ДополнительныеПараметры.ЗакрыватьТолькоБезИзменений
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЗначениеЗаполнено(Соглашение)
		ИЛИ НЕ Документы.ЗаказКлиента.ЗначениеРеквизитаОбъектаТипаБулево(Соглашение, "ИспользуютсяДоговорыКонтрагентов") Тогда
