...
Процедура ТоварыДокументОснованиеПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.ДокументОснование) Тогда
		...
		ТекущиеДанные.ВернутьМногооборотнуюТару = Ложь;
		ТекущиеДанные.ТребуетсяЗалогЗаТару 		= Ложь;
		Возврат;
	КонецЕсли;
	...
	ЗаполнитьРеквизитыМногооборотнойТарыВСтроке(ТекущиеДанные, НайденнаяСтрока);		
...
Процедура ПослеВыбораВариантаРазбиенияСтроки(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияУТКлиент.РазбитьСтрокуТЧ(ДанныеТаблицы,
	                                         ТаблицаФормы,
...
Процедура РазбитьСтрокуЗавершение(НоваяСтрока, ДополнительныеПараметры) Экспорт 
	...
	Иначе
		...
		СтруктураДействий = Новый Структура;
		СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиницСуффикс", ПараметрыРасчетаКоличестваЕдиниц);
...
Процедура УстановитьВидимостьЭлементовПоОперацииСервер()
	...
	ТипыАктовХранения = Новый Массив;
	
	Если ТипыАктовХранения.Найти(Объект.ТипОснованияАктаОРасхождении) <> Неопределено Тогда
...
Функция ЗаполнитьПоПриемкеСервер()
	...
	Для каждого СтрокаТовары Из ТаблицаНакладная Цикл
		СтрокаТовары.КоличествоМаксимум = Макс(СтрокаТовары.КоличествоПоДокументу, СтрокаТовары.Количество);
...
&НаСервере
Процедура ЗаполнитьРеквизитыМногооборотнойТарыВСтроке(СтрокаТаблицыТовары, СтрокаДокументыОснования)
	Если Объект.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПриобретениеТоваровУслуг Тогда		
