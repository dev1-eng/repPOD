...
Процедура КакНастроитьAdobeReader(Команда)
	...
	ОткрытьФорму("Обработка.СправкаСПАРКРиски.Форма.СправочнаяИнформация",
		Новый Структура(
			"ИмяМакета",
			"КакНастроитьПроверкуЦифровойПодписиAdobeAcrobat_ru"));
...
Процедура ОбработатьОшибку(РезультатДействия, ОповещениеПриПодключенииИнтернетПоддержки)
	...
	ИначеЕсли РезультатДействия.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ТребуетсяОплатаИлиПревышенЛимит") Тогда
...
Функция ЗапроситьНовуюСправкуВСервисеСПАРКРиски()
	...
	ИначеЕсли Не СПАРКРиски.ИННСоответствуетТребованиям(ДанныеКонтрагента.ИНН, ОшибкаИНН) Тогда
