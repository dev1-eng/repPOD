...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеСерверУТ.ЕстьТаблицаЗапроса("ВтТаблицаИсправлений", ТекстыЗапроса) Тогда
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|	ВидыЗапасов.ВидЗапасовСписания 						 КАК ВидЗапасов,
	|	ВидыЗапасов.Количество 								 КАК Количество,
	|
	|	&Организация 										 КАК КорОрганизация,
	|	ВидыЗапасов.РазделУчета 							 КАК КорРазделУчета,
	|	(ВЫБОР
	|		КОГДА &УчитыватьСебестоимостьТоваровПоНазначениям
	|			ТОГДА ВидыЗапасов.АналитикаУчетаНоменклатуры
	|		ИНАЧЕ ВидыЗапасов.АналитикаУчетаНоменклатурыБезНазначения
	|	КОНЕЦ)												 КАК КорАналитикаУчетаНоменклатуры,
	|	ВидыЗапасов.ВидЗапасовОприходования 				 КАК КорВидЗапасов,
	|	&ХозяйственнаяОперация 								 КАК ХозяйственнаяОперация,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление) КАК ТипЗаписи
	|ИЗ
	|	ВтТаблицаИсправлений КАК ВидыЗапасов
	|ГДЕ
	|	ВидыЗапасов.Количество <> 0
	|	И ВидыЗапасов.ВидЗапасовОприходования <> ВидыЗапасов.ВидЗапасовСписания
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ИНАЧЕ ВидыЗапасов.АналитикаУчетаНоменклатурыБезНазначения
	|	КОНЕЦ)												 КАК АналитикаУчетаНоменклатуры,
	|	ВидыЗапасов.ВидЗапасовОприходования 				 КАК ВидЗапасов,
	|	ВидыЗапасов.Количество 								 КАК Количество,
	|
