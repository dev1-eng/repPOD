...
Функция ВременныеТаблицыДокументаДляРасчетаСостояний() Экспорт
	...
	ТекстЗапросаТовары =
		...
		|	РеквизитыЗаказа.НачалоСборкиРазборки КАК ДатаОтгрузки,
		|	Таблица.ВариантОбеспечения           КАК ВариантОбеспечения,
		|	РеквизитыЗаказа.Склад                КАК Склад,
		|	Таблица.Номенклатура                 КАК Номенклатура,
		|	Таблица.Характеристика               КАК Характеристика,
		|	
		|	ВЫБОР КОГДА РеквизитыЗаказа.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|				И НЕ РеквизитыЗаказа.ОбосабливатьПоНазначениюЗаказа ТОГДА
		|				РеквизитыЗаказа.Назначение
		|			ИНАЧЕ
		|				РеквизитыЗаказа.НазначениеТовары
		...
		|		ПО РеквизитыЗаказа.Ссылка = Таблица.Ссылка
		|ГДЕ
		|	Таблица.Ссылка В(&МассивЗаказов)
		|	И НЕ Таблица.Отменено
		|	И РеквизитыЗаказа.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СборкаТоваров)
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
