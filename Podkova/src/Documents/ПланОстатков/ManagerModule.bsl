...
Функция МакетыОперандовПлана(Макеты = Неопределено) Экспорт 
	...
	Макеты.Вставить("СвободныеОстатки",                    ПолучитьМакет("СКД_СвободныеОстатки"));
...
Функция СформироватьПечатнуюФормуПлана(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ДопПоля.Количество() > 0 Тогда
			ШиринаКолонкиТовара = ШиринаКолонкиТовара -Макет.Область("Товар" + "ДопПоле"+ Формат(ДопПоля.Количество(), "ЧДЦ=0; ЧГ=0")).ШиринаКолонки * ДопПоля.Количество();
		КонецЕсли; 
		...
		ОбластьДанных                   = Макет.ПолучитьОбласть("ШапкаТаблицы"+?(ВыводитьШапку ИЛИ КроссТаблица,"","НХ")+"|Количество" + СуффиксКроссТаблицы);
		ОбластьДанныхКроссОкончание     = Макет.ПолучитьОбласть("ШапкаТаблицы|Количество" + "КроссОкончание");
		ОбластьКроссЕдиницаИзмерения    = Макет.ПолучитьОбласть("ШапкаТаблицы|КроссЕдиницаИзмерения");
		Если ДопПоля.Количество() > 0 Тогда
			ОбластьТоварДопПоле         = Макет.ПолучитьОбласть("ШапкаТаблицы|Товар" + "ДопПоле"+ Формат(ДопПоля.Количество(), "ЧДЦ=0; ЧГ=0"));
		КонецЕсли; 
		...
		ОбластьДанныхКроссОкончание     = Макет.ПолучитьОбласть("СтрокаТаблицы|Количество" + "КроссОкончание");
		ОбластьКроссЕдиницаИзмерения    = Макет.ПолучитьОбласть("СтрокаТаблицы|КроссЕдиницаИзмерения");
		Если ДопПоля.Количество() > 0 Тогда
			ОбластьТоварДопПоле         = Макет.ПолучитьОбласть("СтрокаТаблицы|Товар" + "ДопПоле"+ Формат(ДопПоля.Количество(), "ЧДЦ=0; ЧГ=0") );
		КонецЕсли;
		...
		Если ВыводитьШапку Тогда
			...
			ОбластьДанныхКроссОкончание     = Макет.ПолучитьОбласть("ПодвалТаблицы|Количество" + "КроссОкончание");
			ОбластьКроссЕдиницаИзмерения    = Макет.ПолучитьОбласть("ПодвалТаблицы|КроссЕдиницаИзмерения");
			Если ДопПоля.Количество() > 0 Тогда
				ОбластьТоварДопПоле         = Макет.ПолучитьОбласть("ПодвалТаблицы|Товар" + "ДопПоле"+ Формат(ДопПоля.Количество(), "ЧДЦ=0; ЧГ=0") );
			КонецЕсли;
...
Функция ТекстЗапросаДанныеКОбработке(ВыбратьДляРегистрацииДвиженийОборотыДвижений = Ложь) Экспорт
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ПланОстатков.Ссылка КАК Ссылка,
	|	ПланОстатков.ВидПлана КАК ВидПлана,
	|	ПланОстатков.НачалоПериода КАК НачалоПериода,
	|	ПланОстатков.ОкончаниеПериода КАК ОкончаниеПериода,
	|	ПланОстатков.Статус.Порядок КАК СтатусПорядок,
	|	ПланОстатков.Дата КАК Дата,
	|	ПланОстатков.Назначение КАК Назначение,
	|	ПланОстатков.Склад КАК Склад
	|ПОМЕСТИТЬ ПланыОстатков
	|ИЗ
	|	Документ.ПланОстатков КАК ПланОстатков
	|ГДЕ
	|	ПланОстатков.Замещающий
	|	И ПланОстатков.Проведен
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПланыОстатков.Ссылка КАК ЗамещающийПлан,
	|	ПланыОстатков1.Ссылка КАК ЗамещенныйПлан
	|ПОМЕСТИТЬ ЗамещающиеИЗамещенныеПланыОстатков
	|ИЗ
	|	ПланыОстатков КАК ПланыОстатков
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланыОстатков КАК ПланыОстатков1
	|		ПО ПланыОстатков.ОкончаниеПериода >= ПланыОстатков1.НачалоПериода
	|			И ПланыОстатков.НачалоПериода <= ПланыОстатков1.ОкончаниеПериода
	|			И ПланыОстатков.Ссылка <> ПланыОстатков1.Ссылка
	|			И ПланыОстатков.ВидПлана = ПланыОстатков1.ВидПлана
	|			И ПланыОстатков.СтатусПорядок >= ПланыОстатков1.СтатусПорядок
	|			И ПланыОстатков.Дата > ПланыОстатков1.Дата
	|			И ПланыОстатков.Назначение = ПланыОстатков1.Назначение
	|			И ПланыОстатков.Склад = ПланыОстатков1.Склад
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗамещающиеИЗамещенныеПланыОстатков.ЗамещенныйПлан КАК Ссылка
	|ИЗ
	|	ЗамещающиеИЗамещенныеПланыОстатков КАК ЗамещающиеИЗамещенныеПланыОстатков
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗамещениеПланов КАК ЗамещениеПланов
	|		ПО ЗамещающиеИЗамещенныеПланыОстатков.ЗамещающийПлан = ЗамещениеПланов.ЗамещающийПлан
	|			И ЗамещающиеИЗамещенныеПланыОстатков.ЗамещенныйПлан = ЗамещениеПланов.ЗамещенныйПлан
	|ГДЕ
	|	ЗамещениеПланов.ЗамещенныйПлан ЕСТЬ NULL";
	Если ВыбратьДляРегистрацииДвиженийОборотыДвижений Тогда
		ТекстЗапроса = ТекстЗапроса +
		"
		|	И ЗамещающиеИЗамещенныеПланыОстатков.ЗамещающийПлан.ОтражаетсяВБюджетировании";
	КонецЕсли;
...
// выполняет замещение планов остатков.
//
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.ПланОстатков";
	МетаданныеДокумента = Метаданные.Документы.ПланОстатков;
