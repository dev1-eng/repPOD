...
Процедура ПолучитьОстаткиПрочихРасходов(ДанныеОтбора, Расходы) Экспорт
	...
	|		Регистр.СтатьяРасходов КАК СтатьяРасходов,
	|		Регистр.АналитикаРасходов КАК АналитикаРасходов,
	|		Регистр.СтоимостьОстаток КАК Сумма,
	|		Регистр.СтоимостьБезНДСОстаток КАК СуммаБезНДС,
	|		(ВЫБОР
	|			КОГДА Регистр.СтоимостьРеглОстаток = 0
	|				ТОГДА Регистр.НДСРеглОстаток
	|			ИНАЧЕ Регистр.СтоимостьРеглОстаток КОНЕЦ) КАК СуммаРегл,
	|		Регистр.НДСРеглОстаток КАК НДСРегл,
	|		Регистр.ПостояннаяРазницаОстаток КАК ПостояннаяРазница,
	|		Регистр.ВременнаяРазницаОстаток КАК ВременнаяРазница
	|	ИЗ
	|		РегистрНакопления.ПартииПрочихРасходов.Остатки(&Граница, Организация = &Организация) КАК Регистр
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ВЫБОР
	|			КОГДА Регистр.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|				ТОГДА Регистр.Стоимость
	|			ИНАЧЕ -Регистр.Стоимость
	|		КОНЕЦ КАК Сумма,
	|		ВЫБОР
	|			КОГДА Регистр.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|				ТОГДА Регистр.СтоимостьБезНДС
	|			ИНАЧЕ -Регистр.СтоимостьБезНДС
	|		КОНЕЦ КАК СуммаБезНДС,
	|		ВЫБОР
	|			КОГДА Регистр.СтоимостьРегл = 0 И Регистр.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|				ТОГДА Регистр.НДСРегл
	|			КОГДА Регистр.СтоимостьРегл = 0 И Регистр.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА -Регистр.НДСРегл
	|			КОГДА Регистр.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|				ТОГДА Регистр.СтоимостьРегл
	|			ИНАЧЕ -Регистр.СтоимостьРегл
	|		КОНЕЦ КАК СуммаРегл,
	|		ВЫБОР
	|			КОГДА Регистр.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|				ТОГДА Регистр.НДСРегл
	...
	|		КОНЕЦ
	|	ИЗ
	|		РегистрНакопления.ПартииПрочихРасходов КАК Регистр
	|	ГДЕ
	|		Регистр.Регистратор = &Ссылка
	|
	|	) КАК ВложенныйЗапрос
