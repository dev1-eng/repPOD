...
Процедура ТоварыНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентПереопределяемый.ПриНачалеВыбораНоменклатуры(
		ЭтотОбъект, ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Алкогольная"), СтандартнаяОбработка,,
...
Процедура ТоварыАлкогольнаяПродукцияПриИзмененииНаСервере(Идентификатор)
	...
	СобытияФормЕГАИС.ОкруглитьКоличествоДляУпакованнойПродукции(Элементы.Товары.ТекущиеДанные,
...
Процедура ЗаполнитьСопоставленныеТовары(АлкогольнаяПродукция = Неопределено)
	...
	|	Товары.НомерСтроки КАК НомерСтроки,
	|	Товары.АлкогольнаяПродукция КАК АлкогольнаяПродукция,
	|	ЕСТЬNULL(КоличествоСопоставлено.КоличествоНоменклатура, 0)   КАК КоличествоВариантовНоменклатуры,
	|	ЕСТЬNULL(КоличествоСопоставлено.КоличествоХарактеристика, 0) КАК КоличествоВариантовХарактеристик,
	|	ВЫБОР
	|		КОГДА Товары.Номенклатура = &ПустаяНоменклатура
	|			ТОГДА СопоставленнаяНоменклатура.Номенклатура
	|		ИНАЧЕ Товары.Номенклатура
	|	КОНЕЦ КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА Товары.Номенклатура = &ПустаяНоменклатура
	|			ТОГДА ПРЕДСТАВЛЕНИЕ(СопоставленнаяНоменклатура.Номенклатура)
	|		ИНАЧЕ ПРЕДСТАВЛЕНИЕ(Товары.Номенклатура)
	|	КОНЕЦ КАК НоменклатураПредставление,
	|	ВЫБОР
	|		КОГДА Товары.Характеристика = &ПустаяХарактеристика
	|			ТОГДА СопоставленныеХарактеристики.Характеристика
	...
	|ИЗ
	|	Товары КАК Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСопоставлено КАК СопоставленнаяНоменклатура
	|		ПО (СопоставленнаяНоменклатура.АлкогольнаяПродукция = Товары.АлкогольнаяПродукция)
	|		И (СопоставленнаяНоменклатура.ИдентификаторУпаковки = Товары.ИдентификаторУпаковки)
	|		И (СопоставленнаяНоменклатура.КоличествоНоменклатура = 1)
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСопоставлено КАК СопоставленныеХарактеристики
	|		ПО (СопоставленныеХарактеристики.АлкогольнаяПродукция = Товары.АлкогольнаяПродукция)
	|			И (СопоставленныеХарактеристики.ИдентификаторУпаковки = Товары.ИдентификаторУпаковки)
	|			И (СопоставленныеХарактеристики.КоличествоНоменклатура = 1)
	|			И (СопоставленныеХарактеристики.КоличествоХарактеристика < 2)
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтКоличествоСопоставлено КАК КоличествоСопоставлено
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаТЧ.СопоставленоКоличество = Выборка.КоличествоВариантовНоменклатуры;
		Если Выборка.КоличествоВариантовНоменклатуры = 1
			И Выборка.КоличествоВариантовХарактеристик > 1 Тогда
			СтрокаТЧ.СопоставленоКоличество = Выборка.КоличествоВариантовХарактеристик;
		КонецЕсли;
		...
		Если Выборка.КоличествоВариантовНоменклатуры = 1
			...
			Если Не ЗначениеЗаполнено(СтрокаТЧ.Характеристика)
				И ЗначениеЗаполнено(Выборка.Характеристика) Тогда
				СтрокаТЧ.Характеристика = Выборка.Характеристика;
				Модифицированность = Истина;
			КонецЕсли;
			
		ИначеЕсли Выборка.КоличествоВариантовНоменклатуры = 1 И Выборка.КоличествоВариантовХарактеристик > 1 Тогда
			СтрокаТЧ.СопоставлениеНоменклатура   = Выборка.НоменклатураПредставление;
			СтрокаТЧ.СопоставлениеХарактеристика = СтрШаблон(НСтр("ru = '<Несколько позиций (%1)>'"), Выборка.КоличествоВариантовХарактеристик);
		ИначеЕсли Выборка.КоличествоВариантовНоменклатуры > 1 Тогда
			СтрокаТЧ.СопоставлениеНоменклатура   = СтрШаблон(НСтр("ru = '<Несколько позиций (%1)>'"), Выборка.КоличествоВариантовНоменклатуры);
