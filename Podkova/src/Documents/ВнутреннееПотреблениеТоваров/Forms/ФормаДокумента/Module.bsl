...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВнутреннееПотреблениеТоваровЛокализация.ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ВнутреннееПотреблениеТоваровЛокализация.ПослеЗаписиНаСервере(ЭтаФорма, ТекущийОбъект, ПараметрыЗаписи);
...
Процедура ХозяйственнаяОперацияПриИзменении(Элемент)
	...
	ВнутреннееПотреблениеТоваровКлиентЛокализация.ХозяйственнаяОперацияПриИзменении(ЭтаФорма);
...
Процедура ТоварыКатегорияЭксплуатацииПриИзменении(Элемент)
	...
	ВнутреннееПотреблениеТоваровКлиентЛокализация.ТоварыКатегорияЭксплуатацииПриИзменении(ЭтаФорма, КэшированныеЗначения);
	
	ТоварыКатегорияЭксплуатацииПриИзмененииНаСервере(КэшированныеЗначения);
...
Процедура НастроитьЗависимыеЭлементыФормыНаСервере(Знач ИзмененныеРеквизитыИлиЭлемент = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("ЗаказНаВнутреннееПотребление")Тогда
	...
	ВнутреннееПотреблениеТоваровЛокализация.НастроитьЗависимыеЭлементыФормы(ЭтаФорма, СтруктураИзмененныхРеквизитов);
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер(ДополнительныеПараметры)
	...
	ВнутреннееПотреблениеТоваровЛокализация.ХозяйственнаяОперацияПриИзменении(ЭтаФорма, ИзмененныеРеквизиты, ДополнительныеПараметры);
...
Процедура АктуализироватьПотреблениеПодДеятельность(Проверить = Истина)
	...
	ПараметрыЗаполнения = Документы.ВнутреннееПотреблениеТоваров.ПараметрыЗаполненияВидаДеятельностиНДС(Объект);
...
Функция АдресТоварыНакладной()
	...
	Таблица.Колонки.Добавить("ЗаказНаВнутреннееПотребление", Новый ОписаниеТипов("ДокументСсылка.ЗаказНаВнутреннееПотребление,
		|ДокументСсылка.ВнутреннееПотреблениеТоваров"));
...
Процедура РазбитьСтроку(Команда)
	...
	Если НЕ ВнутреннееПотреблениеТоваровКлиентЛокализация.МожноРазбитьСтроку(ТекущаяСтрока) Тогда
		Возврат;
	КонецЕсли; 
...
Процедура ОбработкаПодбораТоваровИзЗаказа(АдресВХранилище)
	...
	ВнутреннееПотреблениеТоваровЛокализация.ОбработатьДокументПослеДобавленияСтрок(Объект);
...
Функция ПривязатьСтрокиКЗаказамНаВнутреннееПотребление(ОтобратьПоЗаказу = Ложь)
	...
	|	ВЫБОР КОГДА НЕ ЗаказТовары.СтатьяРасходов ЕСТЬ NULL ТОГДА
	|				ЗаказТовары.СтатьяРасходов
	|			ИНАЧЕ
	|				НЕОПРЕДЕЛЕНО
	|		КОНЕЦ КАК СтатьяРасходов,
	|	ВЫБОР КОГДА НЕ ЗаказТовары.АналитикаРасходов ЕСТЬ NULL ТОГДА
	|				ЗаказТовары.АналитикаРасходов
	|			ИНАЧЕ
	|				НЕОПРЕДЕЛЕНО
	|		КОНЕЦ КАК АналитикаРасходов,
	|	ЕСТЬNULL(ЗаказТовары.ФизическоеЛицо, ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка)) КАК ФизическоеЛицо,
	|	ЕСТЬNULL(ЗаказТовары.КатегорияЭксплуатации, ЗНАЧЕНИЕ(Справочник.КатегорииЭксплуатации.ПустаяСсылка)) КАК КатегорияЭксплуатации,
	|	ЕСТЬNULL(ЗаказТовары.ГруппаПродукции, ЗНАЧЕНИЕ(Справочник.ГруппыАналитическогоУчетаНоменклатуры.ПустаяСсылка)) КАК ГруппаПродукции,
	|	ЕСТЬNULL(ЗаказТовары.АналитикаАктивовПассивов, НЕОПРЕДЕЛЕНО) КАК АналитикаАктивовПассивов,
	|	
	...
	|			И Заказы.ФизическоеЛицо = ДублиТоваров.ФизическоеЛицо
	...
	ТаблицаТоваров.Свернуть("Номенклатура,Характеристика,Упаковка,КодСтроки,СтатусУказанияСерий,СтатьяРасходов,АналитикаРасходов,
		|ЗаказНаВнутреннееПотребление,ФизическоеЛицо,Назначение,КатегорияЭксплуатации,ПартияТМЦВЭксплуатации,ИнвентарныйНомер,
		|Серия,ГруппаПродукции,АналитикаАктивовПассивов,СчетУчета,Субконто1,Субконто2,Субконто3,ИдентификаторСтроки",
		"Количество");
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ВнутреннееПотреблениеТоваровЛокализация.ПриЧтенииСозданииНаСервере(ЭтаФорма);
...
Процедура ЗаполнитьСтатьиРасходовВТабличнойЧасти(ВыделенныеСтроки)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьСтатьиРасходовВТабличнойЧастиЗавершение", ЭтотОбъект, Новый Структура("ВыделенныеСтроки", ВыделенныеСтроки));
		ОткрытьФорму(
...
Процедура ЗаполнитьСтатьиРасходовВТабличнойЧастиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьСтатьиРасходовВТабличнойЧастиЗавершениеСервер(Результат, ДополнительныеПараметры);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСтатьиРасходовВТабличнойЧастиЗавершениеСервер(Результат, ДополнительныеПараметры)
...
Процедура УстановитьУсловноеОформление()
	...
	ВнутреннееПотреблениеТоваровЛокализация.УстановитьУсловноеОформление(ЭтаФорма);
...
Процедура УстановитьДоступностьКомандБуфераОбмена()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(
		...
		МассивЭлементов, 
		"Доступность",
		НЕ ОбщегоНазначения.ПустойБуферОбмена("Строки") И (НЕ Объект.ПотреблениеПоЗаказам ИЛИ ВнутреннееПотреблениеСверхЗаказа));
...
Процедура ЗаполнитьСлужебныеРеквизитыТабличнойЧасти()
	...
	ВнутреннееПотреблениеТоваровЛокализация.ЗаполнитьСлужебныеРеквизитыТабличнойЧасти(Объект);
...
&НаСервере
Процедура ТоварыКатегорияЭксплуатацииПриИзмененииНаСервере(Знач КэшированныеЗначения)
