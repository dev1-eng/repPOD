...
Процедура УправлениеДоступностьюЭлементовФормы(Форма)
	...
	ИнтеграцияИСКлиентСервер.УправлениеДоступностьюЭлементовФормы(Форма, ДоступныеДляЧерновика, ЭтоЧерновик);
	...
	ИнтеграцияИСКлиентСервер.УправлениеДоступностьюЭлементовФормы(Форма, ЗависимыеОтСтатусаЕГАИС, Не ОграничениеПоСтатусуЕГАИС);
...
Процедура СканированиеАлкогольнойПродукцииПослеЗакрытияНаСервере(АдресВременногоХранилища)
	...
	ЗаполнитьТипШтрихкодаВДеревеУпаковок(ДеревоМаркируемойПродукции);
...
&НаСервере
Процедура ЗаполнитьТипШтрихкодаВДеревеУпаковок(ДеревоМаркируемойПродукции)
	
	ДеревоМаркируемойПродукции.Колонки.Добавить("ТипШтрихкода");
	
	ЗначенияШтрихкодов = Новый Массив;
	ЗаполнитьШтрихкодыАкцизныхМарокПоСтрокамДерева(ДеревоМаркируемойПродукции.Строки, ЗначенияШтрихкодов);
	ДанныеШтрихкодовУпаковок = ШтрихкодированиеИС.ШтрихкодыУпаковок(ЗначенияШтрихкодов);
	
	ЗаполнитьТипыШтрихкодовВСтрокахДерева(ДеревоМаркируемойПродукции.Строки, ДанныеШтрихкодовУпаковок);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьШтрихкодыАкцизныхМарокПоСтрокамДерева(СтрокиДерева, ЗначенияШтрихкодов)
	
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		
		ЗначенияШтрихкодов.Добавить(СтрокаДерева.Штрихкод);
		
		ЗаполнитьШтрихкодыАкцизныхМарокПоСтрокамДерева(СтрокаДерева.Строки, ЗначенияШтрихкодов);
...
&НаСервере
Процедура ЗаполнитьТипыШтрихкодовВСтрокахДерева(СтрокиДерева, ДанныеШтрихкодовУпаковок)
	
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		
		НайденнаяСтрока = ДанныеШтрихкодовУпаковок.Найти(СтрокаДерева.Штрихкод, "Штрихкод");
		Если НайденнаяСтрока <> Неопределено Тогда
			СтрокаДерева.ТипШтрихкода = НайденнаяСтрока.ТипШтрихкода;
		КонецЕсли;
		
		ЗаполнитьТипыШтрихкодовВСтрокахДерева(СтрокаДерева.Строки, ДанныеШтрихкодовУпаковок);
...
Процедура ПриИзмененииАлкогольнойПродукции(Идентификатор)
	...
	СобытияФормЕГАИС.ОкруглитьКоличествоДляУпакованнойПродукцииСУчетомУпаковок(Объект.Товары.НайтиПоИдентификатору(Идентификатор),
