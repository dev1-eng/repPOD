...
//
// Параметры:
//   ДокументСсылка - ДокументСсылка.ПриобретениеТоваровУслуг - Документ.
...
Функция ПараметрыОперацииФискализацииЧека(ДокументСсылка, СуммаПредоплатыКорректировка = Неопределено, ВерсияФФД = Неопределено) Экспорт
	...
	|	ПриобретениеТоваровУслугТовары.Упаковка КАК УпаковкаНаименование,
	...
	|				ТОГДА ПриобретениеТоваровУслугТовары.СуммаСНДС
	|			ИНАЧЕ (ПриобретениеТоваровУслугТовары.СуммаСНДС + ПриобретениеТоваровУслугТовары.СуммаРучнойСкидки) / ПриобретениеТоваровУслугТовары.КоличествоУпаковок
	|		КОНЕЦ КАК ЧИСЛО(31, 2)) КАК Цена,
	|	ПриобретениеТоваровУслугТовары.СуммаРучнойСкидки КАК СуммаСкидки,
	...
	|		ПО (СпрНоменклатура.Ссылка = ПриобретениеТоваровУслугТовары.Номенклатура)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК СпрХарактеристикиНоменклатуры
	|		ПО (СпрХарактеристикиНоменклатуры.Ссылка = ПриобретениеТоваровУслугТовары.Характеристика)
	|ГДЕ
	...
	ПараметрыФискализацииЧека.ТипРасчета = Перечисления.ТипыРасчетаДенежнымиСредствами.РасходДенежныхСредств;
...
#Область ИнтеграцияМОТП

Процедура ИзменитьДокументПоРезультатамПоРезультатамПроверкиИПодбораМОТП(ДокументОбъект, ПараметрыОкончанияСканирования) Экспорт
...
Функция ПараметрыЗаполненияНалогообложенияНДСЗакупки(Объект) Экспорт
	...
	Если ХозОперацииЗакупкаУПоставщика.Найти(ДанныеОбъекта.ХозяйственнаяОперация) <> Неопределено
...
Функция ДополнительныеИсточникиДанныхДляДвижений(ИмяРегистра) Экспорт
	...
	ИначеЕсли ИмяРегистра = "ТоварыОрганизаций" Тогда
		ИсточникиДанных.Вставить("Договор");
	КонецЕсли;
...
Процедура ИнициализироватьКлючиАналитикиУчетаНоменклатуры(Запрос)
	...
	|	ТаблицаТовары.АналитикаУчетаНоменклатуры.Серия КАК Серия,
	|	ТаблицаТовары.Ссылка.Склад КАК МестоХранения
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг.Товары КАК ТаблицаТовары
	|
	...
	|		И АналитикаПоСкладам.МестоХранения = ТаблицаТовары.Ссылка.Склад
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
...
Процедура ИнициализироватьКлючиАналитикиУчетаПоПартнерам(Запрос)
	...
	|	ВЫБОР 
	|		КОГДА &РасчетыПоЗаказам
	|			ТОГДА Этапы.Заказ.НаправлениеДеятельности 	
	|		ИНАЧЕ
	|			&НаправлениеДеятельности
	|	КОНЕЦ  		 КАК НаправлениеДеятельности 
	|ИЗ
...
Функция ТекстЗапросаТаблицаРасчетыСПоставщиками(Запрос, ТекстыЗапроса, Регистры)
	...
	|
	|	ЭтапыГрафикаОплаты.Заказ КАК ЗакупкаПоЗаказу,
	|
	|	&ВалютаВзаиморасчетов КАК Валюта,
	|	&ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	&ФормаОплаты КАК ФормаОплаты,
	|	
	|	СУММА(ЭтапыГрафикаОплаты.СуммаВзаиморасчетов + ЭтапыГрафикаОплаты.СуммаВзаиморасчетовПоТаре) КАК Сумма,
	|	ВЫБОР КОГДА &ВалютаВзаиморасчетов = &РегламентированногоУчетаВалюта И &Валюта <> &РегламентированногоУчетаВалюта
	|		ТОГДА СУММА(ЭтапыГрафикаОплаты.СуммаВзаиморасчетов + ЭтапыГрафикаОплаты.СуммаВзаиморасчетовПоТаре)
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ЭтапыГрафикаОплаты.СуммаПлатежа + ЭтапыГрафикаОплаты.СуммаЗалогаЗаТару) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2))
	|	КОНЕЦ КАК СуммаРегл,
	|	ВЫБОР КОГДА &ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|		ТОГДА СУММА(ЭтапыГрафикаОплаты.СуммаВзаиморасчетов + ЭтапыГрафикаОплаты.СуммаВзаиморасчетовПоТаре)
	|		ИНАЧЕ ВЫРАЗИТЬ(СУММА(ЭтапыГрафикаОплаты.СуммаПлатежа + ЭтапыГрафикаОплаты.СуммаЗалогаЗаТару) * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31,2))
	|	КОНЕЦ КАК СуммаУпр,
	|	0 КАК КОплате,
	|	0 КАК КПоступлению,
	|	СУММА(ЭтапыГрафикаОплаты.СуммаВзаиморасчетовПоТаре) КАК ЗалогЗаТару,
	|	&Организация КАК Организация,
	...
	|		И ВЫБОР 
	|				КОГДА &РасчетыПоЗаказам 
	|					ТОГДА ЕСТЬNULL(ЭтапыГрафикаОплаты.Заказ.НаправлениеДеятельности, &НаправлениеДеятельности)
	|				ИНАЧЕ
	|					&НаправлениеДеятельности
	|		  КОНЕЦ      = Аналитика.НаправлениеДеятельности
	|
	...
	|	&Период КАК ДатаРегистратора,
	|	&Номер КАК НомерРегистратора,
	|	НАЧАЛОПЕРИОДА(&Период,ДЕНЬ) КАК ДатаПлатежа,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	ЕСТЬNULL(Аналитика.КлючАналитики, Неопределено) КАК АналитикаУчетаПоПартнерам,
	|
	|	ВЫБОР КОГДА &РасчетыПоДоговорам ТОГДА
	...
	|	КОНЕЦ КАК ЗаказПоставщику,
	|
	|	ТаблицаТовары.ЗаказПоставщику КАК ЗакупкаПоЗаказу,
	|	&ВалютаВзаиморасчетов КАК Валюта,
	|	&ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	...
	|		И ВЫБОР 
	|				КОГДА &РасчетыПоЗаказам 
	|					ТОГДА ЕСТЬNULL(ТаблицаТовары.ЗаказПоставщику.НаправлениеДеятельности, &НаправлениеДеятельности)
	|				ИНАЧЕ
	|					&НаправлениеДеятельности
	|		  КОНЕЦ      = Аналитика.НаправлениеДеятельности
	|
	...
	|
	|	ВЫБОР 
	|		КОГДА ТаблицаТовары.ЗаказПоставщику = ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ТаблицаТовары.ЗаказПоставщику
	...
	|	И (ТаблицаТовары.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|		ИЛИ НЕ &ВернутьМногооборотнуюТару
	|		ИЛИ (&ТребуетсяЗалогЗаТару И ТаблицаТовары.КодСтроки = 0) 
	|		ИЛИ (ТаблицаТовары.КодСтроки <> 0))
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.ЗаказПоставщику,
	...
	|	СУММА(0) КАК СуммаРегл,
	|	СУММА(0) КАК СуммаУпр,
	|	СУММА(ЭтапыГрафикаОплаты.СуммаВзаиморасчетов + ЭтапыГрафикаОплаты.СуммаВзаиморасчетовПоТаре) КАК КОплате,
	...
	|// В версиях от 2.4 в расшифровке платежа приобретения только регл. могут присутствовать только платежи.
	|
	|ВЫБРАТЬ
	|	&Период КАК Период,
	|	&Период КАК ДатаРегистратора,
	...
	|	ВЫБОР КОГДА ТаблицаРасшифровкаПлатежа.ВалютаВзаиморасчетов = &РегламентированногоУчетаВалюта И &Валюта <> &РегламентированногоУчетаВалюта
	...
	|	И НЕ ТИПЗНАЧЕНИЯ(ТаблицаРасшифровкаПлатежа.Заказ) В (Тип(Документ.СписаниеБезналичныхДенежныхСредств),
	...
	|	ВЫБОР КОГДА &Валюта = &РегламентированногоУчетаВалюта
	...
	ТекстЗапроса=СтрЗаменить(ТекстЗапроса,"//ЗачетАвансовПоставщикуПоНакладной",
		ВзаиморасчетыСервер.ТекстПроведенияЗачетАвансовПоставщикуПоНакладной("ПриобретениеТоваровУслуг", ШаблонПолей));
...
Функция ТекстЗапросаВтТаблицаТовары(Запрос, ТекстыЗапроса)
	...
	|			ТОГДА ТаблицаТовары.СуммаВзаиморасчетов - ТаблицаТовары.СуммаНДСВзаиморасчетов
	|			ИНАЧЕ ВЫРАЗИТЬ((ТаблицаТовары.СуммаСНДС - ТаблицаТовары.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2)) 
	|		КОНЕЦ 
	|	) КАК СуммаБезНДСРегл,
	|
...
Процедура ПересчитатьТаблицуТоваров(Запрос)
	...
	ВалютыДляПересчета.Вставить("Регл", Запрос.Параметры.РегламентированногоУчетаВалюта);
