...
Процедура ЗаполнитьЭтапыГрафикаОплаты() Экспорт
	...
	СуммаТары        = ПолучитьСуммуЗалоговойТары();
	...
	Если СуммаДокумента + СуммаТары > 0 Тогда
		...
		Если ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным
			...
			Иначе
				ЖелаемаяДата = ЗакупкиСервер.ПолучитьПоследнююДатуПоГрафику(Дата, Соглашение);
			КонецЕсли;
			...
			ПараметрыДляДобавления.ВариантОплаты = Перечисления.ВариантыОплатыПоставщику.КредитПослеПоступления;
		...
		Иначе
			...
			ПараметрыЗаполненияЭтапов.ТабличнаяЧасть = Товары.Выгрузить(,"КодСтроки, Номенклатура, ЗаказПоставщику, СуммаСНДС, СуммаВзаиморасчетов");
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ПараметрыПолученияКоэффициентаРНПТ = УчетПрослеживаемыхТоваровКлиентСерверЛокализация.ПараметрыПолученияКоэффициентаРНПТ(ЭтотОбъект, "Склад", Истина);
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	ИначеЕсли ИспользоватьРаздельноеОформлениеЗакупок
		...
		Если ЭтоНеотфактуровка Тогда
			ЗаполнитьАналитикамиСУчетамиСерий();
		КонецЕсли;
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(Измерения)
	...
	ПереченьДопПолей = "АналитикаУчетаНоменклатуры, ЗаказПоставщику, Цена, СтавкаНДС";
...
Процедура ЗаполнитьАналитикамиСУчетамиСерий()
...
Функция ПолучитьСуммуЗалоговойТары()
