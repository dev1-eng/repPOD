...
Процедура СкопироватьИзКроссТаблицы(КэшированныеЗначения)
	...
	Если Объект.КроссТаблица 
		...
		Планирование.ЗаполнитьТаблицуИзКроссТаблицы(Объект.ТоварныеКатегории, ТоварныеКатегорииПоПериодам, ПараметрыПреобразования);
...
Функция ЗагрузитьИзExcelНаСервере(МассивСтруктурТовары, ЕстьДобавленныеСтроки)
	...
	Если ПланированиеПоНазначениямВТЧ И ЕстьДобавленныеСтроки Тогда
		...
		Для Каждого СтруктураТовар Из МассивСтруктурТовары Цикл
			...
			ПроверяемаяТоварнаяКатегория = Неопределено;
			Если СтруктураТовар.Свойство("ЗагружаемаяТоварнаяКатегория") И ЗначениеЗаполнено(СтруктураТовар.ЗагружаемаяТоварнаяКатегория) Тогда
				ПроверяемаяТоварнаяКатегория = Справочники.ТоварныеКатегории.НайтиПоНаименованию(СтруктураТовар.ЗагружаемаяТоварнаяКатегория);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ПроверяемаяТоварнаяКатегория) Тогда
				
				СтруктураТовар.Вставить("ТоварнаяКатегория", ПроверяемаяТоварнаяКатегория.Ссылка);
				...
				Если СтруктураТовар.Свойство("ЗагружаемыйРейтингПродаж") Тогда
					
					Если ЗначениеЗаполнено(СтруктураТовар.ЗагружаемыйРейтингПродаж) Тогда
						
						ПроверяемыйРейтингПродаж = Справочники.РейтингиПродажНоменклатуры.НайтиПоНаименованию(СтруктураТовар.ЗагружаемыйРейтингПродаж);
						Если ЗначениеЗаполнено(ПроверяемыйРейтингПродаж) Тогда
							СтруктураТовар.Вставить("РейтингПродаж", ПроверяемыйРейтингПродаж.Ссылка);
						КонецЕсли;
						ПроверяемыйРейтингПродаж = Неопределено;
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru='УИ_Назначение'"),"ИдентификаторНазначение");
...
Процедура ДобавитьШаблоныФормулРасчетаПлановогоКоличества(Дерево, ГруппаОператоров)
	...
	Реквизиты = "ИспользоватьВПланированииОстатков, ИспользоватьВПланированииПродаж, ИспользоватьВПланированииЗакупок, ИспользоватьВПланированииСборкиРазборки, ИспользоватьРасчетПоСкоростиПродаж";
	...
	Если Сценарий.ИспользоватьРасчетПоСкоростиПродаж Тогда
		...
		РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План продаж с учетом корректировки по факту продаж'"), 
			"Макс([ФактПродажПоКатегориям], [ПланыПродажПоКатегориямПоСкоростиПродаж])" +
	...
	Иначе
		...
		РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru='План продаж с учетом корректировки по факту продаж'"), 
			"Макс([ФактПродажПоКатегориям], [ПланыПродажПоКатегориям])" +
				?(ИспользоватьПланыОстатков," - [ПланОстатков]",?(ПланированиеПоНазначениям," - [ФактическиеНачальныеОстатки]"," - [СвободныеОстатки]")));
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ЗаполнятьСклад, ЗаполнятьФорматМагазина, ПланированиеПоНазначениям, ЗаполнятьНазначениеВТЧ");
