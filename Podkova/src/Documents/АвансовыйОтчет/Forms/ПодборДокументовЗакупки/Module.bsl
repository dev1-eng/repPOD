...
Функция ВыборкаДокументовЗакупки(Параметры)
	...
	ПараметрыПодбора = Новый Структура("Ссылка, ПодотчетноеЛицо, Организация, Подразделение, Валюта, Мультивалютный");
	...
	КонецЦикла;
	Запрос.УстановитьПараметр("СсылкаЗаполнена", ЗначениеЗаполнено(ПараметрыПодбора.Ссылка));
