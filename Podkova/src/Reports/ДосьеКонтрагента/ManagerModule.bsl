...
Процедура ЗаполнитьСреднююРентабельностьОКВЭД2(Результат, ВидыДеятельности, Год, ИмяОбласти)
	
	КодОКВЭД2 = ВидыДеятельности[0].КодОКВЭД;
	
	ИмяМакета = "ПредельныеЗначенияРентабельности" + Год;
	
	Если Метаданные.Отчеты.ДосьеКонтрагента.Макеты.Найти(ИмяМакета) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ОбластьКодОКВЭД2 = Макет.Область("КодОКВЭД2");
	ОбластьСтроки = Макет.НайтиТекст(КодОКВЭД2, , ОбластьКодОКВЭД2, , Истина);
	Если ОбластьСтроки = Неопределено Тогда
		НомерСтроки = 2; // Значение "Всего"
	Иначе
		НомерСтроки = ОбластьСтроки.Верх;
	КонецЕсли;
	
	ОбластьРентабельностьАктивов = Макет.Область(ИмяОбласти);
	НомерКолонки = ОбластьРентабельностьАктивов.Лево;
...
Функция СредняяРентабельностьАктивовПоОтрасли(ВидыДеятельности, Год)
	...
	Если ВидыДеятельности[0].ЭтоОКВЭД2 Тогда
		Если Год > "2016" Тогда
			ЗаполнитьСреднююРентабельностьОКВЭД2(Результат, ВидыДеятельности, Год, "РентабельностьАктивов");
			Возврат Результат;
		Иначе
...
Функция СредняяРентабельностьПродажПоОтрасли(ВидыДеятельности, Год)
	...
	Если ВидыДеятельности[0].ЭтоОКВЭД2 Тогда
		Если Год > "2016" Тогда
			ЗаполнитьСреднююРентабельностьОКВЭД2(Результат, ВидыДеятельности, Год, "РентабельностьПродаж");
			Возврат Результат;
		Иначе
			ОсновнойКодОКВЭД = ГруппаРентабельностиПоКодуОКВЭД2(ВидыДеятельности[0].КодОКВЭД);
		КонецЕсли;
