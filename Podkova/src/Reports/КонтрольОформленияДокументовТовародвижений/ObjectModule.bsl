...
//		* СписокНоменкатуры - СписокЗначений - список номенклатуры для оформления складских актов.
...
Процедура УстановитьОбязательныеНастройки(ПользовательскиеНастройкиМодифицированы)
	...
	Для Каждого ЭлементСоответствия Из ПредставленияРекомендаций Цикл
		КомпоновкаДанныхКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СтрокаРекомендация" + ЭлементСоответствия.Ключ, ЭлементСоответствия.Значение);
	КонецЦикла;
...
Функция ТекстЗапросаЗакупки()
	...
	|				ИЗ
	|					РаспоряженияНаПоступлениеПоЗакупке КАК РаспоряженияНаПоступление) {(ДокументПоступления).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОформлениюПоступления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыРаспоряжения
	|		ПО ТоварыКОформлениюПоступления.ДокументПоступления = РеквизитыРаспоряжения.Ссылка
	|			И (НЕ РеквизитыРаспоряжения.ДополнительнаяЗапись)
	|{ГДЕ
...
Процедура ПодключитьЗапрос(ДинамическийТекстЗапроса, ПодключаемыйЗапрос, КоличествоПодключенныхЗапросов,
	...
	ТекстЗапросаВременнойТаблицы = ?(ЗначениеЗаполнено(ТекстЗапросаВременнойТаблицы),
		ТекстЗапросаВременнойТаблицы + ОбщегоНазначенияУТ.РазделительЗапросовВПакете(), "");
