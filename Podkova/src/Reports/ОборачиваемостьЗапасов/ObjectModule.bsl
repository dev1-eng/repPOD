...
Функция ТаблицаПоказателейОборачиваемости(Настройки, РазвернутаяТаблица)
	...
	Для Каждого СтрНоменклатура Из ТаблицаНоменклатуры Цикл
		...
		Если ЗначениеЗаполнено(КонецПериода) И ПеременныеРасчета.ТочкаОтсчета < КонецПериода Тогда
			// Это случай когда контрольные точки расположены между датой последнего изменения остатка и датой конца периода
			// формирования отчета.
		...
		// Остаток на последнюю контрольную точку должен делиться на 2, а в цикле при суммировании учтен весь остаток, поэтому
		// вычтем половину последнего остатка из суммы среднего остатка за период.
