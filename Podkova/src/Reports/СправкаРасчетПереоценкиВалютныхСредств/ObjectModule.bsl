...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьДоговорыКредитовИДепозитов") И 
		НЕ ПолучитьФункциональнуюОпцию("ИспользоватьЛизинг") Тогда
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		СхемаКомпоновкиДанных.НаборыДанных.Движения.Элементы.РасчетыСПоставщиками.Запрос = ТекстЗапросаРасчетыСПоставщиками();
		СхемаКомпоновкиДанных.НаборыДанных.Движения.Элементы.РасчетыСКлиентами.Запрос = ТекстЗапросаРасчетыСКлиентами();
	КонецЕсли;
...
Функция ТекстЗапросаРасчетыСПоставщиками()
	...
	|			&Период,
	|			Валюта <> &ВалютаУпр
	|				ИЛИ Валюта <> &ВалютаРегл {(АналитикаУчетаПоПартнерам).*, (ЗаказПоставщику).*, (РасчетныйДокумент).*, (Валюта).*}) КАК Расчеты
	...
	|	И (Расчеты.Валюта <> &ВалютаУпр ИЛИ Расчеты.Валюта <> &ВалютаРегл)
	|{ГДЕ
	|	Расчеты.Регистратор,
	|	Расчеты.Валюта.*,
	|	Расчеты.Период,
...
Функция ТекстЗапросаРасчетыСКлиентами()
	...
	|			&Период,
	|			Валюта <> &ВалютаУпр
	|				ИЛИ Валюта <> &ВалютаРегл {(АналитикаУчетаПоПартнерам).*, (ЗаказКлиента).*, (РасчетныйДокумент).*, (Валюта).*}) КАК Расчеты
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК Аналитика
	|	ПО Расчеты.АналитикаУчетаПоПартнерам = Аналитика.КлючАналитики
	|{ГДЕ
	|	Аналитика.Организация.*,
