...
Процедура ВывестиДанныеОАктуальныхЗапросахКоммерческихПредложений(РезультатЗапросаЗапросыКоммерческихПредложений, ТаблицаОтчета, Макет)
	...
	// Вывод таблицы
	ВывестиОбластьБезПараметров("ЗапросыКоммерческихПредложенийШапка",Макет,ТаблицаОтчета);
	...
	Пока ВыборкаДетали.Следующий() Цикл
		...
		Область.Параметры.НомерДата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='№ %1 от %2'"),
		                                                                                      ...
		                                                                                      ВыборкаДетали.Дата);
		Если ЧетнаяСтрока Тогда
			Область.Области.СтрокаТаблицыЗКП.ЦветФона = ЦветаСтиля.АльтернативныйЦветФонаПоля;
		КонецЕсли;
		ТаблицаОтчета.Вывести(Область,2);
...
Процедура ВывестиДанныеОДействующихКоммерческихПредложениях(РезультатЗапросаПоКоммерческимПредложениям, ТаблицаОтчета, Макет, ТипКоммерческогоПредложения)
...
Процедура СформироватьОтчет(ТаблицаОтчета, Партнер)  Экспорт
	...
	Если ДанныеПоставщика Тогда
		...
		Если СоответствиеЗапросыДанные["КоммерческиеПредложенияПоставщика"] <> Неопределено Тогда
			ВывестиДанныеОДействующихКоммерческихПредложениях(МассивРезультатовЗапросовПоПартнеру[СоответствиеЗапросыДанные["КоммерческиеПредложенияПоставщика"]],
			                                                  ТаблицаОтчета, Макет, "Поставщику");
		КонецЕсли;
	КонецЕсли;
	Если ДанныеКлиента ИЛИ ДанныеПоставщика Тогда
	...
	Если ДанныеКлиента Тогда
		...
		Если СоответствиеЗапросыДанные["ЗапросКоммерческогоПредложенияОтКлиента"] <> Неопределено Тогда
			ВывестиДанныеОАктуальныхЗапросахКоммерческихПредложений(МассивРезультатовЗапросовПоПартнеру[СоответствиеЗапросыДанные["ЗапросКоммерческогоПредложенияОтКлиента"]],
			                                                        ТаблицаОтчета, Макет);
		КонецЕсли;
