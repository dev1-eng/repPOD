...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ПараметрКоманды")
		...
		ИначеЕсли Параметры.ОписаниеКоманды.ДополнительныеПараметры.ИмяКоманды = "СостояниеРасчетовСПоставщикомПоДокументам" Тогда
			...
			Параметры.КлючВарианта = "ЗадолженностьПоставщикамПоОбъектуРасчетовКонтекст" + СтрокаБазовая;
...
Функция ТекстЗапроса()
	...
	|		
	|		ВЫБОР
	|			КОГДА РасчетыПланОплат.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыПоставщику.АвансДоПодтверждения)
	|				ТОГДА РасчетыПланОплат.КОплатеОстаток 
	|			ИНАЧЕ 0 
	|		КОНЕЦ                                                         КАК ПлановаяОплатаАванс,
	|		ВЫБОР
	|			КОГДА РасчетыПланОплат.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыПоставщику.ПредоплатаДоПоступления)
	|				ТОГДА РасчетыПланОплат.КОплатеОстаток 
	|			ИНАЧЕ 0 
	|		КОНЕЦ                                                         КАК ПлановаяОплатаПредоплата,
	|		ВЫБОР
	|			КОГДА РасчетыПланОплат.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыПоставщику.КредитПослеПоступления)
	|				ИЛИ РасчетыПланОплат.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыПоставщику.КредитСдвиг)
	|				ТОГДА РасчетыПланОплат.КОплатеОстаток 
	|			ИНАЧЕ 0 
	...
	|		
	|		0                                         КАК ПлановоеПоступление,
	|		0                                         КАК ПлановоеПоступлениеаПросрочено
	|	ИЗ
...
Процедура СформироватьПараметрыСостояниеРасчетовСПоставщиком(ПараметрКоманды, ПараметрыФормы, Параметры)
	...
	Если ЕстьПодчиненныеПартнеры Тогда
		...
		Параметры.КлючВарианта = "ЗадолженностьПоставщикамКонтекст" + СтрокаБазовая;
	Иначе
		...
		Если ЭтоМассив И ПараметрКоманды.Количество() = 1 Тогда
			Параметры.КлючВарианта = "ЗадолженностьПоставщикуКонтекст" + СтрокаБазовая;
		Иначе
			Параметры.КлючВарианта = "ЗадолженностьПоставщикамКонтекст" + СтрокаБазовая;
		КонецЕсли;
	КонецЕсли;
