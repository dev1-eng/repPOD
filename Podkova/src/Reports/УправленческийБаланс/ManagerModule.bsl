...
Процедура ВывестиАктуальностьОтчета(КомпоновщикНастроек, ДокументРезультат) Экспорт
	...
	СостояниеРасчета = РасчетСебестоимостиПрикладныеАлгоритмы.ТекущееСостояниеРасчета(ПериодОтчета.ДатаОкончания, МассивОрганизаций);
	ВыполненоРаспределениеВзаиморасчетов = ВзаиморасчетыАктуальны(МассивОрганизаций, ПериодОтчета)
		ИЛИ ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов");
