...
Процедура ОбновитьЗаголовокФормы()
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи Тогда
		ПредставлениеТипа = НСтр("ru='Договор с хранителем'");
	Иначе
...
Процедура ОбновитьСостояниеЭлементовУправленияИтоговПоРасчетам()
	...
	Расчеты = НСтр("ru='Оплачено'") + ": " + Формат(СуммаОплаты, "ЧДЦ=2; ЧН=") 
				+ " " + Объект.ВалютаВзаиморасчетов + "  " + ПроцентОплаты + "%";
	Расчеты = Расчеты + ", " + СтрокаОтгружено + ": " + Формат(СуммаОтгрузки, "ЧДЦ=2; ЧН=") 
...
Процедура ОбновитьОграничениеЗадолженности()
	...
	Элементы.ТекстОстатокДопустимогоКредита.РастягиватьПоГоризонтали = Не Элементы.КартинкаОтгрузкаЗапрещена.Видимость;
...
Процедура ПриИзмененииНастроекПриемкиСервер();
	...
	Если Элементы.ОформлениеОрдера.СписокВыбора.Количество() = 1 Тогда
		ОформлениеОрдера = Элементы.ОформлениеОрдера.СписокВыбора[0].Значение;
	КонецЕсли;
...
Процедура УстановитьВидимостьПорядокСписанияПринятыхТоваров(Форма)
	...
	Форма.Элементы.ПорядокОформленияСписанияНедостачПринятыхНаХранениеТоваров.Видимость = ?(
		Форма.Объект.ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоров.СПоклажедателем"), Истина, Ложь);
