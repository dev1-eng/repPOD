...
Процедура ЗадатьПроизвольнуюФормулу(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СмещениеРедактируемое <> 0 
		...
		ТекстВопроса = НСтр("ru = 'В формуле присутсвует операнд по вычислению планируемого остатка, данные могут быть получены только из текущего периода. Изменить текущие настройки смещения и продолжить?'");
		ПоказатьВопрос(
