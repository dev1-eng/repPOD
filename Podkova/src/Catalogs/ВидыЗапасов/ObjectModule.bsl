...
Функция НаименованиеВидаЗапасов(СправочникОбъект)
	...
	Если СправочникОбъект.ТипЗапасов = Перечисления.ТипыЗапасов.КомиссионныйТовар Тогда
		...
		НаименованиеМассив.Добавить(НСтр("ru='Комитент:'") + Символы.НПП + Строка(СправочникОбъект.ВладелецТовара));
		Если ЗначениеЗаполнено(СправочникОбъект.Соглашение) Тогда
			НаименованиеМассив.Добавить(НСтр("ru='Соглашение:'") + Символы.НПП + Строка(СправочникОбъект.Соглашение));
		КонецЕсли;
		Если ЗначениеЗаполнено(СправочникОбъект.Договор) Тогда
			НаименованиеМассив.Добавить(НСтр("ru='Договор:'") + Символы.НПП + Строка(СправочникОбъект.Договор));
		КонецЕсли;
		НаименованиеМассив.Добавить(СтрокаНалогообложение);
		НаименованиеМассив.Добавить(НСтр("ru='Валюта:'") + Символы.НПП + Строка(СправочникОбъект.Валюта));
		
	ИначеЕсли СправочникОбъект.ТипЗапасов = Перечисления.ТипыЗапасов.ТоварНаХраненииСПравомПродажи Тогда
		
		НаименованиеМассив.Добавить(НСтр("ru='Поклажедатель:'") + Символы.НПП + Строка(СправочникОбъект.ВладелецТовара));
		НаименованиеМассив.Добавить(НСтр("ru='Договор:'") + Символы.НПП + Строка(СправочникОбъект.Договор));
		НаименованиеМассив.Добавить(НСтр("ru='Вид цены:'") + Символы.НПП + Строка(СправочникОбъект.ВидЦены));
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СправочникОбъект.ГруппаФинансовогоУчета) Тогда
		НаименованиеМассив.Добавить(НСтр("ru='Группа:'") + Символы.НПП + Строка(СправочникОбъект.ГруппаФинансовогоУчета));
	КонецЕсли;
	
	НаименованиеМассив.Добавить(НСтр("ru='Организация:'") + Символы.НПП + Строка(СправочникОбъект.Организация));
		
	Если ЗначениеЗаполнено(СправочникОбъект.ГруппаПродукции) Тогда
		НаименованиеМассив.Добавить(НСтр("ru='Группа продукции:'") + Символы.НПП + Строка(СправочникОбъект.ГруппаПродукции));
	КонецЕсли;	
