...
Функция ПолучитьРеквизитыОрганизации(Организация) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		ИндивидуальныйПредприниматель = Выборка.ИндивидуальныйПредприниматель;
	...
	СтруктураРеквизитов.Вставить("КодПоОКАТО", 			КодПоОКАТО);
	СтруктураРеквизитов.Вставить("КодПоОКТМО", 			КодПоОКТМО);
	СтруктураРеквизитов.Вставить("КодОКВЭД", 			КодОКВЭД);
	СтруктураРеквизитов.Вставить("ЮрФизЛицо", 			ЮрФизЛицо);
	СтруктураРеквизитов.Вставить("ИндивидуальныйПредприниматель", ИндивидуальныйПредприниматель);
...
// Устарела. Следует использовать методы программного интерфейса УчетНДСУП.
//
...
//
Функция НалогообложениеНДС(Организация = Неопределено, Склад = Неопределено, Дата = Неопределено, Подразделение = Неопределено) Экспорт
	...
	//++ Локализация
	|	,РегУчетнаяПолитика.ПрименяетсяПСН КАК ПрименяетсяПСН
	//-- Локализация
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		//++ Локализация
		Если Выборка.ПрименяетсяПСН
			И Справочники.Патенты.ЕстьДействующийПатент(Организация, Подразделение, ?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса())) Тогда
...
//	ПеречислениеСсылка.ТипыНалогообложенияНДС - тип налогообложения.
//
Функция ЗакупкаПодДеятельность(Организация, Склад = Неопределено, Знач Дата = Неопределено) Экспорт
