...
Процедура ПередЗаписью(Отказ)
	...
	ПодконтрольнаяПродукцияВЕТИС = ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияВЕТИС;
	МолочнаяПродукцияВЕТИС       = ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.МолочнаяПродукцияПодконтрольнаяВЕТИС;
	...
	Если Не ИспользоватьСерии Тогда
		...
		ИспользоватьПроизводителяВЕТИССерии = Ложь;
		ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии  = Ложь;
		ИспользоватьИдентификаторПартииВЕТИССерии      = Ложь;
	...
	Иначе
		...
		Если НастройкиСерийБерутсяИзДругогоВидаНоменклатуры
			...
			|	ВидыНоменклатуры.ЕдиницаИзмеренияСрокаГодности,
			...
			|	ВидыНоменклатуры.ИспользоватьПроизводителяВЕТИССерии,
			|	ВидыНоменклатуры.ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии,
			|	ВидыНоменклатуры.ИспользоватьИдентификаторПартииВЕТИССерии
			|ИЗ
			...
			Если ВыборкаШапки.Следующий() Тогда
				...
				ЕдиницаИзмеренияСрокаГодности       = ВыборкаШапки.ЕдиницаИзмеренияСрокаГодности;
				...
				ИспользоватьПроизводителяВЕТИССерии = ВыборкаШапки.ИспользоватьПроизводителяВЕТИССерии;
				ИспользоватьИдентификаторПартииВЕТИССерии = ВыборкаШапки.ИспользоватьИдентификаторПартииВЕТИССерии;
				ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии = ВыборкаШапки.ИспользоватьЗаписьСкладскогоЖурналаВЕТИССерии;
		...
		ИначеЕсли АлкогольнаяПродукция
				Или ПодконтрольнаяПродукцияВЕТИС
				Или МолочнаяПродукцияВЕТИС Тогда
		...
		Если Не (АлкогольнаяПродукция
			Или ПодконтрольнаяПродукцияВЕТИС
			Или ПродукцияИСМП.Найти(ОсобенностьУчета) <> Неопределено) Тогда
		...
		Если Не ИспользоватьСрокГодностиСерии Тогда
			...
			Если Не ИспользоватьДатуПроизводстваСерии Тогда
				ТочностьУказанияСрокаГодностиСерии = Перечисления.ТочностиУказанияСрокаГодности.ПустаяСсылка();
			КонецЕсли;
		Иначе
			ЕдиницаИзмеренияСрокаГодности = Справочники.ВидыНоменклатуры.ЕдиницаИзмеренияСрокаГодности(
	...
	Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга
		И ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПустаяСсылка() Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Элемент Из СтруктураШаблонов Цикл
		...
		Если ЗначениеЗаполнено(Шаблон) Тогда
			...
			Если Не РаботаСФормулами.ПроверитьФормулу(
				Шаблон,
				РаботаСФормуламиКлиентСервер.ОперандыТекстовойФормулы(Шаблон),
				Элемент.Ключ,
				ТекстОшибки,
				Истина,
				"Объект") Тогда
