...
Процедура БИКБанкаДляРасчетовГоловнойКонтрагентОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ЗаполнитьБИКиКоррСчет(
		...
		СВИФТБанкаДляРасчетовГоловнойКонтрагент,
...
Процедура ВыбратьГоловногоКонтрагентаПриИзменении(Элемент)
	...
	УправлениеДоступностьюГоловнойКонтрагент();
...
Процедура УправлениеДоступностьюДополнительныеСведения()
	...
	Элементы.ЮридическийАдрес.АвтоОтметкаНезаполненного = УказыватьЮридическиеРеквизиты И ЮрАдресКонтрагентаОбязателен;
	Элементы.ФактическийАдрес.АвтоОтметкаНезаполненного = УказыватьЮридическиеРеквизиты И ФактАдресКонтрагентаОбязателен;
	
	Элементы.ФактическийАдресЧастноеЛицо.АвтоОтметкаНезаполненного = ТребуетсяУказаниеАдресаЧастногоЛица(ЭтотОбъект);
...
&НаКлиенте
Процедура УправлениеДоступностьюГоловнойКонтрагент()
...
&НаСервереБезКонтекста
Процедура ЗаполнитьБИКиКоррСчет(Банк, Бик, Свифт, КоррСчет)
...
Функция ПервичнаяИнформацияЗаполненаНеправильно()
	...
	Если ЭтоКомпания = 0 Тогда
		...
		Если АдресЭППартнераОбязателен
			...
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать адрес электронной почты партнера'"),"АдресЭППартнера");
			Отказ = Истина;
		...
		Если ТелефонПартнераОбязателен
			...
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать телефон партнера'"),"ТелефонПартнера");
			Отказ = Истина;
			ОбязательностьКонтактнойИнформацииОтработана = Истина;
		...
		Если УказатьДанныеКонтактногоЛица Тогда
			...
			Если АдресЭПКонтактногоЛицаОбязателен
				...
				ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать адрес электронной почты контактного лица'"), "АдресЭПКЛ");
				Отказ = Истина;
				ОбязательностьКонтактнойИнформацииОтработана = Истина;
			...
			Если ТелефонКонтактногоЛицаОбязателен 
				...
				ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать телефон контактного лица'"), "ТелефонКЛ");
				Отказ = Истина;
				ОбязательностьКонтактнойИнформацииОтработана = Истина;
			...
			Если МобильныйТелефонКонтактногоЛицаОбязателен 
				...
				ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать мобильный телефон контактного лица'"), "МобильныйТелефонКЛ");
				Отказ = Истина;
				ОбязательностьКонтактнойИнформацииОтработана = Истина;
	...
	Иначе
		...
		Если АдресЭППартнераОбязателен
			...
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать адрес электронной почты партнера'"),"АдресЭППартнераФизЛицо");
			Отказ = Истина;
			ОбязательностьКонтактнойИнформацииОтработана = Истина;
		...
		Если ТелефонПартнераОбязателен
			...
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать телефон партнера'"),"ТелефонПартнераФизЛицо");
			Отказ = Истина;
			ОбязательностьКонтактнойИнформацииОтработана = Истина;
	...
	Если УказатьДанныеКонтактногоЛица И ЭтоКомпания = 0 Тогда
		...
		Если ПустаяСтрока(ТелефонКЛ) И ПустаяСтрока(АдресЭПКЛ) И ПустаяСтрока(МобильныйТелефонКЛ) Тогда
			ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать хотя бы одно значение контактной информации контактного лица'"),"АдресЭПКЛ");
			Отказ = Истина;
		КонецЕсли;
...
Процедура РазобратьФайлНаСервере(СсылкаНаФайл, ЕстьОшибка);
	...
	Если Не ЕстьОшибка Тогда
		...
		АдресЭППартнераФизЛицо = АдресЭППартнера;
		
		ЗначенияПолейКонтактнойИнформацииСервер(ТелефонКЛ,
		                                        ТелефонКЛЗначенияПолей,
		                                        ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон"));
		
		ЗначенияПолейКонтактнойИнформацииСервер(ТелефонПартнера,
		                                        ТелефонПартнераЗначенияПолей,
		                                        ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон"));
		
		ЗначенияПолейКонтактнойИнформацииСервер(ТелефонПартнераФизЛицо,
		                                        ТелефонПартнераФизЛицоЗначенияПолей,
		                                        ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон"));
...
Функция ТребуетсяУказаниеАдресаЧастногоЛица(Форма)
	...
	Возврат Форма.УказыватьЮридическиеРеквизиты
			И (Форма.ЮрАдресКонтрагентаОбязателен
			И Не ЗначениеЗаполнено(Форма.ЮридическийАдрес))
			Или (Форма.ФактАдресКонтрагентаОбязателен
			И Не ЗначениеЗаполнено(Форма.ФактическийАдрес));
...
Процедура ЗаполнитьРеквизитыКонтрагентаПоПолученнымДанным(РеквизитыКонтрагента)
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаГоловнойКонтрагент Тогда
		...
		ПартнерыИКонтрагентыКлиент.ЗаполнитьИсториюНаименований(ЭтотОбъект, РеквизитыКонтрагента);
		Если РеквизитыКонтрагента.Свойство("КПП") Тогда
			КППГоловногоКонтрагента = РеквизитыКонтрагента.КПП;
			ПартнерыИКонтрагентыКлиент.ЗаполнитьИсториюКПП(ЭтотОбъект, РеквизитыКонтрагента);
		КонецЕсли;
