...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПоКонтрагенту                    = ЗначениеЗаполнено(Контрагент) И Не ИспользоватьПартнеровКакКонтрагентов;
	
	Элементы.ПоПартнеру.Видимость    = ПоПартнеру;
	Элементы.ПоКонтрагенту.Видимость = ПоКонтрагенту;
	
	Элементы.ПоКонтрагенту.Доступность = Не Параметры.ИзменениеКонтрагентаНедоступно;
	Элементы.ПоПартнеру.Доступность    = Не Параметры.ИзменениеПартнераНедоступно;
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		Элементы.ПоКонтрагенту.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'По контрагенту ""%1""'"), Контрагент);
	КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Контрагент", Контрагент, ВидСравненияКомпоновкиДанных.Равно,, ПоКонтрагенту);
