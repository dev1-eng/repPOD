...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИзменениеКонтрагентаНедоступно = Параметры.ИзменениеКонтрагентаНедоступно;
	ИзменениеПартнераНедоступно    = Параметры.ИзменениеПартнераНедоступно;	
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
		...
		Если ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыКартЛояльности.Действует")
			...
			ИначеЕсли ЗначениеЗаполнено(Партнер)
				И ТекущиеДанные.Партнер <> Партнер
				И ИзменениеПартнераНедоступно Тогда
				
				Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
					ТекстСообщения = НСтр("ru = 'Карта ""%1"" со штрихкодом ""%2"" зарегистрирована на другого партнера ""%3"".'");
				Иначе
					ТекстСообщения = НСтр("ru = 'Карта ""%1"" со магнитным кодом ""%2"" зарегистрирована на другого партнера ""%3"".'");
				КонецЕсли;
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(ТекстСообщения, ТекущиеДанные.ВидКарты, КодКарты, ТекущиеДанные.Партнер),
					,
					Поле);
					
						ИначеЕсли ЗначениеЗаполнено(Контрагент)
				И ТекущиеДанные.Контрагент <> Контрагент
				И ИзменениеКонтрагентаНедоступно Тогда
				
				Если ТипКода = ПредопределенноеЗначение("Перечисление.ТипыКодовКарт.Штрихкод") Тогда
					ТекстСообщения = НСтр("ru = 'Карта ""%1"" со штрихкодом ""%2"" зарегистрирована на другого контрагента ""%3"".'");
				Иначе
					ТекстСообщения = НСтр("ru = 'Карта ""%1"" со магнитным кодом ""%2"" зарегистрирована на другого контрагента ""%3"".'");
				КонецЕсли;
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрШаблон(ТекстСообщения, ТекущиеДанные.ВидКарты, КодКарты, ТекущиеДанные.Контрагент),
					,
					Поле);				
			Иначе
				Закрыть();
