...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.СпособЗаданияЦены = ПредопределенноеЗначение("Перечисление.СпособыЗаданияЦен.РассчитыватьПоФормуламОтДругихВидовЦен") Тогда
		...
		Если Не РаботаСФормулами.ПроверитьФормулу(Объект.Формула, БазовыеВидыЦенСервер(), "Объект.Формула") Тогда
			Отказ = Истина;
		КонецЕсли;
		...
		Если ИспользоватьЦеновыеГруппы Тогда
			...
			Для Каждого Формула Из Объект.ЦеновыеГруппы Цикл
				...
				Если Не РаботаСФормулами.ПроверитьФормулу(Формула.Формула, БазовыеВидыЦенСервер(), "Объект.Формулы[" + ТекИндекс + "].Формула", ТекстОшибки) Тогда
					Отказ = Истина;
				КонецЕсли;
...
Процедура РедактироватьСхемуКомпоновкиДанных(Команда)
	...
	ПараметрыФормы.Вставить("НеРедактироватьСхемуКомпоновкиДанных",
		Объект.СпособЗаданияЦены = ПредопределенноеЗначение("Перечисление.СпособыЗаданияЦен.НаценкаНаЦенуПоступления"));
		
	ПараметрыФормы.Вставить("НеЗагружатьСхемуКомпоновкиДанныхИзФайла",
		Объект.СпособЗаданияЦены = ПредопределенноеЗначение("Перечисление.СпособыЗаданияЦен.НаценкаНаЦенуПоступления"));
...
Процедура ПроверитьФормулы(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Формула) Тогда
		Если Не РаботаСФормуламиКлиент.ПроверитьФормулу(Объект.Формула, БазовыеВидыЦен(), "Объект.Формула") Тогда
	...
	Для Каждого Формула Из Объект.ЦеновыеГруппы Цикл
		...
		Если Не РаботаСФормуламиКлиент.ПроверитьФормулу(Формула.Формула, БазовыеВидыЦен(), "Объект.ЦеновыеГруппы[" + ИндексЭлемента + "].Формула") Тогда
			ОбнаруженыОшибки = Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьПараметрыФормыРедактированияЦены(Формула)
	...
	СтруктураВозврата.Вставить("Операнды", ПоместитьБазовыеВидыЦенВХранилище());
