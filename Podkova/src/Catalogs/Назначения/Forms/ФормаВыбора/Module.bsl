...
Процедура УстановитьТекстЗапросаДинамическогоСписка()
	...
	Если ТипЗнч(Источник) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары")
		...
		СвойстваСписка.ТекстЗапроса = ВыполнитьПодстановки(Справочники.Назначения.ТекстЗапросаНазначенийРасширенный(),
			Параметры.Отбор.Свойство("Партнер"));
	...
	Если ЗначениеЗаполнено(ИмяОбъекта) Тогда
		...
		ТекстЗапроса = ВыполнитьПодстановки(ТекстЗапроса, Параметры.Отбор.Свойство("Партнер"));
	ИначеЕсли Источник <> Неопределено Тогда
...
Процедура НастроитьФормуПоПараметрам()
	...
	// Для перемещений обособленных товаров не имеет смысла переключатель "По всем складам", так как перемещение идет на
	// конкретный склад.
