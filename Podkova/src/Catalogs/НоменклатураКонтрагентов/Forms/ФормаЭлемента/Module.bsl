...
Процедура НадписьВладельцаНажатие(Элемент)
	...
	ОткрытьФорму("Справочник." + ИмяМетаданныхВладельца + ".ФормаВыбора",
		Новый Структура("ТекущаяСтрока", Объект.ВладелецНоменклатуры), ЭтаФорма, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПроверитьИдентификаторИРазбитьНаЧасти(Знач Ссылка, Знач ВладелецНоменклатуры, НоменклатураКонтрагента, Идентификатор,
	...
	Если ТребуетсяЗаполнениеИдентификатора Тогда
		...
		Идентификатор = ЭлектронноеВзаимодействиеСлужебный.ИдентификаторТовараПоСтроке(Идентификатор, 300);
...
Функция ИдентификаторыРазделенныеНаЧасти(Знач Идентификатор)
	...
	СопоставлениеНоменклатурыКонтрагентовСлужебныйКлиентСервер.РазделитьИдентификаторНаЧасти(Идентификатор, Идентификаторы);
