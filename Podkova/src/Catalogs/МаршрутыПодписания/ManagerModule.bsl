...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	// Заполнение схемы подписания предопределенных элементов.
	Данные_ЗаполнитьСхемуПодписания = ДанныеКОбработке_ЗаполнитьСхемуПодписания();
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Данные, Данные_ЗаполнитьСхемуПодписания, Истина);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для каждого СтрокаТаблицы Из ДанныеДляОбновления Цикл
		...
		НачатьТранзакцию();
		Попытка
			ЭлектронноеВзаимодействиеСлужебный.УстановитьУправляемуюБлокировкуПоСсылке(СтрокаТаблицы.Ссылка);
			
			Записать = Ложь;
			
			Объект = СтрокаТаблицы.Ссылка.ПолучитьОбъект();
			Если Объект <> Неопределено Тогда
				ОбработатьДанные_ЗаполнитьСхемуПодписания(Объект, Записать);
...
Функция ДанныеКОбработке_ЗаполнитьСхемуПодписания() 
...
Процедура ОбработатьДанные_ЗаполнитьСхемуПодписания(Объект, Записать) 
	
	СхемаПодписания = ?(Объект.Ссылка = Справочники.МаршрутыПодписания.ОднойДоступнойПодписью,
		Перечисления.СхемыПодписанияЭД.ОднойДоступнойПодписью, Перечисления.СхемыПодписанияЭД.УказыватьПриСоздании);
	
	Если Объект.Предопределенный И Объект.СхемаПодписания <> СхемаПодписания Тогда
		Объект.СхемаПодписания = СхемаПодписания;
		Записать = Истина;
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбработатьДанные_СоздатьПредопределенныеМаршруты() 
