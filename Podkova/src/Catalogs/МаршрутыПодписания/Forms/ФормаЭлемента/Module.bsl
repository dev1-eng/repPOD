...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ВидПодписи = ПредопределенноеЗначение("Перечисление.ВидыЭлектронныхПодписей.Простая") Тогда
		...
		Для каждого СтрокаТаблицы Из ТекущийОбъект.ТаблицаТребований Цикл
			...
			Для каждого Строка Из СтрокиДерева Цикл
				Строка.Сертификат = ПредопределенноеЗначение("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.ПустаяСсылка");
			КонецЦикла;
			СтрокаТаблицы.Сертификат = ПредопределенноеЗначение("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.ПустаяСсылка");
		КонецЦикла;
...
Процедура ПроверитьВалидностьМаршрутаПоВсемЗависимымНастройкам(Отказ)
	...
	Если ЕстьОбменСКонтрагентами ИЛИ ЕстьОбменСБанками Тогда
		...
		Если ЕстьОбменСКонтрагентами Тогда
			...
			|	НЕОПРЕДЕЛЕНО КАК Сертификат
			|ИЗ
			|	РегистрСведений.НастройкиВнутреннегоЭДО КАК НастройкиВнутреннегоЭДО
			|ГДЕ
			|	НастройкиВнутреннегоЭДО.МаршрутПодписания = &МаршрутПодписания";
		...
		Если ЕстьОбменСКонтрагентами Тогда
			...
			|ГДЕ
			|	НастройкиВнутреннегоЭДО.МаршрутПодписания = &МаршрутПодписания
			|	И НастройкиВнутреннегоЭДО.ВидПодписи <> &ВидПодписи
			|ИТОГИ ПО
			|	Организация";
			
			Запрос.УстановитьПараметр("МаршрутПодписания", Объект.Ссылка);
			Запрос.УстановитьПараметр("ВидПодписи", Объект.ВидПодписи);
			НачатьТранзакцию();
			Попытка
				Поля = Новый Структура;
				Поля.Вставить("Организация", Объект.Организация);
				ЭлектронноеВзаимодействиеСлужебный.УстановитьУправляемуюБлокировку("РегистрСведений.НастройкиВнутреннегоЭДО", Поля,,
					РежимБлокировкиДанных.Разделяемый);
					
				РезультатЗапроса = Запрос.Выполнить();
				
				Если Не РезультатЗапроса.Пустой() Тогда
					ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Вид подписи в маршруте не соответствует виду подписи в настройках внутреннего ЭДО по организациям:'"));
					...
					Пока ВыборкаОрганизация.Следующий() Цикл
						...
						Если ВыборкаДетальныеЗаписи.Следующий() Тогда
							...
							ЗначенияКлюча.Вставить("Организация", ВыборкаОрганизация.Организация);
							ЗначенияКлюча.Вставить("ВидВнутреннегоДокумента", ВыборкаДетальныеЗаписи.ВидВнутреннегоДокумента);
							МенеджерНастроек = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("РегистрСведений.НастройкиВнутреннегоЭДО");
							КлючЗаписи = МенеджерНастроек.СоздатьКлючЗаписи(ЗначенияКлюча);
			...
			Исключение
				ОтменитьТранзакцию();
				ИмяСобытия = НСтр("ru = 'Проверка валидности маршрута подписания'", ОбщегоНазначения.КодОсновногоЯзыка());
