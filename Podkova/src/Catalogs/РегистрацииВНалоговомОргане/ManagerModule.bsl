...
// Процедура заполняет реквизит КодРегиона (по коду налогового органа).
// Отличается от ЗаполнитьКодыРегионовИСлужебныеНаименования() тем, что выполняется во всех версиях поставки (не только КОРП).
// Обрабатываем все элементы справочника одной порцией, так как элементов в справочнике, как правило, совсем немного.
Процедура ЗаполнитьКодыРегионов() Экспорт
	...
	ВыборкаРегистраций = Запрос.Выполнить().Выбрать();
