...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("1ad2605f-34ec-4980-b656-d8ac6fe52a3b");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если НЕ ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(Параметры.Очередь, "РегистрНакопления.РасчетыСКлиентамиПоДокументам") Тогда
		Возврат;
	КонецЕсли;
	...
	|ГДЕ
	|	РегистраторРасчетов.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом)
	|;
	|ВЫБРАТЬ
...
Функция ТекстЗапросаНекорректныеРегистраторы(ИмяВТ = "")
	...
	ТекстЗапроса = "
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаданияКРаспределениюРасчетовСКлиентами КАК Задания
...
Функция ТекстЗапросаИсправленныеДвижения()
	...
	ТекстЗапроса = "
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КортежиКПроверке КАК КортежиКПроверке
		|			ПО КортежиКПроверке.АналитикаУчетаПоПартнерам = РасчетыСКлиентами.АналитикаУчетаПоПартнерам
		|				И КортежиКПроверке.ОбъектРасчетов         = РасчетыСКлиентами.ЗаказКлиента
		|				И КортежиКПроверке.Валюта                 = РасчетыСКлиентами.Валюта
		|ГДЕ
		...
		|		ПО РасчетыСКлиентамиПоДокументам.РасчетныйДокумент = ПорядокДокументов.Регистратор
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КортежиКПроверке КАК КортежиКПроверке
		|			ПО КортежиКПроверке.АналитикаУчетаПоПартнерам = РасчетыСКлиентамиПоДокументам.АналитикаУчетаПоПартнерам
		|				И КортежиКПроверке.ОбъектРасчетов         = РасчетыСКлиентамиПоДокументам.ЗаказКлиента
		|				И КортежиКПроверке.Валюта                 = РасчетыСКлиентамиПоДокументам.Валюта
		|ГДЕ
