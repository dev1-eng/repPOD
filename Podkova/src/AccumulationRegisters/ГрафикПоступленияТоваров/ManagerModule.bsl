...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("171779d4-af1f-4b7a-ae51-a161e8bb34d9");
	...
	Обработчик.Комментарий = НСтр("ru = 'Актуализирует движения по регистру накопления График поступления товаров по документам Заказ поставщику, Приходный ордер на товары и Акт о расхождениях после поступления.'");
	...
	Читаемые.Добавить(Метаданные.РегистрыНакопления.ГрафикПоступленияТоваров.ПолноеИмя());
	Читаемые.Добавить(Метаданные.РегистрыНакопления.ДвижениеТоваров.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.РегистрыНакопления.ГрафикПоступленияТоваров.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Регистратор.Ссылка КАК Регистратор
	|ИЗ
	|	Документ.РегистраторГрафикаДвиженияТоваров КАК Регистратор
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтДублиПоДоговорам КАК Дубли
	|			ПО Регистратор.Распоряжение = Дубли.Распоряжение
	|ГДЕ
	|	НЕ Дубли.Распоряжение ЕСТЬ NULL
	...
	|
	|ВЫБРАТЬ
	|	Регистратор.Ссылка.Распоряжение КАК Регистратор
	|ИЗ
	...
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|	И Регистратор.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказПоставщику
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Регистратор.ЗаказПоставщику КАК Регистратор
	|ИЗ
	...
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|	И НЕ Регистратор.ЗаказПоставщику В(
	|		НЕОПРЕДЕЛЕНО,
